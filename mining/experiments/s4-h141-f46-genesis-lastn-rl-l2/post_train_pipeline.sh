#!/usr/bin/env bash
# After H141 train.done: merge LoRA → chall:8002 → n80 vs Tok331102 king.
# Train base = Genesis; sim king = live Tok331102. Prewarm holds teacher+king.
set -euo pipefail
...
TRAIN_DIR=${TRAIN_DIR:-/root/h141/train}
ADAPTER=${ADAPTER:-$TRAIN_DIR/adapter}
CKPT_ROOT=${CKPT_ROOT:-$TRAIN_DIR/checkpoints}
MERGED=${MERGED:-/root/h141/merged}
SIM_N80=/root/affine_data/h141_sim_result.json
PROG=/root/affine_data/h141_sim_progress.json
LOG=/root/logs/h141_pipeline.nohup
...
log() { echo "[h141-pipe] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }
...
_train_alive() {
  if [[ -f /root/logs/h141_train.pid ]]; then
    local tpid
    tpid=$(cat /root/logs/h141_train.pid 2>/dev/null || true)
    if [[ -n "${tpid:-}" ]] && kill -0 "$tpid" 2>/dev/null; then
      return 0
    fi
  fi
  # Narrow fallback: real trainer argv starts with python3 + full script path.
  pgrep -f "python3 /root/mining_src/s4-h141-f46-genesis-lastn-rl-l2/train_rl_lastn.py --base" >/dev/null 2>&1
}
...
_abort_on_exit() {
  ...
  if [[ -f /root/logs/h141_pipeline.done ]]; then
    return 0
  fi
  if [[ ! -f /root/logs/h141_pipeline.aborted ]]; then
    echo "aborted_err_rc=${rc} $(date -u +%Y-%m-%dT%H:%M:%SZ)" \
      >/root/logs/h141_pipeline.aborted
    echo "[h141-pipe] $(date -u +%Y-%m-%dT%H:%M:%SZ) EXIT trap wrote aborted_err_rc=${rc}" \
      | tee -a "$LOG" >/dev/null 2>&1 || true
  fi
}
trap _abort_on_exit EXIT

mkdir -p /root/logs /root/affine_data /root/h141
rm -f /root/logs/h141_pipeline.aborted /root/logs/h141_pipeline.done \
  /root/logs/h141_merge.done /root/logs/h141_chall_serve.done \
  /root/logs/h141_sim_n80.done
...
while true; do
  ...
  if [[ -e "$TRAIN_DIR/full_ft" || -f "$ADAPTER/FULL_FT_MARKER" || -f "$ADAPTER/adapter_config.json" ]] && ! _train_alive; then
    log "full_ft/adapter present and train proc gone - proceed"
    break
  fi
  now=$(date -u +%s)
  ...
  if (( now > soft - 3600 )); then
    ...
    echo "aborted_no_train $(date -u +%Y-%m-%dT%H:%M:%SZ)" > /root/logs/h141_pipeline.aborted
    exit 1
  fi
  _wait_i=$((_wait_i + 1))
...
if _train_alive; then
  ...
  echo "aborted_train_stuck $(date -u +%Y-%m-%dT%H:%M:%SZ)" > /root/logs/h141_pipeline.aborted
  exit 1
fi
sleep 15
...
if [[ -e "$FULL_FT" ]]; then
  log "full_ft present → symlink MERGED=$MERGED -> $FULL_FT (no LoRA merge)"
  rm -rf "$MERGED"
  ln -sfn "$(readlink -f "$FULL_FT")" "$MERGED"
  echo "{"mode":"full_ft_lastn","full_ft":"$FULL_FT"}" > /root/affine_data/h141_merge_meta.json
  date -u +%Y-%m-%dT%H:%M:%SZ > /root/logs/h141_merge.done
elif [[ -f "$ADAPTER/adapter_config.json" ]]; then
  ...
  python /root/mining_src/s4-h1-sft/merge_lora.py \
    ...
    --device-map auto \
    | tee -a "$LOG"
  cp -f "$MERGED/merge_meta.json" /root/affine_data/h141_merge_meta.json 2>/dev/null || true
  date -u +%Y-%m-%dT%H:%M:%SZ > /root/logs/h141_merge.done
else
  log "ERROR: no full_ft and no adapter"
...
Path("/root/affine_data/h141_identity.json").write_text(
    json.dumps(payload, indent=2) + "\n"
)
print(json.dumps(payload, indent=2), flush=True)
if identical_base:
    sys.exit("REFUSE: merged weight-identical to Genesis init base")
if identical_king:
    sys.exit("REFUSE: merged weight-identical to Tok331102 king")
print("[h141] OK_NON_IDENTICAL", flush=True)
PY

HF_LORA_REPO=${HF_LORA_REPO:-unconst/Affine-5czsc2fc98-h141-lora}
HF_MERGED_REPO=${HF_MERGED_REPO:-unconst/Affine-5czsc2fc98-h141-merged}
HF_BASE_HUB=${HF_BASE_HUB:-dendriteholdings/albedo-qwen3.6-35b-king-genesis}
SEEN_MID=${SEEN_MID:-/root/h141/mid_ckpt_salvaged.txt}
if [[ -n "${HF_TOKEN:-}" ]]; then
  if [[ -f "$SEEN_MID" ]] && grep -qx "adapter-final" "$SEEN_MID"; then
    ...
    echo "{\"skipped\":true,\"reason\":\"mid adapter-final present\"}" \
      >/root/affine_data/h141_adapter_salvage.json
    rm -f /root/logs/h141_push_adapter.pid
  elif pgrep -f "s4-h141-f46-genesis-lastn-rl-l2/mid_ckpt_salvage.sh" >/dev/null 2>&1; then
    ...
    nohup python3 /root/mining_src/s4-h1-sft/salvage_adapter.py \
      ...
      --commit-message "H141 Genesis-init winner-zA thought LoRA salvage (TTL insurance; not a submission)" \
      --out-meta /root/affine_data/h141_adapter_salvage.json \
      >>/root/logs/h141_push_adapter.nohup 2>&1 &
    echo $! >/root/logs/h141_push_adapter.pid
  fi
  ...
  nohup python3 /root/mining_src/s4-h1-sft/push_merged.py \
    ...
    --repo "$HF_MERGED_REPO" \
    --public \
    --commit-message "H141 Genesis-init winner-zA thought merged salvage (TTL insurance; not a submission)" \
    --out-meta /root/affine_data/h141_merged_salvage.json \
    >>/root/logs/h141_push_merged.nohup 2>&1 &
  echo $! >/root/logs/h141_push_merged.pid
  log "adapter push pid=$(cat /root/logs/h141_push_adapter.pid 2>/dev/null || echo skipped) merged push pid=$(cat /root/logs/h141_push_merged.pid)"
else
  log "WARN: HF_TOKEN unset; skipping H141 HF salvage pushes"
fi
...
if [[ "$code_t" != "200" || "$code_k" != "200" ]]; then
  ...
  echo "aborted_engines_unhealthy $(date -u +%Y-%m-%dT%H:%M:%SZ)" > /root/logs/h141_pipeline.aborted
  exit 1
fi
...
date -u +%Y-%m-%dT%H:%M:%SZ > /root/logs/h141_chall_serve.done
...
if (( dead - now < 2400 )); then
  ...
  echo "aborted_no_n80_budget $(date -u +%Y-%m-%dT%H:%M:%SZ)" > /root/logs/h141_pipeline.aborted
  exit 1
fi

N80_MAX_ATTEMPTS=${N80_MAX_ATTEMPTS:-3}
# Fresh block_hash per attempt (H32/H34/H141): default 0*64 dies teacher 400 @~40/80.
...
for attempt in $(seq 1 "$N80_MAX_ATTEMPTS"); do
  ...
  if ps -eo args | awk '/[r]un_sim_duel.py/ && /local-h141/' | grep -q .; then
    log "n80 already running under retry — skip post_train launch"
    n80_ok=1
    break
  fi
  if ps -eo args | awk '/[w]atch_n80_retry\.sh/ && / h141 /' | grep -q . \
    || ps -eo args | awk '/[r]etry_h141_n80\.sh/' | grep -q .; then
    log "watch_n80_retry/retry_h141 armed — defer n80 to retry; skip post_train launch"
    n80_ok=1
    break
  fi
  rm -f "$SIM_N80" "$PROG" /root/logs/h141_sim_n80.done
  ...
  python /root/mining_src/s4-h2-merge/run_sim_duel.py \
    ...
    --progress-out "$PROG" \
    --save-artifact \
    2>&1 | tee -a /root/logs/h141_sim.nohup
  sim_rc=${PIPESTATUS[0]}
  set -e
  ...
  echo "n80_attempt_${attempt}_failed rc=$sim_rc $(date -u +%Y-%m-%dT%H:%M:%SZ)" \
    >> /root/logs/h141_sim_retries.log
  sleep 15
done
if [[ "$n80_ok" -ne 1 ]]; then
...
date -u +%Y-%m-%dT%H:%M:%SZ > /root/logs/h141_sim_n80.done
date -u +%Y-%m-%dT%H:%M:%SZ > /root/logs/h141_pipeline.done
