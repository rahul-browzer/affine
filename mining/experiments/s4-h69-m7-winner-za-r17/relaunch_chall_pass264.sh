...
LOG=/root/logs/h69_chall_recover_pass264.log
CHALL_LOG=/root/logs/vllm_chall.log
PIDF=/root/logs/vllm_chall.pid
MERGE=/root/h69/merged
export HYP=h69
export GPUS=4,5
UTIL=0.72
...
log() { echo "[recover264-h69-chall] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }

log "START chall relaunch (outer×3 + king-seed WRITABLE diverse-warm + post-diverse freeze; p247 prefreeze-before-w1 dead)"

ts=$(date -u +%Y%m%dT%H%M%SZ)
for f in h69_decision.json h69_sim_result.json h69_sim_result_artifact.json \
         h69_sim_progress.json; do
  if [[ -f "/root/affine_data/$f" ]]; then
    mv "/root/affine_data/$f" "$QDIR/${f%.json}_pass264_${ts}.json"
    log "quarantine $f → $QDIR"
  fi
done
rm -f /root/logs/h69_n80.done /root/logs/h69_n80_retry.aborted \
  /root/logs/h69_pipeline.aborted /root/logs/h69_chall_serve.done \
  /root/logs/h69_sim_n80.done /root/logs/h69_chall_freeze_pass247.done \
  /root/logs/h69_chall_freeze_pass251.done \
  /root/logs/h69_chall_freeze_pass237.done \
  /root/logs/h69_chall_freeze_pass239.done \
  /root/logs/h69_chall_freeze_pass240.done
...
while read -r pid; do
  ...
  /[r]un_sim_duel\.py/ && /local-h69/ {print $1}
  /[r]etry_h69_n80\.sh/ {print $1}
')
...
print(f"[recover264-h69-chall] reap gpu={sorted(want)} workers={sorted(pids)} parents={sorted(parents)} grandparents={sorted(grandparents)}")
for pid in kill_set:
    try:
...
wipe_caches() {
  ...
  chmod -R u+w /root/.triton/isolated/h69_chall_* 2>/dev/null || true
  rm -rf /root/.triton/cache/chall /root/.triton/cache/chall_* || true
  rm -rf /root/.triton/isolated/h69_chall_* || true
  rm -rf /root/.cache/flashinfer/cached_ops/sampling || true
  rm -rf /root/.cache/vllm/torch_compile_cache || true
  rm -rf /tmp/torchinductor_* /root/.cache/torch/inductor /root/.cache/torchinductor_chall_* /root/.cache/torchinductor_h69_* || true
...
for attempt in 1 2 3; do
  ...
  wipe_caches
  TAG=h69_chall_p260_a${attempt}_$(date +%s)_$$
...
while read -r pid; do
  ...
  sleep 1
  kill -9 "$pid" 2>/dev/null || true
done < <(ps -eo pid,args | awk '/[w]atch_n80_retry\.sh/ && / h69 / {print $1}')
while read -r pid; do
  [[ -n "${pid:-}" ]] || continue
  ...
  sleep 1
  kill -9 "$pid" 2>/dev/null || true
done < <(ps -eo pid,args | awk '/[r]etry_h69_n80\.sh/ {print $1}')
while read -r pid; do
  [[ -n "${pid:-}" ]] || continue
  ...
  sleep 1
  kill -9 "$pid" 2>/dev/null || true
done < <(ps -eo pid,args | awk '/[w]atch_form_decision\.sh/ && / h69 / {print $1}')
...
nohup bash /root/mining_src/s4-h2-merge/watch_form_decision.sh h69 \
  /root/affine_data/h69_sim_result.json \
  /root/affine_data/h69_decision.json \
  /root/logs/h69_form_decision.nohup \
  >/root/logs/h69_form_decision.launch.nohup 2>&1 &
echo $! >/root/logs/h69_form_decision.pid
log "rearmed form pid=$(cat /root/logs/h69_form_decision.pid)"

nohup bash /root/mining_src/s4-h2-merge/watch_n80_retry.sh h69 \
  /root/mining_src/s4-h69-m7-winner-za-r17/retry_h69_n80.sh \
  >/root/logs/h69_watch_retry.launch.nohup 2>&1 &
echo $! >/root/logs/h69_watch_retry.pid
log "rearmed watcher pid=$(cat /root/logs/h69_watch_retry.pid)"
...
echo "TCACHE=$TCACHE mode=$(stat -c %a $TCACHE) probe=200 attempts_ok post_diverse_freeze=1 king_seed=1" \
  > /root/logs/h69_chall_freeze_pass264.done
