#!/usr/bin/env bash
# mine-h14-1 bootstrap: pinned eval stack + TalentPigs/kkkk/teacher → merge α0.75.
# Secrets from /root/mine.env only (never argv).
set -euo pipefail

LOG=/root/logs/bootstrap_h14.log
mkdir -p /root/logs /root/hf /root/affine_data /root/mining_src /root/merges
exec > >(tee -a "$LOG") 2>&1

echo "[bootstrap-h14] $(date -u +%Y-%m-%dT%H:%M:%SZ) start host=$(hostname)"
...
if [[ -z "${HF_TOKEN:-}" ]]; then
  echo "[bootstrap-h14] FATAL: HF_TOKEN missing in /root/mine.env"
  exit 1
fi
...
uv pip install \
  ...
  "numpy" \
  "scipy" \
  2>&1 | tee /root/logs/pip_h14.log | tail -40

python - <<'PY'
import torch, transformers, vllm
print("[bootstrap-h14] VERSIONS",
...
if txt == orig:
    print("[bootstrap-h14] vllm_client already patched or pattern miss", flush=True)
else:
    p.write_text(txt)
    print("[bootstrap-h14] patched vllm_client timeout=480 retries=5", flush=True)
PY

B_REPO=vincentwarrior/affine-5ccebdzvsj-kkkk
B_REV=3ca1ebe6a952b7ad935ca53f7dd8e85b498bfcb0
...
for repo, rev, label in jobs:
    ...
    if os.path.isfile(done):
        print(f"[bootstrap-h14] SKIP {label} (have {done})", flush=True)
        continue
    print(f"[bootstrap-h14] DOWNLOAD start {label} {repo} rev={rev}", flush=True)
    path = snapshot_download(repo, revision=rev, token=token)
    print(f"[bootstrap-h14] DOWNLOAD done {label} -> {path}", flush=True)
    open(done, "w").write(path + "\n")
print("[bootstrap-h14] ALL_DOWNLOADS_OK", flush=True)
...
python /root/mining_src/s4-h2-merge/merge_linear.py \
  ...
  --out "$OUT" \
  --hf-home "$HF_HOME" \
  2>&1 | tee /root/logs/h14_merge.log

date -u +%Y-%m-%dT%H:%M:%SZ > /root/logs/h14_merge.done
echo "[bootstrap-h14] MERGE_DONE alpha=$ALPHA out=$OUT b=$B_REPO@$B_REV"
...
export CHALL_REPO=/root/merges/h14-tp75
export CHALL_REV=
bash /root/mining_src/s3-duel-sim/serve_three.sh

touch /root/logs/bootstrap_h14.done
echo "[bootstrap-h14] $(date -u +%Y-%m-%dT%H:%M:%SZ) DONE — engines launching; next: wait_ready + n80"
