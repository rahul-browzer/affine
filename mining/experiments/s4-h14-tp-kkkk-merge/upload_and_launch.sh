#!/usr/bin/env bash
# Host → mine-h14-1: upload harness + mine.env, start bootstrap+n80 under nohup.
set -euo pipefail

ROOT=/home/const/subnet120
DST_HOST=${DST_HOST:?set DST_HOST}
DST_PORT=${DST_PORT:?set DST_PORT}
KNOWN=${KNOWN:-/tmp/mine-h14-1.known_hosts}
...
STAGE=$(mktemp -d /tmp/mine-h14-stack.XXXXXX)
trap 'rm -rf "$STAGE"' EXIT

mkdir -p "$STAGE/affine_pkg/affine" "$STAGE/affine_pkg/evalsrv" \
         "$STAGE/s3-duel-sim" "$STAGE/s4-h2-merge" "$STAGE/s4-h14-tp-kkkk-merge"
...
ENV_TMP=$(mktemp /tmp/mine-h14.env.XXXXXX)
...
"${SCP[@]}" "$TAR" "root@${DST_HOST}:/tmp/mine-h14-stack.tar.gz"
"${SCP[@]}" "$ENV_TMP" "root@${DST_HOST}:/root/mine.env"
rm -f "$ENV_TMP"

"${SSH[@]}" 'set -e
  tar -C /root/mining_src -xzf /tmp/mine-h14-stack.tar.gz
  ...
  nohup bash -lc "
    set -euo pipefail
    bash /root/mining_src/s4-h14-tp-kkkk-merge/bootstrap_h14.sh
    bash /root/mining_src/s4-h14-tp-kkkk-merge/start_h14_n80.sh
  " >/root/logs/h14_pipeline.nohup 2>&1 &
  echo $! > /root/logs/h14_pipeline.pid
  ...
  ps -p "$(cat /root/logs/h14_pipeline.pid)" -o pid,etime,cmd || true
'
