#!/usr/bin/env bash
# Pass 229: H49 chall died mid-n80 on Triton __triton_launcher.so
# (FALSE_PROBE ConnectError @ a203). Same recipe as H48 p225: reap GPUs
# 4,5 → wipe → isolated TCACHE → warmup → chmod a-w freeze → rearm n80.
set -euo pipefail
...
LOG=/root/logs/h49_chall_recover_pass229.log
CHALL_LOG=/root/logs/vllm_chall.log
PIDF=/root/logs/vllm_chall.pid
MERGE=/root/h49/merged
export GPUS=4,5
UTIL=0.72
TAG=h49_chall_p229_$(date +%s)_$$
TCACHE=/root/.triton/isolated/$TAG
QDIR=/root/affine_data/false_probes
mkdir -p /root/logs /root/affine_data "$QDIR"
: >"$LOG"

log() { echo "[recover229-h49-chall] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }

log "START chall relaunch (isolated TCACHE + freeze; FALSE_PROBE ConnectError)"
...
for f in h49_decision.json h49_sim_result.json h49_sim_result_artifact.json \
  ...
  if [[ -f "/root/affine_data/$f" ]]; then
    mv "/root/affine_data/$f" "$QDIR/${f%.json}_pass229_${ts}.json"
...
print(f"[recover229-h49-chall] reap gpu={sorted(want)} workers={sorted(pids)} parents={sorted(parents)} grandparents={sorted(grandparents)}")
for pid in kill_set:
    try:
...
log "wipe role chall caches (default + prior isolated + p228 frozen)"
# unfreeze then wipe (p228 chmod a-w blocks rm)
...
log "triple-promptable after freeze — rearm watcher + form"
...
echo "TCACHE=$TCACHE mode=$(stat -c %a $TCACHE) probe=200" \
