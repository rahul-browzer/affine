#!/usr/bin/env bash
# Pass 227: H49 teacher died — Triton race deleted
# /root/.triton/cache/teacher/.../__triton_launcher.so during init.
# King :8001 OK on 2,3 (promptable); merge_lora on 6,7 writing shards.
# GPUs 0,1 free. Wipe teacher_* FIRST → ≥20s settle → unique TCACHE.
# Do NOT touch king/merge/chall. Prewarm + n80_retry will pick up t=1.
set -euo pipefail
...
LOG=/root/logs/h49_teacher_recover_pass227.log
TEACHER_LOG=/root/logs/vllm_teacher.log
PIDF=/root/logs/vllm_teacher.pid
export GPUS=0,1
UTIL=0.80
TEACHER_REPO=zai-org/GLM-4.5-Air-FP8
TAG=teacher_p227_$(date +%s)_$$
mkdir -p /root/logs
: >"$LOG"

log() { echo "[recover227-h49] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }

log "START teacher relaunch (wipe→settle20→unique cache); leave king+merge alone"
...
print(f"[recover227-h49] reap gpu={sorted(want)} workers={sorted(pids)} parents={sorted(parents)} grandparents={sorted(grandparents)}")
for pid in kill_set:
    try:
...
log "DONE_LAUNCH (merge still on 6,7; king :8001 stays; prewarm/n80_retry pick up t=1)"
