#!/usr/bin/env bash
# Pass 230: H49 p229 aborted on warmup #1 — Triton __triton_launcher.so race
# during first completion (health=200 then 500 in 4s). Upgrade: outer retry
# (3×) with fresh isolated TCACHE each attempt + 45s settle after health
# before first warmup (p229 fired warmup immediately → race).
set -euo pipefail
...
LOG=/root/logs/h49_chall_recover_pass230.log
CHALL_LOG=/root/logs/vllm_chall.log
PIDF=/root/logs/vllm_chall.pid
MERGE=/root/h49/merged
export GPUS=4,5
UTIL=0.72
...
log() { echo "[recover230-h49-chall] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }

log "START chall relaunch (outer×3 + 45s post-health settle; p229 warmup#1 Triton.so)"

ts=$(date -u +%Y%m%dT%H%M%SZ)
for f in h49_decision.json h49_sim_result.json h49_sim_result_artifact.json \
         h49_sim_progress.json; do
  if [[ -f "/root/affine_data/$f" ]]; then
    mv "/root/affine_data/$f" "$QDIR/${f%.json}_pass230_${ts}.json"
    log "quarantine $f → $QDIR"
  fi
done
rm -f /root/logs/h49_n80.done /root/logs/h49_n80_retry.aborted \
  /root/logs/h49_pipeline.aborted /root/logs/h49_chall_serve.done \
  /root/logs/h49_sim_n80.done /root/logs/h49_chall_freeze_pass228.done \
  /root/logs/h49_chall_freeze_pass229.done
...
while read -r pid; do
  ...
  /[r]un_sim_duel\.py/ && /local-h49/ {print $1}
  /[r]etry_h49_n80\.sh/ {print $1}
')
...
print(f"[recover230-h49-chall] reap gpu={sorted(want)} workers={sorted(pids)} parents={sorted(parents)} grandparents={sorted(grandparents)}")
for pid in kill_set:
    try:
...
wipe_caches() {
  ...
  chmod -R u+w /root/.triton/isolated/h49_chall_* 2>/dev/null || true
  rm -rf /root/.triton/cache/chall /root/.triton/cache/chall_* || true
  rm -rf /root/.triton/isolated/h49_chall_* || true
  rm -rf /root/.cache/flashinfer/cached_ops/sampling || true
  rm -rf /root/.cache/vllm/torch_compile_cache || true
  rm -rf /tmp/torchinductor_* /root/.cache/torch/inductor /root/.cache/torchinductor_chall_* /root/.cache/torchinductor_h49_* || true
...
_comp() {
  ...
  code=$(curl -s -o "/tmp/h49_warmup_${label}.json" -w "%{http_code}" --max-time 120 \
...
for attempt in 1 2 3; do
  ...
  wipe_caches
  TAG=h49_chall_p230_a${attempt}_$(date +%s)_$$
  ...
  # p229 raced here: warmup immediately after health. Settle so TP workers
  # finish writing launcher.so before first completion loads it.
  log "settle 45s after health before warmup (p229 race mitigation)"
  sleep 45
  ...
  if ! _comp "a${attempt}_w1"; then
    log "attempt $attempt: warmup #1 failed — chall up? pid=$(kill -0 $CHALL_PID 2>/dev/null && echo yes || echo no)"
...
while read -r pid; do
  ...
  sleep 1
  kill -9 "$pid" 2>/dev/null || true
done < <(ps -eo pid,args | awk '/[w]atch_n80_retry\.sh/ && / h49 / {print $1}')
while read -r pid; do
  [[ -n "${pid:-}" ]] || continue
  ...
  sleep 1
  kill -9 "$pid" 2>/dev/null || true
done < <(ps -eo pid,args | awk '/[r]etry_h49_n80\.sh/ {print $1}')
while read -r pid; do
  [[ -n "${pid:-}" ]] || continue
  ...
  sleep 1
  kill -9 "$pid" 2>/dev/null || true
done < <(ps -eo pid,args | awk '/[w]atch_form_decision\.sh/ && / h49 / {print $1}')
sleep 1

nohup bash /root/mining_src/s4-h2-merge/watch_form_decision.sh h49 \
  /root/affine_data/h49_sim_result.json \
  /root/affine_data/h49_decision.json \
  /root/logs/h49_form_decision.nohup \
  >/root/logs/h49_form_decision.launch.nohup 2>&1 &
echo $! >/root/logs/h49_form_decision.pid
log "rearmed form pid=$(cat /root/logs/h49_form_decision.pid)"

nohup bash /root/mining_src/s4-h2-merge/watch_n80_retry.sh h49 \
  /root/mining_src/s4-h49-m7-winner-za-a4/retry_h49_n80.sh \
  >/root/logs/h49_watch_retry.launch.nohup 2>&1 &
echo $! >/root/logs/h49_watch_retry.pid
log "rearmed watcher pid=$(cat /root/logs/h49_watch_retry.pid)"
date -u +%Y-%m-%dT%H:%M:%SZ > /root/logs/h49_chall_serve.done
echo "TCACHE=$TCACHE mode=$(stat -c %a $TCACHE) probe=200 attempts_ok" \
  > /root/logs/h49_chall_freeze_pass230.done
