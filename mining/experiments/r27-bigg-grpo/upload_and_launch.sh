...
POD_NAME=${POD_NAME:-mine-r27-bigg-1}
...
STAGE=$(mktemp -d /tmp/mine-r27-stack.XXXXXX)
...
_rem_raw=$(lium describe "$POD_NAME" --json 2>/dev/null \
  ...
  || true)
if [[ -z "${_rem_raw}" ]]; then
  echo "[r27-up] FATAL: no billing.removal_scheduled_at for POD_NAME=$POD_NAME" >&2
  exit 1
fi
...
echo "[r27-up] Removal=$REMOVAL Soft=$SOFT Dead=$DEAD (from lium describe $POD_NAME)"
...
"${SCP[@]}" "$TAR" "root@${DST_HOST}:/tmp/mine-r27-stack.tar.gz"
...
"${SSH[@]}" 'set -e
  tar -C /root/mining_src -xzf /tmp/mine-r27-stack.tar.gz
  ...
  ps -p "$(cat /root/logs/r27_pipeline.pid)" -o pid,etime,cmd || true
  nvidia-smi -L | wc -l
'
