#!/usr/bin/env bash
# mine-h15-1 bootstrap: pinned eval stack + TalentPigs/leary/teacher → merge α0.75.
# Secrets from /root/mine.env only (never argv).
set -euo pipefail

LOG=/root/logs/bootstrap_h15.log
mkdir -p /root/logs /root/hf /root/affine_data /root/mining_src /root/merges
exec > >(tee -a "$LOG") 2>&1

echo "[bootstrap-h15] $(date -u +%Y-%m-%dT%H:%M:%SZ) start host=$(hostname)"
...
if [[ -z "${HF_TOKEN:-}" ]]; then
  echo "[bootstrap-h15] FATAL: HF_TOKEN missing in /root/mine.env"
  exit 1
fi
...
uv pip install \
  ...
  "numpy" \
  "scipy" \
  2>&1 | tee /root/logs/pip_h15.log | tail -40

python - <<'PY'
import torch, transformers, vllm
print("[bootstrap-h15] VERSIONS",
...
if txt == orig:
    print("[bootstrap-h15] vllm_client already patched or pattern miss", flush=True)
else:
    p.write_text(txt)
    print("[bootstrap-h15] patched vllm_client timeout=480 retries=5", flush=True)
PY

B_REPO=leary-criste/affine-5g4yy75zuz-test
B_REV=1e6d6d02ebe771b767c5d2bfaf3c0a1538605fc3
...
for repo, rev, label in jobs:
    ...
    if os.path.isfile(done):
        print(f"[bootstrap-h15] SKIP {label} (have {done})", flush=True)
        continue
    print(f"[bootstrap-h15] DOWNLOAD start {label} {repo} rev={rev}", flush=True)
    path = snapshot_download(repo, revision=rev, token=token)
    print(f"[bootstrap-h15] DOWNLOAD done {label} -> {path}", flush=True)
    open(done, "w").write(path + "\n")
print("[bootstrap-h15] ALL_DOWNLOADS_OK", flush=True)
...
python /root/mining_src/s4-h2-merge/merge_linear.py \
  ...
  --out "$OUT" \
  --hf-home "$HF_HOME" \
  2>&1 | tee /root/logs/h15_merge.log

date -u +%Y-%m-%dT%H:%M:%SZ > /root/logs/h15_merge.done
echo "[bootstrap-h15] MERGE_DONE alpha=$ALPHA out=$OUT b=$B_REPO@$B_REV"
...
export CHALL_REPO=/root/merges/h15-tp75
export CHALL_REV=
bash /root/mining_src/s3-duel-sim/serve_three.sh

touch /root/logs/bootstrap_h15.done
echo "[bootstrap-h15] $(date -u +%Y-%m-%dT%H:%M:%SZ) DONE — engines launching; next: wait_ready + n80"
