#!/usr/bin/env python3
"""R8: classic REINFORCE on thought tokens maximizing Reason (EMA baseline).

Reward per sample z (via live teacher :8000 echo+logprobs):
  Reason = lpC(y_C | z_A) − lpC(y_C | ∅)

EMA baseline (not group-relative GRPO). Distinct from R3 (G=4, r=16):
higher LoRA rank + running-mean advantage. No CE on harvested z —
...
def main() -> None:
    ...
    ap.add_argument("--samples-per-step", type=int, default=1,
                    help="rollouts per update (REINFORCE; EMA baseline, not GRPO group)")
    ap.add_argument("--baseline-ema", type=float, default=0.9,
    ...
    print(f"[r8] kept={len(rows)}/{len(raw)} budget_chars={budget}", flush=True)
    if len(rows) < 50:
    ...
    print(f"[r8] loading base {args.base}", flush=True)
    ...
    while steps < args.max_steps and epoch < math.ceil(args.epochs):
        ...
        for row in rows:
            ...
            texts: list[str] = []
            for _g in range(args.samples_per_step):
                # Mid-sample hb so host wedge watch sees life during long generates.
                print(
                    f"[r8-hb] {json.dumps({'utc': time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime()), 'step': steps, 'phase': 'sample', 'g': _g})}",
                    flush=True,
                )
                ...
                print(
                    f"[r8-hb] {json.dumps({'utc': time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime()), 'step': steps, 'phase': 'score', 'g': _g})}",
                    flush=True,
                )
                try:
                    ...
                    print(f"[r8] teacher score fail step={steps}: {e}", flush=True)
                    r = 0.0
            ...
            # Classic REINFORCE: advantage vs EMA baseline (≠ R3 group-mean).
            if baseline is None:
                baseline = mean_r
            else:
                baseline = args.baseline_ema * baseline + (1.0 - args.baseline_ema) * mean_r
            ...
            # between the sparse [r8-log] %5 prints (false-kill at p2066).
            print(
                f"[r8-hb] {json.dumps({'utc': time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime()), 'step': steps, 'mean_r': mean_r})}",
                flush=True,
            )
            if steps % 5 == 0 or steps <= 3:
                print(f"[r8-log] {json.dumps(rec)}", flush=True)
            if steps % 50 == 0:
    ...
    print(f"[r8] DONE {json.dumps(result)}", flush=True)
