...
MERGED=${MERGED:-/root/r8/merged}
SIM_N80=/root/affine_data/r8_sim_result.json
PROG=/root/affine_data/r8_sim_progress.json
LOG=/root/logs/r8_pipeline.nohup
...
log() { echo "[r8-pipe] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }

# Prefer pidfile over `pgrep -f train_reason_reinforce.py`: host SSH scrapes that embed the
# pattern in argv false-match and can stall the post-train.done GPU wait forever.
_train_alive() {
  if [[ -f /root/logs/r8_train.pid ]]; then
    local tpid
    tpid=$(cat /root/logs/r8_train.pid 2>/dev/null || true)
    if [[ -n "${tpid:-}" ]] && kill -0 "$tpid" 2>/dev/null; then
      return 0
    fi
  fi
  # Narrow fallback: real trainer argv starts with python3 + full script path.
  pgrep -f "python3 /root/mining_src/r8-reinforce-reason/train_reason_reinforce.py --base" >/dev/null 2>&1
}
...
_abort_on_exit() {
  ...
  if [[ -f /root/logs/r8_pipeline.done ]]; then
    return 0
  fi
  if [[ ! -f /root/logs/r8_pipeline.aborted ]]; then
    echo "aborted_err_rc=${rc} $(date -u +%Y-%m-%dT%H:%M:%SZ)" \
      >/root/logs/r8_pipeline.aborted
    echo "[r8-pipe] $(date -u +%Y-%m-%dT%H:%M:%SZ) EXIT trap wrote aborted_err_rc=${rc}" \
      | tee -a "$LOG" >/dev/null 2>&1 || true
  fi
}
trap _abort_on_exit EXIT

mkdir -p /root/logs /root/affine_data /root/r8
rm -f /root/logs/r8_pipeline.aborted /root/logs/r8_pipeline.done \
  /root/logs/r8_merge.done /root/logs/r8_chall_serve.done \
  /root/logs/r8_sim_n80.done
...
while true; do
  ...
  if (( now > soft - 3600 )); then
    ...
    echo "aborted_no_train $(date -u +%Y-%m-%dT%H:%M:%SZ)" > /root/logs/r8_pipeline.aborted
    exit 1
  fi
  _wait_i=$((_wait_i + 1))
...
if _train_alive; then
  ...
  echo "aborted_train_stuck $(date -u +%Y-%m-%dT%H:%M:%SZ)" > /root/logs/r8_pipeline.aborted
  exit 1
fi
sleep 15
...
python /root/mining_src/s4-h1-sft/merge_lora.py \
  ...
  --device-map auto \
  | tee -a "$LOG"
cp -f "$MERGED/merge_meta.json" /root/affine_data/r8_merge_meta.json 2>/dev/null || true
date -u +%Y-%m-%dT%H:%M:%SZ > /root/logs/r8_merge.done
...
Path("/root/affine_data/r3_identity.json").write_text(
...
print("[r8] OK_NON_IDENTICAL", flush=True)
PY

HF_LORA_REPO=${HF_LORA_REPO:-unconst/Affine-5czsc2fc98-r8-lora}
HF_MERGED_REPO=${HF_MERGED_REPO:-unconst/Affine-5czsc2fc98-r8-merged}
HF_BASE_HUB=${HF_BASE_HUB:-Tok331102/affine-5EqYW8McUc-af10}
SEEN_MID=${SEEN_MID:-/root/r8/mid_ckpt_salvaged.txt}
if [[ -n "${HF_TOKEN:-}" ]]; then
  if [[ -f "$SEEN_MID" ]] && grep -qx "adapter-final" "$SEEN_MID"; then
  ...
  elif pgrep -f "r8-reinforce-reason/mid_ckpt_salvage.sh" >/dev/null 2>&1; then
    ...
    nohup python3 /root/mining_src/s4-h1-sft/salvage_adapter.py \
      ...
      --out-meta /root/affine_data/r3_adapter_salvage.json \
      >>/root/logs/r3_push_adapter.nohup 2>&1 &
  ...
  nohup python3 /root/mining_src/s4-h1-sft/push_merged.py \
    ...
    --out-meta /root/affine_data/r8_merged_salvage.json \
    >>/root/logs/r3_push_merged.nohup 2>&1 &
...
if [[ "$code_t" != "200" || "$code_k" != "200" ]]; then
  ...
  echo "aborted_engines_unhealthy $(date -u +%Y-%m-%dT%H:%M:%SZ)" > /root/logs/r8_pipeline.aborted
  exit 1
fi
...
date -u +%Y-%m-%dT%H:%M:%SZ > /root/logs/r8_chall_serve.done
...
if (( dead - now < 2400 )); then
  ...
  echo "aborted_no_n80_budget $(date -u +%Y-%m-%dT%H:%M:%SZ)" > /root/logs/r8_pipeline.aborted
  exit 1
fi
...
for attempt in $(seq 1 "$N80_MAX_ATTEMPTS"); do
  ...
  rm -f "$SIM_N80" "$PROG" /root/logs/r8_sim_n80.done
...
date -u +%Y-%m-%dT%H:%M:%SZ > /root/logs/r8_sim_n80.done
date -u +%Y-%m-%dT%H:%M:%SZ > /root/logs/r8_pipeline.done
