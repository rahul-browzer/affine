#!/usr/bin/env bash
# Host → mine-f6-1: upload stack + data, start bootstrap under nohup.
set -euo pipefail

ROOT=/home/const/subnet120
DST_HOST=${DST_HOST:?set DST_HOST}
DST_PORT=${DST_PORT:?set DST_PORT}
KNOWN=${KNOWN:-/tmp/mine-f6-1.known_hosts}
...
STAGE=$(mktemp -d /tmp/mine-h101-stack.XXXXXX)
...
TAR=/tmp/mine-h101-stack.tar.gz
tar -C "$STAGE" -czf "$TAR" .
ls -lh "$TAR"

ENV_TMP=$(mktemp /tmp/mine-h101.env.XXXXXX)
...
# H101/F6 data = ultrashort≤80 format of high-Λ2 z_A (1058 ex); init = Tok331102 king.
DATA="$ROOT/mining/experiments/s4-h101-f6-short-format/results/za_ultrashort80.jsonl"
test -s "$DATA"
test "$(wc -l <"$DATA")" -ge 300

"${SSH[@]}" 'mkdir -p /root/mining_src /root/affine_data /root/logs /root/h101 /root/hf'
"${SCP[@]}" "$TAR" "root@${DST_HOST}:/tmp/mine-h101-stack.tar.gz"
...
for repo in (
    "unconst/Affine-5czsc2fc98-h101-lora",
    "unconst/Affine-5czsc2fc98-h101-merged",
):
    try:
...
"${SSH[@]}" 'set -e
  tar -C /root/mining_src -xzf /tmp/mine-h101-stack.tar.gz
  ...
  nohup bash /root/mining_src/s4-h101-f6-short-format/bootstrap_h101.sh \
    >/root/logs/h101_pipeline.nohup 2>&1 &
  echo $! > /root/logs/h101_pipeline.pid
  nohup bash /root/mining_src/s4-h2-merge/watch_form_decision.sh h101 \
    ...
    >/root/logs/h101_form_decision.launch.out 2>&1 &
  ...
  nohup bash /root/mining_src/s4-h101-f6-short-format/watch_preempt_bare_tcache_pass264.sh \
    >/root/logs/h101_preempt_bare_pass264.launch.nohup 2>&1 &
  echo $! > /root/logs/h101_preempt_bare_pass264.pid
  echo PREEMPT_PID=$(cat /root/logs/h101_preempt_bare_pass264.pid)
  echo PIPELINE_PID=$(cat /root/logs/h101_pipeline.pid)
  sleep 3
  head -n 40 /root/logs/bootstrap_h101.log 2>/dev/null || head -n 40 /root/logs/h101_pipeline.nohup || true
  ps -p "$(cat /root/logs/h101_pipeline.pid)" -o pid,etime,cmd || true
'
