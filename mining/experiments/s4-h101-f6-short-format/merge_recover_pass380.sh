#!/usr/bin/env bash
# H101/F6 pass380: GPU merge save hung on gocryptfs request_wait_answer
# (same as H95 p352 / H100/F4 p376). Kill hung merge+post_train, relaunch
# --device-map cpu, then resume post_train SKIP_MERGE=1. T/K already live.
set -euo pipefail

EXP=/root/mining_src/s4-h101-f6-short-format
LOG=/root/logs/h101_merge_recover_pass380.nohup
PIDF=/root/logs/h101_merge_recover_pass380.pid
MERGED=/root/h101/merged
ADAPTER=/root/h101/train/adapter
BASE=/root/hf/hub/models--Tok331102--affine-5EqYW8McUc-af10/snapshots/eb8bf9a356a254f71faaa439e8abc3cfba572c53

log() { echo "[h101-merge380] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }

mkdir -p /root/logs /root/affine_data
: >"$LOG"
echo $$ >"$PIDF"

MERGE_PID=$(ps -eo pid,cmd | awk '/[p]ython .*merge_lora.py --base .*h101\/merged/{print $1; exit}')
POST_PID=$(ps -eo pid,cmd | awk '/[b]ash .*s4-h101-f6-short-format\/post_train_pipeline\.sh/{print $1; exit}')
...
if [[ -x "$EXP/watch_preempt_bare_tcache_pass264.sh" ]]; then
  nohup bash "$EXP/watch_preempt_bare_tcache_pass264.sh" \
    >>/root/logs/h101_preempt_bare_pass264.log 2>&1 &
  echo $! >/root/logs/h101_preempt_bare_pass264.pid
  log "rearmed preempt264 pid=$(cat /root/logs/h101_preempt_bare_pass264.pid)"
fi

export SKIP_MERGE=1
export MERGE_DEVICE_MAP=cpu
export CUDA_VISIBLE_DEVICES=6,7
export SOFT_DEADLINE_UTC=2026-08-09T07:42:00Z
export DEADMAN_UTC=2026-08-09T08:12:00Z
nohup bash "$EXP/post_train_pipeline.sh" \
  >>/root/logs/h101_pipeline.nohup 2>&1 &
echo $! >/root/logs/h101_post_train.pid
log "post_train resumed pid=$(cat /root/logs/h101_post_train.pid) SKIP_MERGE=1"
log "DONE recover380"
