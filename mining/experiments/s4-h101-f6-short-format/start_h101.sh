#!/usr/bin/env bash
# H101/F6: Tok-init thought-only LoRA on ultrashort<=80 format of high-L2 z_A (1058).
set -euo pipefail
...
n=$(wc -l <"$DATA")
echo "[h101] $(date -u +%Y-%m-%dT%H:%M:%SZ) examples=$n data=$DATA base=$BASE gpus=$CUDA_VISIBLE_DEVICES"
test "$n" -ge 200

echo "[h101] verify thought cuts (CPU)"
...
echo "[h101] $(date -u +%Y-%m-%dT%H:%M:%SZ) launch thought-only train lr=5e-6 r=16 on GPUs $CUDA_VISIBLE_DEVICES"
...
meta = {
    ...
    "out": "$TRAIN_DIR",
    "log": "$LOG",
    "note": "F6 Tok-init x ultrashort<=80 format of high-L2 z_A @r16/a32 (format/length axis) vs Tok331102",
}
Path("/root/affine_data/h101_train_launched.json").write_text(json.dumps(meta, indent=2) + "\n")
print(json.dumps(meta, indent=2))
PY
echo "[h101] TRAIN_LAUNCHED pid=$(cat /root/logs/h101_train.pid)"
