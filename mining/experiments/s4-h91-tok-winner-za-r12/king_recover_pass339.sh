#!/usr/bin/env bash
# Pass 339: king-only relaunch after Triton ENOENT on bare cache/king
# (FHFMBKEV…/__triton_launcher.so @16:56:12Z → EngineDead; :8001=000).
# Isolated TCACHE + util=0.72 (bare→ENOENT again; util0.80→OOM H80 p314).
# Merge on GPUs 6,7 + post_train live — leave them alone; chall not up yet.
set -euo pipefail
...
LOGN=/root/logs/h91_king_recover_pass339.nohup
log() { echo "[king339-h91] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOGN"; }

log "START king-only recover (p339 isolated util=0.72 after bare-cache ENOENT @16:56:12Z; leave merge/post_train/teacher)"
...
for line in apps.strip().splitlines():
    ...
    if len(parts) >= 2 and parts[0] in uuids:
        try:
            ...
            pass
print(f"[king339-h91] reap gpus2,3 pids={sorted(pids)}")
for pid in pids:
    try:
...
# Also kill hung APIServer if pid file stale but process on :8001 still listed
python - <<'PY'
...
for line in subprocess.check_output(["ps", "aux"], text=True).splitlines():
    if "vllm" in line and "serve" in line and "--port 8001" in line:
        ...
        print(f"[king339-h91] kill leftover :8001 pid={pid}")
        try:
            os.kill(pid, signal.SIGTERM)
        except OSError:
            pass
...
TCACHE="/root/.triton/isolated/h91_king_p339_$(date +%s)_$$"
mkdir -p "$TCACHE"
echo "$TCACHE" >/root/logs/h91_king_tcache_pass339.path
...
for line in out.strip().splitlines():
    ...
    if int(idx) in (2, 3):
        print(f"[king339-h91] gpu{idx} used_mib={used}")
        if float(used) < 500:
            ok += 1
if ok < 2:
    print("[king339-h91] ABORT: gpus 2,3 not free")
    sys.exit(1)
PY
...
for i in $(seq 1 180); do
  ...
  # Health + /v1/models only during settle — no completions probe (H85 CUDA illegal-access).
  ...
  if [[ "$code" == "200" ]]; then
    ...
    if [[ -n "$mid" ]]; then
      ...
      date -u +%Y-%m-%dT%H:%M:%SZ >/root/logs/h91_king_recover_pass339.done
      echo "$REPO $REV $TCACHE util=0.72" >>/root/logs/h91_king_recover_pass339.done
