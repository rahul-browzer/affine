...
LOG=/root/logs/h91_chall_recover_pass344.log
CHALL_LOG=/root/logs/vllm_chall.log
PIDF=/root/logs/vllm_chall.pid
MERGE=/root/h91/merged
export HYP=h91
export GPUS=4,5
UTIL=0.72
...
log() { echo "[recover344-h91-chall] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }

log "START chall relaunch (p344: seed live king isolated TCACHE + early-abort launcher ENOENT; outer×3 post-diverse freeze)"

ts=$(date -u +%Y%m%dT%H%M%SZ)
for f in h91_decision.json h91_sim_result.json h91_sim_result_artifact.json \
         h91_sim_progress.json; do
  if [[ -f "/root/affine_data/$f" ]]; then
    mv "/root/affine_data/$f" "$QDIR/${f%.json}_pass344_${ts}.json"
    log "quarantine $f → $QDIR"
  fi
done
rm -f /root/logs/h91_n80.done /root/logs/h91_n80_retry.aborted \
  /root/logs/h91_pipeline.aborted /root/logs/h91_chall_serve.done \
  /root/logs/h91_sim_n80.done /root/logs/h91_chall_freeze_pass247.done \
  /root/logs/h91_chall_freeze_pass251.done \
  /root/logs/h91_chall_freeze_pass237.done \
  /root/logs/h91_chall_freeze_pass239.done \
  /root/logs/h91_chall_freeze_pass240.done \
  /root/logs/h91_chall_freeze_pass264.done
...
while read -r pid; do
  ...
  /[r]un_sim_duel\.py/ && /local-h91/ {print $1}
  /[r]etry_h91_n80\.sh/ {print $1}
')
...
print(f"[recover344-h91-chall] reap gpu={sorted(want)} workers={sorted(pids)} parents={sorted(parents)} grandparents={sorted(grandparents)}")
for pid in kill_set:
    try:
...
wipe_caches() {
  ...
  chmod -R u+w /root/.triton/isolated/h91_chall_* 2>/dev/null || true
  rm -rf /root/.triton/cache/chall /root/.triton/cache/chall_* || true
  rm -rf /root/.triton/isolated/h91_chall_* || true
  rm -rf /root/.cache/flashinfer/cached_ops/sampling || true
  rm -rf /root/.cache/vllm/torch_compile_cache || true
  rm -rf /tmp/torchinductor_* /root/.cache/torch/inductor /root/.cache/torchinductor_chall_* /root/.cache/torchinductor_h91_* || true
...
for attempt in 1 2 3; do
  ...
  wipe_caches
  TAG=h91_chall_p260_a${attempt}_$(date +%s)_$$
  ...
  if [[ -z "${KING_SEED:-}" || ! -d "$KING_SEED" ]]; then
    KING_SEED=$(ls -td /root/.triton/isolated/h91_king_* 2>/dev/null | head -1 || true)
  fi
  if [[ -z "${KING_SEED:-}" || ! -d "$KING_SEED" ]]; then
...
while read -r pid; do
  ...
  sleep 1
  kill -9 "$pid" 2>/dev/null || true
done < <(ps -eo pid,args | awk '/[w]atch_n80_retry\.sh/ && / h91 / {print $1}')
while read -r pid; do
  [[ -n "${pid:-}" ]] || continue
  ...
  sleep 1
  kill -9 "$pid" 2>/dev/null || true
done < <(ps -eo pid,args | awk '/[r]etry_h91_n80\.sh/ {print $1}')
while read -r pid; do
  [[ -n "${pid:-}" ]] || continue
  ...
  sleep 1
  kill -9 "$pid" 2>/dev/null || true
done < <(ps -eo pid,args | awk '/[w]atch_form_decision\.sh/ && / h91 / {print $1}')
...
nohup bash /root/mining_src/s4-h2-merge/watch_form_decision.sh h91 \
  /root/affine_data/h91_sim_result.json \
  /root/affine_data/h91_decision.json \
  /root/logs/h91_form_decision.nohup \
  >/root/logs/h91_form_decision.launch.nohup 2>&1 &
echo $! >/root/logs/h91_form_decision.pid
log "rearmed form pid=$(cat /root/logs/h91_form_decision.pid)"

nohup bash /root/mining_src/s4-h2-merge/watch_n80_retry.sh h91 \
  /root/mining_src/s4-h91-tok-winner-za-r12/retry_h91_n80.sh \
  >/root/logs/h91_watch_retry.launch.nohup 2>&1 &
echo $! >/root/logs/h91_watch_retry.pid
log "rearmed watcher pid=$(cat /root/logs/h91_watch_retry.pid)"
date -u +%Y-%m-%dT%H:%M:%SZ > /root/logs/h91_chall_serve.done
echo "TCACHE=$TCACHE mode=$(stat -c %a $TCACHE) probe=200 attempts_ok post_diverse_freeze=1 king_seed=1 seed_src=live_king_isolated" \
  > /root/logs/h91_chall_freeze_pass264.done
echo "TCACHE=$TCACHE mode=$(stat -c %a $TCACHE) probe=200 attempts_ok post_diverse_freeze=1 king_seed=1 seed_src=live_king_isolated" \
  > /root/logs/h91_chall_freeze_pass344.done
