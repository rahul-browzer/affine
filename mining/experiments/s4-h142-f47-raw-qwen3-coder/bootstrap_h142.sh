#!/usr/bin/env bash
# mine-f47-1: raw Qwen3-Coder chall (no train) vs Tok king — F47/H142 screen.
set -euo pipefail

LOG=/root/logs/bootstrap_h142.log
mkdir -p /root/logs /root/hf /root/affine_data /root/mining_src /root/h142
exec > >(tee -a "$LOG") 2>&1

echo "[bootstrap-h142] $(date -u +%Y-%m-%dT%H:%M:%SZ) start host=$(hostname)"
...
if [[ -z "${HF_TOKEN:-}" ]]; then
  echo "[bootstrap-h142] FATAL: HF_TOKEN missing in /root/mine.env"
  exit 1
fi

nvidia-smi -L || true
NGPU=$(nvidia-smi -L | wc -l)
echo "[bootstrap-h142] GPU_COUNT=$NGPU"
test "$NGPU" -ge 8
...
uv pip install \
  ...
  "numpy" \
  "scipy" \
  2>&1 | tee /root/logs/pip_h142.log | tail -40

python - <<'PY'
import torch, transformers, vllm
print("[bootstrap-h142] VERSIONS",
...
if p.is_file():
    ...
    if txt != orig:
        p.write_text(txt)
        print("[bootstrap-h142] patched vllm_client timeout=480 retries=5", flush=True)
    else:
        print("[bootstrap-h142] vllm_client already patched or pattern miss", flush=True)
PY

# Download chall=coder, king=Tok, teacher — then serve + stamp chall ready.
python - <<'PY'
import os
from pathlib import Path
...
print("[bootstrap-h142] DOWNLOAD coder chall start", flush=True)
gpath = snapshot_download(
    "Qwen/Qwen3-Coder-30B-A3B-Instruct",
    revision="b2cff646eb4bb1d68355c01b18ae02e7cf42d120",
    token=token,
)
print(f"[bootstrap-h142] DOWNLOAD coder done -> {gpath}", flush=True)
chall = Path("/root/h142/chall")
...
Path("/root/logs/coder_chall.done").write_text(gpath + "\n")
# retry script expects merge.done for legacy gate — stamp it (no merge occurred)
Path("/root/logs/h142_merge.done").write_text(
    "RAW_QWEN3_CODER_NO_MERGE " + gpath + "\n"
)
print("[bootstrap-h142] DOWNLOAD teacher start", flush=True)
tpath = snapshot_download("zai-org/GLM-4.5-Air-FP8", token=token)
print(f"[bootstrap-h142] DOWNLOAD teacher done -> {tpath}", flush=True)
Path("/root/logs/teacher.done").write_text(tpath + "\n")
print("[bootstrap-h142] DOWNLOAD tok331102 king start", flush=True)
kpath = snapshot_download(
    "Tok331102/affine-5EqYW8McUc-af10",
    revision="eb8bf9a356a254f71faaa439e8abc3cfba572c53",
    token=token,
)
print(f"[bootstrap-h142] DOWNLOAD tok331102 done -> {kpath}", flush=True)
Path("/root/logs/tok331102.done").write_text(kpath + "\n")
print("[bootstrap-h142] ALL_DOWNLOADS_OK", flush=True)
PY

bash /root/mining_src/s3-duel-sim/sync_corpus.sh || true

echo "[bootstrap-h142] $(date -u +%Y-%m-%dT%H:%M:%SZ) serving teacher+Tok+coder"
...
for i in $(seq 1 240); do
  ...
  if [[ "$code" == "200" ]]; then
    echo "chall_serve_ok poll=$i $(date -u +%Y-%m-%dT%H:%M:%SZ)" \
      | tee /root/logs/h142_chall_serve.done
    break
  fi
  sleep 15
done
...
echo "[bootstrap-h142] $(date -u +%Y-%m-%dT%H:%M:%SZ) DONE — engines up; n80 watcher owns screen"
