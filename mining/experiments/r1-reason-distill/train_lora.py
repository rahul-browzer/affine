#!/usr/bin/env python3
"""R1 LoRA SFT: thought-only (or full) loss to raise Reason under v3.

Train z_A thoughts that raise teacher lpC(y_C|z) - lpC(y_C|empty).
Default --loss-on thought masks labels from the first bash fence onward
so we fit thoughts, not actions. Runs on free GPUs (default 6,7) while
teacher/king/chall vLLM stay on 0-5.
"""
