...
for i in $(seq 1 1440); do
  ...
  if (( i % 12 == 0 )); then
    ...
    crumb=$(tail -n 5 /root/logs/r1c_train.log 2>/dev/null | tr '\r' '\n' | grep -E "'loss'|step|%|" | tail -1 || true)
    echo "[r1c-merge] wait-train iter=$i $(date -u +%Y-%m-%dT%H:%M:%SZ) crumb=${crumb:-none}"
  fi
  if (( i == 1440 )); then
    echo "[r1c-merge] TIMEOUT waiting for train" >&2
    exit 2
  fi
  sleep 10
done
...
if [[ -x "${_CU13}/bin/nvcc" && -f "${_CU13}/include/cuda_fp16.h" ]]; then
  ...
  echo "[r1c-merge] CUDA_HOME=$CUDA_HOME"
else
  echo "[r1c-merge] FATAL no nvcc under ${_CU13}" >&2
  exit 2
fi
if [[ -L /usr/local/cuda ]]; then
  rm -f /usr/local/cuda
  echo "[r1c-merge] removed /usr/local/cuda symlink"
fi

BASE=${BASE:-/root/hf/hub/models--Tok331102--affine-5EqYW8McUc-af10/snapshots/eb8bf9a356a254f71faaa439e8abc3cfba572c53}
MERGED=${MERGED:-/root/r1_out/r1c_lora_merged}
LINK=${LINK:-/tmp/r1c_lora_merged}

if [[ ! -d "$BASE" ]]; then
  BASE=$(ls -d /root/hf/hub/models--Tok331102--affine-5EqYW8McUc-af10/snapshots/* 2>/dev/null | head -1 || true)
fi
if [[ -z "$BASE" || ! -d "$BASE" ]]; then
  echo "[r1c-merge] FATAL missing Tok base" >&2
  exit 2
fi
if [[ ! -f "$ADAPTER/adapter_config.json" ]]; then
  echo "[r1c-merge] FATAL missing adapter at $ADAPTER" >&2
  exit 2
fi

# Merge on GPUs 6,7 (freed by train). merge_lora.py grafts visual + copies MM config.
export CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-6,7}
echo "[r1c-merge] merging adapter → $MERGED"
rm -rf "$MERGED"
python /root/mining_src/r1-reason-distill/merge_lora.py \
  ...
  --out "$MERGED"
ln -sfn "$MERGED" "$LINK"
echo "[r1c-merge] link $LINK -> $(readlink -f "$LINK")"
...
if [[ -f "$CHALL_PID_FILE" ]]; then
  ...
  if [[ -n "${CPID:-}" ]] && kill -0 "$CPID" 2>/dev/null; then
    echo "[r1c-merge] stopping chall pid=$CPID"
    kill "$CPID" || true
    ...
    if kill -0 "$CPID" 2>/dev/null; then
      echo "[r1c-merge] chall still alive; kill -9 $CPID"
      kill -9 "$CPID" || true
    fi
  fi
fi
...
if [[ ! -d /root/.triton/cache/chall ]] || [[ -z "$(find /root/.triton/cache/chall -name '*.so' 2>/dev/null | head -1)" ]]; then
  if [[ -d /root/.triton/cache/king ]]; then
    echo "[r1c-merge] seeding chall Triton cache from king"
...
echo "[r1c-merge] launching chall :8002 on $LINK"
...
echo "[r1c-merge] chall pid=$(cat /root/logs/vllm_chall.pid)"
...
for i in $(seq 1 480); do
  ...
  if [[ "$c0$c1$c2" == "200200200" ]]; then
    echo "[r1c-merge] engines 200/200/200 at iter=$i $(date -u +%Y-%m-%dT%H:%M:%SZ)"
    break
  fi
  if (( i % 12 == 0 )); then
    echo "[r1c-merge] wait-engines iter=$i codes=${c0}/${c1}/${c2}"
  fi
  if (( i == 480 )); then
    echo "[r1c-merge] TIMEOUT engines; see vllm_chall.log" >&2
    exit 2
  fi
  sleep 5
done

# Fresh n80 vs live king with R1c LoRA-merged chall.
OUT=/root/affine_data/r1c_lora_reason_sim.json
DEC=/root/affine_data/r1c_lora_decision.json
PROG=/root/affine_data/r1c_lora_reason_progress.json
rm -f "$OUT" "$DEC" "$PROG"
BH=$(python - <<'PY'
import hashlib, time
print(hashlib.sha256(f"r1c-lora-{time.time_ns()}".encode()).hexdigest())
PY
)

echo "[r1c-merge] launching R1c n80 block_hash=${BH:0:16}…"
python /root/mining_src/r1-reason-distill/run_reason_sim.py \
  --n-turns 80 \
  --block-hash "$BH" \
  --hotkey "local-r1c-lora-$(date -u +%Y%m%dT%H%M%SZ)" \
  ...
  --progress-out "$PROG" \
  --save-artifact \
  2>&1 | tee /root/logs/r1c_lora_reason_sim.log

python /root/mining_src/r1-reason-distill/write_reason_decision.py \
  --sim-result "$OUT" --out "$DEC" \
  2>&1 | tee -a /root/logs/r1c_lora_reason_sim.log

echo "[r1c-merge] DONE $(date -u +%Y-%m-%dT%H:%M:%SZ)"
cat "$DEC"
touch /root/logs/r1c_merge_reload.done
