#!/usr/bin/env bash
# After H128 train.done: finalize visual → chall:8002 → n80 vs Tok331102 king.
set -euo pipefail
...
TRAIN_DIR=${TRAIN_DIR:-/root/h128/train}
# Prefer /tmp staging — gocryptfs /root copytree hangs (WCHAN=request_wait_answer; p472).
FULL_FT=${FULL_FT:-/tmp/h128_full_ft_save}
if [[ ! -d "$FULL_FT" ]]; then
  FULL_FT=$TRAIN_DIR/full_ft
fi
MERGED=${MERGED:-/tmp/h128_merged}
MERGED_LINK=${MERGED_LINK:-/root/h128/merged}
SIM_N80=/root/affine_data/h128_sim_result.json
PROG=/root/affine_data/h128_sim_progress.json
LOG=/root/logs/h128_pipeline.nohup
...
log() { echo "[h128-pipe] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }

_train_alive() {
  if [[ -f /root/logs/h128_train.pid ]]; then
    local tpid
    tpid=$(cat /root/logs/h128_train.pid 2>/dev/null || true)
    if [[ -n "${tpid:-}" ]] && kill -0 "$tpid" 2>/dev/null; then
      return 0
    fi
  fi
  pgrep -f "python3 /root/mining_src/s4-h128-f33-pandora-full-ft/train_full.py --base" >/dev/null 2>&1
}
...
_abort_on_exit() {
  ...
  if [[ -f /root/logs/h128_pipeline.done ]]; then
    return 0
  fi
  if [[ ! -f /root/logs/h128_pipeline.aborted ]]; then
    echo "aborted_err_rc=${rc} $(date -u +%Y-%m-%dT%H:%M:%SZ)" \
      >/root/logs/h128_pipeline.aborted
    echo "[h128-pipe] $(date -u +%Y-%m-%dT%H:%M:%SZ) EXIT trap wrote aborted_err_rc=${rc}" \
      | tee -a "$LOG" >/dev/null 2>&1 || true
  fi
}
trap _abort_on_exit EXIT

mkdir -p /root/logs /root/affine_data /root/h128
rm -f /root/logs/h128_pipeline.aborted /root/logs/h128_pipeline.done \
  /root/logs/h128_merge.done /root/logs/h128_chall_serve.done \
  /root/logs/h128_sim_n80.done
...
while true; do
  ...
  if (( now > soft - 3600 )); then
    ...
    echo "aborted_no_train $(date -u +%Y-%m-%dT%H:%M:%SZ)" > /root/logs/h128_pipeline.aborted
    exit 1
  fi
  _wait_i=$((_wait_i + 1))
...
if _train_alive; then
  ...
  echo "aborted_train_stuck $(date -u +%Y-%m-%dT%H:%M:%SZ)" > /root/logs/h128_pipeline.aborted
  exit 1
fi
sleep 15
...
if [[ ! -d "$FULL_FT" ]]; then
  ...
  echo "aborted_no_full_ft $(date -u +%Y-%m-%dT%H:%M:%SZ)" >/root/logs/h128_pipeline.aborted
  exit 1
fi
...
if [[ "${SKIP_MERGE:-0}" == "1" && -f "$MERGED/config.json" ]] \
  ...
  rm -rf "$MERGED"
  python3 /root/mining_src/s4-h128-f33-pandora-full-ft/finalize_full_ft.py \
    --base "$BASE" \
    --full-ft "$FULL_FT" \
    --out "$MERGED" \
    --king "$KING_LOCAL" \
...
cp -f "$MERGED/finalize_meta.json" /root/affine_data/h128_finalize_meta.json 2>/dev/null || true
date -u +%Y-%m-%dT%H:%M:%SZ > /root/logs/h128_merge.done

HF_MERGED_REPO=${HF_MERGED_REPO:-unconst/Affine-5czsc2fc98-h128-pandorafullft}
if [[ -n "${HF_TOKEN:-}" ]]; then
  ...
  nohup python3 /root/mining_src/s4-h1-sft/push_merged.py \
    ...
    --repo "$HF_MERGED_REPO" \
    --public \
    --commit-message "H128 pandora-box full-FT high-Λ2 salvage (TTL insurance; not a submission)" \
    --out-meta /root/affine_data/h128_merged_salvage.json \
    >>/root/logs/h128_push_merged.nohup 2>&1 &
  echo $! >/root/logs/h128_push_merged.pid
fi
...
if [[ -x /root/mining_src/s4-h128-f33-pandora-full-ft/watch_preempt_bare_tcache_pass264.sh ]]; then
  nohup bash /root/mining_src/s4-h128-f33-pandora-full-ft/watch_preempt_bare_tcache_pass264.sh \
    >/root/logs/h128_preempt.nohup 2>&1 &
  echo $! >/root/logs/h128_preempt.pid
fi

date -u +%Y-%m-%dT%H:%M:%SZ > /root/logs/h128_chall_serve.done
log "CHALL_SERVE_DONE — n80 owned by watch_n80_retry / retry_h128_n80_d203first"
date -u +%Y-%m-%dT%H:%M:%SZ > /root/logs/h128_pipeline.done
