#!/usr/bin/env bash
# Host → mine-h22-1: upload harness + mine.env, start bootstrap+n80 under nohup.
set -euo pipefail

ROOT=/home/const/subnet120
DST_HOST=${DST_HOST:?set DST_HOST}
DST_PORT=${DST_PORT:?set DST_PORT}
KNOWN=${KNOWN:-/tmp/mine-h22-1.known_hosts}
...
STAGE=$(mktemp -d /tmp/mine-h22-stack.XXXXXX)
trap 'rm -rf "$STAGE"' EXIT

mkdir -p "$STAGE/affine_pkg/affine" "$STAGE/affine_pkg/evalsrv" \
         "$STAGE/s3-duel-sim" "$STAGE/s4-h2-merge" "$STAGE/s4-h22-tp-kevin-a90"
...
cp -a "$ROOT/mining/experiments/s4-h22-tp-kevin-a90/bootstrap_h22.sh" "$STAGE/s4-h22-tp-kevin-a90/"
cp -a "$ROOT/mining/experiments/s4-h22-tp-kevin-a90/start_h22_n80.sh" "$STAGE/s4-h22-tp-kevin-a90/"
cp -a "$ROOT/mining/experiments/s4-h22-tp-kevin-a90/retry_h22_n80.sh" "$STAGE/s4-h22-tp-kevin-a90/"
cp -a "$ROOT/mining/experiments/s4-h22-tp-kevin-a90/plan.md" "$STAGE/s4-h22-tp-kevin-a90/"

TAR=/tmp/mine-h22-stack.tar.gz
tar -C "$STAGE" -czf "$TAR" .
ls -lh "$TAR"

ENV_TMP=$(mktemp /tmp/mine-h22.env.XXXXXX)
...
"${SCP[@]}" "$TAR" "root@${DST_HOST}:/tmp/mine-h22-stack.tar.gz"
"${SCP[@]}" "$ENV_TMP" "root@${DST_HOST}:/root/mine.env"
rm -f "$ENV_TMP"

"${SSH[@]}" 'set -e
  tar -C /root/mining_src -xzf /tmp/mine-h22-stack.tar.gz
  ...
  chmod +x /root/mining_src/s3-duel-sim/*.sh \
           ...
           /root/mining_src/s4-h22-tp-kevin-a90/*.sh
  test -f /root/mining_src/affine_pkg/affine/score.py
  test -x /root/mining_src/s4-h22-tp-kevin-a90/bootstrap_h22.sh
  test -x /root/mining_src/s4-h22-tp-kevin-a90/start_h22_n80.sh
  test -x /root/mining_src/s4-h22-tp-kevin-a90/retry_h22_n80.sh
  echo STACK_UPLOAD_OK
  nohup bash -lc "
    set -euo pipefail
    bash /root/mining_src/s4-h22-tp-kevin-a90/bootstrap_h22.sh
    bash /root/mining_src/s4-h22-tp-kevin-a90/start_h22_n80.sh
  " >/root/logs/h22_pipeline.nohup 2>&1 &
  echo $! > /root/logs/h22_pipeline.pid
  nohup bash /root/mining_src/s4-h2-merge/watch_fix_decision.sh h22 \
    >/root/logs/h22_fix_decision.nohup 2>&1 &
  echo $! > /root/logs/h22_fix_decision.pid
  nohup bash /root/mining_src/s4-h2-merge/watch_n80_retry.sh h22 \
    /root/mining_src/s4-h22-tp-kevin-a90/retry_h22_n80.sh \
    >/root/logs/h22_watch_retry.nohup 2>&1 &
  echo $! > /root/logs/h22_watch_retry.pid
  echo PIPELINE_PID=$(cat /root/logs/h22_pipeline.pid)
  sleep 2
  head -n 20 /root/logs/h22_pipeline.nohup || true
  ps -p "$(cat /root/logs/h22_pipeline.pid)" -o pid,etime,cmd || true
'
