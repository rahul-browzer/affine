#!/usr/bin/env bash
# Pass 241: p240 a1 still raced at warmup #1 — RuntimeError ENOENT on
...
log() { echo "[recover241-h51-chall] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }

log "START chall relaunch (outer×3 + king-seed + pre-freeze before w1; p240 a1_w1 ENOENT)"

ts=$(date -u +%Y%m%dT%H%M%SZ)
for f in h51_decision.json h51_sim_result.json h51_sim_result_artifact.json \
         h51_sim_progress.json; do
  if [[ -f "/root/affine_data/$f" ]]; then
    mv "/root/affine_data/$f" "$QDIR/${f%.json}_pass241_${ts}.json"
    log "quarantine $f → $QDIR"
  fi
done
rm -f /root/logs/h51_n80.done /root/logs/h51_n80_retry.aborted \
  /root/logs/h51_pipeline.aborted /root/logs/h51_chall_serve.done \
  /root/logs/h51_sim_n80.done /root/logs/h51_chall_freeze_pass237.done \
  /root/logs/h51_chall_freeze_pass239.done \
  /root/logs/h51_chall_freeze_pass240.done
...
while read -r pid; do
  ...
  /[r]un_sim_duel\.py/ && /local-h51/ {print $1}
  /[r]etry_h51_n80\.sh/ {print $1}
')
...
print(f"[recover240-h51-chall] reap gpu={sorted(want)} workers={sorted(pids)} parents={sorted(parents)} grandparents={sorted(grandparents)}")
for pid in kill_set:
    try:
...
wipe_caches() {
  ...
  chmod -R u+w /root/.triton/isolated/h51_chall_* 2>/dev/null || true
  rm -rf /root/.triton/cache/chall /root/.triton/cache/chall_* || true
  rm -rf /root/.triton/isolated/h51_chall_* || true
  rm -rf /root/.cache/flashinfer/cached_ops/sampling || true
  rm -rf /root/.cache/vllm/torch_compile_cache || true
  rm -rf /tmp/torchinductor_* /root/.cache/torch/inductor /root/.cache/torchinductor_chall_* /root/.cache/torchinductor_h51_* || true
...
_comp() {
  ...
  code=$(curl -s -o "/tmp/h51_warmup_${label}.json" -w "%{http_code}" --max-time 120 \
...
for attempt in 1 2 3; do
  ...
  wipe_caches
  TAG=h51_chall_p241_a${attempt}_$(date +%s)_$$
...
while read -r pid; do
  ...
  sleep 1
  kill -9 "$pid" 2>/dev/null || true
done < <(ps -eo pid,args | awk '/[w]atch_n80_retry\.sh/ && / h51 / {print $1}')
while read -r pid; do
  [[ -n "${pid:-}" ]] || continue
  ...
  sleep 1
  kill -9 "$pid" 2>/dev/null || true
done < <(ps -eo pid,args | awk '/[r]etry_h51_n80\.sh/ {print $1}')
while read -r pid; do
  [[ -n "${pid:-}" ]] || continue
  ...
  sleep 1
  kill -9 "$pid" 2>/dev/null || true
done < <(ps -eo pid,args | awk '/[w]atch_form_decision\.sh/ && / h51 / {print $1}')
sleep 1

nohup bash /root/mining_src/s4-h2-merge/watch_form_decision.sh h51 \
  /root/affine_data/h51_sim_result.json \
  /root/affine_data/h51_decision.json \
  /root/logs/h51_form_decision.nohup \
  >/root/logs/h51_form_decision.launch.nohup 2>&1 &
echo $! >/root/logs/h51_form_decision.pid
log "rearmed form pid=$(cat /root/logs/h51_form_decision.pid)"

nohup bash /root/mining_src/s4-h2-merge/watch_n80_retry.sh h51 \
  /root/mining_src/s4-h51-m7-winner-za-a16/retry_h51_n80.sh \
  >/root/logs/h51_watch_retry.launch.nohup 2>&1 &
echo $! >/root/logs/h51_watch_retry.pid
log "rearmed watcher pid=$(cat /root/logs/h51_watch_retry.pid)"
...
echo "TCACHE=$TCACHE mode=$(stat -c %a $TCACHE) probe=200 attempts_ok prefreeze=1 king_seed=1" \
  > /root/logs/h51_chall_freeze_pass241.done
