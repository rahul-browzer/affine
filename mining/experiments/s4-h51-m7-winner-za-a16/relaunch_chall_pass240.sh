#!/usr/bin/env bash
# Pass 240: H51 p239 aborted on warmup #1 — Triton __triton_launcher.so race
...
# before first warmup (p239 fired warmup immediately → race).
set -euo pipefail
...
log() { echo "[recover240-h51-chall] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }

log "START chall relaunch (outer×3 + 45s post-health settle; p239 warmup#1 Triton.so)"
...
for f in h51_decision.json h51_sim_result.json h51_sim_result_artifact.json \
  ...
  if [[ -f "/root/affine_data/$f" ]]; then
    mv "/root/affine_data/$f" "$QDIR/${f%.json}_pass240_${ts}.json"
...
for attempt in 1 2 3; do
  ...
  wipe_caches
  TAG=h51_chall_p240_a${attempt}_$(date +%s)_$$
  ...
  # p239 raced here: warmup immediately after health. Settle so TP workers
  # finish writing launcher.so before first completion loads it.
  log "settle 45s after health before warmup (p239 race mitigation)"
  sleep 45
