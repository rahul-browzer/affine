#!/usr/bin/env bash
# Host → mine-f34-1: upload stack + data, start H129 full-FT bootstrap.
set -euo pipefail

ROOT=/home/const/subnet120
DST_HOST=${DST_HOST:?set DST_HOST}
DST_PORT=${DST_PORT:?set DST_PORT}
KNOWN=${KNOWN:-/tmp/mine-f34-1.known_hosts}
...
STAGE=$(mktemp -d /tmp/mine-h129-stack.XXXXXX)
trap 'rm -rf "$STAGE"' EXIT

EXP=s4-h129-f34-diane-full-ft
...
TAR=/tmp/mine-h129-stack.tar.gz
tar -C "$STAGE" -czf "$TAR" .
ls -lh "$TAR"

ENV_TMP=$(mktemp /tmp/mine-h129.env.XXXXXX)
...
"${SSH[@]}" 'mkdir -p /root/mining_src /root/affine_data /root/logs /root/h129 /root/hf'
"${SCP[@]}" "$TAR" "root@${DST_HOST}:/tmp/mine-h129-stack.tar.gz"
"${SCP[@]}" "$ENV_TMP" "root@${DST_HOST}:/root/mine.env"
"${SCP[@]}" "$DATA" "root@${DST_HOST}:/root/h129/winner_za_high_l2.jsonl"
rm -f "$ENV_TMP"
...
repo = "unconst/Affine-5czsc2fc98-h129-dianefullft"
try:
...
"${SSH[@]}" 'set -e
  tar -C /root/mining_src -xzf /tmp/mine-h129-stack.tar.gz
  chmod 600 /root/mine.env
  chmod +x /root/mining_src/s3-duel-sim/*.sh \
           /root/mining_src/s4-h2-merge/*.sh \
           /root/mining_src/s4-h129-f34-diane-full-ft/*.sh
  test -f /root/mining_src/affine_pkg/affine/score.py
  test -s /root/h129/winner_za_high_l2.jsonl
  test -x /root/mining_src/s4-h129-f34-diane-full-ft/bootstrap_h129.sh
  echo STACK_UPLOAD_OK
  nohup bash /root/mining_src/s4-h129-f34-diane-full-ft/bootstrap_h129.sh \
    >/root/logs/h129_pipeline.nohup 2>&1 &
  echo $! > /root/logs/h129_pipeline.pid
  nohup bash /root/mining_src/s4-h2-merge/watch_form_decision.sh h129 \
    /root/affine_data/h129_sim_result.json /root/affine_data/h129_decision.json \
    /root/logs/h129_form_decision.nohup \
    >/root/logs/h129_form_decision.launch.out 2>&1 &
  ...
  echo PIPELINE_PID=$(cat /root/logs/h129_pipeline.pid)
  sleep 5
  head -n 40 /root/logs/bootstrap_h129.log 2>/dev/null || head -n 40 /root/logs/h129_pipeline.nohup || true
  ps -p "$(cat /root/logs/h129_pipeline.pid)" -o pid,etime,cmd || true
  nvidia-smi -L | wc -l
'
