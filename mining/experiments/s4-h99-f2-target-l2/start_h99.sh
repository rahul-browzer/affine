#!/usr/bin/env bash
# H99: Tok331102/Tok-init thought-only LoRA on winner-zA high clip-L1 z_A (406→fit-filter).
set -euo pipefail
...
n=$(wc -l <"$DATA")
echo "[h99] $(date -u +%Y-%m-%dT%H:%M:%SZ) examples=$n data=$DATA base=$BASE gpus=$CUDA_VISIBLE_DEVICES"
test "$n" -ge 200

echo "[h99] verify thought cuts (CPU)"
...
echo "[h99] $(date -u +%Y-%m-%dT%H:%M:%SZ) launch thought-only train lr=5e-6 r=16 on GPUs $CUDA_VISIBLE_DEVICES"
nohup python3 "$SRC/train_lora.py" \
  ...
  --loss-on thought \
  >"$LOG" 2>&1 &
echo $! | tee /root/logs/h99_train.pid >"$OUT/train.pid"
python3 - <<PY
import json, time
...
meta = {
    ...
    "hypo": "H99",
    "pid": int(Path("/root/logs/h99_train.pid").read_text().strip()),
    "base": "$BASE",
    ...
    "out": "$TRAIN_DIR",
    "log": "$LOG",
    "note": "F2 Tok-init × high-Λ2 z_A @r16/α32 (data-axis; not clip-L1 winner-zA) vs Tok331102",
}
Path("/root/affine_data/h99_train_launched.json").write_text(json.dumps(meta, indent=2) + "\n")
print(json.dumps(meta, indent=2))
PY
echo "[h99] TRAIN_LAUNCHED pid=$(cat /root/logs/h99_train.pid)"
