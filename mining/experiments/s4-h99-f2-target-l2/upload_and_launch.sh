#!/usr/bin/env bash
# Host → mine-f2-1: upload stack + data, start bootstrap under nohup.
set -euo pipefail

ROOT=/home/const/subnet120
DST_HOST=${DST_HOST:?set DST_HOST}
DST_PORT=${DST_PORT:?set DST_PORT}
KNOWN=${KNOWN:-/tmp/mine-f2-1.known_hosts}
...
STAGE=$(mktemp -d /tmp/mine-h99-stack.XXXXXX)
...
TAR=/tmp/mine-h99-stack.tar.gz
tar -C "$STAGE" -czf "$TAR" .
ls -lh "$TAR"

ENV_TMP=$(mktemp /tmp/mine-h99.env.XXXXXX)
...
# H99/F2 data = high-Λ2 z_A harvest (1059 ex); init = Tok331102 king.
DATA="$ROOT/mining/experiments/s4-h99-f2-target-l2/results/winner_za_high_l2.jsonl"
test -s "$DATA"
test "$(wc -l <"$DATA")" -ge 300

"${SSH[@]}" 'mkdir -p /root/mining_src /root/affine_data /root/logs /root/h99 /root/hf'
"${SCP[@]}" "$TAR" "root@${DST_HOST}:/tmp/mine-h99-stack.tar.gz"
...
for repo in (
    "unconst/Affine-5czsc2fc98-h99-lora",
    "unconst/Affine-5czsc2fc98-h99-merged",
):
    try:
...
"${SSH[@]}" 'set -e
  tar -C /root/mining_src -xzf /tmp/mine-h99-stack.tar.gz
  ...
  nohup bash /root/mining_src/s4-h99-f2-target-l2/bootstrap_h99.sh \
    >/root/logs/h99_pipeline.nohup 2>&1 &
  echo $! > /root/logs/h99_pipeline.pid
  nohup bash /root/mining_src/s4-h2-merge/watch_form_decision.sh h99 \
    ...
    >/root/logs/h99_form_decision.launch.out 2>&1 &
  ...
  nohup bash /root/mining_src/s4-h99-f2-target-l2/watch_preempt_bare_tcache_pass264.sh \
    >/root/logs/h99_preempt_bare_pass264.launch.nohup 2>&1 &
  echo $! > /root/logs/h99_preempt_bare_pass264.pid
  echo PREEMPT_PID=$(cat /root/logs/h99_preempt_bare_pass264.pid)
  echo PIPELINE_PID=$(cat /root/logs/h99_pipeline.pid)
  sleep 3
  head -n 40 /root/logs/bootstrap_h99.log 2>/dev/null || head -n 40 /root/logs/h99_pipeline.nohup || true
  ps -p "$(cat /root/logs/h99_pipeline.pid)" -o pid,etime,cmd || true
'
