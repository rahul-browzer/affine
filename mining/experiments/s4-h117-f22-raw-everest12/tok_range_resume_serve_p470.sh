#!/usr/bin/env bash
# Pass 470: Range-resume frozen Tok king shards on mine-f22-1, then serve_three + n80 path.
...
for pid in $(pgrep -f 'bash /root/mining_src/s4-h117-f22-raw-everest12/bootstrap_h117.sh' || true); do
  # children
...
for i in $(seq 1 360); do
  ...
  if [[ "$code" == "200" && "$kcode" == "200" && "$tcode" == "200" ]]; then
    echo "chall_serve_ok poll=$i $(date -u +%Y-%m-%dT%H:%M:%SZ)" | tee /root/logs/h117_chall_serve.done
    break
  fi
  sleep 15
done
...
rm -f /root/logs/h117_n80_retry.aborted /root/affine_data/h117_N80_DONE /root/logs/h117_N80_DONE 2>/dev/null || true
