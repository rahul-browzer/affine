...
LOGN=/root/logs/h117_king_recover_pass478.nohup
log() { echo "[king478-h117] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOGN"; }
...
for line in apps.strip().splitlines():
    ...
    if len(parts) >= 2 and parts[0] in uuids:
        try:
            ...
            pass
print(f"[king478-h117] reap gpus2,3 pids={sorted(pids)}")
for pid in pids:
    try:
...
rm -f /root/affine_data/h117_sim_result.json /root/affine_data/h117_sim_progress.json \
  /root/affine_data/h117_decision.json /root/logs/h117_n80.done /root/logs/h117_n80_retry.aborted \
  /root/logs/h117_king_recover_pass311.done /root/logs/h117_sim_n80.done \
  /root/logs/h117_king_recover_pass332.done /root/logs/h117_king_recover_pass478.done
log "cleared stale h117 sim progress/result"

log "wipe bare king + failed p332 isolated"
rm -rf /root/.triton/cache/king
rm -rf /root/.triton/cache/king_* 2>/dev/null || true
rm -rf /root/.triton/isolated/h117_king_p332_* 2>/dev/null || true
rm -rf /root/.triton/isolated/h117_king_p478_* 2>/dev/null || true

SEED=""
# Prefer live chall TRITON_CACHE_DIR (frozen, promptable)
for pid in $(pgrep -f 'vllm serve /root/h117/chall' || true); do
...
if [[ -z "$SEED" ]]; then
  for d in /root/.triton/isolated/h117_chall_*; do
    [[ -d "$d" ]] || continue
...
TCACHE="/root/.triton/isolated/h117_king_p478_$(date +%s)_$$"
mkdir -p "$TCACHE"
# Copy contents; leave SEED frozen (do not chmod seed)
cp -a "$SEED"/. "$TCACHE/"
chmod -R u+w "$TCACHE" || true
n_so=$(find "$TCACHE" -name '__triton_launcher*.so' | wc -l)
echo "$TCACHE" >/root/logs/h117_king_tcache_pass332.path
echo "$TCACHE" >/root/logs/h117_king_tcache_pass478.path
...
print(f"[king478-h117] launcher ghosts={bad}")
...
for line in out.strip().splitlines():
    ...
    if int(idx) in (2, 3):
        print(f"[king478-h117] gpu{idx} used_mib={used}")
        if float(used) < 500:
            ok += 1
if ok < 2:
    print("[king478-h117] ABORT: gpus 2,3 not free")
    sys.exit(1)
PY
  ...
  if [[ "$code" == "200" ]]; then
    ...
    if [[ -n "$mid" ]]; then
      ...
      if [[ "$pcode" == "200" ]]; then
        ...
        date -u +%Y-%m-%dT%H:%M:%SZ >/root/logs/h117_king_recover_pass478.done
        echo "$REPO $REV $TCACHE util=0.72 seeded_from_chall" >>/root/logs/h117_king_recover_pass478.done
        # also stamp 332.done so older watchers that look for it proceed
        cp /root/logs/h117_king_recover_pass478.done /root/logs/h117_king_recover_pass332.done
        log "DONE — retry_h117_n80 should proceed (watcher already armed)"
        exit 0
      fi
    fi
  fi
