#!/usr/bin/env bash
# Host → mine-r18-sbs-grpo-1: R3 stack + R18 sbs-v2-GRPO overlays, bootstrap.
# Axis R18: pure sbs-v2 Reason-GRPO (≠ R3 Tok, ≠ R10 merge, ≠ R2ay screen).
set -euo pipefail

ROOT=/home/const/subnet120
POD_NAME=${POD_NAME:-mine-r18-sbs-grpo-1}
...
cp -a "$ROOT/mining/experiments/r18-sbs-grpo/plan.md" "$STAGE/r18-sbs-grpo/"
cp -a "$ROOT/mining/experiments/r18-sbs-grpo/start_r18.sh" "$STAGE/r18-sbs-grpo/"
cp -a "$ROOT/mining/experiments/r18-sbs-grpo/bootstrap_r18.sh" "$STAGE/r18-sbs-grpo/"
# Overlay: bootstrap_r3 + start_r3 → R18 sbs-v2 Reason-GRPO.
cp -a "$ROOT/mining/experiments/r18-sbs-grpo/start_r18.sh" "$STAGE/$EXP/start_r3.sh"
cp -a "$ROOT/mining/experiments/r18-sbs-grpo/bootstrap_r18.sh" "$STAGE/$EXP/bootstrap_r3.sh"
...
TAR=/tmp/mine-r18-stack.tar.gz
tar -C "$STAGE" -czf "$TAR" .
ls -lh "$TAR"

ENV_TMP=$(mktemp /tmp/mine-r18.env.XXXXXX)
...
umask 077
{
  ...
  echo "export HF_LORA_REPO=unconst/Affine-5czsc2fc98-r18-lora"
  echo "export HF_MERGED_REPO=unconst/Affine-5czsc2fc98-r18-merged"
  echo "export R18_AXIS=sbs_grpo_reason"
  echo "export R18_LR=5e-6"
  echo "export R18_LORA_R=16"
  echo "export R18_LORA_ALPHA=32"
  echo "export R18_GROUP_SIZE=4"
  echo "export R18_MAX_STEPS=200"
} >"$ENV_TMP"
chmod 600 "$ENV_TMP"
...
"${SSH[@]}" 'mkdir -p /root/mining_src /root/affine_data /root/logs /root/r3 /root/r18 /root/hf'
"${SCP[@]}" "$TAR" "root@${DST_HOST}:/tmp/mine-r18-stack.tar.gz"
"${SCP[@]}" "$ENV_TMP" "root@${DST_HOST}:/root/mine.env"
"${SCP[@]}" "$DATA" "root@${DST_HOST}:/root/r3/winner_za_high_l1.jsonl"
"${SCP[@]}" "$DATA" "root@${DST_HOST}:/root/r18/winner_za_high_l1.jsonl"
rm -f "$ENV_TMP"
...
for repo in (
    "unconst/Affine-5czsc2fc98-r18-lora",
    "unconst/Affine-5czsc2fc98-r18-merged",
):
    try:
...
"${SSH[@]}" 'set -e
  tar -C /root/mining_src -xzf /tmp/mine-r18-stack.tar.gz
  ...
  chmod +x /root/mining_src/s3-duel-sim/*.sh \
           ...
           /root/mining_src/r18-sbs-grpo/*.sh
  ...
  # Prove overlay is R18, not stock R3 / R3b.
  ...
  echo "R18_DEADLINES soft=$SOFT_DEADLINE_UTC dead=$DEADMAN_UTC axis=$R18_AXIS"
  echo "R18_KNOBS lr=${R18_LR} r=${R18_LORA_R} G=${R18_GROUP_SIZE}"
  echo STACK_UPLOAD_OK
  nohup bash /root/mining_src/r3-reason-grpo/bootstrap_r3.sh \
    >/root/logs/r18_pipeline.nohup 2>&1 &
  echo $! > /root/logs/r18_pipeline.pid
  cp -f /root/logs/r18_pipeline.pid /root/logs/r3_pipeline.pid
  echo PIPELINE_PID=$(cat /root/logs/r18_pipeline.pid)
  sleep 5
  head -n 40 /root/logs/bootstrap_r3.log 2>/dev/null || head -n 40 /root/logs/r18_pipeline.nohup || true
  ps -p "$(cat /root/logs/r18_pipeline.pid)" -o pid,etime,cmd || true
  nvidia-smi -L | wc -l
'
