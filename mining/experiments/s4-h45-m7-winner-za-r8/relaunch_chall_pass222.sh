#!/usr/bin/env bash
# Pass 218: H45 p216 came up on isolated TCACHE, first completions 200,
...
for f in h45_decision.json h45_sim_result.json h45_sim_result_artifact.json \
         h45_sim_progress.json; do
  if [[ -f "/root/affine_data/$f" ]]; then
    mv "/root/affine_data/$f" "$QDIR/${f%.json}_pass222_${ts}.json"
    log "quarantine $f → $QDIR"
  fi
done
rm -f /root/logs/h45_n80.done /root/logs/h45_n80_retry.aborted \
  /root/logs/h45_pipeline.aborted /root/logs/h45_chall_serve.done
...
while read -r pid; do
  ...
  /[r]un_sim_duel\.py/ && /local-h45/ {print $1}
  /[r]etry_h45_n80\.sh/ {print $1}
')
...
print(f"[recover222-h45-chall] reap gpu={sorted(want)} workers={sorted(pids)} parents={sorted(parents)} grandparents={sorted(grandparents)}")
for pid in kill_set:
    try:
...
rm -rf /root/.triton/isolated/h45_chall_* || true
...
rm -rf /tmp/torchinductor_* /root/.cache/torch/inductor /root/.cache/torchinductor_chall_* /root/.cache/torchinductor_h45_* || true
...
_comp() {
  ...
  code=$(curl -s -o "/tmp/h45_warmup_${label}.json" -w "%{http_code}" --max-time 120 \
...
while read -r pid; do
  ...
  sleep 1
  kill -9 "$pid" 2>/dev/null || true
done < <(ps -eo pid,args | awk '/[w]atch_n80_retry\.sh/ && / h45 / {print $1}')
...
while read -r pid; do
  ...
  sleep 1
  kill -9 "$pid" 2>/dev/null || true
done < <(ps -eo pid,args | awk '/[r]etry_h45_n80\.sh/ {print $1}')
sleep 1
...
log "rearmed watcher pid=$(cat /root/logs/h45_watch_retry.pid)"
