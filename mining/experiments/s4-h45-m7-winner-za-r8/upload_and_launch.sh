#!/usr/bin/env bash
# Host → mine-h45-1: upload stack + data, start bootstrap under nohup.
set -euo pipefail

ROOT=/home/const/subnet120
DST_HOST=${DST_HOST:?set DST_HOST}
DST_PORT=${DST_PORT:?set DST_PORT}
KNOWN=${KNOWN:-/tmp/mine-h45-1.known_hosts}
...
STAGE=$(mktemp -d /tmp/mine-h45-stack.XXXXXX)
trap 'rm -rf "$STAGE"' EXIT

EXP=s4-h45-m7-winner-za-r8
...
TAR=/tmp/mine-h45-stack.tar.gz
tar -C "$STAGE" -czf "$TAR" .
ls -lh "$TAR"

ENV_TMP=$(mktemp /tmp/mine-h45.env.XXXXXX)
...
# H45 data = H29 king-self harvest (686 ex); init = m7 (same as H28).
DATA="$ROOT/mining/experiments/s4-h27-clip-l1-shape/results/winner_za_high_l1.jsonl"
test -s "$DATA"
test "$(wc -l <"$DATA")" -ge 300

"${SSH[@]}" 'mkdir -p /root/mining_src /root/affine_data /root/logs /root/h45 /root/hf'
"${SCP[@]}" "$TAR" "root@${DST_HOST}:/tmp/mine-h45-stack.tar.gz"
"${SCP[@]}" "$ENV_TMP" "root@${DST_HOST}:/root/mine.env"
"${SCP[@]}" "$DATA" "root@${DST_HOST}:/root/h45/winner_za_high_l1.jsonl"
rm -f "$ENV_TMP"
...
for repo in (
    "unconst/Affine-5czsc2fc98-h45-lora",
    "unconst/Affine-5czsc2fc98-h45-merged",
):
    try:
...
"${SSH[@]}" 'set -e
  tar -C /root/mining_src -xzf /tmp/mine-h45-stack.tar.gz
  chmod 600 /root/mine.env
  chmod +x /root/mining_src/s3-duel-sim/*.sh \
           /root/mining_src/s4-h2-merge/*.sh \
           /root/mining_src/s4-h45-m7-winner-za-r8/*.sh
  test -f /root/mining_src/affine_pkg/affine/score.py
  test -s /root/h45/winner_za_high_l1.jsonl
  test -x /root/mining_src/s4-h45-m7-winner-za-r8/bootstrap_h45.sh
  echo STACK_UPLOAD_OK
  nohup bash /root/mining_src/s4-h45-m7-winner-za-r8/bootstrap_h45.sh \
    >/root/logs/h45_pipeline.nohup 2>&1 &
  echo $! > /root/logs/h45_pipeline.pid
  nohup bash /root/mining_src/s4-h2-merge/watch_form_decision.sh h45 \
    /root/affine_data/h45_sim_result.json /root/affine_data/h45_decision.json \
    /root/logs/h45_form_decision.nohup \
    >/root/logs/h45_form_decision.launch.out 2>&1 &
  echo $! > /root/logs/h45_form_decision.pid
  nohup bash /root/mining_src/s4-h2-merge/watch_n80_retry.sh h45 \
    /root/mining_src/s4-h45-m7-winner-za-r8/retry_h45_n80.sh \
    >/root/logs/h45_watch_retry.launch.nohup 2>&1 &
  echo $! > /root/logs/h45_watch_retry.pid
  echo PIPELINE_PID=$(cat /root/logs/h45_pipeline.pid)
  sleep 3
  head -n 40 /root/logs/bootstrap_h45.log 2>/dev/null || head -n 40 /root/logs/h45_pipeline.nohup || true
  ps -p "$(cat /root/logs/h45_pipeline.pid)" -o pid,etime,cmd || true
'
