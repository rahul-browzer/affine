#!/usr/bin/env bash
# Host → mine-h55-1: upload stack + data, start bootstrap under nohup.
set -euo pipefail

ROOT=/home/const/subnet120
DST_HOST=${DST_HOST:?set DST_HOST}
DST_PORT=${DST_PORT:?set DST_PORT}
KNOWN=${KNOWN:-/tmp/mine-h55-1.known_hosts}
...
STAGE=$(mktemp -d /tmp/mine-h55-stack.XXXXXX)
trap 'rm -rf "$STAGE"' EXIT

EXP=s4-h55-m7-winner-za-lr55e6
...
TAR=/tmp/mine-h55-stack.tar.gz
tar -C "$STAGE" -czf "$TAR" .
ls -lh "$TAR"

ENV_TMP=$(mktemp /tmp/mine-h55.env.XXXXXX)
...
# H55 data = H29 king-self harvest (686 ex); init = m7 (same as H28).
DATA="$ROOT/mining/experiments/s4-h27-clip-l1-shape/results/winner_za_high_l1.jsonl"
test -s "$DATA"
test "$(wc -l <"$DATA")" -ge 300

"${SSH[@]}" 'mkdir -p /root/mining_src /root/affine_data /root/logs /root/h55 /root/hf'
"${SCP[@]}" "$TAR" "root@${DST_HOST}:/tmp/mine-h55-stack.tar.gz"
"${SCP[@]}" "$ENV_TMP" "root@${DST_HOST}:/root/mine.env"
"${SCP[@]}" "$DATA" "root@${DST_HOST}:/root/h55/winner_za_high_l1.jsonl"
rm -f "$ENV_TMP"
...
for repo in (
    "unconst/Affine-5czsc2fc98-h55-lora",
    "unconst/Affine-5czsc2fc98-h55-merged",
):
    try:
...
"${SSH[@]}" 'set -e
  tar -C /root/mining_src -xzf /tmp/mine-h55-stack.tar.gz
  chmod 600 /root/mine.env
  chmod +x /root/mining_src/s3-duel-sim/*.sh \
           /root/mining_src/s4-h2-merge/*.sh \
           /root/mining_src/s4-h55-m7-winner-za-lr55e6/*.sh
  test -f /root/mining_src/affine_pkg/affine/score.py
  test -s /root/h55/winner_za_high_l1.jsonl
  test -x /root/mining_src/s4-h55-m7-winner-za-lr55e6/bootstrap_h55.sh
  echo STACK_UPLOAD_OK
  nohup bash /root/mining_src/s4-h55-m7-winner-za-lr55e6/bootstrap_h55.sh \
    >/root/logs/h55_pipeline.nohup 2>&1 &
  echo $! > /root/logs/h55_pipeline.pid
  nohup bash /root/mining_src/s4-h2-merge/watch_form_decision.sh h55 \
    /root/affine_data/h55_sim_result.json /root/affine_data/h55_decision.json \
    /root/logs/h55_form_decision.nohup \
    >/root/logs/h55_form_decision.launch.out 2>&1 &
  echo $! > /root/logs/h55_form_decision.pid
  nohup bash /root/mining_src/s4-h2-merge/watch_n80_retry.sh h55 \
    /root/mining_src/s4-h55-m7-winner-za-lr55e6/retry_h55_n80.sh \
    >/root/logs/h55_watch_retry.launch.nohup 2>&1 &
  echo $! > /root/logs/h55_watch_retry.pid
  echo PIPELINE_PID=$(cat /root/logs/h55_pipeline.pid)
  sleep 3
  head -n 40 /root/logs/bootstrap_h55.log 2>/dev/null || head -n 40 /root/logs/h55_pipeline.nohup || true
  ps -p "$(cat /root/logs/h55_pipeline.pid)" -o pid,etime,cmd || true
'
