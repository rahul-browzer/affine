#!/usr/bin/env bash
# Pass 248: H55 king :8001 died mid-n80 (~16/80) → ConnectError. Orphan
# VLLM::Worker ppid=1 on GPUs 2,3. Reap → wipe king caches → settle → unique
# TCACHE. Teacher :8000 + chall :8002 stay up; n80_retry waits engines.
set -euo pipefail
...
LOG=/root/logs/h55_king_recover_pass248.log
...
TAG=king_p248_$(date +%s)_$$
mkdir -p /root/logs
: >"$LOG"

log() { echo "[recover248-h55] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }

log "START king relaunch (wipe→settle20→unique cache)"
...
# nvidia-smi compute-apps can show stale "[Not Found]" PIDs while real
# orphans are VLLM::Worker with ppid=1 — kill those by process table first.
ps -eo pid=,ppid=,cmd= | awk '$2==1 && /VLLM::Worker/ {print $1}' | while read -r op; do
  log "kill ppid1 Worker pid=$op"
...
print(f"[recover248-h55] reap gpu={sorted(want)} workers={sorted(pids)} parents={sorted(parents)}")
for pid in kill_set:
    try:
...
log "DONE_LAUNCH (merge still running — prewarm/post_train will pick up health)"
