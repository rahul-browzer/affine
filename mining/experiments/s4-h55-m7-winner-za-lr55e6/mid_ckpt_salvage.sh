#!/usr/bin/env bash
# Watch H55 Trainer mid-checkpoints + final adapter; push to HF (TTL insurance).
set -euo pipefail
...
TRAIN_DIR=/root/h55/train
CKPT_ROOT=$TRAIN_DIR/checkpoints
ADAPTER=$TRAIN_DIR/adapter
TRAIN_DONE=$TRAIN_DIR/train.done
REPO=${H5C_LORA_REPO:-unconst/Affine-5czsc2fc98-h55-lora}
SEEN=/root/h55/mid_ckpt_salvaged.txt
LOG_TAG=h55-mid-salvage
BASE_HUB=${H5C_BASE_HUB:-kevin954/Affine-5dfqbbh8ev-sft}

mkdir -p /root/h55 /root/logs /root/affine_data
touch "$SEEN"
...
salvage_one() {
  ...
  local meta=/root/affine_data/h55_mid_${tag}_salvage.json
  if python3 /root/mining_src/s4-h1-sft/salvage_adapter.py \
    --adapter "$src" \
    --repo "$REPO" \
    --path-in-repo "$path_in_repo" \
    --base-hub "$BASE_HUB" \
    --commit-message "H55 mid-ckpt salvage $tag (TTL insurance)" \
    --out-meta "$meta"; then
    echo "$tag" >>"$SEEN"
