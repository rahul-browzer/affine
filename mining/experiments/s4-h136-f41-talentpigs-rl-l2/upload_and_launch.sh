#!/usr/bin/env bash
# Host → mine-f41-1: upload stack + data, start bootstrap under nohup.
set -euo pipefail

ROOT=/home/const/subnet120
DST_HOST=${DST_HOST:?set DST_HOST}
DST_PORT=${DST_PORT:?set DST_PORT}
KNOWN=${KNOWN:-/tmp/mine-f41-1.known_hosts}
...
STAGE=$(mktemp -d /tmp/mine-h136-stack.XXXXXX)
trap 'rm -rf "$STAGE"' EXIT

EXP=s4-h136-f41-talentpigs-rl-l2
...
test -f "$STAGE/$EXP/train_rl_l2.py"
test -f "$STAGE/$EXP/bootstrap_h136.sh"
test -f "$STAGE/$EXP/start_h136.sh"

TAR=/tmp/mine-h136-stack.tar.gz
tar -C "$STAGE" -czf "$TAR" .
ls -lh "$TAR"

ENV_TMP=$(mktemp /tmp/mine-h136.env.XXXXXX)
...
umask 077
{
  ...
  if [[ -n "${SOFT_DEADLINE_UTC:-}" ]]; then echo "SOFT_DEADLINE_UTC=${SOFT_DEADLINE_UTC}"; fi
  if [[ -n "${DEADMAN_UTC:-}" ]]; then echo "DEADMAN_UTC=${DEADMAN_UTC}"; fi
} >"$ENV_TMP"
chmod 600 "$ENV_TMP"

# H136 data = H27 winner_za_high_l1 (406 ex); init = TalentPigs @dbfbb3e2; n80 king = Tok.
DATA="$ROOT/mining/experiments/s4-h27-clip-l1-shape/results/winner_za_high_l1.jsonl"
test -s "$DATA"
test "$(wc -l <"$DATA")" -ge 300

"${SSH[@]}" 'mkdir -p /root/mining_src /root/affine_data /root/logs /root/h136 /root/hf'
"${SCP[@]}" "$TAR" "root@${DST_HOST}:/tmp/mine-h136-stack.tar.gz"
"${SCP[@]}" "$ENV_TMP" "root@${DST_HOST}:/root/mine.env"
"${SCP[@]}" "$DATA" "root@${DST_HOST}:/root/h136/winner_za_high_l1.jsonl"
rm -f "$ENV_TMP"
...
for repo in (
    "unconst/Affine-5czsc2fc98-h136-lora",
    "unconst/Affine-5czsc2fc98-h136-merged",
):
    try:
...
"${SSH[@]}" 'set -e
  tar -C /root/mining_src -xzf /tmp/mine-h136-stack.tar.gz
  chmod 600 /root/mine.env
  chmod +x /root/mining_src/s3-duel-sim/*.sh \
           /root/mining_src/s4-h2-merge/*.sh \
           /root/mining_src/s4-h136-f41-talentpigs-rl-l2/*.sh
  test -f /root/mining_src/affine_pkg/affine/score.py
  test -s /root/h136/winner_za_high_l1.jsonl
  test -x /root/mining_src/s4-h136-f41-talentpigs-rl-l2/bootstrap_h136.sh
  echo STACK_UPLOAD_OK
  nohup bash /root/mining_src/s4-h136-f41-talentpigs-rl-l2/bootstrap_h136.sh \
    >/root/logs/h136_pipeline.nohup 2>&1 &
  echo $! > /root/logs/h136_pipeline.pid
  nohup bash /root/mining_src/s4-h2-merge/watch_form_decision.sh h136 \
    /root/affine_data/h136_sim_result.json /root/affine_data/h136_decision.json \
    /root/logs/h136_form_decision.nohup \
    >/root/logs/h136_form_decision.launch.out 2>&1 &
  echo $! > /root/logs/h136_form_decision.pid
  nohup bash /root/mining_src/s4-h2-merge/watch_n80_retry.sh h136 \
    /root/mining_src/s4-h136-f41-talentpigs-rl-l2/retry_h136_n80_d203first.sh \
    >/root/logs/h136_watch_retry.launch.nohup 2>&1 &
  echo $! > /root/logs/h136_watch_retry.pid
  nohup bash /root/mining_src/s4-h136-f41-talentpigs-rl-l2/watch_preempt_bare_tcache_pass264.sh \
    >/root/logs/h136_preempt_bare_pass264.launch.nohup 2>&1 &
  echo $! > /root/logs/h136_preempt_bare_pass264.pid
  echo PREEMPT_PID=$(cat /root/logs/h136_preempt_bare_pass264.pid)
  echo PIPELINE_PID=$(cat /root/logs/h136_pipeline.pid)
  sleep 3
  head -n 40 /root/logs/bootstrap_h136.log 2>/dev/null || head -n 40 /root/logs/h136_pipeline.nohup || true
  ps -p "$(cat /root/logs/h136_pipeline.pid)" -o pid,etime,cmd || true
'
