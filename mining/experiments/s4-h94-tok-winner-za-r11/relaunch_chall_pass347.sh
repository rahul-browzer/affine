#!/usr/bin/env bash
# Pass 344: chall recover after load-time NODUTTS4 ImportError under empty seed.
# King lives on isolated TCACHE (not /root/.triton/cache/king) → p264 "no king
# TCACHE to seed" left chall cold → ghost launcher ENOENT @ profile_run.
# Fix: seed from live :8001 TRITON_CACHE_DIR; early-abort wait_health on
# launcher ImportError (don't burn 120×10s). Outer×3 + post-diverse freeze.
set -euo pipefail
...
LOG=/root/logs/h94_chall_recover_pass347.log
CHALL_LOG=/root/logs/vllm_chall.log
PIDF=/root/logs/vllm_chall.pid
MERGE=/root/h94/merged
export HYP=h94
export GPUS=4,5
UTIL=0.72
...
log() { echo "[recover347-h94-chall] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }

log "START chall relaunch (p344: seed live king isolated TCACHE + early-abort launcher ENOENT; outer×3 post-diverse freeze)"

ts=$(date -u +%Y%m%dT%H%M%SZ)
for f in h94_decision.json h94_sim_result.json h94_sim_result_artifact.json \
         h94_sim_progress.json; do
  if [[ -f "/root/affine_data/$f" ]]; then
    mv "/root/affine_data/$f" "$QDIR/${f%.json}_pass347_${ts}.json"
    log "quarantine $f → $QDIR"
  fi
done
rm -f /root/logs/h94_n80.done /root/logs/h94_n80_retry.aborted \
  /root/logs/h94_pipeline.aborted /root/logs/h94_chall_serve.done \
  /root/logs/h94_sim_n80.done /root/logs/h94_chall_freeze_pass247.done \
  /root/logs/h94_chall_freeze_pass251.done \
  /root/logs/h94_chall_freeze_pass237.done \
  /root/logs/h94_chall_freeze_pass239.done \
  /root/logs/h94_chall_freeze_pass240.done \
  /root/logs/h94_chall_freeze_pass264.done
...
while read -r pid; do
  ...
  /[r]un_sim_duel\.py/ && /local-h94/ {print $1}
  /[r]etry_h94_n80\.sh/ {print $1}
')
...
print(f"[recover347-h94-chall] reap gpu={sorted(want)} workers={sorted(pids)} parents={sorted(parents)} grandparents={sorted(grandparents)}")
for pid in kill_set:
    try:
...
wipe_caches() {
  ...
  chmod -R u+w /root/.triton/isolated/h94_chall_* 2>/dev/null || true
  rm -rf /root/.triton/cache/chall /root/.triton/cache/chall_* || true
  rm -rf /root/.triton/isolated/h94_chall_* || true
  rm -rf /root/.cache/flashinfer/cached_ops/sampling || true
  rm -rf /root/.cache/vllm/torch_compile_cache || true
  rm -rf /tmp/torchinductor_* /root/.cache/torch/inductor /root/.cache/torchinductor_chall_* /root/.cache/torchinductor_h94_* || true
...
wait_health() {
  log "wait health=200 (max 120×10s; early-abort on launcher ImportError)"
  local i code
  for i in $(seq 1 120); do
    if grep -q '__triton_launcher.*cannot open shared object file' "$CHALL_LOG" 2>/dev/null \
      || grep -q 'ImportError: .*/__triton_launcher' "$CHALL_LOG" 2>/dev/null; then
      log "ABORT early: Triton launcher ENOENT in chall log at poll=$i — next attempt"
      return 1
    fi
...
for attempt in 1 2 3; do
  ...
  wipe_caches
  TAG=h94_chall_p260_a${attempt}_$(date +%s)_$$
  TCACHE=/root/.triton/isolated/$TAG
  mkdir -p "$TCACHE" /root/.cache/torchinductor_$TAG
  n_seed=0
  KING_SEED=""
  # Live king on :8001 often uses isolated TCACHE, not /root/.triton/cache/king.
  for kpid in $(ss -lptn 'sport = :8001' 2>/dev/null | awk -Fpid= 'NF>1{split($2,a,","); print a[1]}'); do
    [[ -r "/proc/$kpid/environ" ]] || continue
    KING_SEED=$(tr '\0' '\n' <"/proc/$kpid/environ" | awk -F= '/^TRITON_CACHE_DIR=/{print $2; exit}')
    [[ -n "$KING_SEED" ]] && break
  done
  if [[ -z "${KING_SEED:-}" || ! -d "$KING_SEED" ]]; then
    KING_SEED=$(ls -td /root/.triton/isolated/h94_king_* 2>/dev/null | head -1 || true)
  fi
  if [[ -z "${KING_SEED:-}" || ! -d "$KING_SEED" ]]; then
    KING_SEED=/root/.triton/cache/king
  fi
  if [[ -d "$KING_SEED" ]]; then
    n_seed=$(find "$KING_SEED" -name '__triton_launcher*.so' 2>/dev/null | wc -l)
    log "seed TCACHE from $KING_SEED (launcher.so=$n_seed) — leave WRITABLE"
    cp -a "$KING_SEED"/. "$TCACHE/" 2>/dev/null || true
  else
...
while read -r pid; do
  ...
  sleep 1
  kill -9 "$pid" 2>/dev/null || true
done < <(ps -eo pid,args | awk '/[w]atch_n80_retry\.sh/ && / h94 / {print $1}')
while read -r pid; do
  [[ -n "${pid:-}" ]] || continue
  ...
  sleep 1
  kill -9 "$pid" 2>/dev/null || true
done < <(ps -eo pid,args | awk '/[r]etry_h94_n80\.sh/ {print $1}')
while read -r pid; do
  [[ -n "${pid:-}" ]] || continue
  ...
  sleep 1
  kill -9 "$pid" 2>/dev/null || true
done < <(ps -eo pid,args | awk '/[w]atch_form_decision\.sh/ && / h94 / {print $1}')
...
while read -r pid; do
  [[ -n "${pid:-}" ]] || continue
  # Do not kill self ($0 is pass347).
  [[ "$pid" -eq "$$" ]] && continue
  log "kill old p264 recover pid=$pid"
  kill "$pid" 2>/dev/null || true
  sleep 1
  kill -9 "$pid" 2>/dev/null || true
done < <(ps -eo pid,args | awk '/[r]elaunch_chall_pass264\.sh/ {print $1}')
sleep 1

nohup bash /root/mining_src/s4-h2-merge/watch_form_decision.sh h94 \
  /root/affine_data/h94_sim_result.json \
  /root/affine_data/h94_decision.json \
  /root/logs/h94_form_decision.nohup \
  >/root/logs/h94_form_decision.launch.nohup 2>&1 &
echo $! >/root/logs/h94_form_decision.pid
log "rearmed form pid=$(cat /root/logs/h94_form_decision.pid)"

nohup bash /root/mining_src/s4-h2-merge/watch_n80_retry.sh h94 \
  /root/mining_src/s4-h94-tok-winner-za-r11/retry_h94_n80.sh \
  >/root/logs/h94_watch_retry.launch.nohup 2>&1 &
echo $! >/root/logs/h94_watch_retry.pid
log "rearmed watcher pid=$(cat /root/logs/h94_watch_retry.pid)"
date -u +%Y-%m-%dT%H:%M:%SZ > /root/logs/h94_chall_serve.done
echo "TCACHE=$TCACHE mode=$(stat -c %a $TCACHE) probe=200 attempts_ok post_diverse_freeze=1 king_seed=1 seed_src=live_king_isolated" \
  > /root/logs/h94_chall_freeze_pass264.done
echo "TCACHE=$TCACHE mode=$(stat -c %a $TCACHE) probe=200 attempts_ok post_diverse_freeze=1 king_seed=1 seed_src=live_king_isolated" \
  > /root/logs/h94_chall_freeze_pass347.done
