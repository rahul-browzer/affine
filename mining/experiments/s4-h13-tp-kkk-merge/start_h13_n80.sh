#!/usr/bin/env bash
# After bootstrap_h13.done + engines up: run n80 sim vs TalentPigs.
set -euo pipefail
...
test -d /root/merges/h13-tp75

TIMEOUT_S=2400 bash /root/mining_src/s3-duel-sim/wait_ready.sh

for attempt in 1 2 3; do
  echo "[h13-n80] attempt=$attempt $(date -u +%Y-%m-%dT%H:%M:%SZ)"
  set +e
  python /root/mining_src/s4-h2-merge/run_sim_duel.py \
    ...
    --chall-repo /root/merges/h13-tp75 \
    --chall-rev local \
    --n-turns 80 \
    --hotkey local-h13 \
    --out /root/affine_data/h13_sim_result.json \
    --progress-out /root/affine_data/h13_sim_progress.json \
    --save-artifact \
