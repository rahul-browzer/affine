...
LOGN=/root/logs/h126_king_recover_pass332.nohup
log() { echo "[king332-h126] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOGN"; }
...
for line in apps.strip().splitlines():
    ...
    if len(parts) >= 2 and parts[0] in uuids:
        try:
            ...
            pass
print(f"[king332-h126] reap gpus2,3 pids={sorted(pids)}")
for pid in pids:
    try:
...
rm -f /root/affine_data/h126_sim_result.json /root/affine_data/h126_sim_progress.json \
  /root/affine_data/h126_decision.json /root/logs/h126_n80.done /root/logs/h126_n80_retry.aborted \
  /root/logs/h126_king_recover_pass311.done /root/logs/h126_sim_n80.done \
  /root/logs/h126_king_recover_pass332.done
log "cleared stale h126 sim progress/result"
...
TCACHE="/root/.triton/isolated/h126_king_p332_$(date +%s)_$$"
mkdir -p "$TCACHE"
echo "$TCACHE" >/root/logs/h126_king_tcache_pass332.path
...
for line in out.strip().splitlines():
    ...
    if int(idx) in (2, 3):
        print(f"[king332-h126] gpu{idx} used_mib={used}")
        if float(used) < 500:
            ok += 1
if ok < 2:
    print("[king332-h126] ABORT: gpus 2,3 not free")
    sys.exit(1)
PY
  ...
  if [[ "$code" == "200" ]]; then
    ...
    if [[ -n "$mid" ]]; then
      ...
      if [[ "$pcode" == "200" ]]; then
        ...
        date -u +%Y-%m-%dT%H:%M:%SZ >/root/logs/h126_king_recover_pass332.done
        echo "$REPO $REV $TCACHE util=0.72" >>/root/logs/h126_king_recover_pass332.done
        log "DONE — retry_h126_n80 should proceed (watcher already armed)"
        exit 0
      fi
    fi
  fi
