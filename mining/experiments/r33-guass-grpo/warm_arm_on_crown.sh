#!/usr/bin/env bash
# Host → lean warm-arm R33 onto TKC-hot mine-crown-1 after R26 SIGNAL_POS_BELOW.
# Keeps teacher:8000 + guass:8001; trains guass-init Reason-GRPO on GPUs 6–7.
set -euo pipefail
...
STAGE=$(mktemp -d /tmp/mine-r33-warm.XXXXXX)
...
cp -a "$ROOT/mining/experiments/r33-guass-grpo/"*.sh "$STAGE/r33-guass-grpo/"
cp -a "$ROOT/mining/experiments/r33-guass-grpo/plan.md" "$STAGE/r33-guass-grpo/"
# Overlay start for R33 identity grep.
...
TAR=/tmp/mine-r33-warm-stack.tar.gz
tar -C "$STAGE" -czf "$TAR" .
ls -lh "$TAR"
...
ENV_TMP=$(mktemp /tmp/mine-r33.env.XXXXXX)
{
  ...
  echo "export BASE=/root/hf/hub/models--ttttxxxxsada--Affine-5guassq3tu/snapshots/e86758f5080d1e373e5fbbd7b4fbf6af327aeb44"
} >"$ENV_TMP"
chmod 600 "$ENV_TMP"
...
"${SCP[@]}" "$TAR" "root@${DST_HOST}:/tmp/mine-r33-warm-stack.tar.gz"
...
"${SSH[@]}" 'set -euo pipefail
  tar -C /root/mining_src -xzf /tmp/mine-r33-warm-stack.tar.gz
  ...
  # Stop leftover R26/R3 post/form/sim by pidfile only (never pkill -f).
  for f in /root/logs/r26_lean_warm.pid /root/logs/r26_post_train.pid \
           /root/logs/r26_form_decision.pid /root/logs/r26_train.pid \
  ...
  nohup bash /root/mining_src/r33-guass-grpo/lean_warm_boot.sh \
    >/root/logs/r33_lean_warm.nohup 2>&1 &
  echo $! >/root/logs/r33_lean_warm.pid
  sleep 35
  echo LEAN_PID=$(cat /root/logs/r33_lean_warm.pid)
  head -n 80 /root/logs/r33_lean_warm.log 2>/dev/null \
    || head -n 80 /root/logs/r33_lean_warm.nohup || true
  ps -p "$(cat /root/logs/r33_lean_warm.pid)" -o pid,etime,cmd || true
  test -f /root/logs/r3_train.pid && echo TRAIN_PID=$(cat /root/logs/r3_train.pid)
  test -f /root/logs/r33_post_train.pid && echo POST_PID=$(cat /root/logs/r33_post_train.pid)
...
echo "R33_WARM_ARM_OK pod=$POD_NAME soft=$SOFT dead=$DEAD $(date -u +%Y-%m-%dT%H:%M:%SZ)"
