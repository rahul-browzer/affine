...
LOGN=/root/logs/h140_king_recover_pass311.nohup
log() { echo "[king311-h140] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOGN"; }
...
for line in apps.strip().splitlines():
    ...
    if len(parts) >= 2 and parts[0] in uuids:
        try:
            ...
            pass
print(f"[king311-h140] reap gpus2,3 pids={sorted(pids)}")
for pid in pids:
    try:
...
for line in out.strip().splitlines():
    ...
    if int(idx) in (2, 3):
        print(f"[king311-h140] gpu{idx} used_mib={used}")
        if float(used) < 500:
            ok += 1
if ok < 2:
    print("[king311-h140] ABORT: gpus 2,3 not free")
    sys.exit(1)
PY
  ...
  if [[ "$code" == "200" ]]; then
    ...
    if [[ -n "$mid" ]]; then
      ...
      if [[ "$pcode" == "200" ]]; then
        ...
        date -u +%Y-%m-%dT%H:%M:%SZ >/root/logs/h140_king_recover_pass311.done
        echo "$REPO $REV" >>/root/logs/h140_king_recover_pass311.done
