...
SOFT_DEADLINE_UTC=${SOFT_DEADLINE_UTC:-2026-08-13T23:00:00Z}
DEADMAN_UTC=${DEADMAN_UTC:-2026-08-13T23:30:00Z}
...
_train_alive() {
  if [[ -f /root/logs/r3_train.pid ]]; then
    local tpid
    tpid=$(cat /root/logs/r3_train.pid 2>/dev/null || true)
    if [[ -n "${tpid:-}" ]] && kill -0 "$tpid" 2>/dev/null; then
      return 0
    fi
  fi
  # Narrow fallback: real trainer argv starts with python3 + full script path.
  pgrep -f "python3 /root/mining_src/r3-reason-grpo/train_reason_grpo.py --base" >/dev/null 2>&1
}
...
_abort_on_exit() {
  ...
  if [[ -f /root/logs/r3_pipeline.done ]]; then
    return 0
  fi
  if [[ ! -f /root/logs/r3_pipeline.aborted ]]; then
...
rm -f /root/logs/r3_pipeline.aborted /root/logs/r3_pipeline.done \
  /root/logs/r3_merge.done /root/logs/r3_chall_serve.done \
  /root/logs/r3_sim_n80.done
...
while true; do
  ...
  if (( now > soft - 3600 )); then
    ...
    echo "aborted_no_train $(date -u +%Y-%m-%dT%H:%M:%SZ)" > /root/logs/r3_pipeline.aborted
    exit 1
  fi
  _wait_i=$((_wait_i + 1))
...
if _train_alive; then
  ...
  echo "aborted_train_stuck $(date -u +%Y-%m-%dT%H:%M:%SZ)" > /root/logs/r3_pipeline.aborted
  exit 1
fi
sleep 15
...
python /root/mining_src/s4-h1-sft/merge_lora.py \
  ...
  --device-map auto \
  | tee -a "$LOG"
cp -f "$MERGED/merge_meta.json" /root/affine_data/r3_merge_meta.json 2>/dev/null || true
...
HF_LORA_REPO=${HF_LORA_REPO:-unconst/Affine-5czsc2fc98-r3-lora}
HF_MERGED_REPO=${HF_MERGED_REPO:-unconst/Affine-5czsc2fc98-r3-merged}
...
if [[ -n "${HF_TOKEN:-}" ]]; then
  ...
  nohup python3 /root/mining_src/s4-h1-sft/push_merged.py \
    ...
    --out-meta /root/affine_data/r3_merged_salvage.json \
...
if [[ "$code_t" != "200" || "$code_k" != "200" ]]; then
  ...
  echo "aborted_engines_unhealthy $(date -u +%Y-%m-%dT%H:%M:%SZ)" > /root/logs/r3_pipeline.aborted
  exit 1
fi
...
if (( dead - now < 2400 )); then
  ...
  echo "aborted_no_n80_budget $(date -u +%Y-%m-%dT%H:%M:%SZ)" > /root/logs/r3_pipeline.aborted
  exit 1
fi
...
# p2171/p2227: Reason v3 decision — submit bar = live crown (k_sigma·SE), no 1.5×.
python /root/mining_src/r1-reason-distill/write_reason_decision.py \
  --sim-result "$SIM_N80" --out "$SIM_DEC" --hyp R3b --k-sigma 2.0 --headroom-bar 1.0 \
  2>&1 | tee -a /root/logs/r3_sim.nohup
