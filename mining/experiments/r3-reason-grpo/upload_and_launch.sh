#!/usr/bin/env bash
# Host → mine-r3-grpo-1: upload stack + data, start bootstrap under nohup.
set -euo pipefail

ROOT=/home/const/subnet120
DST_HOST=${DST_HOST:?set DST_HOST}
DST_PORT=${DST_PORT:?set DST_PORT}
KNOWN=${KNOWN:-/tmp/mine-r3-grpo-1.known_hosts}
...
STAGE=$(mktemp -d /tmp/mine-r3-stack.XXXXXX)
...
test -f "$STAGE/$EXP/bootstrap_r3.sh"
test -f "$STAGE/$EXP/start_r3.sh"

TAR=/tmp/mine-r3-stack.tar.gz
tar -C "$STAGE" -czf "$TAR" .
ls -lh "$TAR"

ENV_TMP=$(mktemp /tmp/mine-r3.env.XXXXXX)
...
"${SSH[@]}" 'mkdir -p /root/mining_src /root/affine_data /root/logs /root/r3 /root/hf'
"${SCP[@]}" "$TAR" "root@${DST_HOST}:/tmp/mine-r3-stack.tar.gz"
...
for repo in (
    "unconst/Affine-5czsc2fc98-r3-lora",
    "unconst/Affine-5czsc2fc98-r3-merged",
):
    try:
...
"${SSH[@]}" 'set -e
  tar -C /root/mining_src -xzf /tmp/mine-r3-stack.tar.gz
  ...
  nohup bash /root/mining_src/r3-reason-grpo/bootstrap_r3.sh \
    >/root/logs/r3_pipeline.nohup 2>&1 &
  echo $! > /root/logs/r3_pipeline.pid
  echo PIPELINE_PID=$(cat /root/logs/r3_pipeline.pid)
  sleep 5
  head -n 50 /root/logs/bootstrap_r3.log 2>/dev/null || head -n 50 /root/logs/r3_pipeline.nohup || true
  ps -p "$(cat /root/logs/r3_pipeline.pid)" -o pid,etime,cmd || true
'
