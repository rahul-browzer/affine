#!/usr/bin/env bash
# Restart H17 n80 after httpx.ReadTimeout / crash. Engines must already be up.
...
MERGED=${MERGED:-/root/merges/h17-tp90}
SIM=/root/affine_data/h17_sim_result.json
PROG=/root/affine_data/h17_sim_progress.json
DEC=/root/affine_data/h17_decision.json
LOG=/root/logs/h17_n80_retry.nohup
MAX_ATTEMPTS=${MAX_ATTEMPTS:-3}

log() { echo "[h17-n80-retry] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }
...
if [[ -f "$SIM" ]]; then
  ...
  python3 /root/mining_src/s4-h2-merge/write_merge_decision.py \
    --hyp h17 --sim-result "$SIM" --out "$DEC"
  date -u +%Y-%m-%dT%H:%M:%SZ > /root/logs/h17_n80.done
  exit 0
fi
if pgrep -f "run_sim_duel.py .*local-h17" >/dev/null 2>&1; then
...
if ! _engines_ok; then
  ...
  echo "aborted_engines_unhealthy $(date -u +%Y-%m-%dT%H:%M:%SZ)" \
    >/root/logs/h17_n80_retry.aborted
  exit 1
fi
test -d "$MERGED"
