#!/usr/bin/env bash
# Pre-warm teacher:8000 + TalentPigs king:8001 while H47 trains on GPUs 6,7.
...
LOG=/root/logs/h47_prewarm.nohup
mkdir -p /root/logs /root/affine_data

log() { echo "[h47-prewarm] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }
...
while true; do
  ...
  if [[ $t -eq 1 && $k -eq 1 ]]; then
    date -u +%Y-%m-%dT%H:%M:%SZ > /root/logs/h47_prewarm.done
    log "PREWARM_READY"
    exit 0
  fi
