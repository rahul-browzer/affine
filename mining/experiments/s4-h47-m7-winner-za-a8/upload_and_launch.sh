#!/usr/bin/env bash
# Host → mine-h47-1: upload stack + data, start bootstrap under nohup.
set -euo pipefail

ROOT=/home/const/subnet120
DST_HOST=${DST_HOST:?set DST_HOST}
DST_PORT=${DST_PORT:?set DST_PORT}
KNOWN=${KNOWN:-/tmp/mine-h47-1.known_hosts}
...
STAGE=$(mktemp -d /tmp/mine-h47-stack.XXXXXX)
trap 'rm -rf "$STAGE"' EXIT

EXP=s4-h47-m7-winner-za-a8
...
TAR=/tmp/mine-h47-stack.tar.gz
tar -C "$STAGE" -czf "$TAR" .
ls -lh "$TAR"

ENV_TMP=$(mktemp /tmp/mine-h47.env.XXXXXX)
...
# H47 data = H29 king-self harvest (686 ex); init = m7 (same as H28).
DATA="$ROOT/mining/experiments/s4-h27-clip-l1-shape/results/winner_za_high_l1.jsonl"
test -s "$DATA"
test "$(wc -l <"$DATA")" -ge 300

"${SSH[@]}" 'mkdir -p /root/mining_src /root/affine_data /root/logs /root/h47 /root/hf'
"${SCP[@]}" "$TAR" "root@${DST_HOST}:/tmp/mine-h47-stack.tar.gz"
"${SCP[@]}" "$ENV_TMP" "root@${DST_HOST}:/root/mine.env"
"${SCP[@]}" "$DATA" "root@${DST_HOST}:/root/h47/winner_za_high_l1.jsonl"
rm -f "$ENV_TMP"
...
for repo in (
    "unconst/Affine-5czsc2fc98-h47-lora",
    "unconst/Affine-5czsc2fc98-h47-merged",
):
    try:
...
"${SSH[@]}" 'set -e
  tar -C /root/mining_src -xzf /tmp/mine-h47-stack.tar.gz
  chmod 600 /root/mine.env
  chmod +x /root/mining_src/s3-duel-sim/*.sh \
           /root/mining_src/s4-h2-merge/*.sh \
           /root/mining_src/s4-h47-m7-winner-za-a8/*.sh
  test -f /root/mining_src/affine_pkg/affine/score.py
  test -s /root/h47/winner_za_high_l1.jsonl
  test -x /root/mining_src/s4-h47-m7-winner-za-a8/bootstrap_h47.sh
  echo STACK_UPLOAD_OK
  nohup bash /root/mining_src/s4-h47-m7-winner-za-a8/bootstrap_h47.sh \
    >/root/logs/h47_pipeline.nohup 2>&1 &
  echo $! > /root/logs/h47_pipeline.pid
  nohup bash /root/mining_src/s4-h2-merge/watch_form_decision.sh h47 \
    /root/affine_data/h47_sim_result.json /root/affine_data/h47_decision.json \
    /root/logs/h47_form_decision.nohup \
    >/root/logs/h47_form_decision.launch.out 2>&1 &
  echo $! > /root/logs/h47_form_decision.pid
  nohup bash /root/mining_src/s4-h2-merge/watch_n80_retry.sh h47 \
    /root/mining_src/s4-h47-m7-winner-za-a8/retry_h47_n80.sh \
    >/root/logs/h47_watch_retry.launch.nohup 2>&1 &
  echo $! > /root/logs/h47_watch_retry.pid
  echo PIPELINE_PID=$(cat /root/logs/h47_pipeline.pid)
  sleep 3
  head -n 40 /root/logs/bootstrap_h47.log 2>/dev/null || head -n 40 /root/logs/h47_pipeline.nohup || true
  ps -p "$(cat /root/logs/h47_pipeline.pid)" -o pid,etime,cmd || true
'
