#!/usr/bin/env bash
# Host → mine-r24-longctx-1: R3 stack + R24 LongCtx-GRPO overlay, bootstrap.
# Axis R24: Tok-init Reason-GRPO max_len=16384 max_new=1024 (≠ R3 6144/512).
set -euo pipefail

ROOT=/home/const/subnet120
POD_NAME=${POD_NAME:-mine-r24-longctx-1}
...
STAGE=$(mktemp -d /tmp/mine-r24-stack.XXXXXX)
...
cp -a "$ROOT/mining/experiments/r24-longctx-grpo/start_r24.sh" "$STAGE/r24-longctx-grpo/"
# Overlay: bootstrap_r3 calls start_r3.sh — replace with R24 longctx knobs.
...
"${SCP[@]}" "$TAR" "root@${DST_HOST}:/tmp/mine-r24-stack.tar.gz"
...
"${SSH[@]}" 'set -e
  tar -C /root/mining_src -xzf /tmp/mine-r24-stack.tar.gz
  ...
  # Prove overlay is R24 LongCtx, not stock R3 / R3b / parent-GRPO.
  ...
  echo "R24_DEADLINES soft=$SOFT_DEADLINE_UTC dead=$DEADMAN_UTC axis=$R24_AXIS"
  echo "R24_KNOBS max_len=${R24_MAX_LEN} max_new=${R24_MAX_NEW} lr=${R24_LR} r=${R24_LORA_R} G=${R24_GROUP_SIZE}"
  echo STACK_UPLOAD_OK
  nohup bash /root/mining_src/r3-reason-grpo/bootstrap_r3.sh \
    >/root/logs/r24_pipeline.nohup 2>&1 &
  echo $! > /root/logs/r24_pipeline.pid
  cp -f /root/logs/r24_pipeline.pid /root/logs/r3_pipeline.pid
  echo PIPELINE_PID=$(cat /root/logs/r24_pipeline.pid)
  sleep 5
  head -n 40 /root/logs/bootstrap_r3.log 2>/dev/null || head -n 40 /root/logs/r24_pipeline.nohup || true
  ps -p "$(cat /root/logs/r24_pipeline.pid)" -o pid,etime,cmd || true
  nvidia-smi -L | wc -l
'
