#!/usr/bin/env bash
# mine-h8-1 bootstrap: pinned eval stack + TalentPigs/golden-crown/teacher → merge α0.75.
# Secrets from /root/mine.env only (never argv).
set -euo pipefail

LOG=/root/logs/bootstrap_h8.log
mkdir -p /root/logs /root/hf /root/affine_data /root/mining_src /root/merges
exec > >(tee -a "$LOG") 2>&1

echo "[bootstrap-h8] $(date -u +%Y-%m-%dT%H:%M:%SZ) start host=$(hostname)"
...
if [[ -z "${HF_TOKEN:-}" ]]; then
  echo "[bootstrap-h8] FATAL: HF_TOKEN missing in /root/mine.env"
  exit 1
fi
...
uv pip install \
  ...
  "numpy" \
  "scipy" \
  2>&1 | tee /root/logs/pip_h8.log | tail -40

python - <<'PY'
import torch, transformers, vllm
print("[bootstrap-h8] VERSIONS",
...
jobs = [
    ...
    ("golden-crown/Affine-5EpvnXGu8jUAVc67oPGgJ3brR4JZqjBUSaTKhZuBoNAAzSJF",
     "ee37f4f0457df943d957435d7c9c24222a7ca93d", "goldencrown"),
    ("zai-org/GLM-4.5-Air-FP8", None, "teacher"),
]
for repo, rev, label in jobs:
    print(f"[bootstrap-h8] DOWNLOAD start {label} {repo} rev={rev}", flush=True)
    path = snapshot_download(repo, revision=rev, token=token)
    print(f"[bootstrap-h8] DOWNLOAD done {label} -> {path}", flush=True)
    open(f"/root/logs/{label}.done", "w").write(path + "\n")
print("[bootstrap-h8] ALL_DOWNLOADS_OK", flush=True)
PY
...
python /root/mining_src/s4-h2-merge/merge_linear.py \
  ...
  --b-repo golden-crown/Affine-5EpvnXGu8jUAVc67oPGgJ3brR4JZqjBUSaTKhZuBoNAAzSJF \
  --b-rev ee37f4f0457df943d957435d7c9c24222a7ca93d \
  --alpha "$ALPHA" \
  --out "$OUT" \
  --hf-home "$HF_HOME" \
  2>&1 | tee /root/logs/h8_merge.log

date -u +%Y-%m-%dT%H:%M:%SZ > /root/logs/h8_merge.done
echo "[bootstrap-h8] MERGE_DONE alpha=$ALPHA out=$OUT"
...
export CHALL_REPO=/root/merges/h8-tp75
export CHALL_REV=
bash /root/mining_src/s3-duel-sim/serve_three.sh

touch /root/logs/bootstrap_h8.done
echo "[bootstrap-h8] $(date -u +%Y-%m-%dT%H:%M:%SZ) DONE — engines launching; next: wait_ready + n80"
