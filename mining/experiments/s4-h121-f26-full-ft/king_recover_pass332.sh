...
LOGN=/root/logs/h121_king_recover_pass332.nohup
log() { echo "[king332-h121] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOGN"; }
...
for line in apps.strip().splitlines():
    ...
    if len(parts) >= 2 and parts[0] in uuids:
        try:
            ...
            pass
print(f"[king332-h121] reap gpus2,3 pids={sorted(pids)}")
for pid in pids:
    try:
...
rm -f /root/affine_data/h121_sim_result.json /root/affine_data/h121_sim_progress.json \
  /root/affine_data/h121_decision.json /root/logs/h121_n80.done /root/logs/h121_n80_retry.aborted \
  /root/logs/h121_king_recover_pass311.done /root/logs/h121_sim_n80.done \
  /root/logs/h121_king_recover_pass332.done
log "cleared stale h121 sim progress/result"
...
TCACHE="/root/.triton/isolated/h121_king_p332_$(date +%s)_$$"
mkdir -p "$TCACHE"
echo "$TCACHE" >/root/logs/h121_king_tcache_pass332.path
...
for line in out.strip().splitlines():
    ...
    if int(idx) in (2, 3):
        print(f"[king332-h121] gpu{idx} used_mib={used}")
        if float(used) < 500:
            ok += 1
if ok < 2:
    print("[king332-h121] ABORT: gpus 2,3 not free")
    sys.exit(1)
PY
  ...
  if [[ "$code" == "200" ]]; then
    ...
    if [[ -n "$mid" ]]; then
      ...
      if [[ "$pcode" == "200" ]]; then
        ...
        date -u +%Y-%m-%dT%H:%M:%SZ >/root/logs/h121_king_recover_pass332.done
        echo "$REPO $REV $TCACHE util=0.72" >>/root/logs/h121_king_recover_pass332.done
        log "DONE — retry_h121_n80 should proceed (watcher already armed)"
        exit 0
      fi
    fi
  fi
