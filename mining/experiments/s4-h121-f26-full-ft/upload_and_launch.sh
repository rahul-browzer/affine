#!/usr/bin/env bash
# Host → mine-f26-1: upload stack + data, start H121 full-FT bootstrap.
set -euo pipefail

ROOT=/home/const/subnet120
DST_HOST=${DST_HOST:?set DST_HOST}
DST_PORT=${DST_PORT:?set DST_PORT}
KNOWN=${KNOWN:-/tmp/mine-f26-1.known_hosts}
...
STAGE=$(mktemp -d /tmp/mine-h121-stack.XXXXXX)
...
TAR=/tmp/mine-h121-stack.tar.gz
tar -C "$STAGE" -czf "$TAR" .
ls -lh "$TAR"

ENV_TMP=$(mktemp /tmp/mine-h121.env.XXXXXX)
...
"${SSH[@]}" 'mkdir -p /root/mining_src /root/affine_data /root/logs /root/h121 /root/hf'
"${SCP[@]}" "$TAR" "root@${DST_HOST}:/tmp/mine-h121-stack.tar.gz"
...
repo = "unconst/Affine-5czsc2fc98-h121-fullft"
try:
...
"${SSH[@]}" 'set -e
  tar -C /root/mining_src -xzf /tmp/mine-h121-stack.tar.gz
  ...
  nohup bash /root/mining_src/s4-h121-f26-full-ft/bootstrap_h121.sh \
    >/root/logs/h121_pipeline.nohup 2>&1 &
  echo $! > /root/logs/h121_pipeline.pid
  nohup bash /root/mining_src/s4-h2-merge/watch_form_decision.sh h121 \
    ...
    >/root/logs/h121_form_decision.launch.out 2>&1 &
  ...
  echo PIPELINE_PID=$(cat /root/logs/h121_pipeline.pid)
  sleep 5
  head -n 40 /root/logs/bootstrap_h121.log 2>/dev/null || head -n 40 /root/logs/h121_pipeline.nohup || true
  ps -p "$(cat /root/logs/h121_pipeline.pid)" -o pid,etime,cmd || true
  nvidia-smi -L | wc -l
'
