#!/usr/bin/env bash
# H121: Tok-init thought-only full-FT on high-Λ2 z_A (F26; no LoRA).
...
n=$(wc -l <"$DATA")
echo "[h121] $(date -u +%Y-%m-%dT%H:%M:%SZ) examples=$n data=$DATA base=$BASE gpus=$CUDA_VISIBLE_DEVICES"
test "$n" -ge 200

echo "[h121] verify thought cuts (CPU)"
python3 /root/mining_src/s4-h1v2-sft/verify_thought_mask.py \
  --data "$DATA" \
  --out "$OUT/thought_mask_verify.json"

rm -f "$TRAIN_DIR/train.done" "$TRAIN_DIR/train_result.json"
echo "[h121] $(date -u +%Y-%m-%dT%H:%M:%SZ) launch full-FT lr=1e-6 on GPUs $CUDA_VISIBLE_DEVICES"
...
meta = {
    ...
    "out": "$TRAIN_DIR",
    "log": "$LOG",
    "note": "F26 Tok-init dense full-FT × high-Λ2 z_A (no LoRA) vs Tok331102",
}
Path("/root/affine_data/h121_train_launched.json").write_text(json.dumps(meta, indent=2) + "\n")
print(json.dumps(meta, indent=2))
PY
echo "[h121] TRAIN_LAUNCHED pid=$(cat /root/logs/h121_train.pid)"
