#!/usr/bin/env bash
# Host → mine-h53-1: upload stack + data, start bootstrap under nohup.
set -euo pipefail

ROOT=/home/const/subnet120
DST_HOST=${DST_HOST:?set DST_HOST}
DST_PORT=${DST_PORT:?set DST_PORT}
KNOWN=${KNOWN:-/tmp/mine-h53-1.known_hosts}
...
STAGE=$(mktemp -d /tmp/mine-h53-stack.XXXXXX)
trap 'rm -rf "$STAGE"' EXIT

EXP=s4-h53-m7-winner-za-lr4e6
...
TAR=/tmp/mine-h53-stack.tar.gz
tar -C "$STAGE" -czf "$TAR" .
ls -lh "$TAR"

ENV_TMP=$(mktemp /tmp/mine-h53.env.XXXXXX)
...
# H53 data = H29 king-self harvest (686 ex); init = m7 (same as H28).
DATA="$ROOT/mining/experiments/s4-h27-clip-l1-shape/results/winner_za_high_l1.jsonl"
test -s "$DATA"
test "$(wc -l <"$DATA")" -ge 300

"${SSH[@]}" 'mkdir -p /root/mining_src /root/affine_data /root/logs /root/h53 /root/hf'
"${SCP[@]}" "$TAR" "root@${DST_HOST}:/tmp/mine-h53-stack.tar.gz"
"${SCP[@]}" "$ENV_TMP" "root@${DST_HOST}:/root/mine.env"
"${SCP[@]}" "$DATA" "root@${DST_HOST}:/root/h53/winner_za_high_l1.jsonl"
rm -f "$ENV_TMP"
...
for repo in (
    "unconst/Affine-5czsc2fc98-h53-lora",
    "unconst/Affine-5czsc2fc98-h53-merged",
):
    try:
...
"${SSH[@]}" 'set -e
  tar -C /root/mining_src -xzf /tmp/mine-h53-stack.tar.gz
  chmod 600 /root/mine.env
  chmod +x /root/mining_src/s3-duel-sim/*.sh \
           /root/mining_src/s4-h2-merge/*.sh \
           /root/mining_src/s4-h53-m7-winner-za-lr4e6/*.sh
  test -f /root/mining_src/affine_pkg/affine/score.py
  test -s /root/h53/winner_za_high_l1.jsonl
  test -x /root/mining_src/s4-h53-m7-winner-za-lr4e6/bootstrap_h53.sh
  echo STACK_UPLOAD_OK
  nohup bash /root/mining_src/s4-h53-m7-winner-za-lr4e6/bootstrap_h53.sh \
    >/root/logs/h53_pipeline.nohup 2>&1 &
  echo $! > /root/logs/h53_pipeline.pid
  nohup bash /root/mining_src/s4-h2-merge/watch_form_decision.sh h53 \
    /root/affine_data/h53_sim_result.json /root/affine_data/h53_decision.json \
    /root/logs/h53_form_decision.nohup \
    >/root/logs/h53_form_decision.launch.out 2>&1 &
  echo $! > /root/logs/h53_form_decision.pid
  nohup bash /root/mining_src/s4-h2-merge/watch_n80_retry.sh h53 \
    /root/mining_src/s4-h53-m7-winner-za-lr4e6/retry_h53_n80.sh \
    >/root/logs/h53_watch_retry.launch.nohup 2>&1 &
  echo $! > /root/logs/h53_watch_retry.pid
  echo PIPELINE_PID=$(cat /root/logs/h53_pipeline.pid)
  sleep 3
  head -n 40 /root/logs/bootstrap_h53.log 2>/dev/null || head -n 40 /root/logs/h53_pipeline.nohup || true
  ps -p "$(cat /root/logs/h53_pipeline.pid)" -o pid,etime,cmd || true
'
