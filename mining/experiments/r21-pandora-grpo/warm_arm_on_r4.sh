#!/usr/bin/env bash
# Host → lean warm-arm R21 onto TKC-hot mine-r4-fullft-1 after R20 REFUTE.
# Keeps teacher:8000 + guass:8001; trains pandora-GRPO on GPUs 6–7 (≠ R15 REINFORCE).
set -euo pipefail
...
STAGE=$(mktemp -d /tmp/mine-r21-warm.XXXXXX)
...
cp -a "$ROOT/mining/experiments/r21-pandora-grpo/"*.sh "$STAGE/r21-pandora-grpo/"
cp -a "$ROOT/mining/experiments/r21-pandora-grpo/plan.md" "$STAGE/r21-pandora-grpo/"
# Overlay start/bootstrap for R21 identity grep.
...
TAR=/tmp/mine-r21-warm-stack.tar.gz
tar -C "$STAGE" -czf "$TAR" .
ls -lh "$TAR"
...
ENV_TMP=$(mktemp /tmp/mine-r21.env.XXXXXX)
{
  ...
  echo "export HF_MERGED_REPO=unconst/Affine-5czsc2fc98-r21-pandora-grpo"
  ...
  echo "export BASE=/root/hf/hub/models--pandora-box--Affine-5eqdtdzqle-ckpt300-m4/snapshots/5218b1383952ff7a8d49b1d7b82acfe5e1bd448d"
} >"$ENV_TMP"
chmod 600 "$ENV_TMP"
...
"${SCP[@]}" "$TAR" "root@${DST_HOST}:/tmp/mine-r21-warm-stack.tar.gz"
...
for repo in (
    "unconst/Affine-5czsc2fc98-r21-pandora-grpo",
    "unconst/Affine-5czsc2fc98-r21-pandora-grpo-lora",
):
    try:
...
"${SSH[@]}" 'set -euo pipefail
  tar -C /root/mining_src -xzf /tmp/mine-r21-warm-stack.tar.gz
  ...
  # Stop leftover R20 lean/post/form by pidfile only (never pkill -f).
  for f in /root/logs/r20_lean_warm.pid /root/logs/r20_post_train.pid \
           /root/logs/r20_form_decision.pid /root/logs/r20_n80_guass_p2211.pid \
  ...
  nohup bash /root/mining_src/r21-pandora-grpo/lean_warm_boot.sh \
    >/root/logs/r21_lean_warm.nohup 2>&1 &
  echo $! >/root/logs/r21_lean_warm.pid
  sleep 25
  echo LEAN_PID=$(cat /root/logs/r21_lean_warm.pid)
  head -n 80 /root/logs/r21_lean_warm.log 2>/dev/null \
    || head -n 80 /root/logs/r21_lean_warm.nohup || true
  ps -p "$(cat /root/logs/r21_lean_warm.pid)" -o pid,etime,cmd || true
  test -f /root/logs/r3_train.pid && echo TRAIN_PID=$(cat /root/logs/r3_train.pid)
  test -f /root/logs/r21_post_train.pid && echo POST_PID=$(cat /root/logs/r21_post_train.pid)
...
echo "R21_WARM_ARM_OK pod=$POD_NAME soft=$SOFT dead=$DEAD $(date -u +%Y-%m-%dT%H:%M:%SZ)"
