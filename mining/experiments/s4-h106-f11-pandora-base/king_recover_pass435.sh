...
LOGN=/root/logs/h106_king_recover_pass435.nohup
log() { echo "[king435-h106] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOGN"; }
...
for line in apps.strip().splitlines():
    ...
    if len(parts) >= 2 and parts[0] in uuids:
        try:
            ...
            pass
print(f"[king435-h106] reap gpus2,3 pids={sorted(pids)}")
for pid in pids:
    try:
...
rm -f /root/affine_data/h106_sim_result.json /root/affine_data/h106_sim_progress.json \
  ...
  /root/logs/h106_king_recover_pass311.done /root/logs/h106_sim_n80.done \
  /root/logs/h106_king_recover_pass332.done /root/logs/h106_king_recover_pass435.done
...
rm -rf /root/.triton/isolated/h106_king_p332_* 2>/dev/null || true

SEED=""
# Prefer live chall TRITON_CACHE_DIR (frozen, promptable)
for pid in $(pgrep -f 'vllm serve /root/h106/merged' || true); do
...
if [[ -z "$SEED" ]]; then
  for d in /root/.triton/isolated/h106_chall_*; do
    [[ -d "$d" ]] || continue
...
TCACHE="/root/.triton/isolated/h106_king_p435_$(date +%s)_$$"
mkdir -p "$TCACHE"
# Copy contents; leave SEED frozen (do not chmod seed)
cp -a "$SEED"/. "$TCACHE/"
chmod -R u+w "$TCACHE" || true
n_so=$(find "$TCACHE" -name '__triton_launcher*.so' | wc -l)
echo "$TCACHE" >/root/logs/h106_king_tcache_pass332.path
echo "$TCACHE" >/root/logs/h106_king_tcache_pass435.path
...
print(f"[king435-h106] launcher ghosts={bad}")
...
for line in out.strip().splitlines():
    ...
    if int(idx) in (2, 3):
        print(f"[king435-h106] gpu{idx} used_mib={used}")
        if float(used) < 500:
            ok += 1
if ok < 2:
    print("[king435-h106] ABORT: gpus 2,3 not free")
    sys.exit(1)
PY
  ...
  if [[ "$code" == "200" ]]; then
    ...
    if [[ -n "$mid" ]]; then
      ...
      if [[ "$pcode" == "200" ]]; then
        ...
        date -u +%Y-%m-%dT%H:%M:%SZ >/root/logs/h106_king_recover_pass435.done
        echo "$REPO $REV $TCACHE util=0.72 seeded_from_chall" >>/root/logs/h106_king_recover_pass435.done
        # also stamp 332.done so older watchers that look for it proceed
        cp /root/logs/h106_king_recover_pass435.done /root/logs/h106_king_recover_pass332.done
