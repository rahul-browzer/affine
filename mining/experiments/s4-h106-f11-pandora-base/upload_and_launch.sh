#!/usr/bin/env bash
# Host → mine-f11-1: upload stack + data, start bootstrap under nohup.
set -euo pipefail

ROOT=/home/const/subnet120
DST_HOST=${DST_HOST:?set DST_HOST}
DST_PORT=${DST_PORT:?set DST_PORT}
KNOWN=${KNOWN:-/tmp/mine-f11-1.known_hosts}
...
STAGE=$(mktemp -d /tmp/mine-h106-stack.XXXXXX)
trap 'rm -rf "$STAGE"' EXIT

EXP=s4-h106-f11-pandora-base
...
TAR=/tmp/mine-h106-stack.tar.gz
tar -C "$STAGE" -czf "$TAR" .
ls -lh "$TAR"

ENV_TMP=$(mktemp /tmp/mine-h106.env.XXXXXX)
...
# H106/F11 data = high-Λ2 z_A harvest (1059 ex); init = pandora past-crown.
DATA="$ROOT/mining/experiments/s4-h99-f2-target-l2/results/winner_za_high_l2.jsonl"
test -s "$DATA"
test "$(wc -l <"$DATA")" -ge 300

"${SSH[@]}" 'mkdir -p /root/mining_src /root/affine_data /root/logs /root/h106 /root/hf'
"${SCP[@]}" "$TAR" "root@${DST_HOST}:/tmp/mine-h106-stack.tar.gz"
"${SCP[@]}" "$ENV_TMP" "root@${DST_HOST}:/root/mine.env"
"${SCP[@]}" "$DATA" "root@${DST_HOST}:/root/h106/winner_za_high_l2.jsonl"
rm -f "$ENV_TMP"
...
for repo in (
    "unconst/Affine-5czsc2fc98-h106-lora",
    "unconst/Affine-5czsc2fc98-h106-merged",
):
    try:
...
"${SSH[@]}" 'set -e
  tar -C /root/mining_src -xzf /tmp/mine-h106-stack.tar.gz
  chmod 600 /root/mine.env
  chmod +x /root/mining_src/s3-duel-sim/*.sh \
           /root/mining_src/s4-h2-merge/*.sh \
           /root/mining_src/s4-h106-f11-pandora-base/*.sh
  test -f /root/mining_src/affine_pkg/affine/score.py
  test -s /root/h106/winner_za_high_l2.jsonl
  test -x /root/mining_src/s4-h106-f11-pandora-base/bootstrap_h106.sh
  echo STACK_UPLOAD_OK
  nohup bash /root/mining_src/s4-h106-f11-pandora-base/bootstrap_h106.sh \
    >/root/logs/h106_pipeline.nohup 2>&1 &
  echo $! > /root/logs/h106_pipeline.pid
  nohup bash /root/mining_src/s4-h2-merge/watch_form_decision.sh h106 \
    ...
    >/root/logs/h106_form_decision.launch.out 2>&1 &
  ...
  nohup bash /root/mining_src/s4-h106-f11-pandora-base/watch_preempt_bare_tcache_pass264.sh \
    >/root/logs/h106_preempt_bare_pass264.launch.nohup 2>&1 &
  echo $! > /root/logs/h106_preempt_bare_pass264.pid
  echo PREEMPT_PID=$(cat /root/logs/h106_preempt_bare_pass264.pid)
  echo PIPELINE_PID=$(cat /root/logs/h106_pipeline.pid)
  sleep 3
  head -n 40 /root/logs/bootstrap_h106.log 2>/dev/null || head -n 40 /root/logs/h106_pipeline.nohup || true
  ps -p "$(cat /root/logs/h106_pipeline.pid)" -o pid,etime,cmd || true
'
