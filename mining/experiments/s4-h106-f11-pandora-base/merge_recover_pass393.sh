#!/usr/bin/env bash
# H106/F11 pass393: GPU merge save hung on gocryptfs request_wait_answer
...
EXP=/root/mining_src/s4-h106-f11-pandora-base
LOG=/root/logs/h106_merge_recover_pass393.nohup
PIDF=/root/logs/h106_merge_recover_pass393.pid
MERGED=/root/h106/merged
ADAPTER=/root/h106/train/adapter
BASE=/root/hf/hub/models--pandora-box--Affine-5eqdtdzqle-ckpt300-m4/snapshots/5218b1383952ff7a8d49b1d7b82acfe5e1bd448d

log() { echo "[h106-merge393] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }

mkdir -p /root/logs /root/affine_data
: >"$LOG"
echo $$ >"$PIDF"

MERGE_PID=$(ps -eo pid,cmd | awk '/[p]ython .*merge_lora.py --base .*h106\/merged/{print $1; exit}')
POST_PID=$(ps -eo pid,cmd | awk '/[b]ash .*s4-h106-f11-pandora-base\/post_train_pipeline\.sh/{print $1; exit}')
...
rm -f /root/logs/h106_pipeline.aborted
...
if [[ -x "$EXP/watch_preempt_bare_tcache_pass264.sh" ]]; then
  nohup bash "$EXP/watch_preempt_bare_tcache_pass264.sh" \
    >>/root/logs/h106_preempt_bare_pass264.log 2>&1 &
  echo $! >/root/logs/h106_preempt_bare_pass264.pid
  log "rearmed preempt264 pid=$(cat /root/logs/h106_preempt_bare_pass264.pid)"
fi
...
log "post_train resumed pid=$(cat /root/logs/h106_post_train.pid) SKIP_MERGE=1"
log "DONE recover393"
