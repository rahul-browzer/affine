#!/usr/bin/env bash
# Pass 177: king died again on Triton __triton_launcher.so race after health=200.
...
LOG=/root/logs/h23_king_recover_pass177.log
...
log() { echo "[recover177] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }
...
print(f"[recover177] reap gpu={sorted(want)} workers={sorted(pids)} parents={sorted(parents)}")
for pid in kill_set:
    try:
...
# Wipe half-written Triton / flashinfer sampling for king role.
...
set -euo pipefail
LOG=/root/logs/h23_king_recover_pass177.log
echo "[recover-wait177] $(date -u +%Y-%m-%dT%H:%M:%SZ) waiting king health+probe" >>"$LOG"
ok=0
for i in $(seq 1 160); do
  ...
  if [[ "$k" == "200" && "$t" == "200" && "$c" == "200" ]]; then
    echo "[recover-wait177] health t=$t k=$k c=$c at i=$i $(date -u +%Y-%m-%dT%H:%M:%SZ)" >>"$LOG"
    ...
    echo "[recover-wait177] king_completions_probe=$probe i=$i" >>"$LOG"
    if [[ "$probe" == "ok" ]]; then
      ok=1
      break
    fi
  ...
  if (( i % 10 == 0 )); then
    echo "[recover-wait177] i=$i t=$t k=$k c=$c $(date -u +%Y-%m-%dT%H:%M:%SZ)" >>"$LOG"
  fi
  sleep 15
done
if [[ "$ok" != "1" ]]; then
  echo "[recover-wait177] ERROR king never became promptable" >>"$LOG"
  exit 1
fi
for p in $(ps -eo pid,cmd | awk "/[s]tart_h23_n80\\.sh/ {print \$1}"); do
  kill -9 "$p" 2>/dev/null || true
...
nohup bash /root/mining_src/s4-h23-tp-talucampe-a90/start_h23_n80.sh \
  >/root/logs/h23_n80_relaunch_pass177.nohup 2>&1 &
echo $! >/root/logs/h23_n80_relaunch_pass177.pid
echo "[recover-wait177] relaunched start_h23_n80 pid=$(cat /root/logs/h23_n80_relaunch_pass177.pid) $(date -u +%Y-%m-%dT%H:%M:%SZ)" >>"$LOG"
' >/root/logs/h23_recover_wait177.nohup 2>&1 &
