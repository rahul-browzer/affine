# mine-h23-1 bootstrap: TalentPigs×Talucampe ck5 α0.90 + teacher → merge/serve/n80.
# Secrets from /root/mine.env only (never argv).
set -euo pipefail

LOG=/root/logs/bootstrap_h23.log
mkdir -p /root/logs /root/hf /root/affine_data /root/mining_src /root/merges
exec > >(tee -a "$LOG") 2>&1

echo "[bootstrap-h23] $(date -u +%Y-%m-%dT%H:%M:%SZ) start host=$(hostname)"
...
if [[ -z "${HF_TOKEN:-}" ]]; then
  echo "[bootstrap-h23] FATAL: HF_TOKEN missing in /root/mine.env"
  exit 1
fi
...
uv pip install \
  ...
  "numpy" \
  "scipy" \
  2>&1 | tee /root/logs/pip_h23.log | tail -40

python - <<'PY'
import torch, transformers, vllm
print("[bootstrap-h23] VERSIONS",
...
if txt == orig:
    print("[bootstrap-h23] vllm_client already patched or pattern miss", flush=True)
else:
    p.write_text(txt)
    print("[bootstrap-h23] patched vllm_client timeout=480 retries=5", flush=True)
PY

B_REPO=Talucampe037/Affine-5f6xxabdmp-ck5
B_REV=da35105f9e3d43e5d30ddc913d8a3d4f63865ffb
...
jobs = [
    ...
    ("$B_REPO", "$B_REV", "talucampe"),
    ("zai-org/GLM-4.5-Air-FP8", None, "teacher"),
]
for repo, rev, label in jobs:
    done = f"/root/logs/{label}.done"
    if os.path.isfile(done):
        print(f"[bootstrap-h23] SKIP {label} (have {done})", flush=True)
        continue
    print(f"[bootstrap-h23] DOWNLOAD start {label} {repo} rev={rev}", flush=True)
    path = snapshot_download(repo, revision=rev, token=token)
    print(f"[bootstrap-h23] DOWNLOAD done {label} -> {path}", flush=True)
    open(done, "w").write(path + "\n")
print("[bootstrap-h23] ALL_DOWNLOADS_OK", flush=True)
...
python /root/mining_src/s4-h2-merge/merge_linear.py \
  ...
  --out "$OUT" \
  --hf-home "$HF_HOME" \
  2>&1 | tee /root/logs/h23_merge.log

date -u +%Y-%m-%dT%H:%M:%SZ > /root/logs/h23_merge.done
echo "[bootstrap-h23] MERGE_DONE alpha=$ALPHA out=$OUT b=$B_REPO@$B_REV"
...
export CHALL_REPO=/root/merges/h23-tp90
export CHALL_REV=
bash /root/mining_src/s3-duel-sim/serve_three.sh

touch /root/logs/bootstrap_h23.done
echo "[bootstrap-h23] $(date -u +%Y-%m-%dT%H:%M:%SZ) DONE — engines launching; next: wait_ready + n80"
