#!/usr/bin/env bash
# Host → mine-h44-1: upload stack + data, start bootstrap under nohup.
set -euo pipefail

ROOT=/home/const/subnet120
DST_HOST=${DST_HOST:?set DST_HOST}
DST_PORT=${DST_PORT:?set DST_PORT}
KNOWN=${KNOWN:-/tmp/mine-h44-1.known_hosts}
...
STAGE=$(mktemp -d /tmp/mine-h44-stack.XXXXXX)
trap 'rm -rf "$STAGE"' EXIT

EXP=s4-h44-m7-winner-za-clip08
...
TAR=/tmp/mine-h44-stack.tar.gz
tar -C "$STAGE" -czf "$TAR" .
ls -lh "$TAR"

ENV_TMP=$(mktemp /tmp/mine-h44.env.XXXXXX)
...
# H44 data = winner-zA filtered clipL1≥0.08 (305 ex); init = m7 (H28 cell).
DATA="$ROOT/mining/experiments/s4-h44-m7-winner-za-clip08/winner_za_clip08.jsonl"
test -s "$DATA"
test "$(wc -l <"$DATA")" -ge 250

"${SSH[@]}" 'mkdir -p /root/mining_src /root/affine_data /root/logs /root/h44 /root/hf'
"${SCP[@]}" "$TAR" "root@${DST_HOST}:/tmp/mine-h44-stack.tar.gz"
"${SCP[@]}" "$ENV_TMP" "root@${DST_HOST}:/root/mine.env"
"${SCP[@]}" "$DATA" "root@${DST_HOST}:/root/h44/winner_za_clip08.jsonl"
rm -f "$ENV_TMP"
...
for repo in (
    "unconst/Affine-5czsc2fc98-h44-lora",
    "unconst/Affine-5czsc2fc98-h44-merged",
):
    try:
...
"${SSH[@]}" 'set -e
  tar -C /root/mining_src -xzf /tmp/mine-h44-stack.tar.gz
  chmod 600 /root/mine.env
  chmod +x /root/mining_src/s3-duel-sim/*.sh \
           /root/mining_src/s4-h2-merge/*.sh \
           /root/mining_src/s4-h44-m7-winner-za-clip08/*.sh
  test -f /root/mining_src/affine_pkg/affine/score.py
  test -s /root/h44/winner_za_clip08.jsonl
  test -x /root/mining_src/s4-h44-m7-winner-za-clip08/bootstrap_h44.sh
  echo STACK_UPLOAD_OK
  nohup bash /root/mining_src/s4-h44-m7-winner-za-clip08/bootstrap_h44.sh \
    >/root/logs/h44_pipeline.nohup 2>&1 &
  echo $! > /root/logs/h44_pipeline.pid
  nohup bash /root/mining_src/s4-h2-merge/watch_form_decision.sh h44 \
    /root/affine_data/h44_sim_result.json /root/affine_data/h44_decision.json \
    /root/logs/h44_form_decision.nohup \
    >/root/logs/h44_form_decision.launch.out 2>&1 &
  echo $! > /root/logs/h44_form_decision.pid
  nohup bash /root/mining_src/s4-h2-merge/watch_n80_retry.sh h44 \
    /root/mining_src/s4-h44-m7-winner-za-clip08/retry_h44_n80.sh \
    >/root/logs/h44_watch_retry.launch.nohup 2>&1 &
  echo $! > /root/logs/h44_watch_retry.pid
  echo PIPELINE_PID=$(cat /root/logs/h44_pipeline.pid)
  sleep 3
  head -n 40 /root/logs/bootstrap_h44.log 2>/dev/null || head -n 40 /root/logs/h44_pipeline.nohup || true
  ps -p "$(cat /root/logs/h44_pipeline.pid)" -o pid,etime,cmd || true
'
