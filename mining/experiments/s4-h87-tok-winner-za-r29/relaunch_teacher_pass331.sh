#!/usr/bin/env bash
# Pass 331: H87 teacher dead — Triton ENOENT on bare cache/teacher
# (4NLUGJ4U…/__triton_launcher.so @15:55:22Z → EngineDead; :8000=000).
# King :8001 OK. Merge writing on GPUs 6,7. Reap 0,1 → wipe teacher* →
# settle20 → unique TCACHE → vllm serve GLM :8000. Leave king/merge alone.
set -euo pipefail
HYPO=h87
...
LOG=/root/logs/${HYPO}_teacher_recover_pass331.log
TEACHER_LOG=/root/logs/vllm_teacher.log
PIDF=/root/logs/vllm_teacher.pid
export GPUS=0,1
UTIL=0.80
TEACHER_REPO=zai-org/GLM-4.5-Air-FP8
TAG=teacher_p331_$(date +%s)_$$
mkdir -p /root/logs
: >"$LOG"
log() { echo "[recover331-${HYPO}] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }
...
print(f"[recover331] reap gpu={sorted(want)} workers={sorted(pids)} parents={sorted(parents)} grandparents={sorted(grandparents)}")
for pid in kill_set:
...
log "DONE_LAUNCH (king :8001 stays; merge on 6,7; post_train resumes on t=200)"
