#!/usr/bin/env bash
# Host → mine-h11-1: upload harness + mine.env, start bootstrap+n80 under nohup.
set -euo pipefail

ROOT=/home/const/subnet120
DST_HOST=${DST_HOST:?set DST_HOST}
DST_PORT=${DST_PORT:?set DST_PORT}
KNOWN=${KNOWN:-/tmp/mine-h11-1.known_hosts}
...
STAGE=$(mktemp -d /tmp/mine-h11-stack.XXXXXX)
trap 'rm -rf "$STAGE"' EXIT

mkdir -p "$STAGE/affine_pkg/affine" "$STAGE/affine_pkg/evalsrv" \
         "$STAGE/s3-duel-sim" "$STAGE/s4-h2-merge" "$STAGE/s4-h11-tp-adambell-merge"
...
cp -a "$ROOT/mining/experiments/s4-h11-tp-adambell-merge/bootstrap_h11.sh" "$STAGE/s4-h11-tp-adambell-merge/"
cp -a "$ROOT/mining/experiments/s4-h11-tp-adambell-merge/start_h11_n80.sh" "$STAGE/s4-h11-tp-adambell-merge/"
cp -a "$ROOT/mining/experiments/s4-h11-tp-adambell-merge/plan.md" "$STAGE/s4-h11-tp-adambell-merge/"

TAR=/tmp/mine-h11-stack.tar.gz
tar -C "$STAGE" -czf "$TAR" .
ls -lh "$TAR"

ENV_TMP=$(mktemp /tmp/mine-h11.env.XXXXXX)
...
"${SCP[@]}" "$TAR" "root@${DST_HOST}:/tmp/mine-h11-stack.tar.gz"
"${SCP[@]}" "$ENV_TMP" "root@${DST_HOST}:/root/mine.env"
rm -f "$ENV_TMP"

"${SSH[@]}" 'set -e
  tar -C /root/mining_src -xzf /tmp/mine-h11-stack.tar.gz
  chmod 600 /root/mine.env
  chmod +x /root/mining_src/s3-duel-sim/*.sh \
           /root/mining_src/s4-h2-merge/watch_fix_decision.sh \
           /root/mining_src/s4-h11-tp-adambell-merge/*.sh
  test -f /root/mining_src/affine_pkg/affine/score.py
  test -x /root/mining_src/s4-h11-tp-adambell-merge/bootstrap_h11.sh
  test -x /root/mining_src/s4-h11-tp-adambell-merge/start_h11_n80.sh
  echo STACK_UPLOAD_OK
  nohup bash -lc "
    set -euo pipefail
    bash /root/mining_src/s4-h11-tp-adambell-merge/bootstrap_h11.sh
    bash /root/mining_src/s4-h11-tp-adambell-merge/start_h11_n80.sh
  " >/root/logs/h11_pipeline.nohup 2>&1 &
  echo $! > /root/logs/h11_pipeline.pid
  # Sidecar nested-verdict watcher (failsafe if start_*.sh path drifts).
  nohup bash /root/mining_src/s4-h2-merge/watch_fix_decision.sh h11 \
    >/root/logs/h11_fix_decision.nohup 2>&1 &
  echo $! > /root/logs/h11_fix_decision.pid
  echo PIPELINE_PID=$(cat /root/logs/h11_pipeline.pid)
  sleep 2
  head -n 20 /root/logs/h11_pipeline.nohup || true
  ps -p "$(cat /root/logs/h11_pipeline.pid)" -o pid,etime,cmd || true
'
