#!/usr/bin/env bash
# Host → mine-r25-hitemp-1: R3 stack + R25 HiTemp-GRPO overlay, bootstrap.
# Axis R25: Tok-init Reason-GRPO temperature=1.2 (≠ R3 0.8).
set -euo pipefail

ROOT=/home/const/subnet120
POD_NAME=${POD_NAME:-mine-r25-hitemp-1}
...
cp -a "$ROOT/mining/experiments/r25-hitemp-grpo/plan.md" "$STAGE/r25-hitemp-grpo/"
cp -a "$ROOT/mining/experiments/r25-hitemp-grpo/start_r25.sh" "$STAGE/r25-hitemp-grpo/"
# Overlay: bootstrap_r3 calls start_r3.sh — replace with R25 hitemp knobs.
cp -a "$ROOT/mining/experiments/r25-hitemp-grpo/start_r25.sh" "$STAGE/$EXP/start_r3.sh"
...
TAR=/tmp/mine-r25-stack.tar.gz
tar -C "$STAGE" -czf "$TAR" .
ls -lh "$TAR"

ENV_TMP=$(mktemp /tmp/mine-r25.env.XXXXXX)
...
umask 077
{
  ...
  echo "export HF_LORA_REPO=unconst/Affine-5czsc2fc98-r25-lora"
  echo "export HF_MERGED_REPO=unconst/Affine-5czsc2fc98-r25-merged"
  echo "export R25_AXIS=hitemp_grpo_reason"
  echo "export R25_LR=5e-6"
  echo "export R25_LORA_R=16"
  echo "export R25_LORA_ALPHA=32"
  echo "export R25_GROUP_SIZE=4"
  echo "export R25_MAX_STEPS=200"
  echo "export R25_MAX_LEN=6144"
  echo "export R25_MAX_NEW=512"
  echo "export R25_TEMPERATURE=1.2"
} >"$ENV_TMP"
chmod 600 "$ENV_TMP"
...
"${SSH[@]}" 'mkdir -p /root/mining_src /root/affine_data /root/logs /root/r3 /root/r25 /root/hf'
"${SCP[@]}" "$TAR" "root@${DST_HOST}:/tmp/mine-r25-stack.tar.gz"
"${SCP[@]}" "$ENV_TMP" "root@${DST_HOST}:/root/mine.env"
"${SCP[@]}" "$DATA" "root@${DST_HOST}:/root/r3/winner_za_high_l1.jsonl"
"${SCP[@]}" "$DATA" "root@${DST_HOST}:/root/r25/winner_za_high_l1.jsonl"
rm -f "$ENV_TMP"
...
for repo in (
    "unconst/Affine-5czsc2fc98-r25-lora",
    "unconst/Affine-5czsc2fc98-r25-merged",
):
    try:
...
"${SSH[@]}" 'set -e
  tar -C /root/mining_src -xzf /tmp/mine-r25-stack.tar.gz
  ...
  chmod +x /root/mining_src/s3-duel-sim/*.sh \
           ...
           /root/mining_src/r25-hitemp-grpo/*.sh
  ...
  # Prove overlay is R25 HiTemp, not stock R3 / R24 / parent-GRPO.
  grep -q "R25: HiTemp-GRPO" /root/mining_src/r3-reason-grpo/start_r3.sh
  grep -q "temperature=1.2" /root/mining_src/r3-reason-grpo/start_r3.sh
  set -a; source /root/mine.env; set +a
  echo "R25_DEADLINES soft=$SOFT_DEADLINE_UTC dead=$DEADMAN_UTC axis=$R25_AXIS"
  echo "R25_KNOBS temp=${R25_TEMPERATURE} lr=${R25_LR} r=${R25_LORA_R} G=${R25_GROUP_SIZE} max_len=${R25_MAX_LEN}"
  echo STACK_UPLOAD_OK
  nohup bash /root/mining_src/r3-reason-grpo/bootstrap_r3.sh \
    >/root/logs/r25_pipeline.nohup 2>&1 &
  echo $! > /root/logs/r25_pipeline.pid
  cp -f /root/logs/r25_pipeline.pid /root/logs/r3_pipeline.pid
  echo PIPELINE_PID=$(cat /root/logs/r25_pipeline.pid)
  sleep 5
  head -n 40 /root/logs/bootstrap_r3.log 2>/dev/null || head -n 40 /root/logs/r25_pipeline.nohup || true
  ps -p "$(cat /root/logs/r25_pipeline.pid)" -o pid,etime,cmd || true
  nvidia-smi -L | wc -l
'
