...
# Teacher+chall stay up. Do NOT touch chall (recover274 / GPUs 4,5).
set -euo pipefail
...
LOGN=/root/logs/h85_king_recover_pass311.nohup
log() { echo "[king311-h85] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOGN"; }
...
for line in apps.strip().splitlines():
    ...
    if len(parts) >= 2 and parts[0] in uuids:
        try:
            ...
            pass
print(f"[king311-h85] reap gpus2,3 pids={sorted(pids)}")
for pid in pids:
    try:
...
rm -f /root/affine_data/h85_sim_result.json /root/affine_data/h85_sim_progress.json \
  /root/affine_data/h85_decision.json /root/logs/h85_n80.done /root/logs/h85_n80_retry.aborted
log "cleared stale h85 sim progress/result"
...
for line in out.strip().splitlines():
    ...
    if int(idx) in (2, 3):
        print(f"[king311-h85] gpu{idx} used_mib={used}")
        if float(used) < 500:
            ok += 1
if ok < 2:
    print("[king311-h85] ABORT: gpus 2,3 not free")
    sys.exit(1)
PY
  ...
  if [[ "$code" == "200" ]]; then
    ...
    if [[ -n "$mid" ]]; then
      ...
      if [[ "$pcode" == "200" ]]; then
        ...
        date -u +%Y-%m-%dT%H:%M:%SZ >/root/logs/h85_king_recover_pass311.done
        echo "$REPO $REV" >>/root/logs/h85_king_recover_pass311.done
        log "DONE — retry_h85_n80 should proceed (watcher already armed)"
        exit 0
      fi
    fi
  fi
