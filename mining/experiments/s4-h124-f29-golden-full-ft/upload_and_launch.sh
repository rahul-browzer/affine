#!/usr/bin/env bash
# Host → mine-f29-1: upload stack + data, start H124 full-FT bootstrap.
set -euo pipefail

ROOT=/home/const/subnet120
DST_HOST=${DST_HOST:?set DST_HOST}
DST_PORT=${DST_PORT:?set DST_PORT}
KNOWN=${KNOWN:-/tmp/mine-f29-1.known_hosts}
...
STAGE=$(mktemp -d /tmp/mine-h124-stack.XXXXXX)
trap 'rm -rf "$STAGE"' EXIT

EXP=s4-h124-f29-golden-full-ft
...
TAR=/tmp/mine-h124-stack.tar.gz
tar -C "$STAGE" -czf "$TAR" .
ls -lh "$TAR"

ENV_TMP=$(mktemp /tmp/mine-h124.env.XXXXXX)
...
"${SSH[@]}" 'mkdir -p /root/mining_src /root/affine_data /root/logs /root/h124 /root/hf'
"${SCP[@]}" "$TAR" "root@${DST_HOST}:/tmp/mine-h124-stack.tar.gz"
"${SCP[@]}" "$ENV_TMP" "root@${DST_HOST}:/root/mine.env"
"${SCP[@]}" "$DATA" "root@${DST_HOST}:/root/h124/winner_za_high_l2.jsonl"
rm -f "$ENV_TMP"
...
repo = "unconst/Affine-5czsc2fc98-h124-goldfullft"
try:
...
"${SSH[@]}" 'set -e
  tar -C /root/mining_src -xzf /tmp/mine-h124-stack.tar.gz
  chmod 600 /root/mine.env
  chmod +x /root/mining_src/s3-duel-sim/*.sh \
           /root/mining_src/s4-h2-merge/*.sh \
           /root/mining_src/s4-h124-f29-golden-full-ft/*.sh
  test -f /root/mining_src/affine_pkg/affine/score.py
  test -s /root/h124/winner_za_high_l2.jsonl
  test -x /root/mining_src/s4-h124-f29-golden-full-ft/bootstrap_h124.sh
  echo STACK_UPLOAD_OK
  nohup bash /root/mining_src/s4-h124-f29-golden-full-ft/bootstrap_h124.sh \
    >/root/logs/h124_pipeline.nohup 2>&1 &
  echo $! > /root/logs/h124_pipeline.pid
  nohup bash /root/mining_src/s4-h2-merge/watch_form_decision.sh h124 \
    /root/affine_data/h124_sim_result.json /root/affine_data/h124_decision.json \
    /root/logs/h124_form_decision.nohup \
    >/root/logs/h124_form_decision.launch.out 2>&1 &
  ...
  echo PIPELINE_PID=$(cat /root/logs/h124_pipeline.pid)
  sleep 5
  head -n 40 /root/logs/bootstrap_h124.log 2>/dev/null || head -n 40 /root/logs/h124_pipeline.nohup || true
  ps -p "$(cat /root/logs/h124_pipeline.pid)" -o pid,etime,cmd || true
  nvidia-smi -L | wc -l
'
