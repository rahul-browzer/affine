#!/usr/bin/env bash
# After H123 train.done: finalize visual → chall:8002 → n80 vs Tok331102 king.
set -euo pipefail
...
log() { echo "[h123-pipe] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }

_train_alive() {
  if [[ -f /root/logs/h123_train.pid ]]; then
    local tpid
    tpid=$(cat /root/logs/h123_train.pid 2>/dev/null || true)
    if [[ -n "${tpid:-}" ]] && kill -0 "$tpid" 2>/dev/null; then
      return 0
    fi
  fi
  pgrep -f "python3 /root/mining_src/s4-h123-f28-teacher-refs-ft/train_full.py --base" >/dev/null 2>&1
}
...
_abort_on_exit() {
  ...
  if [[ -f /root/logs/h123_pipeline.done ]]; then
    return 0
  fi
  if [[ ! -f /root/logs/h123_pipeline.aborted ]]; then
    echo "aborted_err_rc=${rc} $(date -u +%Y-%m-%dT%H:%M:%SZ)" \
      >/root/logs/h123_pipeline.aborted
    echo "[h123-pipe] $(date -u +%Y-%m-%dT%H:%M:%SZ) EXIT trap wrote aborted_err_rc=${rc}" \
      | tee -a "$LOG" >/dev/null 2>&1 || true
  fi
}
trap _abort_on_exit EXIT

mkdir -p /root/logs /root/affine_data /root/h123
rm -f /root/logs/h123_pipeline.aborted /root/logs/h123_pipeline.done \
  /root/logs/h123_merge.done /root/logs/h123_chall_serve.done \
  /root/logs/h123_sim_n80.done
...
while true; do
  ...
  if (( now > soft - 3600 )); then
    ...
    echo "aborted_no_train $(date -u +%Y-%m-%dT%H:%M:%SZ)" > /root/logs/h123_pipeline.aborted
    exit 1
  fi
  _wait_i=$((_wait_i + 1))
...
if _train_alive; then
  ...
  echo "aborted_train_stuck $(date -u +%Y-%m-%dT%H:%M:%SZ)" > /root/logs/h123_pipeline.aborted
  exit 1
fi
sleep 15
...
if [[ ! -d "$FULL_FT" ]]; then
  ...
  echo "aborted_no_full_ft $(date -u +%Y-%m-%dT%H:%M:%SZ)" >/root/logs/h123_pipeline.aborted
  exit 1
fi
...
if [[ "${SKIP_MERGE:-0}" == "1" && -f "$MERGED/config.json" ]] \
  ...
  rm -rf "$MERGED"
  python3 /root/mining_src/s4-h123-f28-teacher-refs-ft/finalize_full_ft.py \
    --base "$BASE" \
    --full-ft "$FULL_FT" \
    --out "$MERGED" \
    --king "$KING_LOCAL" \
...
HF_MERGED_REPO=${HF_MERGED_REPO:-unconst/Affine-5czsc2fc98-h123-trefsft}
if [[ -n "${HF_TOKEN:-}" ]]; then
  ...
  nohup python3 /root/mining_src/s4-h1-sft/push_merged.py \
    ...
    --repo "$HF_MERGED_REPO" \
    --public \
    --commit-message "H123 Tok full-FT high-Λ2 salvage (TTL insurance; not a submission)" \
    --out-meta /root/affine_data/h123_merged_salvage.json \
    >>/root/logs/h123_push_merged.nohup 2>&1 &
  echo $! >/root/logs/h123_push_merged.pid
fi
...
if [[ -x /root/mining_src/s4-h123-f28-teacher-refs-ft/watch_preempt_bare_tcache_pass264.sh ]]; then
  nohup bash /root/mining_src/s4-h123-f28-teacher-refs-ft/watch_preempt_bare_tcache_pass264.sh \
    >/root/logs/h123_preempt.nohup 2>&1 &
  echo $! >/root/logs/h123_preempt.pid
fi

date -u +%Y-%m-%dT%H:%M:%SZ > /root/logs/h123_chall_serve.done
log "CHALL_SERVE_DONE — n80 owned by watch_n80_retry / retry_h123_n80_d203first"
date -u +%Y-%m-%dT%H:%M:%SZ > /root/logs/h123_pipeline.done
