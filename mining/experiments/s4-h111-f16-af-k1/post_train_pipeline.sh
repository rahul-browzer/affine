#!/usr/bin/env bash
# After H111 train.done: merge LoRA → chall:8002 → n80 vs Tok331102 king.
# Base = af-k1 (train init ≠ king); sim king = live Tok331102. Prewarm holds teacher+king.
set -euo pipefail
...
BASE=${BASE:-/root/hf/hub/models--af-k1--Affine-5ECeJJpEMjW4pxM9eGyJ5ua3Sebfyr8kcVwLAdaiJLUC8pkW/snapshots/ff6eb4bcff3e7c6b8c0e097bc0cffa4fa2ba8e01}
...
TRAIN_DIR=${TRAIN_DIR:-/root/h111/train}
ADAPTER=${ADAPTER:-$TRAIN_DIR/adapter}
CKPT_ROOT=${CKPT_ROOT:-$TRAIN_DIR/checkpoints}
MERGED=${MERGED:-/root/h111/merged}
SIM_N80=/root/affine_data/h111_sim_result.json
PROG=/root/affine_data/h111_sim_progress.json
LOG=/root/logs/h111_pipeline.nohup
# Pass376: calm-wolf-30 remove≈2026-08-09T07:18Z → soft=TTL−1h, deadman=TTL−30m
# remove_at≈2026-08-09T12:37Z → soft=TTL−1h, deadman=TTL−30m
SOFT_DEADLINE_UTC=${SOFT_DEADLINE_UTC:-2026-08-09T12:57:00Z}
DEADMAN_UTC=${DEADMAN_UTC:-2026-08-09T13:27:00Z}

log() { echo "[h111-pipe] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }
...
_train_alive() {
  if [[ -f /root/logs/h111_train.pid ]]; then
    local tpid
    tpid=$(cat /root/logs/h111_train.pid 2>/dev/null || true)
...
_abort_on_exit() {
  ...
  if [[ -f /root/logs/h111_pipeline.done ]]; then
    return 0
  fi
  if [[ ! -f /root/logs/h111_pipeline.aborted ]]; then
    echo "aborted_err_rc=${rc} $(date -u +%Y-%m-%dT%H:%M:%SZ)" \
      >/root/logs/h111_pipeline.aborted
    echo "[h111-pipe] $(date -u +%Y-%m-%dT%H:%M:%SZ) EXIT trap wrote aborted_err_rc=${rc}" \
      | tee -a "$LOG" >/dev/null 2>&1 || true
  fi
}
trap _abort_on_exit EXIT

mkdir -p /root/logs /root/affine_data /root/h111
rm -f /root/logs/h111_pipeline.aborted /root/logs/h111_pipeline.done \
  /root/logs/h111_merge.done /root/logs/h111_chall_serve.done \
  /root/logs/h111_sim_n80.done
...
while true; do
  ...
  if (( now > soft - 3600 )); then
    ...
    echo "aborted_no_train $(date -u +%Y-%m-%dT%H:%M:%SZ)" > /root/logs/h111_pipeline.aborted
    exit 1
  fi
  _wait_i=$((_wait_i + 1))
...
if _train_alive; then
  ...
  echo "aborted_train_stuck $(date -u +%Y-%m-%dT%H:%M:%SZ)" > /root/logs/h111_pipeline.aborted
  exit 1
fi
sleep 15
...
# mid-shard on gocryptfs (H95@18:56Z; H111/F16 p376).
...
cp -f "$MERGED/merge_meta.json" /root/affine_data/h111_merge_meta.json 2>/dev/null || true
date -u +%Y-%m-%dT%H:%M:%SZ > /root/logs/h111_merge.done
...
Path("/root/affine_data/h111_identity.json").write_text(
...
print("[h111] OK_NON_IDENTICAL", flush=True)
PY

HF_LORA_REPO=${HF_LORA_REPO:-unconst/Affine-5czsc2fc98-h111-lora}
HF_MERGED_REPO=${HF_MERGED_REPO:-unconst/Affine-5czsc2fc98-h111-merged}
HF_BASE_HUB=${HF_BASE_HUB:-af-k1/Affine-5ECeJJpEMjW4pxM9eGyJ5ua3Sebfyr8kcVwLAdaiJLUC8pkW}
SEEN_MID=${SEEN_MID:-/root/h111/mid_ckpt_salvaged.txt}
if [[ -n "${HF_TOKEN:-}" ]]; then
  if [[ -f "$SEEN_MID" ]] && grep -qx "adapter-final" "$SEEN_MID"; then
    ...
    echo "{\"skipped\":true,\"reason\":\"mid adapter-final present\"}" \
      >/root/affine_data/h111_adapter_salvage.json
    rm -f /root/logs/h111_push_adapter.pid
  elif pgrep -f "s4-h111-f16-af-k1/mid_ckpt_salvage.sh" >/dev/null 2>&1; then
    ...
    nohup python3 /root/mining_src/s4-h1-sft/salvage_adapter.py \
      ...
      --commit-message "H111 afk1-init winner-zA thought LoRA salvage (TTL insurance; not a submission)" \
      --out-meta /root/affine_data/h111_adapter_salvage.json \
      >>/root/logs/h111_push_adapter.nohup 2>&1 &
    echo $! >/root/logs/h111_push_adapter.pid
  fi
  ...
  nohup python3 /root/mining_src/s4-h1-sft/push_merged.py \
    ...
    --repo "$HF_MERGED_REPO" \
    --public \
    --commit-message "H111 afk1-init winner-zA thought merged salvage (TTL insurance; not a submission)" \
    --out-meta /root/affine_data/h111_merged_salvage.json \
    >>/root/logs/h111_push_merged.nohup 2>&1 &
  echo $! >/root/logs/h111_push_merged.pid
  log "adapter push pid=$(cat /root/logs/h111_push_adapter.pid 2>/dev/null || echo skipped) merged push pid=$(cat /root/logs/h111_push_merged.pid)"
else
  log "WARN: HF_TOKEN unset; skipping H111 HF salvage pushes"
fi
...
if [[ "$code_t" != "200" || "$code_k" != "200" ]]; then
  ...
  echo "aborted_engines_unhealthy $(date -u +%Y-%m-%dT%H:%M:%SZ)" > /root/logs/h111_pipeline.aborted
  exit 1
fi
...
date -u +%Y-%m-%dT%H:%M:%SZ > /root/logs/h111_chall_serve.done
...
if (( dead - now < 2400 )); then
  ...
  echo "aborted_no_n80_budget $(date -u +%Y-%m-%dT%H:%M:%SZ)" > /root/logs/h111_pipeline.aborted
  exit 1
fi

N80_MAX_ATTEMPTS=${N80_MAX_ATTEMPTS:-3}
# Fresh block_hash per attempt (H32/H34/H111): default 0*64 dies teacher 400 @~40/80.
...
for attempt in $(seq 1 "$N80_MAX_ATTEMPTS"); do
  ...
  if ps -eo args | awk '/[r]un_sim_duel.py/ && /local-h111/' | grep -q .; then
    log "n80 already running under retry — skip post_train launch"
    n80_ok=1
    break
  fi
  if ps -eo args | awk '/[w]atch_n80_retry\.sh/ && / h111 /' | grep -q . \
    || ps -eo args | awk '/[r]etry_h111_n80\.sh/' | grep -q .; then
    log "watch_n80_retry/retry_h111 armed — defer n80 to retry; skip post_train launch"
    n80_ok=1
    break
  fi
  rm -f "$SIM_N80" "$PROG" /root/logs/h111_sim_n80.done
  ...
  python /root/mining_src/s4-h2-merge/run_sim_duel.py \
    ...
    --progress-out "$PROG" \
    --save-artifact \
    2>&1 | tee -a /root/logs/h111_sim.nohup
  sim_rc=${PIPESTATUS[0]}
  set -e
  ...
  echo "n80_attempt_${attempt}_failed rc=$sim_rc $(date -u +%Y-%m-%dT%H:%M:%SZ)" \
    >> /root/logs/h111_sim_retries.log
  sleep 15
done
if [[ "$n80_ok" -ne 1 ]]; then
...
date -u +%Y-%m-%dT%H:%M:%SZ > /root/logs/h111_sim_n80.done
date -u +%Y-%m-%dT%H:%M:%SZ > /root/logs/h111_pipeline.done
