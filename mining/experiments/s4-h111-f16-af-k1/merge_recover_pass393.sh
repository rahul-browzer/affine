#!/usr/bin/env bash
# H111/F16 pass393: GPU merge save hung on gocryptfs request_wait_answer
...
EXP=/root/mining_src/s4-h111-f16-af-k1
LOG=/root/logs/h111_merge_recover_pass393.nohup
PIDF=/root/logs/h111_merge_recover_pass393.pid
MERGED=/root/h111/merged
ADAPTER=/root/h111/train/adapter
BASE=/root/hf/hub/models--af-k1--Affine-5ECeJJpEMjW4pxM9eGyJ5ua3Sebfyr8kcVwLAdaiJLUC8pkW/snapshots/ff6eb4bcff3e7c6b8c0e097bc0cffa4fa2ba8e01

log() { echo "[h111-merge393] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }

mkdir -p /root/logs /root/affine_data
: >"$LOG"
echo $$ >"$PIDF"

MERGE_PID=$(ps -eo pid,cmd | awk '/[p]ython .*merge_lora.py --base .*h111\/merged/{print $1; exit}')
POST_PID=$(ps -eo pid,cmd | awk '/[b]ash .*s4-h111-f16-af-k1\/post_train_pipeline\.sh/{print $1; exit}')
...
rm -f /root/logs/h111_pipeline.aborted
...
if [[ -x "$EXP/watch_preempt_bare_tcache_pass264.sh" ]]; then
  nohup bash "$EXP/watch_preempt_bare_tcache_pass264.sh" \
    >>/root/logs/h111_preempt_bare_pass264.log 2>&1 &
  echo $! >/root/logs/h111_preempt_bare_pass264.pid
  log "rearmed preempt264 pid=$(cat /root/logs/h111_preempt_bare_pass264.pid)"
fi
...
log "post_train resumed pid=$(cat /root/logs/h111_post_train.pid) SKIP_MERGE=1"
log "DONE recover393"
