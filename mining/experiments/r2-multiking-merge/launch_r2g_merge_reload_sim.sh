#!/usr/bin/env bash
# R2g: after R2e (Talent×awesome) resolves below bar + Talent×saysth skew
# premerge ready (gated on chal-00440 Reason hr>0) → reload chall → fresh n80.
# If R2g premerge SKIP (Reason− / mismatch / timeout) → exit without chall kill.
# Does NOT touch teacher:8000 or king:8001. Kill chall by PID file only.
# Pre-registered: submit only if headroom ≥ 1.5×(3·SE).
set -euo pipefail
LOG=/root/logs/r2g_merge_reload.log
DONE=/root/logs/r2g_merge_reload.done
PIDF=/root/logs/r2g_merge_reload.pid
mkdir -p /root/logs /root/affine_data /root/r2_out
echo $$ >"$PIDF"
exec > >(tee -a "$LOG") 2>&1

echo "[r2g-merge] $(date -u +%Y-%m-%dT%H:%M:%SZ) start"
if [[ -f "$DONE" ]]; then
  echo "[r2g-merge] already done: $(cat "$DONE")"
  exit 0
fi

PREMERGE_DONE=${PREMERGE_DONE:-/root/logs/r2g_premerge.done}
PREMERGE_SKIP=${PREMERGE_SKIP:-/root/logs/r2g_premerge.skip}
...
LINK=${LINK:-/tmp/r2g_alpha_merged}
...
# 1) Wait R2g CPU premerge outcome (done with weights, or skip).
for i in $(seq 1 2880); do
  if [[ -f "$PREMERGE_SKIP" ]]; then
    echo "SKIP_R2G_PREMERGE_SKIPPED $(cat "$PREMERGE_SKIP") $(date -u +%Y-%m-%dT%H:%M:%SZ)" | tee "$DONE"
    exit 0
  fi
  if [[ -f "$PREMERGE_DONE" && -f "$MERGED/model.safetensors.index.json" ]]; then
    echo "[r2g-merge] premerge ready at iter=$i $(date -u +%Y-%m-%dT%H:%M:%SZ) $(cat "$PREMERGE_DONE")"
    break
  fi
  if (( i % 12 == 0 )); then
    crumb=$(tail -n 3 /root/logs/r2g_premerge.log 2>/dev/null | tr '\r' '\n' | tail -1 || true)
    echo "[r2g-merge] wait-premerge iter=$i crumb=${crumb:-none}"
  fi
  if (( i == 2880 )); then
    echo "SKIP_R2G_PREMERGE_TIMEOUT $(date -u +%Y-%m-%dT%H:%M:%SZ)" | tee "$DONE"
    exit 0
  fi
  sleep 10
done
...
for i in $(seq 1 2880); do
  ...
  if [[ -f "$R2E_DEC" ]]; then
    ...
    # Also treat live sim as busy if decision exists but parent still writing.
    if [[ -f /root/affine_data/r2e_alpha_reason_progress.json ]] && [[ ! -f "$R2E_DONE" ]]; then
      if [[ -f /root/logs/r2e_merge_reload.pid ]]; then
        ppid=$(cat /root/logs/r2e_merge_reload.pid 2>/dev/null || true)
        if [[ -n "${ppid:-}" ]] && kill -0 "$ppid" 2>/dev/null; then
          r2e_busy=1
...
if [[ ! -f "$MERGED/model.safetensors.index.json" ]]; then
  echo "[r2g-merge] FATAL missing merged index at $MERGED" >&2
  exit 2
fi

ln -sfn "$MERGED" "$LINK"
echo "[r2g-merge] link $LINK -> $(readlink -f "$LINK")"

CHALL_PID_FILE=/root/logs/vllm_chall.pid
if [[ -f "$CHALL_PID_FILE" ]]; then
  CPID=$(cat "$CHALL_PID_FILE" || true)
  if [[ -n "${CPID:-}" ]] && kill -0 "$CPID" 2>/dev/null; then
    echo "[r2g-merge] stopping chall pid=$CPID"
    kill "$CPID" || true
...
if [[ ! -d /root/.triton/cache/chall ]] || [[ -z "$(find /root/.triton/cache/chall -name '*.so' 2>/dev/null | head -1)" ]]; then
  if [[ -d /root/.triton/cache/king ]]; then
    echo "[r2g-merge] seeding chall Triton cache from king"
...
echo "[r2g-merge] launching chall :8002 on $LINK"
...
echo "[r2g-merge] chall pid=$(cat /root/logs/vllm_chall.pid)"
...
for i in $(seq 1 480); do
  ...
  if [[ "$c0$c1$c2" == "200200200" ]]; then
    echo "[r2g-merge] engines 200/200/200 at iter=$i"
    break
  fi
  if (( i % 12 == 0 )); then
    echo "[r2g-merge] wait-engines iter=$i codes=${c0}/${c1}/${c2}"
  fi
  if (( i == 480 )); then
    echo "[r2g-merge] TIMEOUT engines; see vllm_chall.log" >&2
    exit 2
  fi
  sleep 5
done

OUT=/root/affine_data/r2g_alpha_reason_sim.json
DEC=/root/affine_data/r2g_alpha_decision.json
PROG=/root/affine_data/r2g_alpha_reason_progress.json
rm -f "$OUT" "$DEC" "$PROG"
BH=$(python - <<'PY'
import hashlib, time
print(hashlib.sha256(f"r2g-talent-saysth-{time.time_ns()}".encode()).hexdigest())
PY
)

echo "[r2g-merge] launching R2g n80 block_hash=${BH:0:16}…"
python /root/mining_src/r1-reason-distill/run_reason_sim.py \
  --n-turns 80 \
  --block-hash "$BH" \
  --hotkey "local-r2g-talent-saysth-$(date -u +%Y%m%dT%H%M%SZ)" \
  ...
  --progress-out "$PROG" \
  --save-artifact \
  2>&1 | tee /root/logs/r2g_alpha_reason_sim.log

python /root/mining_src/r1-reason-distill/write_reason_decision.py \
  --sim-result "$OUT" --out "$DEC" --hyp R2g \
  2>&1 | tee -a /root/logs/r2g_alpha_reason_sim.log

echo "[r2g-merge] DONE $(date -u +%Y-%m-%dT%H:%M:%SZ)"
cat "$DEC"
