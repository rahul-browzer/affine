#!/usr/bin/env bash
# R2k: after R2g/R2i/R2j resolve below bar + Talent×BKN-six skew premerge
# ready (gated on chal-00431 Reason hr>0) → reload chall → fresh n80.
# If R2k premerge SKIP (Reason− / mismatch / timeout) → exit without chall kill.
# Does NOT touch teacher:8000 or king:8001. Kill chall by PID file only.
# Pre-registered: submit only if headroom ≥ 1.5×(3·SE).
set -euo pipefail
LOG=/root/logs/r2k_merge_reload.log
DONE=/root/logs/r2k_merge_reload.done
PIDF=/root/logs/r2k_merge_reload.pid
mkdir -p /root/logs /root/affine_data /root/r2_out
echo $$ >"$PIDF"
exec > >(tee -a "$LOG") 2>&1

echo "[r2k-merge] $(date -u +%Y-%m-%dT%H:%M:%SZ) start"
if [[ -f "$DONE" ]]; then
  echo "[r2k-merge] already done: $(cat "$DONE")"
  exit 0
fi

PREMERGE_DONE=${PREMERGE_DONE:-/root/logs/r2k_premerge.done}
PREMERGE_SKIP=${PREMERGE_SKIP:-/root/logs/r2k_premerge.skip}
...
LINK=${LINK:-/tmp/r2k_alpha_merged}
...
# 1) Wait R2k CPU premerge outcome (done with weights, or skip).
for i in $(seq 1 2880); do
  if [[ -f "$PREMERGE_SKIP" ]]; then
    echo "SKIP_R2K_PREMERGE_SKIPPED $(cat "$PREMERGE_SKIP") $(date -u +%Y-%m-%dT%H:%M:%SZ)" | tee "$DONE"
    exit 0
  fi
  if [[ -f "$PREMERGE_DONE" && -f "$MERGED/model.safetensors.index.json" ]]; then
    echo "[r2k-merge] premerge ready at iter=$i $(date -u +%Y-%m-%dT%H:%M:%SZ) $(cat "$PREMERGE_DONE")"
    break
  fi
  if (( i % 12 == 0 )); then
    crumb=$(tail -n 3 /root/logs/r2k_premerge.log 2>/dev/null | tr '\r' '\n' | tail -1 || true)
    echo "[r2k-merge] wait-premerge iter=$i crumb=${crumb:-none}"
  fi
  if (( i == 2880 )); then
    echo "SKIP_R2K_PREMERGE_TIMEOUT $(date -u +%Y-%m-%dT%H:%M:%SZ)" | tee "$DONE"
    exit 0
  fi
  sleep 10
done

# 2) Wait R2g + R2i + R2j lanes free (or prior clears 1.5×). Pidfile kill -0 only.
for i in $(seq 1 2880); do
  for f in "$R2D_DEC" "$R2E_DEC" "$R2H_DEC" "$R2G_DEC" "$R2I_DEC" "$R2J_DEC"; do
    if [[ -f "$f" ]] && headroom_ok "$f"; then
      echo "SKIP_R2K_PRIOR_CLEARS file=$f $(date -u +%Y-%m-%dT%H:%M:%SZ)" | tee "$DONE"
      exit 0
    fi
  done
  if [[ -f "$PREMERGE_SKIP" ]]; then
  ...
  if [[ -f "$R2J_DONE" ]] || [[ -f "$R2J_PREMERGE_SKIP" ]] || [[ -f "$R2J_DEC" ]]; then
    ...
    # R2j waiters still expected (432 not stamped); do not race chall.
    r2j_busy=1
  fi

  if (( r2g_busy == 0 && r2i_busy == 0 && r2j_busy == 0 )); then
    echo "[r2k-merge] R2g/R2i/R2j below bar or skipped; lane free at iter=$i"
    break
  fi
  if (( i % 12 == 0 )); then
    echo "[r2k-merge] wait-lane iter=$i $(date -u +%Y-%m-%dT%H:%M:%SZ) r2g_busy=$r2g_busy r2i_busy=$r2i_busy r2j_busy=$r2j_busy r2g_dec=$([[ -f $R2G_DEC ]] && echo y || echo n) r2j_done=$([[ -f $R2J_DONE ]] && echo y || echo n)"
  fi
  if (( i == 2880 )); then
    echo "[r2k-merge] TIMEOUT waiting R2g/R2i/R2j lane" >&2
    exit 2
  fi
  sleep 10
done
...
if [[ ! -f "$MERGED/model.safetensors.index.json" ]]; then
  echo "[r2k-merge] FATAL missing merged index at $MERGED" >&2
  exit 2
fi

ln -sfn "$MERGED" "$LINK"
echo "[r2k-merge] link $LINK -> $(readlink -f "$LINK")"

# Wait if R2q pure-saysth currently owns chall (do not yank mid-n80).
_WAIT_R2Q_TAG=r2k-merge
# shellcheck disable=SC1091
...
if [[ -f "$CHALL_PID_FILE" ]]; then
  ...
  if [[ -n "${CPID:-}" ]] && kill -0 "$CPID" 2>/dev/null; then
    echo "[r2k-merge] stopping chall pid=$CPID"
    kill "$CPID" || true
...
if [[ ! -d /root/.triton/cache/chall ]] || [[ -z "$(find /root/.triton/cache/chall -name '*.so' 2>/dev/null | head -1)" ]]; then
  if [[ -d /root/.triton/cache/king ]]; then
    echo "[r2k-merge] seeding chall Triton cache from king"
...
echo "[r2k-merge] launching chall :8002 on $LINK"
...
echo "[r2k-merge] chall pid=$(cat /root/logs/vllm_chall.pid)"
...
for i in $(seq 1 480); do
  ...
  if [[ "$c0$c1$c2" == "200200200" ]]; then
    echo "[r2k-merge] engines 200/200/200 at iter=$i"
    break
  fi
  if (( i % 12 == 0 )); then
    echo "[r2k-merge] wait-engines iter=$i codes=${c0}/${c1}/${c2}"
  fi
  if (( i == 480 )); then
    echo "[r2k-merge] TIMEOUT engines; see vllm_chall.log" >&2
    exit 2
  fi
  sleep 5
done

OUT=/root/affine_data/r2k_alpha_reason_sim.json
DEC=/root/affine_data/r2k_alpha_decision.json
PROG=/root/affine_data/r2k_alpha_reason_progress.json
rm -f "$OUT" "$DEC" "$PROG"
BH=$(python - <<'PY'
import hashlib, time
print(hashlib.sha256(f"r2k-talent-bkn6-{time.time_ns()}".encode()).hexdigest())
PY
)

echo "[r2k-merge] launching R2k n80 block_hash=${BH:0:16}…"
python /root/mining_src/r1-reason-distill/run_reason_sim.py \
  --n-turns 80 \
  --block-hash "$BH" \
  --hotkey "local-r2k-talent-bkn6-$(date -u +%Y%m%dT%H%M%SZ)" \
  ...
  --progress-out "$PROG" \
  --save-artifact \
  2>&1 | tee /root/logs/r2k_alpha_reason_sim.log

python /root/mining_src/r1-reason-distill/write_reason_decision.py \
  --sim-result "$OUT" --out "$DEC" --hyp R2k \
  2>&1 | tee -a /root/logs/r2k_alpha_reason_sim.log

echo "[r2k-merge] DONE $(date -u +%Y-%m-%dT%H:%M:%SZ)"
cat "$DEC"
