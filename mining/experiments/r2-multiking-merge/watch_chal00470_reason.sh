#!/usr/bin/env bash
# CPU watcher: when chal-00470 (tojointhecommunity/affine-5efg6cm3yl-google) publishes a duel gzip,
...
set -euo pipefail
LOG=/root/logs/watch_chal00470_reason.log
DONE=/root/logs/watch_chal00470_reason.done
PIDF=/root/logs/watch_chal00470_reason.pid
OUT=${OUT:-/root/affine_data/chal00470_reason.json}
CHAL=${CHAL:-chal-00470}
...
echo "[watch-470] $(date -u +%Y-%m-%dT%H:%M:%SZ) start chal=$CHAL"
if [[ -f "$DONE" && -f "$OUT" ]]; then
  echo "[watch-470] already done: $(cat "$DONE")"
  exit 0
fi
...
print(f"[watch-470] polling {url}", flush=True)
for i in range(1, 2880):  # ~8h @10s
    try:
        raw = fetch_bytes(url)
        print(f"[watch-470] got {len(raw)} bytes at iter={i}", flush=True)
        ...
        print(f"[watch-470] {line}", flush=True)
        raise SystemExit(0)
    except Exception as e:
        if i % 12 == 0:
            print(f"[watch-470] wait iter={i} err={type(e).__name__}: {e}", flush=True)
        time.sleep(10)
print("[watch-470] TIMEOUT", flush=True)
raise SystemExit(2)
PY
