#!/usr/bin/env bash
# When R2aq local n80 stamps r2aq_now_decision.json with headroom ≥ 1.5×(3·SE),
# purge old public HF merges if needed and upload pure-now chall dir to
# unconst/Affine-5czsc2fc98-r2aq-now (public).
...
set -euo pipefail
LOG=/root/logs/watch_r2aq_stage5_push.log
DONE=/root/logs/watch_r2aq_stage5_push.done
PIDF=/root/logs/watch_r2aq_stage5_push.pid
DEC=${DEC:-/root/affine_data/r2aq_now_decision.json}
STAGE5=${STAGE5:-/root/affine_data/r2aq_stage5_ready.json}
CHALL=${CHALL:-/root/r2_out/now_chall}
REPO=${REPO:-unconst/Affine-5czsc2fc98-r2aq-now}
OUT_META=${OUT_META:-/root/affine_data/r2aq_stage5_hf_push.json}
PURGE_META=${PURGE_META:-/root/affine_data/r2aq_stage5_hf_purge.json}
HEADROOM_BAR=${HEADROOM_BAR:-1.5}
KEEP_REPOS=${KEEP_REPOS:-unconst/Affine-5czsc2fc98-r1lora,unconst/Affine-5czsc2fc98-r2v-sft3,unconst/Affine-5czsc2fc98-r2ao-af17,unconst/Affine-5czsc2fc98-r2ap-h44,unconst/Affine-5czsc2fc98-r2aq-now}
mkdir -p /root/logs /root/affine_data
echo $$ >"$PIDF"
exec > >(tee -a "$LOG") 2>&1

echo "[r2aq-stage5-push] $(date -u +%Y-%m-%dT%H:%M:%SZ) start"
if [[ -f "$DONE" && -f "$OUT_META" ]]; then
  echo "[r2aq-stage5-push] already done: $(cat "$DONE")"
  exit 0
fi

echo "[r2aq-stage5-push] waiting for $DEC with headroom≥$HEADROOM_BAR"
...
PY
        echo "[r2aq-stage5-push] STAGE5 stamped hr=$hr at iter=$i"
        break
      fi
...
print(p.read_text().strip() if p.is_file() else 'no-progress')" 2>/dev/null || echo none)
    echo "[r2aq-stage5-push] wait iter=$i crumb=${crumb:-none}"
  fi
  if (( i == 2880 )); then
...
if [[ -z "${HF_TOKEN:-}" ]]; then
  echo "[r2aq-stage5-push] FATAL HF_TOKEN missing" >&2
  exit 2
fi
...
for sz, m in cands:
    ...
    try:
        ...
        freed += sz
        print(f"[r2aq-stage5-push] purged {m.id} ({sz/1e9:.1f}G)", flush=True)
    except Exception as e:
        errors.append({"id": m.id, "error": str(e)})
        print(f"[r2aq-stage5-push] purge err {m.id}: {e}", flush=True)
...
if not readme.is_file():
    readme.write_text(
        "---\n"
        "base_model: tojointhecommunity/affine-5efg6cm3yl-now\n"
        ...
        "- reason-v3\n"
        "---\n\n"
        "# R2aq pure now challenger (Reason v3)\n\n"
...
print(
    f"[r2aq-stage5-push] uploading {chall} ({total/(1<<30):.1f} GiB) → {repo} public",
    flush=True,
)
info = api.upload_folder(
    ...
    repo_id=repo,
    repo_type="model",
    commit_message="R2aq pure now Stage-5 pre-submit (Reason v3 local n80 ≥1.5×)",
    allow_patterns=[
        "*.safetensors",
        "*.json",
        "*.txt",
...
print("[r2aq-stage5-push] DONE", flush=True)
PY
