#!/usr/bin/env bash
# CPU watcher: when chal-00458 (marsplan0624/…-whoami) publishes a duel gzip,
# recompute Reason = mean(lpC_yc_za − lpC_yc_e) per turn, paired vs Tok af10,
# stamp headroom_vs_3se. Gates Talent×whoami (R2r) after Reason+ without waiting
# for the next Ralph pass. No GPU. Does not submit.
set -euo pipefail
LOG=/root/logs/watch_chal00458_reason.log
DONE=/root/logs/watch_chal00458_reason.done
PIDF=/root/logs/watch_chal00458_reason.pid
OUT=${OUT:-/root/affine_data/chal00458_reason.json}
CHAL=${CHAL:-chal-00458}
...
echo "[watch-458] $(date -u +%Y-%m-%dT%H:%M:%SZ) start chal=$CHAL"
if [[ -f "$DONE" && -f "$OUT" ]]; then
  echo "[watch-458] already done: $(cat "$DONE")"
  exit 0
fi
...
for i in range(1, 2880):  # ~8h @10s
    try:
        ...
        time.sleep(10)
print("[watch-458] TIMEOUT", flush=True)
raise SystemExit(2)
PY
