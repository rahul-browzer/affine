#!/usr/bin/env bash
# When R2bf local n80 stamps r2bf_dpo2_decision.json with headroom ≥ 1.5×(3·SE),
# purge old public HF merges if needed and upload pure dpo2 chall dir to
# unconst/Affine-5czsc2fc98-r2bf-dpo2 (public).
...
set -euo pipefail
LOG=/root/logs/watch_r2bf_stage5_push.log
DONE=/root/logs/watch_r2bf_stage5_push.done
PIDF=/root/logs/watch_r2bf_stage5_push.pid
DEC=${DEC:-/root/affine_data/r2bf_dpo2_decision.json}
STAGE5=${STAGE5:-/root/affine_data/r2bf_stage5_ready.json}
CHALL=${CHALL:-/root/r2_out/dpo2_chall}
REPO=${REPO:-unconst/Affine-5czsc2fc98-r2bf-dpo2}
OUT_META=${OUT_META:-/root/affine_data/r2bf_stage5_hf_push.json}
PURGE_META=${PURGE_META:-/root/affine_data/r2bf_stage5_hf_purge.json}
HEADROOM_BAR=${HEADROOM_BAR:-1.5}
KEEP_REPOS=${KEEP_REPOS:-unconst/Affine-5czsc2fc98-r1lora,unconst/Affine-5czsc2fc98-r2v-sft3,unconst/Affine-5czsc2fc98-r2ao-af17,unconst/Affine-5czsc2fc98-r2ap-h44,unconst/Affine-5czsc2fc98-r2aq-now,unconst/Affine-5czsc2fc98-r2ar-iynocr2p,unconst/Affine-5czsc2fc98-r2as-726,unconst/Affine-5czsc2fc98-r2at-hope11,unconst/Affine-5czsc2fc98-r2au-sft4,unconst/Affine-5czsc2fc98-r2av-v2,unconst/Affine-5czsc2fc98-r2ax-tt,unconst/Affine-5czsc2fc98-r2az-vvv,unconst/Affine-5czsc2fc98-r2ba-awesome-v10,unconst/Affine-5czsc2fc98-r2bb-ckp333,unconst/Affine-5czsc2fc98-r2bd-ckp55,unconst/Affine-5czsc2fc98-r2be-hope12,unconst/Affine-5czsc2fc98-r2bf-dpo2}
mkdir -p /root/logs /root/affine_data
echo $$ >"$PIDF"
exec > >(tee -a "$LOG") 2>&1

echo "[r2bf-stage5-push] $(date -u +%Y-%m-%dT%H:%M:%SZ) start"
if [[ -f "$DONE" && -f "$OUT_META" ]]; then
  echo "[r2bf-stage5-push] already done: $(cat "$DONE")"
  exit 0
fi

echo "[r2bf-stage5-push] waiting for $DEC with headroom≥$HEADROOM_BAR"
...
PY
        echo "[r2bf-stage5-push] STAGE5 stamped hr=$hr at iter=$i"
        break
      fi
  ...
  if (( i % 12 == 0 )); then
    crumb=$(python3 -c "import json;from pathlib import Path;p=Path('/root/affine_data/r2bf_dpo2_reason_progress.json');
print(p.read_text().strip() if p.is_file() else 'no-progress')" 2>/dev/null || echo none)
    echo "[r2bf-stage5-push] wait iter=$i crumb=${crumb:-none}"
  fi
  if (( i == 2880 )); then
...
if [[ -z "${HF_TOKEN:-}" ]]; then
  echo "[r2bf-stage5-push] FATAL HF_TOKEN missing" >&2
  exit 2
fi
...
for sz, m in cands:
    ...
    try:
        ...
        freed += sz
        print(f"[r2bf-stage5-push] purged {m.id} ({sz/1e9:.1f}G)", flush=True)
    except Exception as e:
        errors.append({"id": m.id, "error": str(e)})
        print(f"[r2bf-stage5-push] purge err {m.id}: {e}", flush=True)
...
if not readme.is_file():
    readme.write_text(
        "---\n"
        "base_model: Shatoria/Affine-5ghntktyzq-dpo2\n"
        ...
        "- reason-v3\n"
        "---\n\n"
        "# R2bf pure dpo2 challenger (Reason v3)\n\n"
...
print(
    f"[r2bf-stage5-push] uploading {chall} ({total/(1<<30):.1f} GiB) → {repo} public",
    flush=True,
)
info = api.upload_folder(
    ...
    repo_id=repo,
    repo_type="model",
    commit_message="R2bf pure dpo2 Stage-5 pre-submit (Reason v3 local n80 ≥1.5×)",
    allow_patterns=[
        "*.safetensors",
        "*.json",
        "*.txt",
...
print("[r2bf-stage5-push] DONE", flush=True)
PY
