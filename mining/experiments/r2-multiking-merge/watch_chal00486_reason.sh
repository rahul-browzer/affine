#!/usr/bin/env bash
# CPU watcher: when chal-00486 publishes duel gzip OR history verdict,
# stamp Reason headroom_vs_3se (or hr=None on unservable). No GPU. No submit.
set -euo pipefail
LOG=/root/logs/watch_chal00486_reason.log
DONE=/root/logs/watch_chal00486_reason.done
PIDF=/root/logs/watch_chal00486_reason.pid
OUT=${OUT:-/root/affine_data/chal00486_reason.json}
CHAL=${CHAL:-chal-00486}
...
echo "[watch-486] $(date -u +%Y-%m-%dT%H:%M:%SZ) start chal=$CHAL"
if [[ -f "$DONE" && -f "$OUT" ]]; then
  echo "[watch-486] already done: $(cat "$DONE")"
  exit 0
fi
...
print(f"[watch-486] polling {url} (+ history fast-path)", flush=True)
...
for i in range(1, 2880):
    try:
        ...
        if hist is not None:
            ...
            print(f"[watch-486] {line}", flush=True)
            raise SystemExit(0)
        raw = fetch_bytes(url)
        print(f"[watch-486] got {len(raw)} bytes at iter={i}", flush=True)
        ...
        if i % 12 == 0:
            print(f"[watch-486] wait iter={i} err={type(e).__name__}: {e}", flush=True)
        time.sleep(10)
print("[watch-486] TIMEOUT", flush=True)
raise SystemExit(2)
PY
