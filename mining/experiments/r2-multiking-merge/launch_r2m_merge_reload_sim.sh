#!/usr/bin/env bash
# R2m: after R2g/R2i/R2j/R2k/R2l resolve below bar + Talent×cp200 skew premerge
# ready (gated on chal-00456 Reason hr>0) → reload chall → fresh n80.
# If R2m premerge SKIP (Reason− / mismatch / timeout) → exit without chall kill.
# Does NOT touch teacher:8000 or king:8001. Kill chall by PID file only.
# Pre-registered: submit only if headroom ≥ 1.5×(3·SE).
set -euo pipefail
LOG=/root/logs/r2m_merge_reload.log
DONE=/root/logs/r2m_merge_reload.done
PIDF=/root/logs/r2m_merge_reload.pid
mkdir -p /root/logs /root/affine_data /root/r2_out
echo $$ >"$PIDF"
exec > >(tee -a "$LOG") 2>&1

echo "[r2m-merge] $(date -u +%Y-%m-%dT%H:%M:%SZ) start"
if [[ -f "$DONE" ]]; then
  echo "[r2m-merge] already done: $(cat "$DONE")"
  exit 0
fi

PREMERGE_DONE=${PREMERGE_DONE:-/root/logs/r2m_premerge.done}
PREMERGE_SKIP=${PREMERGE_SKIP:-/root/logs/r2m_premerge.skip}
...
# 1) Wait R2m CPU premerge outcome (done with weights, or skip).
for i in $(seq 1 2880); do
  if [[ -f "$PREMERGE_SKIP" ]]; then
    echo "SKIP_R2M_PREMERGE_SKIPPED $(cat "$PREMERGE_SKIP") $(date -u +%Y-%m-%dT%H:%M:%SZ)" | tee "$DONE"
    exit 0
  fi
  if [[ -f "$PREMERGE_DONE" && -f "$MERGED/model.safetensors.index.json" ]]; then
    echo "[r2m-merge] premerge ready at iter=$i $(date -u +%Y-%m-%dT%H:%M:%SZ) $(cat "$PREMERGE_DONE")"
    break
  fi
  if (( i % 12 == 0 )); then
    crumb=$(tail -n 3 /root/logs/r2m_premerge.log 2>/dev/null | tr '\r' '\n' | tail -1 || true)
    echo "[r2m-merge] wait-premerge iter=$i crumb=${crumb:-none}"
  fi
  if (( i == 2880 )); then
    echo "SKIP_R2M_PREMERGE_TIMEOUT $(date -u +%Y-%m-%dT%H:%M:%SZ)" | tee "$DONE"
    exit 0
  fi
  sleep 10
done
...
for i in $(seq 1 2880); do
  ...
  if [[ -f "$R2L_DONE" ]] || [[ -f "$R2L_PREMERGE_SKIP" ]] || [[ -f "$R2L_DEC" ]]; then
    ...
    # R2l waiters still expected (450 not stamped); do not race chall.
    r2l_busy=1
  fi
...
if [[ ! -f "$MERGED/model.safetensors.index.json" ]]; then
  echo "[r2m-merge] FATAL missing merged index at $MERGED" >&2
  exit 2
fi

ln -sfn "$MERGED" "$LINK"
echo "[r2m-merge] link $LINK -> $(readlink -f "$LINK")"

# Wait if R2q pure-saysth currently owns chall (do not yank mid-n80).
_WAIT_R2Q_TAG=r2m-merge
# shellcheck disable=SC1091
...
if [[ -f "$CHALL_PID_FILE" ]]; then
  ...
  if [[ -n "${CPID:-}" ]] && kill -0 "$CPID" 2>/dev/null; then
    echo "[r2m-merge] stopping chall pid=$CPID"
    kill "$CPID" || true
...
if [[ ! -d /root/.triton/cache/chall ]] || [[ -z "$(find /root/.triton/cache/chall -name '*.so' 2>/dev/null | head -1)" ]]; then
  if [[ -d /root/.triton/cache/king ]]; then
    echo "[r2m-merge] seeding chall Triton cache from king"
...
echo "[r2m-merge] launching chall :8002 on $LINK"
...
echo "[r2m-merge] chall pid=$(cat /root/logs/vllm_chall.pid)"
...
for i in $(seq 1 480); do
  ...
  if [[ "$c0$c1$c2" == "200200200" ]]; then
    echo "[r2m-merge] engines 200/200/200 at iter=$i"
    break
  fi
  if (( i % 12 == 0 )); then
    echo "[r2m-merge] wait-engines iter=$i codes=${c0}/${c1}/${c2}"
  fi
  if (( i == 480 )); then
    echo "[r2m-merge] TIMEOUT engines; see vllm_chall.log" >&2
    exit 2
  fi
  sleep 5
done

OUT=/root/affine_data/r2m_alpha_reason_sim.json
DEC=/root/affine_data/r2m_alpha_decision.json
PROG=/root/affine_data/r2m_alpha_reason_progress.json
rm -f "$OUT" "$DEC" "$PROG"
BH=$(python - <<'PY'
import hashlib, time
print(hashlib.sha256(f"r2m-talent-cp200-{time.time_ns()}".encode()).hexdigest())
PY
)

echo "[r2m-merge] launching R2m n80 block_hash=${BH:0:16}…"
python /root/mining_src/r1-reason-distill/run_reason_sim.py \
  --n-turns 80 \
  --block-hash "$BH" \
  --hotkey "local-r2m-talent-cp200-$(date -u +%Y%m%dT%H%M%SZ)" \
  ...
  --progress-out "$PROG" \
  --save-artifact \
  2>&1 | tee /root/logs/r2m_alpha_reason_sim.log

python /root/mining_src/r1-reason-distill/write_reason_decision.py \
  --sim-result "$OUT" --out "$DEC" --hyp R2m \
  2>&1 | tee -a /root/logs/r2m_alpha_reason_sim.log

echo "[r2m-merge] DONE $(date -u +%Y-%m-%dT%H:%M:%SZ)"
cat "$DEC"
