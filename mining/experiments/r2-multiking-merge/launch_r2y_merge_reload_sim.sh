#!/usr/bin/env bash
# R2y: after R2l…R2x (+ R2q/R2v/R2w) resolve below bar + Talent×tpc9 skew
# premerge ready (gated on chal-00463 Reason hr>0) → reload chall → fresh n80.
# If R2y premerge SKIP (Reason− / mismatch / timeout) → exit without chall kill.
# Does NOT touch teacher:8000 or king:8001. Kill chall by PID file only.
# Pre-registered: submit only if headroom ≥ 1.5×(3·SE).
set -euo pipefail
LOG=/root/logs/r2y_merge_reload.log
DONE=/root/logs/r2y_merge_reload.done
PIDF=/root/logs/r2y_merge_reload.pid
mkdir -p /root/logs /root/affine_data /root/r2_out
echo $$ >"$PIDF"
exec > >(tee -a "$LOG") 2>&1

echo "[r2y-merge] $(date -u +%Y-%m-%dT%H:%M:%SZ) start"
if [[ -f "$DONE" ]]; then
  echo "[r2y-merge] already done: $(cat "$DONE")"
  exit 0
fi

PREMERGE_DONE=${PREMERGE_DONE:-/root/logs/r2y_premerge.done}
PREMERGE_SKIP=${PREMERGE_SKIP:-/root/logs/r2y_premerge.skip}
...
LINK=${LINK:-/tmp/r2y_alpha_merged}
...
for i in $(seq 1 2880); do
  if [[ -f "$PREMERGE_SKIP" ]]; then
    echo "SKIP_R2Y_PREMERGE_SKIPPED $(cat "$PREMERGE_SKIP") $(date -u +%Y-%m-%dT%H:%M:%SZ)" | tee "$DONE"
    exit 0
  fi
  if [[ -f "$PREMERGE_DONE" && -f "$MERGED/model.safetensors.index.json" ]]; then
    echo "[r2y-merge] premerge ready at iter=$i $(date -u +%Y-%m-%dT%H:%M:%SZ) $(cat "$PREMERGE_DONE")"
    break
  fi
  if (( i % 12 == 0 )); then
    crumb=$(tail -n 3 /root/logs/r2y_premerge.log 2>/dev/null | tr '\r' '\n' | tail -1 || true)
    echo "[r2y-merge] wait-premerge iter=$i crumb=${crumb:-none}"
  fi
  if (( i == 2880 )); then
    echo "SKIP_R2Y_PREMERGE_TIMEOUT $(date -u +%Y-%m-%dT%H:%M:%SZ)" | tee "$DONE"
    exit 0
  fi
  sleep 10
done
...
for i in $(seq 1 2880); do
  for f in "$R2D_DEC" "$R2E_DEC" "$R2H_DEC" "$R2G_DEC" "$R2I_DEC" "$R2J_DEC" \
           ...
           "$R2Q_DEC" "$R2R_DEC" "$R2S_DEC" "$R2T_DEC" "$R2U_DEC" "$R2V_DEC" "$R2X_DEC"; do
    if [[ -f "$f" ]] && headroom_ok "$f"; then
      echo "SKIP_R2Y_PRIOR_CLEARS file=$f $(date -u +%Y-%m-%dT%H:%M:%SZ)" | tee "$DONE"
      exit 0
    fi
  done
  if [[ -f "$PREMERGE_SKIP" ]]; then
  ...
  for pf in /root/logs/r2{i,j,k,l,m,n,o,p,r}_merge_reload.pid \
            ...
            /root/logs/r2v_sft3_reload.pid /root/logs/r2x_merge_reload.pid; do
  ...
  if (( busy == 0 )); then
    echo "[r2y-merge] R2i…R2x/R2q/R2v below bar or skipped; lane free at iter=$i"
    break
  fi
  if (( i % 12 == 0 )); then
    echo "[r2y-merge] wait-lane iter=$i $(date -u +%Y-%m-%dT%H:%M:%SZ) busy=$busy r2l_term=$(lane_terminal "$R2L_DONE" "$R2L_PREMERGE_SKIP" "$R2L_DEC" && echo y || echo n) r2x_term=$(lane_terminal "$R2X_DONE" "$R2X_PREMERGE_SKIP" "$R2X_DEC" && echo y || echo n)"
  fi
  if (( i == 2880 )); then
    echo "[r2y-merge] TIMEOUT waiting R2i…R2x/R2q/R2v lane" >&2
    exit 2
  fi
  sleep 10
done
...
if [[ ! -f "$MERGED/model.safetensors.index.json" ]]; then
  echo "[r2y-merge] FATAL missing merged index at $MERGED" >&2
  exit 2
fi

ln -sfn "$MERGED" "$LINK"
echo "[r2y-merge] link $LINK -> $(readlink -f "$LINK")"

# Wait if R2q/R2s/R2t/R2u/R2v/R2w currently owns chall (do not yank mid-n80).
_WAIT_R2Q_TAG=r2y-merge
# shellcheck disable=SC1091
...
if [[ -f "$CHALL_PID_FILE" ]]; then
  ...
  if [[ -n "${CPID:-}" ]] && kill -0 "$CPID" 2>/dev/null; then
    echo "[r2y-merge] stopping chall pid=$CPID"
    kill "$CPID" || true
...
if [[ ! -d /root/.triton/cache/chall ]] || [[ -z "$(find /root/.triton/cache/chall -name '*.so' 2>/dev/null | head -1)" ]]; then
  if [[ -d /root/.triton/cache/king ]]; then
    echo "[r2y-merge] seeding chall Triton cache from king"
...
echo "[r2y-merge] launching chall :8002 on $LINK"
...
echo "[r2y-merge] chall pid=$(cat /root/logs/vllm_chall.pid)"
...
for i in $(seq 1 480); do
  ...
  if [[ "$c0$c1$c2" == "200200200" ]]; then
    echo "[r2y-merge] engines 200/200/200 at iter=$i"
    break
  fi
  if (( i % 12 == 0 )); then
    echo "[r2y-merge] wait-engines iter=$i codes=${c0}/${c1}/${c2}"
  fi
  if (( i == 480 )); then
    echo "[r2y-merge] TIMEOUT engines; see vllm_chall.log" >&2
    exit 2
  fi
  sleep 5
done

OUT=/root/affine_data/r2y_alpha_reason_sim.json
DEC=/root/affine_data/r2y_alpha_decision.json
PROG=/root/affine_data/r2y_alpha_reason_progress.json
rm -f "$OUT" "$DEC" "$PROG"
BH=$(python - <<'PY'
import hashlib, time
print(hashlib.sha256(f"r2y-talent-tpc9-{time.time_ns()}".encode()).hexdigest())
PY
)

echo "[r2y-merge] launching R2y n80 block_hash=${BH:0:16}…"
python /root/mining_src/r1-reason-distill/run_reason_sim.py \
  --n-turns 80 \
  --block-hash "$BH" \
  --hotkey "local-r2y-talent-tpc9-$(date -u +%Y%m%dT%H%M%SZ)" \
  ...
  --progress-out "$PROG" \
  --save-artifact \
  2>&1 | tee /root/logs/r2y_alpha_reason_sim.log

python /root/mining_src/r1-reason-distill/write_reason_decision.py \
  --sim-result "$OUT" --out "$DEC" --hyp R2y \
  2>&1 | tee -a /root/logs/r2y_alpha_reason_sim.log

echo "[r2y-merge] DONE $(date -u +%Y-%m-%dT%H:%M:%SZ)"
cat "$DEC"
