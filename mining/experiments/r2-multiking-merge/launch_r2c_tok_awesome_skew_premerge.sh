#!/usr/bin/env bash
# R2c: CPU skew-α merge Tok af10 × awesome-v6 (pull harder toward near-miss).
# Equal-α R2b max_abs_delta≈0.006 (almost Tok). Skew W_TOK=0.25 W_AWESOME=0.75.
# Prefetch already done (awesome-v6). CPU only — safe during R1c/R2/R2b.
# Pre-registered n80 bar later: submit only if headroom ≥ 1.5×(3·SE).
set -euo pipefail
LOG=/root/logs/r2c_premerge.log
DONE=/root/logs/r2c_premerge.done
PIDF=/root/logs/r2c_premerge.pid
...
echo "[r2c-premerge] $(date -u +%Y-%m-%dT%H:%M:%SZ) start"
if [[ -f "$DONE" && -f "$MERGED/model.safetensors.index.json" ]]; then
  echo "[r2c-premerge] already done: $(cat "$DONE")"
  exit 0
fi
...
for i in $(seq 1 360); do
  if [[ -f "$PREFETCH_DONE" ]]; then
    echo "[r2c-premerge] prefetch done at iter=$i $(date -u +%Y-%m-%dT%H:%M:%SZ)"
    break
  fi
  if (( i == 360 )); then
    echo "[r2c-premerge] TIMEOUT prefetch" >&2
    exit 2
  fi
  sleep 10
done
...
TOK=$(resolve_snap Tok331102/affine-5EqYW8McUc-af10 "$TOK_REV") || {
  echo "[r2c-premerge] FATAL missing Tok" >&2
  exit 2
}
AWESOME=$(resolve_snap 0pentensor/Affine-5dflhtkufw-awesome-v6 "$AWESOME_REV") || {
  echo "[r2c-premerge] FATAL missing awesome-v6" >&2
  exit 2
}

rm -rf "$MERGED"
mkdir -p "$MERGED"
echo "[r2c-premerge] α-merge Tok:$W_TOK awesome-v6:$W_AWESOME → $MERGED"
...
d=json.loads(Path("/root/r2_out/alpha_tok_awesome_v6_skew/merge_alpha_meta.json").read_text())
print(f"max_abs_delta={d.get('max_abs_delta')} n_keys={d.get('n_keys')} identical_frac={d.get('identical_frac')}")
PY
)
fi
echo "OK $(date -u +%Y-%m-%dT%H:%M:%SZ) w_tok=$W_TOK w_awesome=$W_AWESOME ${META}" | tee "$DONE"
echo "[r2c-premerge] DONE $(date -u +%Y-%m-%dT%H:%M:%SZ)"
