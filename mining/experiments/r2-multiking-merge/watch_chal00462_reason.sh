#!/usr/bin/env bash
# CPU watcher: when chal-00462 (0pentensor/…-awesome-v8) publishes a duel gzip,
...
set -euo pipefail
LOG=/root/logs/watch_chal00462_reason.log
DONE=/root/logs/watch_chal00462_reason.done
PIDF=/root/logs/watch_chal00462_reason.pid
OUT=${OUT:-/root/affine_data/chal00462_reason.json}
CHAL=${CHAL:-chal-00462}
...
echo "[watch-462] $(date -u +%Y-%m-%dT%H:%M:%SZ) start chal=$CHAL"
if [[ -f "$DONE" && -f "$OUT" ]]; then
  echo "[watch-462] already done: $(cat "$DONE")"
  exit 0
fi
...
print(f"[watch-462] polling {url} (+ history fast-path)", flush=True)
...
for i in range(1, 2880):  # ~8h @10s
    try:
        ...
        if hist is not None:
            ...
            print(f"[watch-462] {line}", flush=True)
            raise SystemExit(0)
        raw = fetch_bytes(url)
        print(f"[watch-462] got {len(raw)} bytes at iter={i}", flush=True)
        ...
        if i % 12 == 0:
            print(f"[watch-462] wait iter={i} err={type(e).__name__}: {e}", flush=True)
        time.sleep(10)
print("[watch-462] TIMEOUT", flush=True)
raise SystemExit(2)
PY
