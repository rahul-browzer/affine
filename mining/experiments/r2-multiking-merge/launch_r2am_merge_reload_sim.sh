#!/usr/bin/env bash
# R2am: after R2l…R2z (+ R2q/R2v/R2aa…R2am) resolve below bar + Talent×sbs-v1 skew
# premerge ready (gated on chal-00480 Reason hr>0) → reload chall → fresh n80.
# Waits R2am terminal; yields only if a later sibling holding stamp appears (none yet).
# If R2am premerge SKIP (Reason− / mismatch / timeout) → exit without chall kill.
# Does NOT touch teacher:8000 or king:8001. Kill chall by PID file only.
# Pre-registered: submit only if headroom ≥ 1.5×(3·SE).
set -euo pipefail
LOG=/root/logs/r2am_merge_reload.log
DONE=/root/logs/r2am_merge_reload.done
PIDF=/root/logs/r2am_merge_reload.pid
mkdir -p /root/logs /root/affine_data /root/r2_out
echo $$ >"$PIDF"
exec > >(tee -a "$LOG") 2>&1

echo "[r2am-merge] $(date -u +%Y-%m-%dT%H:%M:%SZ) start"
if [[ -f "$DONE" ]]; then
  echo "[r2am-merge] already done: $(cat "$DONE")"
  exit 0
fi

PREMERGE_DONE=${PREMERGE_DONE:-/root/logs/r2am_premerge.done}
PREMERGE_SKIP=${PREMERGE_SKIP:-/root/logs/r2am_premerge.skip}
...
MERGED=${MERGED:-/root/r2_out/alpha_talent_sbs_v1_skew}
LINK=${LINK:-/tmp/r2am_alpha_merged}
HOLDING=${HOLDING:-/root/logs/r2am_talent_sbs_v1_holding.stamp}
...
# 1) Wait R2am CPU premerge outcome (done with weights, or skip).
for i in $(seq 1 2880); do
  if [[ -f "$PREMERGE_SKIP" ]]; then
    echo "SKIP_R2AM_PREMERGE_SKIPPED $(cat "$PREMERGE_SKIP") $(date -u +%Y-%m-%dT%H:%M:%SZ)" | tee "$DONE"
    exit 0
  fi
  if [[ -f "$PREMERGE_DONE" && -f "$MERGED/model.safetensors.index.json" ]]; then
    echo "[r2am-merge] premerge ready at iter=$i $(date -u +%Y-%m-%dT%H:%M:%SZ) $(cat "$PREMERGE_DONE")"
    break
  fi
  if (( i % 12 == 0 )); then
    crumb=$(tail -n 3 /root/logs/r2am_premerge.log 2>/dev/null | tr '\r' '\n' | tail -1 || true)
    echo "[r2am-merge] wait-premerge iter=$i crumb=${crumb:-none}"
  fi
  if (( i == 2880 )); then
    echo "SKIP_R2AM_PREMERGE_TIMEOUT $(date -u +%Y-%m-%dT%H:%M:%SZ)" | tee "$DONE"
    exit 0
  fi
  sleep 10
done

# 2) Wait R2i…R2z + R2q/R2v + R2aa/R2ab terminal (or any prior clears 1.5×). Pidfile kill -0 only.
for i in $(seq 1 2880); do
  for f in "$R2D_DEC" "$R2E_DEC" "$R2H_DEC" "$R2G_DEC" "$R2I_DEC" "$R2J_DEC" \
           ...
           "$R2X_DEC" "$R2Y_DEC" "$R2Z_DEC" "$R2AA_DEC" "$R2AB_DEC" "$R2AC_DEC" "$R2AD_DEC"; do
    if [[ -f "$f" ]] && headroom_ok "$f"; then
      echo "SKIP_R2AM_PRIOR_CLEARS file=$f $(date -u +%Y-%m-%dT%H:%M:%SZ)" | tee "$DONE"
      exit 0
    fi
  done
  if [[ -f "$PREMERGE_SKIP" ]]; then
  ...
  for pf in /root/logs/r2{i,j,k,l,n,o,p,r}_merge_reload.pid \
            ...
            /root/logs/r2aa_merge_reload.pid /root/logs/r2ab_merge_reload.pid /root/logs/r2ac_merge_reload.pid /root/logs/r2ad_merge_reload.pid; do
  ...
  if (( busy == 0 )); then
    echo "[r2am-merge] R2i…R2z/R2q/R2v/R2aa…R2ad below bar or skipped; lane free at iter=$i"
    break
  fi
  if (( i % 12 == 0 )); then
    echo "[r2am-merge] wait-lane iter=$i $(date -u +%Y-%m-%dT%H:%M:%SZ) busy=$busy r2p_term=$(lane_terminal "$R2P_DONE" "$R2P_PREMERGE_SKIP" "$R2P_DEC" && echo y || echo n) r2aa_term=$(lane_terminal "$R2AA_DONE" "$R2AA_PREMERGE_SKIP" "$R2AA_DEC" && echo y || echo n) r2ab_term=$(lane_terminal "$R2AB_DONE" "$R2AB_PREMERGE_SKIP" "$R2AB_DEC" && echo y || echo n) r2ac_term=$(lane_terminal "$R2AC_DONE" "$R2AC_PREMERGE_SKIP" "$R2AC_DEC" && echo y || echo n) r2ad_term=$(lane_terminal "$R2AD_DONE" "$R2AD_PREMERGE_SKIP" "$R2AD_DEC" && echo y || echo n)"
  fi
  if (( i == 2880 )); then
    echo "[r2am-merge] TIMEOUT waiting R2i…R2z/R2q/R2v/R2aa…R2ad lane" >&2
    exit 2
  fi
  sleep 10
done
...
if [[ ! -f "$MERGED/model.safetensors.index.json" ]]; then
  echo "[r2am-merge] FATAL missing merged index at $MERGED" >&2
  exit 2
fi

ln -sfn "$MERGED" "$LINK"
echo "[r2am-merge] link $LINK -> $(readlink -f "$LINK")"

# Wait if R2q/R2s/R2t/R2u/R2v/R2w/R2o currently owns chall (do not yank mid-n80).
rm -f "$HOLDING"
_WAIT_R2Q_TAG=r2am-merge
# shellcheck disable=SC1091
...
if [[ -f "$CHALL_PID_FILE" ]]; then
  ...
  if [[ -n "${CPID:-}" ]] && kill -0 "$CPID" 2>/dev/null; then
    echo "[r2am-merge] stopping chall pid=$CPID"
    kill "$CPID" || true
...
if [[ ! -d /root/.triton/cache/chall ]] || [[ -z "$(find /root/.triton/cache/chall -name '*.so' 2>/dev/null | head -1)" ]]; then
  if [[ -d /root/.triton/cache/king ]]; then
    echo "[r2am-merge] seeding chall Triton cache from king"
...
echo "[r2am-merge] launching chall :8002 on $LINK"
...
echo "[r2am-merge] chall pid=$(cat /root/logs/vllm_chall.pid)"
...
for i in $(seq 1 480); do
  ...
  if [[ "$c0$c1$c2" == "200200200" ]]; then
    echo "[r2am-merge] engines 200/200/200 at iter=$i"
    break
  fi
  if (( i % 12 == 0 )); then
    echo "[r2am-merge] wait-engines iter=$i codes=${c0}/${c1}/${c2}"
  fi
  if (( i == 480 )); then
    echo "[r2am-merge] TIMEOUT engines; see vllm_chall.log" >&2
    rm -f "$HOLDING"
    exit 2
  fi
  sleep 5
...
BH=$(python - <<'PY'
import hashlib, time
print(hashlib.sha256(f"r2am-talent-sbs-v1-{time.time_ns()}".encode()).hexdigest())
PY
)

echo "[r2am-merge] launching R2am n80 block_hash=${BH:0:16}…"
python /root/mining_src/r1-reason-distill/run_reason_sim.py \
  --n-turns 80 \
  --block-hash "$BH" \
  --hotkey "local-r2am-talent-sbs-v1-$(date -u +%Y%m%dT%H%M%SZ)" \
  ...
  --progress-out "$PROG" \
  --save-artifact \
  2>&1 | tee /root/logs/r2am_alpha_reason_sim.log

python /root/mining_src/r1-reason-distill/write_reason_decision.py \
  --sim-result "$OUT" --out "$DEC" --hyp R2am \
  2>&1 | tee -a /root/logs/r2am_alpha_reason_sim.log

rm -f "$HOLDING"
echo "[r2am-merge] DONE $(date -u +%Y-%m-%dT%H:%M:%SZ)"
cat "$DEC"
