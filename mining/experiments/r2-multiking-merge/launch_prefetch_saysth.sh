#!/usr/bin/env bash
# Prefetch live chal-00440 challenger saysth/…-v9a (CPU/network only).
# Index probe (p1901): weights_ok=True for unconst. No Reason verdict yet —
# cache now so a post-verdict Reason+ parent can merge without an idle download.
set -euo pipefail
LOG=/root/logs/r2_prefetch_saysth.log
DONE=/root/logs/r2_prefetch_saysth.done
PIDF=/root/logs/r2_prefetch_saysth.pid
...
out = {
    ...
    "note": "p1901 live chal-00440 prefetch; Reason unknown until verdict",
    "parents": [],
    "skipped": [],
}
meta = Path("/root/affine_data/r2_prefetch_saysth.json")
