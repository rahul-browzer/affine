...
PIDF=/root/logs/watch_chal00471_reason.pid
OUT=${OUT:-/root/affine_data/chal00471_reason.json}
CHAL=${CHAL:-chal-00471}
...
echo "[watch-471] $(date -u +%Y-%m-%dT%H:%M:%SZ) start chal=$CHAL"
if [[ -f "$DONE" && -f "$OUT" ]]; then
  echo "[watch-471] already done: $(cat "$DONE")"
  exit 0
fi
...
for i in range(1, 2880):
    try:
        ...
        time.sleep(10)
print("[watch-471] TIMEOUT", flush=True)
raise SystemExit(2)
PY
