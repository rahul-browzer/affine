#!/usr/bin/env bash
# CPU watcher: when chal-00431 (BKN1890/…-six) publishes a duel gzip,
# recompute Reason = mean(lpC_yc_za − lpC_yc_e) per turn, paired vs Tok af10,
# stamp headroom_vs_3se. Gates possible Talent×BKN6 merge after Reason+ without waiting for
# the next Ralph pass. No GPU. Does not submit.
set -euo pipefail
LOG=/root/logs/watch_chal00431_reason.log
DONE=/root/logs/watch_chal00431_reason.done
PIDF=/root/logs/watch_chal00431_reason.pid
OUT=${OUT:-/root/affine_data/chal00431_reason.json}
CHAL=${CHAL:-chal-00431}
...
echo "[watch-431] $(date -u +%Y-%m-%dT%H:%M:%SZ) start chal=$CHAL"
if [[ -f "$DONE" && -f "$OUT" ]]; then
  echo "[watch-431] already done: $(cat "$DONE")"
  exit 0
fi
...
print(f"[watch-431] polling {url}", flush=True)
for i in range(1, 2880):  # ~8h @10s
    try:
        raw = fetch_bytes(url)
        print(f"[watch-431] got {len(raw)} bytes at iter={i}", flush=True)
        ...
        print(f"[watch-431] {line}", flush=True)
        raise SystemExit(0)
    except Exception as e:
        if i % 12 == 0:
            print(f"[watch-431] wait iter={i} err={type(e).__name__}: {e}", flush=True)
        time.sleep(10)
print("[watch-431] TIMEOUT", flush=True)
raise SystemExit(2)
PY
