...
if [[ -f "$SKIP" ]]; then
  echo "[r2o-premerge] already skipped: $(cat "$SKIP")"
  exit 0
fi

echo "[r2o-premerge] waiting for zeus prefetch $PREFETCH_DONE"
for i in $(seq 1 2880); do
  if [[ -f "$PREFETCH_DONE" ]]; then
    echo "[r2o-premerge] prefetch ready: $(cat "$PREFETCH_DONE")"
    break
  fi
  if (( i % 12 == 0 )); then
    crumb=$(tail -n 2 /root/logs/r2_prefetch_zeus.log 2>/dev/null | tr '\r' '\n' | tail -1 || true)
    echo "[r2o-premerge] wait-prefetch iter=$i crumb=${crumb:-none}"
  fi
  if (( i == 2880 )); then
    echo "SKIP $(date -u +%Y-%m-%dT%H:%M:%SZ) zeus prefetch TIMEOUT" | tee "$SKIP"
    exit 2
  fi
  sleep 10
done
...
d = json.loads(Path("/root/affine_data/chal00452_reason.json").read_text())
...
Path("/tmp/r2o_gate_ok").write_text("1\n" if ok else "0\n")
PY
)
