#!/usr/bin/env bash
# When R2bi local n80 stamps r2bi_mt2_decision.json with headroom ≥ 1.5×(3·SE),
# purge old public HF merges if needed and upload pure mt2 chall dir to
...
set -euo pipefail
LOG=/root/logs/watch_r2bi_stage5_push.log
DONE=/root/logs/watch_r2bi_stage5_push.done
PIDF=/root/logs/watch_r2bi_stage5_push.pid
DEC=${DEC:-/root/affine_data/r2bi_mt2_decision.json}
STAGE5=${STAGE5:-/root/affine_data/r2bi_stage5_ready.json}
CHALL=${CHALL:-/root/r2_out/mt2_chall}
REPO=${REPO:-unconst/Affine-5czsc2fc98-r2bi-mt2}
OUT_META=${OUT_META:-/root/affine_data/r2bi_stage5_hf_push.json}
PURGE_META=${PURGE_META:-/root/affine_data/r2bi_stage5_hf_purge.json}
...
echo "[r2bi-stage5-push] $(date -u +%Y-%m-%dT%H:%M:%SZ) start"
if [[ -f "$DONE" && -f "$OUT_META" ]]; then
  echo "[r2bi-stage5-push] already done: $(cat "$DONE")"
  exit 0
fi

echo "[r2bi-stage5-push] waiting for $DEC with headroom≥$HEADROOM_BAR"
...
PY
        echo "[r2bi-stage5-push] STAGE5 stamped hr=$hr at iter=$i"
        break
      fi
...
print(p.read_text().strip() if p.is_file() else 'no-progress')" 2>/dev/null || echo none)
    echo "[r2bi-stage5-push] wait iter=$i crumb=${crumb:-none}"
  fi
  if (( i == 2880 )); then
...
if [[ -z "${HF_TOKEN:-}" ]]; then
  echo "[r2bi-stage5-push] FATAL HF_TOKEN missing" >&2
  exit 2
fi
...
for sz, m in cands:
    ...
    try:
        ...
        freed += sz
        print(f"[r2bi-stage5-push] purged {m.id} ({sz/1e9:.1f}G)", flush=True)
    except Exception as e:
        errors.append({"id": m.id, "error": str(e)})
        print(f"[r2bi-stage5-push] purge err {m.id}: {e}", flush=True)
...
print(
    f"[r2bi-stage5-push] uploading {chall} ({total/(1<<30):.1f} GiB) → {repo} public",
    flush=True,
)
...
print("[r2bi-stage5-push] DONE", flush=True)
PY
