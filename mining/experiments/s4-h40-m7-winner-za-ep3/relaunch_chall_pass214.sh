#!/usr/bin/env bash
# Pass 214: H40 chall EngineDead after first completion —
# `__triton_launcher.so` missing from chall_p212 cache. Orphan Workers on
...
LOG=/root/logs/h40_chall_recover_pass214.log
...
log() { echo "[recover214-h40-chall] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }
...
print(f"[recover214-h40-chall] reap gpu={sorted(want)} workers={sorted(pids)} parents={sorted(parents)} grandparents={sorted(grandparents)}")
for pid in kill_set:
    try:
