#!/usr/bin/env bash
# Pass 217: H40 p216 came up on isolated TCACHE, first completions 200,
...
LOG=/root/logs/h40_chall_recover_pass217.log
CHALL_LOG=/root/logs/vllm_chall.log
PIDF=/root/logs/vllm_chall.pid
MERGE=/root/h40/merged
export GPUS=4,5
UTIL=0.72
TAG=h40_chall_p217_$(date +%s)_$$
TCACHE=/root/.triton/isolated/$TAG
QDIR=/root/affine_data/false_probes
mkdir -p /root/logs /root/affine_data "$QDIR"
: >"$LOG"

log() { echo "[recover217-h40-chall] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }

log "START chall relaunch (freeze TCACHE after warmup — p216 post-comp .so race)"
...
for f in h40_decision.json h40_sim_result.json h40_sim_result_artifact.json \
  ...
  if [[ -f "/root/affine_data/$f" ]]; then
    mv "/root/affine_data/$f" "$QDIR/${f%.json}_pass217_${ts}.json"
...
print(f"[recover217-h40-chall] reap gpu={sorted(want)} workers={sorted(pids)} parents={sorted(parents)} grandparents={sorted(grandparents)}")
for pid in kill_set:
    try:
...
_comp() {
  ...
  code=$(curl -s -o "/tmp/h40_warmup_${label}.json" -w "%{http_code}" --max-time 120 \
...
while read -r pid; do
  ...
  sleep 1
  kill -9 "$pid" 2>/dev/null || true
done < <(ps -eo pid,args | awk '/[r]etry_h40_n80\.sh/ {print $1}')
sleep 1
