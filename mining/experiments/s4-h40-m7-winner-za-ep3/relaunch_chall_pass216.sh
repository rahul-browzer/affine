#!/usr/bin/env bash
# Pass 216: H40 chall died during EngineCore init (p215) —
# `__triton_launcher.so` missing from chall_p215_* mid-profile. GPUs 4,5
# already free. Reap → wipe chall/chall_* + prior isolated → 30s settle →
# TCACHE under /root/.triton/isolated/ (outside chall_* wipe glob) → relaunch.
set -euo pipefail
...
for f in h40_decision.json h40_sim_result.json h40_sim_result_artifact.json \
  ...
  if [[ -f "/root/affine_data/$f" ]]; then
    mv "/root/affine_data/$f" "$QDIR/${f%.json}_pass216_${ts}.json"
...
print(f"[recover216-h40-chall] reap gpu={sorted(want)} workers={sorted(pids)} parents={sorted(parents)} grandparents={sorted(grandparents)}")
for pid in kill_set:
    try:
...
rm -rf /root/.triton/isolated/h40_chall_* || true
rm -rf /root/.cache/flashinfer/cached_ops/sampling || true
rm -rf /tmp/torchinductor_* /root/.cache/torch/inductor /root/.cache/torchinductor_chall_* /root/.cache/torchinductor_h40_* || true
log "settle 30s after wipe (p215 died mid-init on missing launcher.so)"
sleep 30
