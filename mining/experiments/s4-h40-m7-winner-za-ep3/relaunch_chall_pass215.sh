#!/usr/bin/env bash
# Pass 215: H40 chall EngineDead after first completion —
# `__triton_launcher.so` missing from chall_p214 cache. Orphan Workers on
# GPUs 4,5 hold ~135GiB; :8002 dead; retry polling forever. Reap → wipe
# chall/chall_* FIRST → ≥20s settle → unique TCACHE → relaunch + rearm.
set -euo pipefail
...
for f in h40_decision.json h40_sim_result.json h40_sim_result_artifact.json \
  ...
  if [[ -f "/root/affine_data/$f" ]]; then
    mv "/root/affine_data/$f" "$QDIR/${f%.json}_pass215_${ts}.json"
...
print(f"[recover215-h40-chall] reap gpu={sorted(want)} workers={sorted(pids)} parents={sorted(parents)} grandparents={sorted(grandparents)}")
for pid in kill_set:
    try:
