#!/usr/bin/env bash
# Pass 218: H40 p216 came up on isolated TCACHE, first completions 200,
...
for f in h40_decision.json h40_sim_result.json h40_sim_result_artifact.json \
  ...
  if [[ -f "/root/affine_data/$f" ]]; then
    mv "/root/affine_data/$f" "$QDIR/${f%.json}_pass219_${ts}.json"
...
print(f"[recover219-h40-chall] reap gpu={sorted(want)} workers={sorted(pids)} parents={sorted(parents)} grandparents={sorted(grandparents)}")
for pid in kill_set:
    try:
