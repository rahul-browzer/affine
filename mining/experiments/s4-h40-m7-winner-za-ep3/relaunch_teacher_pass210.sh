#!/usr/bin/env bash
# Pass 210: H40 teacher hung — workers on 0,1 hold VRAM but health=000;
# Worker_TP1 died on Triton ImportError cuda_utils.so missing (shared teacher
# cache race). King :8001 OK on 2,3; train on 6,7. Wipe teacher_* FIRST →
# ≥20s settle → unique TCACHE. Prewarm wait loop will write prewarm.done.
set -euo pipefail
...
LOG=/root/logs/h40_teacher_recover_pass210.log
TEACHER_LOG=/root/logs/vllm_teacher.log
PIDF=/root/logs/vllm_teacher.pid
export GPUS=0,1
UTIL=0.80
TEACHER_REPO=zai-org/GLM-4.5-Air-FP8
TAG=teacher_p210_$(date +%s)_$$
mkdir -p /root/logs
: >"$LOG"

log() { echo "[recover210-h40] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }

log "START teacher relaunch (wipe→settle20→unique cache)"
...
for pid in list(pids):
    try:
        ...
        pass
# Also kill APIServer parents of EngineCore parents
grandparents = set()
...
print(f"[recover210-h40] reap gpu={sorted(want)} workers={sorted(pids)} parents={sorted(parents)} grandparents={sorted(grandparents)}")
for pid in kill_set:
    try:
...
log "DONE_LAUNCH (train still on 6,7; king :8001 stays; prewarm will pick up t=1)"
