# mine-h20-1 bootstrap: TalentPigs×leary α0.90 + teacher → merge/serve/n80.
# Secrets from /root/mine.env only (never argv).
set -euo pipefail

LOG=/root/logs/bootstrap_h20.log
mkdir -p /root/logs /root/hf /root/affine_data /root/mining_src /root/merges
exec > >(tee -a "$LOG") 2>&1

echo "[bootstrap-h20] $(date -u +%Y-%m-%dT%H:%M:%SZ) start host=$(hostname)"
...
if [[ -z "${HF_TOKEN:-}" ]]; then
  echo "[bootstrap-h20] FATAL: HF_TOKEN missing in /root/mine.env"
  exit 1
fi
...
uv pip install \
  ...
  "numpy" \
  "scipy" \
  2>&1 | tee /root/logs/pip_h20.log | tail -40

python - <<'PY'
import torch, transformers, vllm
print("[bootstrap-h20] VERSIONS",
...
if txt == orig:
    print("[bootstrap-h20] vllm_client already patched or pattern miss", flush=True)
else:
    p.write_text(txt)
    print("[bootstrap-h20] patched vllm_client timeout=480 retries=5", flush=True)
PY
...
for repo, rev, label in jobs:
    ...
    if os.path.isfile(done):
        print(f"[bootstrap-h20] SKIP {label} (have {done})", flush=True)
        continue
    print(f"[bootstrap-h20] DOWNLOAD start {label} {repo} rev={rev}", flush=True)
    path = snapshot_download(repo, revision=rev, token=token)
    print(f"[bootstrap-h20] DOWNLOAD done {label} -> {path}", flush=True)
    open(done, "w").write(path + "\n")
print("[bootstrap-h20] ALL_DOWNLOADS_OK", flush=True)
...
python /root/mining_src/s4-h2-merge/merge_linear.py \
  ...
  --out "$OUT" \
  --hf-home "$HF_HOME" \
  2>&1 | tee /root/logs/h20_merge.log

date -u +%Y-%m-%dT%H:%M:%SZ > /root/logs/h20_merge.done
echo "[bootstrap-h20] MERGE_DONE alpha=$ALPHA out=$OUT b=$B_REPO@$B_REV"
...
export CHALL_REPO=/root/merges/h20-tp90
export CHALL_REV=
bash /root/mining_src/s3-duel-sim/serve_three.sh

touch /root/logs/bootstrap_h20.done
echo "[bootstrap-h20] $(date -u +%Y-%m-%dT%H:%M:%SZ) DONE — engines launching; next: wait_ready + n80"
