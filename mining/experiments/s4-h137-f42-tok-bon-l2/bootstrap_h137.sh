#!/usr/bin/env bash
# mine-f42-1: Tok331102 init → H137/F42 BoN-CE teacher-Λ2.
# Order: pip → DL tok+teacher → serve teacher → train (6,7) → king DL + post_train.
set -euo pipefail

LOG=/root/logs/bootstrap_h137.log
mkdir -p /root/logs /root/hf /root/affine_data /root/mining_src /root/h137
exec > >(tee -a "$LOG") 2>&1

echo "[bootstrap-h137] $(date -u +%Y-%m-%dT%H:%M:%SZ) start host=$(hostname)"
...
if [[ -z "${HF_TOKEN:-}" ]]; then
  echo "[bootstrap-h137] FATAL: HF_TOKEN missing in /root/mine.env"
  exit 1
fi

nvidia-smi -L || true
NGPU=$(nvidia-smi -L | wc -l)
echo "[bootstrap-h137] GPU_COUNT=$NGPU"
test "$NGPU" -ge 8
...
if p.is_file():
    ...
    if txt != orig:
        p.write_text(txt)
        print("[bootstrap-h137] patched vllm_client timeout=480 retries=5", flush=True)
    else:
        print("[bootstrap-h137] vllm_client already patched or pattern miss", flush=True)
PY
...
print("[bootstrap-h137] DOWNLOAD tok-init start", repo, rev, flush=True)
path = snapshot_download(repo, revision=rev, token=token)
print(f"[bootstrap-h137] DOWNLOAD tok-init done -> {path}", flush=True)
open("/root/logs/tok_init.done", "w").write(path + "\n")
open("/root/logs/tok331102.done", "w").write(path + "\n")
print("[bootstrap-h137] DOWNLOAD teacher start", flush=True)
tpath = snapshot_download("zai-org/GLM-4.5-Air-FP8", token=token)
print(f"[bootstrap-h137] DOWNLOAD teacher done -> {tpath}", flush=True)
open("/root/logs/teacher.done", "w").write(tpath + "\n")
PY

# Serve teacher only on 0,1 (chall placeholder stopped; king later).
echo "[bootstrap-h137] $(date -u +%Y-%m-%dT%H:%M:%SZ) serving teacher before train"
...
# Wait teacher :8000 (king optional for train; start_h137 waits teacher).
for i in $(seq 1 240); do
  if curl -sf --max-time 5 http://127.0.0.1:8000/v1/models >/dev/null 2>&1; then
    echo "[bootstrap-h137] teacher :8000 up at iter=$i"
    break
  fi
  if (( i == 240 )); then
    echo "[bootstrap-h137] FATAL teacher never up"
...
echo "[bootstrap-h137] $(date -u +%Y-%m-%dT%H:%M:%SZ) launching H137/F42 train"
...
echo "[bootstrap-h137] $(date -u +%Y-%m-%dT%H:%M:%SZ) BOOTSTRAP_DONE train=$(cat /root/logs/h137_train.pid) post=$(cat /root/logs/h137_post_train.pid)"
