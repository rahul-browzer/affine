#!/usr/bin/env bash
# Host → mine-f42-1: upload stack + data, start bootstrap under nohup.
set -euo pipefail

ROOT=/home/const/subnet120
DST_HOST=${DST_HOST:?set DST_HOST}
DST_PORT=${DST_PORT:?set DST_PORT}
KNOWN=${KNOWN:-/tmp/mine-f42-1.known_hosts}
...
STAGE=$(mktemp -d /tmp/mine-h137-stack.XXXXXX)
...
test -f "$STAGE/$EXP/train_bon_l2.py"
test -f "$STAGE/$EXP/bootstrap_h137.sh"
test -f "$STAGE/$EXP/start_h137.sh"

TAR=/tmp/mine-h137-stack.tar.gz
tar -C "$STAGE" -czf "$TAR" .
ls -lh "$TAR"

ENV_TMP=$(mktemp /tmp/mine-h137.env.XXXXXX)
...
# H137 data = H27 winner_za_high_l1 (406 ex); init = Tok331102 king.
DATA="$ROOT/mining/experiments/s4-h27-clip-l1-shape/results/winner_za_high_l1.jsonl"
test -s "$DATA"
test "$(wc -l <"$DATA")" -ge 300

"${SSH[@]}" 'mkdir -p /root/mining_src /root/affine_data /root/logs /root/h137 /root/hf'
"${SCP[@]}" "$TAR" "root@${DST_HOST}:/tmp/mine-h137-stack.tar.gz"
...
for repo in (
    "unconst/Affine-5czsc2fc98-h137-lora",
    "unconst/Affine-5czsc2fc98-h137-merged",
):
    try:
...
"${SSH[@]}" 'set -e
  tar -C /root/mining_src -xzf /tmp/mine-h137-stack.tar.gz
  ...
  nohup bash /root/mining_src/s4-h137-f42-tok-bon-l2/bootstrap_h137.sh \
    >/root/logs/h137_pipeline.nohup 2>&1 &
  echo $! > /root/logs/h137_pipeline.pid
  nohup bash /root/mining_src/s4-h2-merge/watch_form_decision.sh h137 \
    ...
    >/root/logs/h137_form_decision.launch.out 2>&1 &
  ...
  nohup bash /root/mining_src/s4-h137-f42-tok-bon-l2/watch_preempt_bare_tcache_pass264.sh \
    >/root/logs/h137_preempt_bare_pass264.launch.nohup 2>&1 &
  echo $! > /root/logs/h137_preempt_bare_pass264.pid
  echo PREEMPT_PID=$(cat /root/logs/h137_preempt_bare_pass264.pid)
  echo PIPELINE_PID=$(cat /root/logs/h137_pipeline.pid)
  sleep 3
  head -n 40 /root/logs/bootstrap_h137.log 2>/dev/null || head -n 40 /root/logs/h137_pipeline.nohup || true
  ps -p "$(cat /root/logs/h137_pipeline.pid)" -o pid,etime,cmd || true
'
