#!/usr/bin/env python3
"""H137/F42: Best-of-N CE on thought tokens maximizing teacher Λ2.

Sample G thoughts, score each with live teacher:
  r = lpC(y | z) − lpC(y | ∅)
Imitate only the argmax sample via token-level CE (−sum log p).

Structurally different from F37 REINFORCE (advantage×logp on all G):
here the policy is pulled toward the high-Λ2 mode, not a mean-baseline
gradient that vanishes when G rewards are similar.
"""
...
def _ce_on_thought(
    ...
    """Token CE (−mean logp) on generated thought continuation."""
    enc = tok(prompt, add_special_tokens=False, return_tensors="pt")
...
def main() -> None:
    ...
    print(f"[bon] kept={len(rows)}/{len(raw)} budget_chars={budget}", flush=True)
    if len(rows) < 50:
    ...
    for i in range(120):
        try:
            ...
            print(f"[bon] wait teacher ({i}): {e}", flush=True)
            time.sleep(15)
    else:
        raise SystemExit(f"teacher not ready at {args.teacher_url}")
    print(f"[bon] teacher_model={teacher_model}", flush=True)

    meta = {
        "experiment": "s4-h137-f42-tok-bon-l2",
        "family": "F42",
    ...
    print(f"[bon] loading base {args.base}", flush=True)
        ...
        for row in rows:
            ...
            for _g in range(args.group_size):
                ...
                try:
                    ...
                    print(f"[bon] teacher score fail step={steps}: {e}", flush=True)
                    r = float("-inf")
            ...
            finite = [(i, r) for i, r in enumerate(rewards) if math.isfinite(r)]
            if not finite:
                print(f"[bon] skip step={steps}: no finite rewards", flush=True)
                continue
            best_i, best_r = max(finite, key=lambda t: t[1])
            mean_r = sum(r for _, r in finite) / len(finite)
            loss = _ce_on_thought(model, tok, prompt, texts[best_i], device)
            ...
            rec = {
                ...
                "z0": texts[best_i][:80] if texts else "",
            }
            hist.append(rec)
            if steps % 5 == 0 or steps <= 3:
                print(f"[bon-log] {json.dumps(rec)}", flush=True)
            if steps % 50 == 0:
    ...
    result = {
        ...
        "mean_best_r_last20": (
            sum(h["best_r"] for h in hist[-20:]) / max(1, len(hist[-20:]))
        ),
        "adapter": str(adapter),
    }
    ...
    print(f"[bon] DONE {json.dumps(result)}", flush=True)
