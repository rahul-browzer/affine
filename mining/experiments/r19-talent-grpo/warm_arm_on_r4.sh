#!/usr/bin/env bash
# Host → lean warm-arm R19 onto TKC-hot mine-r4-fullft-1 after R5b SIGNAL_POS_BELOW.
# Keeps teacher:8000 + guass:8001; trains Talent-GRPO on GPUs 6–7 (≠ R5b Talent full-FT).
set -euo pipefail
...
STAGE=$(mktemp -d /tmp/mine-r19-warm.XXXXXX)
...
cp -a "$ROOT/mining/experiments/r19-talent-grpo/"*.sh "$STAGE/r19-talent-grpo/"
cp -a "$ROOT/mining/experiments/r19-talent-grpo/plan.md" "$STAGE/r19-talent-grpo/"
# Overlay start/bootstrap for R19 identity grep.
...
TAR=/tmp/mine-r19-warm-stack.tar.gz
tar -C "$STAGE" -czf "$TAR" .
ls -lh "$TAR"
...
ENV_TMP=$(mktemp /tmp/mine-r19.env.XXXXXX)
{
  ...
  echo "export BASE=/root/hf/hub/models--TalentPigs--affine-5ekxlcg3fx-abc/snapshots/dbfbb3e2a17c7603e7fc68a3a15b343f42dfdef4"
} >"$ENV_TMP"
chmod 600 "$ENV_TMP"
...
"${SCP[@]}" "$TAR" "root@${DST_HOST}:/tmp/mine-r19-warm-stack.tar.gz"
...
"${SSH[@]}" 'set -euo pipefail
  tar -C /root/mining_src -xzf /tmp/mine-r19-warm-stack.tar.gz
  ...
  test -x /root/mining_src/r19-talent-grpo/lean_warm_boot.sh
  ...
  # Stop leftover R5b lean/post/form/n80 by pidfile only (never pkill -f).
  for f in /root/logs/r5b_form_decision.pid /root/logs/watch_n80_retry.pid \
           /root/logs/h122_n80_retry.pid /root/logs/r5b_post_train.pid \
           /root/logs/r21_lean_warm.pid /root/logs/r21_post_train.pid \
           /root/logs/r21_form_decision.pid /root/logs/r3_train.pid \
           /root/logs/r3_post_train.pid /root/logs/r3_pipeline.pid; do
    if [[ -f "$f" ]]; then
  ...
  nohup bash /root/mining_src/r19-talent-grpo/lean_warm_boot.sh \
    >/root/logs/r19_lean_warm.nohup 2>&1 &
  echo $! >/root/logs/r19_lean_warm.pid
  sleep 35
  echo LEAN_PID=$(cat /root/logs/r19_lean_warm.pid)
  head -n 80 /root/logs/r19_lean_warm.log 2>/dev/null \
    || head -n 80 /root/logs/r19_lean_warm.nohup || true
  ps -p "$(cat /root/logs/r19_lean_warm.pid)" -o pid,etime,cmd || true
  test -f /root/logs/r3_train.pid && echo TRAIN_PID=$(cat /root/logs/r3_train.pid)
  test -f /root/logs/r19_post_train.pid && echo POST_PID=$(cat /root/logs/r19_post_train.pid)
...
echo "R19_WARM_ARM_OK pod=$POD_NAME soft=$SOFT dead=$DEAD $(date -u +%Y-%m-%dT%H:%M:%SZ)"
