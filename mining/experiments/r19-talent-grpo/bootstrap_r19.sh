...
mkdir -p /root/logs /root/hf /root/affine_data /root/mining_src /root/r3 /root/r19
exec > >(tee -a "$LOG") 2>&1

echo "[bootstrap-r19] $(date -u +%Y-%m-%dT%H:%M:%SZ) start host=$(hostname)"
...
if [[ -z "${HF_TOKEN:-}" ]]; then
  echo "[bootstrap-r19] FATAL: HF_TOKEN missing in /root/mine.env"
  exit 1
fi

nvidia-smi -L || true
NGPU=$(nvidia-smi -L | wc -l)
echo "[bootstrap-r19] GPU_COUNT=$NGPU"
test "$NGPU" -ge 8
...
# Prove overlay is R19, not stock R3 / R5b / R18.
...
if p.is_file():
    ...
    if txt != orig:
        p.write_text(txt)
        print("[bootstrap-r19] patched vllm_client timeout=600 retries=5", flush=True)
    else:
...
talent_repo = "TalentPigs/affine-5ekxlcg3fx-abc"
talent_rev = "dbfbb3e2a17c7603e7fc68a3a15b343f42dfdef4"
print("[bootstrap-r19] DOWNLOAD talent-init start", talent_repo, talent_rev, flush=True)
talent = snapshot_download(talent_repo, revision=talent_rev, token=token)
print(f"[bootstrap-r19] DOWNLOAD talent-init done -> {talent}", flush=True)
open("/root/logs/talent.done", "w").write(talent + "\n")
...
print(f"[bootstrap-r19] DOWNLOAD teacher done -> {tpath}", flush=True)
open("/root/logs/teacher.done", "w").write(tpath + "\n")
print("[bootstrap-r19] ALL_DOWNLOADS_OK", flush=True)
PY

export BASE=$(cat /root/r19/r19_base.path)
if [[ -f /root/mine.env ]]; then
...
echo "[bootstrap-r19] $(date -u +%Y-%m-%dT%H:%M:%SZ) serving teacher before train BASE=$BASE"
bash /root/mining_src/r3-reason-grpo/prewarm_engines.sh \
  >/root/logs/r3_prewarm.nohup 2>&1 &
echo $! >/root/logs/r3_prewarm.pid
cp -f /root/logs/r3_prewarm.pid /root/logs/r19_prewarm.pid

for i in $(seq 1 240); do
  if curl -sf --max-time 5 http://127.0.0.1:8000/v1/models >/dev/null 2>&1; then
    echo "[bootstrap-r19] teacher :8000 up at iter=$i"
    break
  fi
  if (( i == 240 )); then
    echo "[bootstrap-r19] FATAL teacher never up"
...
echo "[bootstrap-r19] $(date -u +%Y-%m-%dT%H:%M:%SZ) launching R19 Talent Reason-GRPO train"
...
cp -f /root/logs/r3_extra_dl.pid /root/logs/r19_extra_dl.pid
...
cp -f /root/logs/r3_post_train.pid /root/logs/r19_post_train.pid

echo "[bootstrap-r19] $(date -u +%Y-%m-%dT%H:%M:%SZ) BOOTSTRAP_DONE train=$(cat /root/logs/r3_train.pid) post=$(cat /root/logs/r3_post_train.pid)"
