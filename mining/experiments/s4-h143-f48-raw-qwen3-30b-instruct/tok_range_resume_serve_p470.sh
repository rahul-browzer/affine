#!/usr/bin/env bash
# Pass 470: Range-resume frozen Tok king shards on mine-f48-1, then serve_three + n80 path.
# huggingface_hub≥1.27 never resumes .incomplete (opens fresh "wb"); use HTTP Range.
set -euo pipefail

LOG=/root/logs/h143_tok_range_p470.nohup
mkdir -p /root/logs
exec > >(tee -a "$LOG") 2>&1
...
for pid in $(pgrep -f 'bash /root/mining_src/s4-h143-f48-raw-qwen3-30b-instruct/bootstrap_h143.sh' || true); do
  # children
...
for i in $(seq 1 360); do
  ...
  if [[ "$code" == "200" && "$kcode" == "200" && "$tcode" == "200" ]]; then
    echo "chall_serve_ok poll=$i $(date -u +%Y-%m-%dT%H:%M:%SZ)" | tee /root/logs/h143_chall_serve.done
    break
  fi
  sleep 15
done
...
rm -f /root/logs/h143_n80_retry.aborted /root/affine_data/h143_N80_DONE /root/logs/h143_N80_DONE 2>/dev/null || true
