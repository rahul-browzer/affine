...
log() { echo "[recover-wait193-h31] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }
...
for i in $(seq 1 160); do
  ...
  if [[ "$t" == "200" && "$k" == "200" && "$c" == "200" ]]; then
    ...
    probe=$(curl -s --max-time 180 http://127.0.0.1:8002/v1/completions \
      ...
      -d '{"model":"/root/h31/merged","prompt":"hi","max_tokens":4,"temperature":0}' \
...
nohup bash /root/mining_src/s4-h31-m7-king-self-lr3e5/retry_h31_n80.sh \
  >/root/logs/h31_n80_relaunch_pass193.nohup 2>&1 &
echo $! >/root/logs/h31_n80_relaunch_pass193.pid
log "relaunched retry pid=$(cat /root/logs/h31_n80_relaunch_pass193.pid)"

if ! ps -eo pid,cmd | awk '/[w]atch_form_decision\.sh/ && / h31 /' | grep -q .; then
  nohup bash /root/mining_src/s4-h2-merge/watch_form_decision.sh \
    h31 /root/affine_data/h31_sim_result.json /root/affine_data/h31_decision.json \
    /root/logs/h31_form_decision.nohup \
    >/root/logs/h31_form_decision.launch.out 2>&1 &
  log "relaunched watch_form_decision"
fi
if ! ps -eo pid,cmd | awk '/[w]atch_n80_retry\.sh/ && / h31 /' | grep -q .; then
  nohup bash /root/mining_src/s4-h2-merge/watch_n80_retry.sh \
    h31 /root/mining_src/s4-h31-m7-king-self-lr3e5/retry_h31_n80.sh \
