#!/usr/bin/env bash
# Host → mine-h31-1: upload stack + data, start bootstrap under nohup.
set -euo pipefail

ROOT=/home/const/subnet120
DST_HOST=${DST_HOST:?set DST_HOST}
DST_PORT=${DST_PORT:?set DST_PORT}
KNOWN=${KNOWN:-/tmp/mine-h31-1.known_hosts}
...
STAGE=$(mktemp -d /tmp/mine-h31-stack.XXXXXX)
trap 'rm -rf "$STAGE"' EXIT

EXP=s4-h31-m7-king-self-lr3e5
...
TAR=/tmp/mine-h31-stack.tar.gz
tar -C "$STAGE" -czf "$TAR" .
ls -lh "$TAR"

ENV_TMP=$(mktemp /tmp/mine-h31.env.XXXXXX)
...
# H31 data = H29 king-self harvest (686 ex); init = m7 (same as H28).
DATA="$ROOT/mining/experiments/s4-h29-king-self-clip-l1/results/king_self_high_l1.jsonl"
test -s "$DATA"
test "$(wc -l <"$DATA")" -ge 300

"${SSH[@]}" 'mkdir -p /root/mining_src /root/affine_data /root/logs /root/h31 /root/hf'
"${SCP[@]}" "$TAR" "root@${DST_HOST}:/tmp/mine-h31-stack.tar.gz"
"${SCP[@]}" "$ENV_TMP" "root@${DST_HOST}:/root/mine.env"
"${SCP[@]}" "$DATA" "root@${DST_HOST}:/root/h31/king_self_high_l1.jsonl"
rm -f "$ENV_TMP"
...
for repo in (
    "unconst/Affine-5czsc2fc98-h31-lora",
    "unconst/Affine-5czsc2fc98-h31-merged",
):
    try:
...
"${SSH[@]}" 'set -e
  tar -C /root/mining_src -xzf /tmp/mine-h31-stack.tar.gz
  chmod 600 /root/mine.env
  chmod +x /root/mining_src/s3-duel-sim/*.sh \
           /root/mining_src/s4-h2-merge/*.sh \
           /root/mining_src/s4-h31-m7-king-self-lr3e5/*.sh
  test -f /root/mining_src/affine_pkg/affine/score.py
  test -s /root/h31/king_self_high_l1.jsonl
  test -x /root/mining_src/s4-h31-m7-king-self-lr3e5/bootstrap_h31.sh
  echo STACK_UPLOAD_OK
  nohup bash /root/mining_src/s4-h31-m7-king-self-lr3e5/bootstrap_h31.sh \
    >/root/logs/h31_pipeline.nohup 2>&1 &
  echo $! > /root/logs/h31_pipeline.pid
  nohup bash /root/mining_src/s4-h2-merge/watch_form_decision.sh h31 \
    /root/affine_data/h31_sim_result.json /root/affine_data/h31_decision.json \
    /root/logs/h31_form_decision.nohup \
    >/root/logs/h31_form_decision.launch.out 2>&1 &
  echo $! > /root/logs/h31_form_decision.pid
  nohup bash /root/mining_src/s4-h2-merge/watch_n80_retry.sh h31 \
    /root/mining_src/s4-h31-m7-king-self-lr3e5/retry_h31_n80.sh \
    >/root/logs/h31_watch_retry.launch.nohup 2>&1 &
  echo $! > /root/logs/h31_watch_retry.pid
  echo PIPELINE_PID=$(cat /root/logs/h31_pipeline.pid)
  sleep 3
  head -n 40 /root/logs/bootstrap_h31.log 2>/dev/null || head -n 40 /root/logs/h31_pipeline.nohup || true
  ps -p "$(cat /root/logs/h31_pipeline.pid)" -o pid,etime,cmd || true
'
