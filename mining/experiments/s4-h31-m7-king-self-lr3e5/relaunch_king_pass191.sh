#!/usr/bin/env bash
# Pass 191: H31 king died on Triton __triton_launcher.so race (GPUs 2,3 empty).
...
log() { echo "[recover191-h31] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }

log "START king relaunch TCACHE=$TCACHE"

mkdir -p /root/affine_data/false_probes
for f in h31_decision.json h31_sim_result.json h31_sim_progress.json; do
...
print(f"[recover191-h31] reap gpu={sorted(want)} workers={sorted(pids)} parents={sorted(parents)}")
for pid in kill_set:
    try:
...
nohup bash /root/mining_src/s4-h31-m7-king-self-lr3e5/recover_wait_pass191.sh \
  >/root/logs/h31_recover_wait191.nohup 2>&1 &
echo $! >/root/logs/h31_recover_wait.pid
log "wait_pid=$(cat /root/logs/h31_recover_wait.pid)"
log "DONE_LAUNCH"
