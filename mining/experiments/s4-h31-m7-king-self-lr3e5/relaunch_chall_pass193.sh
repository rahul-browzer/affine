#!/usr/bin/env bash
# Pass 193: H31 chall died on first completions — Triton __triton_launcher.so
...
LOG=/root/logs/h31_chall_recover_pass193.log
CHALL_LOG=/root/logs/vllm_chall.log
PIDF=/root/logs/vllm_chall.pid
MERGE=/root/h31/merged
export GPUS=4,5
UTIL=0.72
...
log() { echo "[recover193-h31-chall] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }

log "START chall relaunch TCACHE=$TCACHE"

for f in h31_decision.json h31_sim_result.json h31_sim_progress.json \
  h31_sim_result_artifact.json; do
...
done < <(ps -eo pid,cmd | awk '/[r]un_sim_duel.py/ && /local-h31/ {print $1}')
...
while read -r pid; do
  ...
  sleep 1
  kill -9 "$pid" 2>/dev/null || true
done < <(ps -eo pid,cmd | awk '/[p]ost_train_pipeline.sh/ && /h31/ {print $1}')
...
print(f"[recover193-h31-chall] reap gpu={sorted(want)} workers={sorted(pids)} parents={sorted(parents)}")
for pid in kill_set:
    try:
...
rm -f /root/logs/h31_pipeline.aborted /root/logs/h31_n80_retry.aborted \
  /root/logs/h31_n80.done /root/logs/h31_chall_serve.done

nohup bash /root/mining_src/s4-h31-m7-king-self-lr3e5/recover_wait_chall_pass193.sh \
  >/root/logs/h31_recover_wait193.nohup 2>&1 &
echo $! >/root/logs/h31_recover_wait193.pid
log "wait_pid=$(cat /root/logs/h31_recover_wait193.pid)"
log "DONE_LAUNCH"
