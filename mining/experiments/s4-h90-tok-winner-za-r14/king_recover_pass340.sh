#!/usr/bin/env bash
# Pass 340: king-only relaunch after mid-n80 sample_tokens TimeoutError
# → EngineDead; :8001=000; GPUs 2,3 empty. Leave chall (recover264 / 4,5).
# Isolated TCACHE + util=0.72. No completions probe during settle (H85).
set -euo pipefail
...
LOGN=/root/logs/h90_king_recover_pass340.nohup
log() { echo "[king340-h90] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOGN"; }

log "START king-only recover (p340 isolated util=0.72 after sample_tokens TimeoutError @17:08:27Z; leave chall)"
...
for line in apps.strip().splitlines():
    ...
    if len(parts) >= 2 and parts[0] in uuids:
        try:
            ...
            pass
print(f"[king340-h90] reap gpus2,3 pids={sorted(pids)}")
for pid in pids:
    try:
...
for line in subprocess.check_output(["ps", "aux"], text=True).splitlines():
    if "vllm" in line and "serve" in line and "--port 8001" in line:
        ...
        print(f"[king340-h90] kill leftover :8001 pid={pid}")
        try:
            os.kill(pid, signal.SIGTERM)
        except OSError:
            pass
...
rm -f /root/affine_data/h90_sim_result.json /root/affine_data/h90_sim_progress.json \
  /root/affine_data/h90_decision.json /root/logs/h90_n80.done /root/logs/h90_n80_retry.aborted \
  /root/logs/h90_king_recover_pass311.done /root/logs/h90_sim_n80.done \
  /root/logs/h90_king_recover_pass332.done /root/logs/h90_king_recover_pass340.done
log "cleared stale h90 sim progress/result"
...
TCACHE="/root/.triton/isolated/h90_king_p340_$(date +%s)_$$"
mkdir -p "$TCACHE"
echo "$TCACHE" >/root/logs/h90_king_tcache_pass340.path
...
for line in out.strip().splitlines():
    ...
    if int(idx) in (2, 3):
        print(f"[king340-h90] gpu{idx} used_mib={used}")
        if float(used) < 500:
            ok += 1
if ok < 2:
    print("[king340-h90] ABORT: gpus 2,3 not free")
    sys.exit(1)
PY
...
for i in $(seq 1 180); do
  ...
  # Health + /v1/models only — no completions probe (H85).
  ...
  if [[ "$code" == "200" ]]; then
    ...
    if [[ -n "$mid" ]]; then
      ...
      date -u +%Y-%m-%dT%H:%M:%SZ >/root/logs/h90_king_recover_pass340.done
      echo "$REPO $REV $TCACHE util=0.72" >>/root/logs/h90_king_recover_pass340.done
      log "DONE — retry_h90_n80 should proceed (watcher already armed)"
      exit 0
    fi
  fi
