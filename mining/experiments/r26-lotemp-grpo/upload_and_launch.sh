#!/usr/bin/env bash
# Host → mine-r26-lotemp-1: R3 stack + R26 LoTemp-GRPO overlay, bootstrap.
# Axis R26: Tok-init Reason-GRPO temperature=0.5 (≠ R3 0.8 / R25 1.2).
set -euo pipefail

ROOT=/home/const/subnet120
POD_NAME=${POD_NAME:-mine-r26-lotemp-1}
...
STAGE=$(mktemp -d /tmp/mine-r26-stack.XXXXXX)
...
cp -a "$ROOT/mining/experiments/r26-lotemp-grpo/start_r26.sh" "$STAGE/r26-lotemp-grpo/"
# Overlay: bootstrap_r3 calls start_r3.sh — replace with R26 lotemp knobs.
...
"${SCP[@]}" "$TAR" "root@${DST_HOST}:/tmp/mine-r26-stack.tar.gz"
...
"${SSH[@]}" 'set -e
  tar -C /root/mining_src -xzf /tmp/mine-r26-stack.tar.gz
  ...
  # Prove overlay is R26 LoTemp, not stock R3 / R24 / parent-GRPO.
  ...
  echo "R26_DEADLINES soft=$SOFT_DEADLINE_UTC dead=$DEADMAN_UTC axis=$R26_AXIS"
  echo "R26_KNOBS temp=${R26_TEMPERATURE} lr=${R26_LR} r=${R26_LORA_R} G=${R26_GROUP_SIZE} max_len=${R26_MAX_LEN}"
  echo STACK_UPLOAD_OK
  nohup bash /root/mining_src/r3-reason-grpo/bootstrap_r3.sh \
    >/root/logs/r26_pipeline.nohup 2>&1 &
  echo $! > /root/logs/r26_pipeline.pid
  cp -f /root/logs/r26_pipeline.pid /root/logs/r3_pipeline.pid
  echo PIPELINE_PID=$(cat /root/logs/r26_pipeline.pid)
  sleep 5
  head -n 40 /root/logs/bootstrap_r3.log 2>/dev/null || head -n 40 /root/logs/r26_pipeline.nohup || true
  ps -p "$(cat /root/logs/r26_pipeline.pid)" -o pid,etime,cmd || true
  nvidia-smi -L | wc -l
'
