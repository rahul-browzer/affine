#!/usr/bin/env bash
# H105/F10 pass393: GPU merge save hung on gocryptfs request_wait_answer
...
EXP=/root/mining_src/s4-h105-f10-talentpigs-base
LOG=/root/logs/h105_merge_recover_pass393.nohup
PIDF=/root/logs/h105_merge_recover_pass393.pid
MERGED=/root/h105/merged
ADAPTER=/root/h105/train/adapter
BASE=/root/hf/hub/models--TalentPigs--affine-5ekxlcg3fx-abc/snapshots/dbfbb3e2a17c7603e7fc68a3a15b343f42dfdef4

log() { echo "[h105-merge393] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }

mkdir -p /root/logs /root/affine_data
: >"$LOG"
echo $$ >"$PIDF"

MERGE_PID=$(ps -eo pid,cmd | awk '/[p]ython .*merge_lora.py --base .*h105\/merged/{print $1; exit}')
POST_PID=$(ps -eo pid,cmd | awk '/[b]ash .*s4-h105-f10-talentpigs-base\/post_train_pipeline\.sh/{print $1; exit}')
...
rm -f /root/logs/h105_pipeline.aborted
...
if [[ -x "$EXP/watch_preempt_bare_tcache_pass264.sh" ]]; then
  nohup bash "$EXP/watch_preempt_bare_tcache_pass264.sh" \
    >>/root/logs/h105_preempt_bare_pass264.log 2>&1 &
  echo $! >/root/logs/h105_preempt_bare_pass264.pid
  log "rearmed preempt264 pid=$(cat /root/logs/h105_preempt_bare_pass264.pid)"
fi
...
log "post_train resumed pid=$(cat /root/logs/h105_post_train.pid) SKIP_MERGE=1"
log "DONE recover393"
