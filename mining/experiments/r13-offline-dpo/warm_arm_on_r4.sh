#!/usr/bin/env bash
# Host → lean warm-arm R13 onto TKC-hot mine-r4-fullft-1 after R12 REFUTE.
# Keeps teacher:8000 + ckp333:8001; trains offline-DPO on GPUs 6–7.
set -euo pipefail
...
STAGE=$(mktemp -d /tmp/mine-r13-warm.XXXXXX)
...
cp -a "$ROOT/mining/experiments/r13-offline-dpo/"*.sh "$STAGE/r13-offline-dpo/"
cp -a "$ROOT/mining/experiments/r13-offline-dpo/plan.md" "$STAGE/r13-offline-dpo/"
# Overlay start/bootstrap for R13 identity grep.
...
TAR=/tmp/mine-r13-warm-stack.tar.gz
tar -C "$STAGE" -czf "$TAR" .
ls -lh "$TAR"
...
"${SCP[@]}" "$TAR" "root@${DST_HOST}:/tmp/mine-r13-warm-stack.tar.gz"
...
"${SSH[@]}" 'set -euo pipefail
  tar -C /root/mining_src -xzf /tmp/mine-r13-warm-stack.tar.gz
  chmod 600 /root/mine.env
  chmod +x /root/mining_src/s4-h138-f43-tok-dpo-l2/*.sh \
  ...
  # Stop leftover R12 lean/post/form by pidfile only (never pkill -f).
  for f in /root/logs/r12_lean_warm.pid /root/logs/r12_post_train.pid \
           /root/logs/h137_post_train.pid /root/logs/r12_form_decision.pid \
           /root/logs/h137_dec_watch.pid /root/logs/r12_form_decision.launch.out; do
    if [[ -f "$f" ]]; then
  ...
  # Also stop form watcher if still the R12 one (pidfile may be text-less launch out).
  if [[ -f /root/logs/r12_form_decision.pid ]]; then
    pid=$(cat /root/logs/r12_form_decision.pid 2>/dev/null || true)
    if [[ -n "${pid:-}" ]] && [[ "$pid" =~ ^[0-9]+$ ]] && kill -0 "$pid" 2>/dev/null; then
      kill "$pid" || true
      echo "killed_r12_form $pid"
    fi
  fi
  nohup bash /root/mining_src/r13-offline-dpo/lean_warm_boot.sh \
    >/root/logs/r13_lean_warm.nohup 2>&1 &
  echo $! >/root/logs/r13_lean_warm.pid
  sleep 10
  echo LEAN_PID=$(cat /root/logs/r13_lean_warm.pid)
  head -n 80 /root/logs/r13_lean_warm.log 2>/dev/null \
    || head -n 80 /root/logs/r13_lean_warm.nohup || true
  ps -p "$(cat /root/logs/r13_lean_warm.pid)" -o pid,etime,cmd || true
  test -f /root/logs/h138_train.pid && echo TRAIN_PID=$(cat /root/logs/h138_train.pid)
  test -f /root/logs/r13_post_train.pid && echo POST_PID=$(cat /root/logs/r13_post_train.pid)
...
echo "R13_WARM_ARM_OK pod=$POD_NAME soft=$SOFT dead=$DEAD $(date -u +%Y-%m-%dT%H:%M:%SZ)"
