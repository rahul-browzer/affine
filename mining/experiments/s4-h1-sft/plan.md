...
3. **After `train.done`:** automated by `post_train_pipeline.sh` (pid 83414
   re-armed 2026-08-07T01:58Z with salvage): `salvage_adapter.py` → private
   `unconst/Affine-5czsc2fc98-h1-lora` (adapter-only TTL insurance);
   `merge_lora.py` → `/root/h1/merged`; restart chall:8002 via
   `restart_for_h2.sh`; `run_sim_duel.py` →
