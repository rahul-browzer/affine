#!/usr/bin/env bash
# Pass 209: H41 king died at EngineCore init during prewarm —
# __triton_launcher.so missing (classic Triton race). Wipe role caches FIRST
# → ≥20s settle → unique TCACHE. Merge still writing on 6,7; GPUs 2,3 free.
# Teacher :8000 stays up. Prewarm wait loop will see k=1 and write prewarm.done.
set -euo pipefail
...
LOG=/root/logs/h41_king_recover_pass209.log
...
TAG=king_p209_$(date +%s)_$$
mkdir -p /root/logs
: >"$LOG"

log() { echo "[recover209-h41] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }
...
print(f"[recover209-h41] reap gpu={sorted(want)} workers={sorted(pids)} parents={sorted(parents)}")
for pid in kill_set:
    try:
