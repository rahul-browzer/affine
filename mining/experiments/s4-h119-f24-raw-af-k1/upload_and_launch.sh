#!/usr/bin/env bash
# Host → mine-f24-1: upload lean stack (no train), start raw-af-k1 bootstrap.
set -euo pipefail

ROOT=/home/const/subnet120
DST_HOST=${DST_HOST:?set DST_HOST}
DST_PORT=${DST_PORT:?set DST_PORT}
KNOWN=${KNOWN:-/tmp/mine-f24-1.known_hosts}
...
STAGE=$(mktemp -d /tmp/mine-h119-stack.XXXXXX)
trap 'rm -rf "$STAGE"' EXIT

EXP=s4-h119-f24-raw-af-k1
...
TAR=/tmp/mine-h119-stack.tar.gz
tar -C "$STAGE" -czf "$TAR" .
ls -lh "$TAR"

ENV_TMP=$(mktemp /tmp/mine-h119.env.XXXXXX)
...
"${SSH[@]}" 'mkdir -p /root/mining_src /root/affine_data /root/logs /root/h119 /root/hf'
"${SCP[@]}" "$TAR" "root@${DST_HOST}:/tmp/mine-h119-stack.tar.gz"
"${SCP[@]}" "$ENV_TMP" "root@${DST_HOST}:/root/mine.env"
rm -f "$ENV_TMP"

"${SSH[@]}" 'set -e
  tar -C /root/mining_src -xzf /tmp/mine-h119-stack.tar.gz
  chmod 600 /root/mine.env
  chmod +x /root/mining_src/s3-duel-sim/*.sh \
           /root/mining_src/s4-h2-merge/*.sh \
           /root/mining_src/s4-h119-f24-raw-af-k1/*.sh
  test -f /root/mining_src/affine_pkg/affine/score.py
  test -x /root/mining_src/s4-h119-f24-raw-af-k1/bootstrap_h119.sh
  echo STACK_UPLOAD_OK
  nohup bash /root/mining_src/s4-h119-f24-raw-af-k1/bootstrap_h119.sh \
    >/root/logs/h119_pipeline.nohup 2>&1 &
  echo $! > /root/logs/h119_pipeline.pid
  nohup bash /root/mining_src/s4-h2-merge/watch_form_decision.sh h119 \
    /root/affine_data/h119_sim_result.json /root/affine_data/h119_decision.json \
    /root/logs/h119_form_decision.nohup \
    >/root/logs/h119_form_decision.launch.out 2>&1 &
  echo $! > /root/logs/h119_form_decision.pid
  nohup bash /root/mining_src/s4-h2-merge/watch_n80_retry.sh h119 \
    /root/mining_src/s4-h119-f24-raw-af-k1/retry_h119_n80_d203first.sh \
    >/root/logs/h119_watch_retry.launch.nohup 2>&1 &
  echo $! > /root/logs/h119_watch_retry.pid
  echo PIPELINE_PID=$(cat /root/logs/h119_pipeline.pid)
  sleep 3
  head -n 40 /root/logs/bootstrap_h119.log 2>/dev/null || head -n 40 /root/logs/h119_pipeline.nohup || true
  ps -p "$(cat /root/logs/h119_pipeline.pid)" -o pid,etime,cmd || true
  # COUNT=8 smoke
  nvidia-smi -L | wc -l
'
