#!/usr/bin/env bash
# mine-f24-1: raw af_k1 chall (no train) vs Tok king — F24/H119 screen.
set -euo pipefail

LOG=/root/logs/bootstrap_h119.log
mkdir -p /root/logs /root/hf /root/affine_data /root/mining_src /root/h119
exec > >(tee -a "$LOG") 2>&1

echo "[bootstrap-h119] $(date -u +%Y-%m-%dT%H:%M:%SZ) start host=$(hostname)"
...
if [[ -z "${HF_TOKEN:-}" ]]; then
  echo "[bootstrap-h119] FATAL: HF_TOKEN missing in /root/mine.env"
  exit 1
fi

nvidia-smi -L || true
NGPU=$(nvidia-smi -L | wc -l)
echo "[bootstrap-h119] GPU_COUNT=$NGPU"
test "$NGPU" -ge 8
...
uv pip install \
  ...
  "numpy" \
  "scipy" \
  2>&1 | tee /root/logs/pip_h119.log | tail -40

python - <<'PY'
import torch, transformers, vllm
print("[bootstrap-h119] VERSIONS",
...
if p.is_file():
    ...
    if txt != orig:
        p.write_text(txt)
        print("[bootstrap-h119] patched vllm_client timeout=480 retries=5", flush=True)
    else:
        print("[bootstrap-h119] vllm_client already patched or pattern miss", flush=True)
PY

# Download chall=af_k1, king=Tok, teacher — then serve + stamp chall ready.
python - <<'PY'
import os
from pathlib import Path
...
print("[bootstrap-h119] DOWNLOAD af_k1 chall start", flush=True)
gpath = snapshot_download(
    "af-k1/Affine-5ECeJJpEMjW4pxM9eGyJ5ua3Sebfyr8kcVwLAdaiJLUC8pkW",
    revision="ff6eb4bcff3e7c6b8c0e097bc0cffa4fa2ba8e01",
    token=token,
)
print(f"[bootstrap-h119] DOWNLOAD af_k1 done -> {gpath}", flush=True)
chall = Path("/root/h119/chall")
...
Path("/root/logs/af_k1_chall.done").write_text(gpath + "\n")
# retry script expects merge.done for legacy gate — stamp it (no merge occurred)
Path("/root/logs/h119_merge.done").write_text(
    "RAW_AF_K1_NO_MERGE " + gpath + "\n"
)
print("[bootstrap-h119] DOWNLOAD teacher start", flush=True)
tpath = snapshot_download("zai-org/GLM-4.5-Air-FP8", token=token)
print(f"[bootstrap-h119] DOWNLOAD teacher done -> {tpath}", flush=True)
Path("/root/logs/teacher.done").write_text(tpath + "\n")
print("[bootstrap-h119] DOWNLOAD tok331102 king start", flush=True)
kpath = snapshot_download(
    "Tok331102/affine-5EqYW8McUc-af10",
    revision="eb8bf9a356a254f71faaa439e8abc3cfba572c53",
    token=token,
)
print(f"[bootstrap-h119] DOWNLOAD tok331102 done -> {kpath}", flush=True)
Path("/root/logs/tok331102.done").write_text(kpath + "\n")
print("[bootstrap-h119] ALL_DOWNLOADS_OK", flush=True)
PY

bash /root/mining_src/s3-duel-sim/sync_corpus.sh || true

echo "[bootstrap-h119] $(date -u +%Y-%m-%dT%H:%M:%SZ) serving teacher+Tok+af_k1"
...
export CHALL_REPO=/root/h119/chall
export CHALL_REV=
...
for i in $(seq 1 240); do
  ...
  if [[ "$code" == "200" ]]; then
    echo "chall_serve_ok poll=$i $(date -u +%Y-%m-%dT%H:%M:%SZ)" \
      | tee /root/logs/h119_chall_serve.done
    break
  fi
  sleep 15
done
test -f /root/logs/h119_chall_serve.done

echo "[bootstrap-h119] $(date -u +%Y-%m-%dT%H:%M:%SZ) DONE — engines up; n80 watcher owns screen"
touch /root/logs/bootstrap_h119.done
