#!/usr/bin/env bash
# H36: Radiant28/m7-init thought-only LoRA on king-self high clip-L1 z_A (686→fit-filter).
set -euo pipefail
...
n=$(wc -l <"$DATA")
echo "[h36] $(date -u +%Y-%m-%dT%H:%M:%SZ) examples=$n data=$DATA base=$BASE gpus=$CUDA_VISIBLE_DEVICES"
test "$n" -ge 300

echo "[h36] verify thought cuts (CPU)"
...
echo "[h36] $(date -u +%Y-%m-%dT%H:%M:%SZ) launch thought-only train lr=1e-5 on GPUs $CUDA_VISIBLE_DEVICES"
nohup python3 "$SRC/train_lora.py" \
  ...
  --loss-on thought \
  >"$LOG" 2>&1 &
echo $! | tee /root/logs/h36_train.pid >"$OUT/train.pid"
python3 - <<PY
import json, time
...
meta = {
    ...
    "hypo": "H36",
    "pid": int(Path("/root/logs/h36_train.pid").read_text().strip()),
    "base": "$BASE",
...
PY
echo "[h36] TRAIN_LAUNCHED pid=$(cat /root/logs/h36_train.pid)"
