#!/usr/bin/env bash
# Pass 314: king-only relaunch after load-time Triton ENOENT on bare cache/king.
# Uses isolated TRITON_CACHE_DIR + early abort on launcher.so ENOENT (don't burn 180 polls).
...
LOGN=/root/logs/h80_king_recover_pass314.nohup
log() { echo "[king314-h80] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOGN"; }

log "START king-only recover (p314 isolated TCACHE; prior bare cache/king ENOENT NODUTTS4 @13:25:32Z; leave chall)"
...
for line in apps.strip().splitlines():
    ...
    if len(parts) >= 2 and parts[0] in uuids:
        try:
            ...
            pass
print(f"[king314-h80] reap gpus2,3 pids={sorted(pids)}")
for pid in pids:
    try:
...
log "cleared stale h80 sim progress/result"
...
TCACHE="/root/.triton/isolated/h80_king_p314_$(date +%s)_$$"
mkdir -p "$TCACHE"
echo "$TCACHE" >/root/logs/h80_king_tcache_pass314.path
...
for line in out.strip().splitlines():
    ...
    if int(idx) in (2, 3):
        print(f"[king314-h80] gpu{idx} used_mib={used}")
        if float(used) < 500:
            ok += 1
if ok < 2:
    print("[king314-h80] ABORT: gpus 2,3 not free")
    sys.exit(1)
PY
...
for i in $(seq 1 180); do
  ...
  if [[ "$code" == "200" ]]; then
    ...
    if [[ -n "$mid" ]]; then
      pcode=$(curl -s -o /tmp/_probe_king314.json -w "%{http_code}" --max-time 90 \
        http://127.0.0.1:8001/v1/completions \
        -H 'Content-Type: application/json' \
        -d "{\"model\":\"${mid}\",\"prompt\":\"hi\",\"max_tokens\":2}" || true)
      if [[ "$pcode" == "200" ]]; then
        log "KING PROMPTABLE mid=$mid poll=$i TCACHE=$TCACHE"
        date -u +%Y-%m-%dT%H:%M:%SZ >/root/logs/h80_king_recover_pass314.done
        echo "$REPO $REV $TCACHE" >>/root/logs/h80_king_recover_pass314.done
