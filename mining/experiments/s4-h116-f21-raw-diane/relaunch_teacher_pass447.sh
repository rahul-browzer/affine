#!/usr/bin/env bash
# Pass 447: H116 teacher dead — Triton ENOENT on bare cache/teacher
...
LOG=/root/logs/${HYPO}_teacher_recover_pass447.log
TEACHER_LOG=/root/logs/vllm_teacher.log
PIDF=/root/logs/vllm_teacher.pid
export GPUS=0,1
UTIL=0.72
TEACHER_REPO=zai-org/GLM-4.5-Air-FP8
TAG=teacher_p447_$(date +%s)_$$
mkdir -p /root/logs
: >"$LOG"
log() { echo "[recover447-${HYPO}] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }
...
print(f"[recover447] reap gpu={sorted(want)} workers={sorted(pids)} parents={sorted(parents)} grandparents={sorted(grandparents)}")
for pid in kill_set:
