#!/usr/bin/env bash
# Pass 207: H38 chall died again on __triton_launcher.so (brief health→500→
...
for f in h38_decision.json h38_sim_result.json h38_sim_result_artifact.json \
  ...
  if [[ -f "/root/affine_data/$f" ]]; then
    mv "/root/affine_data/$f" "$QDIR/${f%.json}_pass207_${ts}.json"
...
print(f"[recover207-h38-chall] reap gpu={sorted(want)} workers={sorted(pids)} parents={sorted(parents)}")
for pid in kill_set:
    try:
