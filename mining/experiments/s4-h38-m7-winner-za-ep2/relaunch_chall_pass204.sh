#!/usr/bin/env bash
# Pass 204: H38 chall EngineCore init failed (__triton_launcher.so missing).
# Wipe→settle→unique TCACHE; post_train already killed; watcher owns n80.
# Completions-gated retry prevents health-only false start.
set -euo pipefail
...
LOG=/root/logs/h38_chall_recover_pass204.log
...
log() { echo "[recover204-h38-chall] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }
...
print(f"[recover204-h38-chall] reap gpu={sorted(want)} workers={sorted(pids)} parents={sorted(parents)}")
for pid in kill_set:
    try:
...
rm -f /root/logs/h38_pipeline.aborted /root/logs/h38_n80_retry.aborted \
  /root/logs/h38_n80.done /root/logs/h38_chall_serve.done

if ! ps -eo pid,cmd | awk '/[w]atch_n80_retry\.sh h38 / {found=1} END{exit !found}'; then
