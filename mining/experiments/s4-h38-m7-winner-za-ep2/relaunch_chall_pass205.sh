#!/usr/bin/env bash
# Pass 205: H38 chall died again on __triton_launcher.so (brief health→500→
# false_probe ConnectError). Quarantine decision, wipe→settle20s→unique TCACHE,
# rearm watcher. Completions-gated retry + double-probe in retry_h38_n80.sh.
set -euo pipefail
...
LOG=/root/logs/h38_chall_recover_pass205.log
CHALL_LOG=/root/logs/vllm_chall.log
PIDF=/root/logs/vllm_chall.pid
MERGE=/root/h38/merged
export GPUS=4,5
UTIL=0.72
TAG=chall_p205_$(date +%s)_$$
TCACHE=/root/.triton/cache/$TAG
QDIR=/root/affine_data/false_probes
mkdir -p /root/logs /root/affine_data "$QDIR"
: >"$LOG"

log() { echo "[recover205-h38-chall] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }

log "START quarantine false ConnectError REFUTE + chall relaunch"

ts=$(date -u +%Y%m%dT%H%M%SZ)
for f in h38_decision.json h38_sim_result.json h38_sim_result_artifact.json \
         h38_sim_progress.json; do
  if [[ -f "/root/affine_data/$f" ]]; then
    mv "/root/affine_data/$f" "$QDIR/${f%.json}_pass205_${ts}.json"
    log "quarantine $f → $QDIR"
  fi
done
rm -f /root/logs/h38_n80.done /root/logs/h38_n80_retry.aborted \
  /root/logs/h38_pipeline.aborted /root/logs/h38_chall_serve.done
...
while read -r pid; do
  ...
  /[r]un_sim_duel\.py/ && /local-h38/ {print $1}
')
...
print(f"[recover205-h38-chall] reap gpu={sorted(want)} workers={sorted(pids)} parents={sorted(parents)}")
for pid in kill_set:
    try:
...
# Always rearm watcher (exits on any decision, incl. false_probe).
pkill -f 'watch_n80_retry\.sh h38 ' 2>/dev/null || true
sleep 1
...
log "rearmed watcher pid=$(cat /root/logs/h38_watch_retry.pid)"
log "DONE_LAUNCH (n80 waits for health+completions probe)"
