#!/usr/bin/env bash
# Pass 248: H54 king :8001 down before/at n80 start (GPUs 2,3 free). Wipe →
# settle → unique TCACHE. Teacher :8000 + chall :8002 stay; kill doomed sim
# so watch_n80_retry can restart once king is promptable.
set -euo pipefail
...
LOG=/root/logs/h54_king_recover_pass248.log
...
log() { echo "[recover248-h54] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }
...
print(f"[recover248-h54] reap gpu={sorted(want)} workers={sorted(pids)} parents={sorted(parents)}")
for pid in kill_set:
    try:
