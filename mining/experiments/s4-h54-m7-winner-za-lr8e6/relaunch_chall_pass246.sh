#!/usr/bin/env bash
# Pass 246: H54 post-merge first warmup still raced at warmup #1 — RuntimeError ENOENT on
# __triton_launcher.so despite 45s post-health settle and n_pre=16.
# Upgrade: seed TCACHE from live king cache + PRE-FREEZE (chmod a-w) before
# w1 when launcher.so already present from profile_run / seed. Outer×3 kept.
set -euo pipefail
...
LOG=/root/logs/h54_chall_recover_pass246.log
CHALL_LOG=/root/logs/vllm_chall.log
PIDF=/root/logs/vllm_chall.pid
MERGE=/root/h54/merged
export GPUS=4,5
UTIL=0.72
...
log() { echo "[recover246-h54-chall] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }

log "START chall relaunch (outer×3 + king-seed + pre-freeze before w1; H54 post-merge first warmup_w1 ENOENT)"

ts=$(date -u +%Y%m%dT%H%M%SZ)
for f in h54_decision.json h54_sim_result.json h54_sim_result_artifact.json \
         h54_sim_progress.json; do
  if [[ -f "/root/affine_data/$f" ]]; then
    mv "/root/affine_data/$f" "$QDIR/${f%.json}_pass246_${ts}.json"
    log "quarantine $f → $QDIR"
  fi
done
rm -f /root/logs/h54_n80.done /root/logs/h54_n80_retry.aborted \
  /root/logs/h54_pipeline.aborted /root/logs/h54_chall_serve.done \
  /root/logs/h54_sim_n80.done /root/logs/h54_chall_freeze_pass237.done \
  /root/logs/h54_chall_freeze_pass239.done \
  /root/logs/h54_chall_freeze_pass240.done
...
while read -r pid; do
  ...
  /[r]un_sim_duel\.py/ && /local-h54/ {print $1}
  /[r]etry_h54_n80\.sh/ {print $1}
')
...
print(f"[recover246-h54-chall] reap gpu={sorted(want)} workers={sorted(pids)} parents={sorted(parents)} grandparents={sorted(grandparents)}")
for pid in kill_set:
    try:
...
wipe_caches() {
  ...
  chmod -R u+w /root/.triton/isolated/h54_chall_* 2>/dev/null || true
  rm -rf /root/.triton/cache/chall /root/.triton/cache/chall_* || true
  rm -rf /root/.triton/isolated/h54_chall_* || true
  rm -rf /root/.cache/flashinfer/cached_ops/sampling || true
  rm -rf /root/.cache/vllm/torch_compile_cache || true
  rm -rf /tmp/torchinductor_* /root/.cache/torch/inductor /root/.cache/torchinductor_chall_* /root/.cache/torchinductor_h54_* || true
...
_comp() {
  ...
  code=$(curl -s -o "/tmp/h54_warmup_${label}.json" -w "%{http_code}" --max-time 120 \
...
for attempt in 1 2 3; do
  ...
  wipe_caches
  TAG=h54_chall_p246_a${attempt}_$(date +%s)_$$
  TCACHE=/root/.triton/isolated/$TAG
  mkdir -p "$TCACHE" /root/.cache/torchinductor_$TAG
  # Seed from live king (same Qwen3.5-MoE arch) so launcher.so exist before
  # any TP race on first completion. Do NOT touch king's live cache dir.
  if [[ -d /root/.triton/cache/king ]]; then
    n_king=$(find /root/.triton/cache/king -name '__triton_launcher*.so' 2>/dev/null | wc -l)
    log "seed TCACHE from /root/.triton/cache/king (launcher.so=$n_king)"
  ...
  if ! _comp "a${attempt}_w1"; then
    log "attempt $attempt: warmup #1 failed — chall up? pid=$(kill -0 $CHALL_PID 2>/dev/null && echo yes || echo no) pre_froze=$pre_froze"
...
while read -r pid; do
  ...
  sleep 1
  kill -9 "$pid" 2>/dev/null || true
done < <(ps -eo pid,args | awk '/[w]atch_n80_retry\.sh/ && / h54 / {print $1}')
while read -r pid; do
  [[ -n "${pid:-}" ]] || continue
  ...
  sleep 1
  kill -9 "$pid" 2>/dev/null || true
done < <(ps -eo pid,args | awk '/[r]etry_h54_n80\.sh/ {print $1}')
while read -r pid; do
  [[ -n "${pid:-}" ]] || continue
  ...
  sleep 1
  kill -9 "$pid" 2>/dev/null || true
done < <(ps -eo pid,args | awk '/[w]atch_form_decision\.sh/ && / h54 / {print $1}')
sleep 1

nohup bash /root/mining_src/s4-h2-merge/watch_form_decision.sh h54 \
  /root/affine_data/h54_sim_result.json \
  /root/affine_data/h54_decision.json \
  /root/logs/h54_form_decision.nohup \
  >/root/logs/h54_form_decision.launch.nohup 2>&1 &
echo $! >/root/logs/h54_form_decision.pid
log "rearmed form pid=$(cat /root/logs/h54_form_decision.pid)"

nohup bash /root/mining_src/s4-h2-merge/watch_n80_retry.sh h54 \
  /root/mining_src/s4-h54-m7-winner-za-lr8e6/retry_h54_n80.sh \
  >/root/logs/h54_watch_retry.launch.nohup 2>&1 &
echo $! >/root/logs/h54_watch_retry.pid
log "rearmed watcher pid=$(cat /root/logs/h54_watch_retry.pid)"
date -u +%Y-%m-%dT%H:%M:%SZ > /root/logs/h54_chall_serve.done
echo "TCACHE=$TCACHE mode=$(stat -c %a $TCACHE) probe=200 attempts_ok prefreeze=1 king_seed=1" \
  > /root/logs/h54_chall_freeze_pass246.done
