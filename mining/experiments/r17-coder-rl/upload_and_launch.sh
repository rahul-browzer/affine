#!/usr/bin/env bash
# Host → mine-r17-coder-rl-1: H135 RL stack + R17 coder-REINFORCE overlays, bootstrap.
# Axis R17: Qwen3-Coder REINFORCE on teacher Reason (≠ R14–R16 Albedo, ≠ R3/R8 Tok RL).
set -euo pipefail

ROOT=/home/const/subnet120
POD_NAME=${POD_NAME:-mine-r17-coder-rl-1}
...
STAGE=$(mktemp -d /tmp/mine-r17-stack.XXXXXX)
...
cp -a "$ROOT/mining/experiments/r17-coder-rl/start_r17.sh" "$STAGE/r17-coder-rl/"
cp -a "$ROOT/mining/experiments/r17-coder-rl/bootstrap_r17.sh" "$STAGE/r17-coder-rl/"
# Overlay: H135 bootstrap + start → R17 Reason coder-REINFORCE.
...
"${SCP[@]}" "$TAR" "root@${DST_HOST}:/tmp/mine-r17-stack.tar.gz"
...
"${SSH[@]}" 'set -e
  tar -C /root/mining_src -xzf /tmp/mine-r17-stack.tar.gz
  ...
  # Prove overlay is R17, not stock H135 / R14–R16.
  grep -q "R17: coder-REINFORCE" /root/mining_src/s4-h135-f40-kevin-rl-l2/start_h135.sh
  grep -q "DOWNLOAD coder-init" /root/mining_src/s4-h135-f40-kevin-rl-l2/bootstrap_h135.sh
  set -a; source /root/mine.env; set +a
  echo "R17_DEADLINES soft=$SOFT_DEADLINE_UTC dead=$DEADMAN_UTC axis=$R17_AXIS"
  echo STACK_UPLOAD_OK
  nohup bash /root/mining_src/s4-h135-f40-kevin-rl-l2/bootstrap_h135.sh \
    >/root/logs/r17_pipeline.nohup 2>&1 &
  echo $! > /root/logs/r17_pipeline.pid
  cp -f /root/logs/r17_pipeline.pid /root/logs/h135_pipeline.pid
  ...
  echo PIPELINE_PID=$(cat /root/logs/r17_pipeline.pid)
  sleep 5
  head -n 40 /root/logs/bootstrap_h135.log 2>/dev/null || head -n 40 /root/logs/r17_pipeline.nohup || true
  ps -p "$(cat /root/logs/r17_pipeline.pid)" -o pid,etime,cmd || true
  nvidia-smi -L | wc -l
'
