#!/usr/bin/env bash
# Host → lean warm-arm R17 onto TKC-hot mine-crown-1 after R16 REFUTE.
# Keeps teacher:8000 + ckp333:8001; trains Qwen3-Coder REINFORCE on GPUs 6–7.
set -euo pipefail
...
STAGE=$(mktemp -d /tmp/mine-r17-warm.XXXXXX)
...
mkdir -p "$STAGE/affine_pkg/affine" "$STAGE/affine_pkg/evalsrv" \
         ...
         "$STAGE/s4-h1v2-sft" "$STAGE/$EXP" "$STAGE/r17-coder-rl"
...
cp -a "$ROOT/mining/experiments/r17-coder-rl/"*.sh "$STAGE/r17-coder-rl/"
cp -a "$ROOT/mining/experiments/r17-coder-rl/plan.md" "$STAGE/r17-coder-rl/"
# Overlay start/bootstrap for R17 identity grep.
cp -a "$ROOT/mining/experiments/r17-coder-rl/start_r17.sh" "$STAGE/$EXP/start_h135.sh"
cp -a "$ROOT/mining/experiments/r17-coder-rl/bootstrap_r17.sh" "$STAGE/$EXP/bootstrap_h135.sh"
...
TAR=/tmp/mine-r17-warm-stack.tar.gz
tar -C "$STAGE" -czf "$TAR" .
ls -lh "$TAR"
...
set +a
umask 077
ENV_TMP=$(mktemp /tmp/mine-r17.env.XXXXXX)
{
  ...
  echo "export HF_MERGED_REPO=unconst/Affine-5czsc2fc98-r17-coder-rl"
  echo "export R17_AXIS=coder_reinforce_reason"
  echo "export R17_LR=5e-6"
  echo "export R17_LORA_R=16"
  echo "export R17_LORA_ALPHA=32"
  echo "export R17_GROUP_SIZE=2"
  echo "export R17_MAX_STEPS=200"
  echo "export R17_MAX_NEW=256"
...
"${SSH[@]}" 'mkdir -p /root/mining_src /root/affine_data /root/logs /root/h135 /root/r17 /root/hf'
"${SCP[@]}" "$TAR" "root@${DST_HOST}:/tmp/mine-r17-warm-stack.tar.gz"
"${SCP[@]}" "$ENV_TMP" "root@${DST_HOST}:/root/mine.env"
"${SCP[@]}" "$DATA" "root@${DST_HOST}:/root/h135/winner_za_high_l1.jsonl"
"${SCP[@]}" "$DATA" "root@${DST_HOST}:/root/r17/winner_za_high_l1.jsonl"
rm -f "$ENV_TMP"
...
for repo in (
    "unconst/Affine-5czsc2fc98-r17-coder-rl",
    "unconst/Affine-5czsc2fc98-r17-coder-rl-lora",
):
    try:
...
"${SSH[@]}" 'set -euo pipefail
  tar -C /root/mining_src -xzf /tmp/mine-r17-warm-stack.tar.gz
  chmod 600 /root/mine.env
  chmod +x /root/mining_src/s4-h135-f40-kevin-rl-l2/*.sh \
           /root/mining_src/r17-coder-rl/*.sh \
  ...
  # Stop leftover R16 lean/post/form by pidfile only (never pkill -f).
  for f in /root/logs/r16_lean_warm.pid /root/logs/r16_post_train.pid \
           /root/logs/r16_form_decision.pid /root/logs/r16_train.pid \
           /root/logs/h135_train.pid /root/logs/h135_post_train.pid \
           /root/logs/h135_pipeline.pid /root/logs/r16_n80.pid; do
    if [[ -f "$f" ]]; then
  ...
  nohup bash /root/mining_src/r17-coder-rl/lean_warm_boot.sh \
    >/root/logs/r17_lean_warm.nohup 2>&1 &
  echo $! >/root/logs/r17_lean_warm.pid
  sleep 20
  echo LEAN_PID=$(cat /root/logs/r17_lean_warm.pid)
  head -n 80 /root/logs/r17_lean_warm.log 2>/dev/null \
    || head -n 80 /root/logs/r17_lean_warm.nohup || true
  ps -p "$(cat /root/logs/r17_lean_warm.pid)" -o pid,etime,cmd || true
  test -f /root/logs/h135_train.pid && echo TRAIN_PID=$(cat /root/logs/h135_train.pid) || echo TRAIN_PENDING
  test -f /root/logs/r17_post_train.pid && echo POST_PID=$(cat /root/logs/r17_post_train.pid) || true
...
echo "R17_WARM_ARM_OK pod=$POD_NAME soft=$SOFT dead=$DEAD $(date -u +%Y-%m-%dT%H:%M:%SZ)"
