#!/usr/bin/env bash
# Pass 183: H28 king :8001 died mid-n80 (500s then shutdown; ConnectError).
# Wipe caches, unique TRITON_CACHE_DIR, relaunch king on GPUs 2,3; probe; retry n80.
set -euo pipefail
...
log() { echo "[recover183] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }

log "START king relaunch TCACHE=$TCACHE"

# Quarantine any false decision / stale progress
mkdir -p /root/affine_data/false_probes
for f in h28_decision.json h28_sim_result.json h28_sim_progress.json; do
  if [[ -f "/root/affine_data/$f" ]]; then
    mv "/root/affine_data/$f" "/root/affine_data/false_probes/${f%.json}_pass183_$(date -u +%H%M%S).json"
    log "quarantined $f"
  fi
done
...
# Reap orphans on GPUs 2,3 by index→uuid→pid
python - <<'PY' | tee -a "$LOG"
...
print(f"[recover183] reap gpu={sorted(want)} workers={sorted(pids)} parents={sorted(parents)}")
for pid in kill_set:
    try:
...
# Background wait+probe+retry n80 (do not embed retry path in a way that self-kills)
nohup bash /root/mining_src/s4-h28-m7-clip-l1-shape/recover_wait_n80_pass183.sh \
  >/root/logs/h28_recover_wait183.nohup 2>&1 &
echo $! >/root/logs/h28_recover_wait.pid
log "wait_pid=$(cat /root/logs/h28_recover_wait.pid)"
log "DONE_LAUNCH"
