#!/usr/bin/env bash
# Host → mine-h28-1: upload stack + data, start bootstrap under nohup.
set -euo pipefail

ROOT=/home/const/subnet120
DST_HOST=${DST_HOST:?set DST_HOST}
DST_PORT=${DST_PORT:?set DST_PORT}
KNOWN=${KNOWN:-/tmp/mine-h28-1.known_hosts}
...
STAGE=$(mktemp -d /tmp/mine-h28-stack.XXXXXX)
trap 'rm -rf "$STAGE"' EXIT

EXP=s4-h28-m7-clip-l1-shape
...
TAR=/tmp/mine-h28-stack.tar.gz
tar -C "$STAGE" -czf "$TAR" .
ls -lh "$TAR"

ENV_TMP=$(mktemp /tmp/mine-h28.env.XXXXXX)
...
# Reuse H27 harvest (same 406-ex winner-zA set; H28 differs by m7 init).
DATA="$ROOT/mining/experiments/s4-h27-clip-l1-shape/results/winner_za_high_l1.jsonl"
test -s "$DATA"

"${SSH[@]}" 'mkdir -p /root/mining_src /root/affine_data /root/logs /root/h28 /root/hf'
"${SCP[@]}" "$TAR" "root@${DST_HOST}:/tmp/mine-h28-stack.tar.gz"
"${SCP[@]}" "$ENV_TMP" "root@${DST_HOST}:/root/mine.env"
"${SCP[@]}" "$DATA" "root@${DST_HOST}:/root/h28/winner_za_high_l1.jsonl"
rm -f "$ENV_TMP"
...
for repo in (
    "unconst/Affine-5czsc2fc98-h28-lora",
    "unconst/Affine-5czsc2fc98-h28-merged",
):
    try:
...
"${SSH[@]}" 'set -e
  tar -C /root/mining_src -xzf /tmp/mine-h28-stack.tar.gz
  chmod 600 /root/mine.env
  chmod +x /root/mining_src/s3-duel-sim/*.sh \
           /root/mining_src/s4-h2-merge/*.sh \
           /root/mining_src/s4-h28-m7-clip-l1-shape/*.sh
  test -f /root/mining_src/affine_pkg/affine/score.py
  test -s /root/h28/winner_za_high_l1.jsonl
  test -x /root/mining_src/s4-h28-m7-clip-l1-shape/bootstrap_h28.sh
  echo STACK_UPLOAD_OK
  nohup bash /root/mining_src/s4-h28-m7-clip-l1-shape/bootstrap_h28.sh \
    >/root/logs/h28_pipeline.nohup 2>&1 &
  echo $! > /root/logs/h28_pipeline.pid
  nohup bash /root/mining_src/s4-h2-merge/watch_form_decision.sh h28 \
    >/root/logs/h28_form_decision.nohup 2>&1 &
  echo $! > /root/logs/h28_form_decision.pid
  echo PIPELINE_PID=$(cat /root/logs/h28_pipeline.pid)
  sleep 3
  head -n 30 /root/logs/bootstrap_h28.log 2>/dev/null || head -n 30 /root/logs/h28_pipeline.nohup || true
  ps -p "$(cat /root/logs/h28_pipeline.pid)" -o pid,etime,cmd || true
'
