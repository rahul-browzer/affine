#!/usr/bin/env bash
# Host → mine-h10-1: upload harness + mine.env, start bootstrap+n80 under nohup.
set -euo pipefail

ROOT=/home/const/subnet120
DST_HOST=${DST_HOST:?set DST_HOST}
DST_PORT=${DST_PORT:?set DST_PORT}
KNOWN=${KNOWN:-/tmp/mine-h10-1.known_hosts}
...
STAGE=$(mktemp -d /tmp/mine-h10-stack.XXXXXX)
trap 'rm -rf "$STAGE"' EXIT

mkdir -p "$STAGE/affine_pkg/affine" "$STAGE/affine_pkg/evalsrv" \
         "$STAGE/s3-duel-sim" "$STAGE/s4-h2-merge" "$STAGE/s4-h10-tp-kevin-merge"
...
cp -a "$ROOT/mining/experiments/s4-h10-tp-kevin-merge/bootstrap_h10.sh" "$STAGE/s4-h10-tp-kevin-merge/"
cp -a "$ROOT/mining/experiments/s4-h10-tp-kevin-merge/start_h10_n80.sh" "$STAGE/s4-h10-tp-kevin-merge/"
cp -a "$ROOT/mining/experiments/s4-h10-tp-kevin-merge/plan.md" "$STAGE/s4-h10-tp-kevin-merge/"

TAR=/tmp/mine-h10-stack.tar.gz
tar -C "$STAGE" -czf "$TAR" .
ls -lh "$TAR"

ENV_TMP=$(mktemp /tmp/mine-h10.env.XXXXXX)
...
"${SCP[@]}" "$TAR" "root@${DST_HOST}:/tmp/mine-h10-stack.tar.gz"
"${SCP[@]}" "$ENV_TMP" "root@${DST_HOST}:/root/mine.env"
rm -f "$ENV_TMP"

"${SSH[@]}" 'set -e
  tar -C /root/mining_src -xzf /tmp/mine-h10-stack.tar.gz
  chmod 600 /root/mine.env
  chmod +x /root/mining_src/s3-duel-sim/*.sh \
           /root/mining_src/s4-h10-tp-kevin-merge/*.sh
  test -f /root/mining_src/affine_pkg/affine/score.py
  test -x /root/mining_src/s4-h10-tp-kevin-merge/bootstrap_h10.sh
  test -x /root/mining_src/s4-h10-tp-kevin-merge/start_h10_n80.sh
  echo STACK_UPLOAD_OK
  nohup bash -lc "
    set -euo pipefail
    bash /root/mining_src/s4-h10-tp-kevin-merge/bootstrap_h10.sh
    bash /root/mining_src/s4-h10-tp-kevin-merge/start_h10_n80.sh
  " >/root/logs/h10_pipeline.nohup 2>&1 &
  echo $! > /root/logs/h10_pipeline.pid
  echo PIPELINE_PID=$(cat /root/logs/h10_pipeline.pid)
  sleep 2
  head -n 20 /root/logs/h10_pipeline.nohup || true
  ps -p "$(cat /root/logs/h10_pipeline.pid)" -o pid,etime,cmd || true
'
