#!/usr/bin/env bash
# R11: Tok-init → online-DPO on live teacher Reason.
# Overlay: upload_and_launch copies this to s4-h139-f44-tok-online-dpo-l2/bootstrap_h139.sh.
# Order: pip → DL tok+teacher → serve teacher → train (6,7) → post_train.
set -euo pipefail

LOG=/root/logs/bootstrap_h139.log
mkdir -p /root/logs /root/hf /root/affine_data /root/mining_src /root/h139 /root/r11
exec > >(tee -a "$LOG") 2>&1

echo "[bootstrap-r11] $(date -u +%Y-%m-%dT%H:%M:%SZ) start host=$(hostname)"
...
if [[ -z "${HF_TOKEN:-}" ]]; then
  echo "[bootstrap-r11] FATAL: HF_TOKEN missing in /root/mine.env"
  exit 1
fi

nvidia-smi -L || true
NGPU=$(nvidia-smi -L | wc -l)
echo "[bootstrap-r11] GPU_COUNT=$NGPU"
test "$NGPU" -ge 8
...
test -x /root/mining_src/s4-h139-f44-tok-online-dpo-l2/post_train_pipeline.sh
# Prove overlay is R11, not stock H139 / F44.
...
uv pip install \
  ...
  "pandas" \
  "pyarrow" \
  2>&1 | tee /root/logs/pip_h139.log | tail -40

python - <<'PY'
import torch, transformers, vllm, peft, accelerate, httpx
print("[bootstrap-r11] VERSIONS",
...
if p.is_file():
    ...
    if txt != orig:
        p.write_text(txt)
        print("[bootstrap-r11] patched vllm_client timeout=600 retries=5", flush=True)
    else:
        print("[bootstrap-r11] vllm_client already patched or pattern miss", flush=True)
PY
...
print("[bootstrap-r11] DOWNLOAD tok-init start", repo, rev, flush=True)
path = snapshot_download(repo, revision=rev, token=token)
print(f"[bootstrap-r11] DOWNLOAD tok-init done -> {path}", flush=True)
open("/root/logs/tok_init.done", "w").write(path + "\n")
open("/root/logs/tok331102.done", "w").write(path + "\n")
print("[bootstrap-r11] DOWNLOAD teacher start", flush=True)
tpath = snapshot_download("zai-org/GLM-4.5-Air-FP8", token=token)
print(f"[bootstrap-r11] DOWNLOAD teacher done -> {tpath}", flush=True)
open("/root/logs/teacher.done", "w").write(tpath + "\n")
PY

echo "[bootstrap-r11] $(date -u +%Y-%m-%dT%H:%M:%SZ) serving teacher before train"
bash /root/mining_src/s4-h139-f44-tok-online-dpo-l2/prewarm_engines.sh \
  >/root/logs/h139_prewarm.nohup 2>&1 &
echo $! >/root/logs/h139_prewarm.pid

for i in $(seq 1 240); do
  if curl -sf --max-time 5 http://127.0.0.1:8000/v1/models >/dev/null 2>&1; then
    echo "[bootstrap-r11] teacher :8000 up at iter=$i"
    break
  fi
  if (( i == 240 )); then
    echo "[bootstrap-r11] FATAL teacher never up"
    tail -80 /root/logs/h139_prewarm.nohup || true
...
echo "[bootstrap-r11] $(date -u +%Y-%m-%dT%H:%M:%SZ) launching R11 online-DPO train"
bash /root/mining_src/s4-h139-f44-tok-online-dpo-l2/start_h139.sh
touch /root/logs/h139_train_launched.stamp
...
' >/root/logs/h139_extra_dl.nohup 2>&1 &
echo $! >/root/logs/h139_extra_dl.pid

nohup bash /root/mining_src/s4-h139-f44-tok-online-dpo-l2/post_train_pipeline.sh \
  >/root/logs/h139_post_train.nohup 2>&1 &
echo $! >/root/logs/h139_post_train.pid
cp -f /root/logs/h139_post_train.pid /root/logs/r11_post_train.pid

echo "[bootstrap-r11] $(date -u +%Y-%m-%dT%H:%M:%SZ) BOOTSTRAP_DONE train=$(cat /root/logs/h139_train.pid) post=$(cat /root/logs/h139_post_train.pid)"
