#!/usr/bin/env bash
# Host → mine-h70-1: upload stack + data, start bootstrap under nohup.
set -euo pipefail

ROOT=/home/const/subnet120
DST_HOST=${DST_HOST:?set DST_HOST}
DST_PORT=${DST_PORT:?set DST_PORT}
KNOWN=${KNOWN:-/tmp/mine-h70-1.known_hosts}
...
STAGE=$(mktemp -d /tmp/mine-h70-stack.XXXXXX)
trap 'rm -rf "$STAGE"' EXIT

EXP=s4-h70-m7-winner-za-lr501e6
...
TAR=/tmp/mine-h70-stack.tar.gz
tar -C "$STAGE" -czf "$TAR" .
ls -lh "$TAR"

ENV_TMP=$(mktemp /tmp/mine-h70.env.XXXXXX)
...
# H70 data = H29 king-self harvest (686 ex); init = m7 (same as H28).
DATA="$ROOT/mining/experiments/s4-h27-clip-l1-shape/results/winner_za_high_l1.jsonl"
test -s "$DATA"
test "$(wc -l <"$DATA")" -ge 300

"${SSH[@]}" 'mkdir -p /root/mining_src /root/affine_data /root/logs /root/h70 /root/hf'
"${SCP[@]}" "$TAR" "root@${DST_HOST}:/tmp/mine-h70-stack.tar.gz"
"${SCP[@]}" "$ENV_TMP" "root@${DST_HOST}:/root/mine.env"
"${SCP[@]}" "$DATA" "root@${DST_HOST}:/root/h70/winner_za_high_l1.jsonl"
rm -f "$ENV_TMP"
...
for repo in (
    "unconst/Affine-5czsc2fc98-h70-lora",
    "unconst/Affine-5czsc2fc98-h70-merged",
):
    try:
...
"${SSH[@]}" 'set -e
  tar -C /root/mining_src -xzf /tmp/mine-h70-stack.tar.gz
  chmod 600 /root/mine.env
  chmod +x /root/mining_src/s3-duel-sim/*.sh \
           /root/mining_src/s4-h2-merge/*.sh \
           /root/mining_src/s4-h70-m7-winner-za-lr501e6/*.sh
  test -f /root/mining_src/affine_pkg/affine/score.py
  test -s /root/h70/winner_za_high_l1.jsonl
  test -x /root/mining_src/s4-h70-m7-winner-za-lr501e6/bootstrap_h70.sh
  echo STACK_UPLOAD_OK
  nohup bash /root/mining_src/s4-h70-m7-winner-za-lr501e6/bootstrap_h70.sh \
    >/root/logs/h70_pipeline.nohup 2>&1 &
  echo $! > /root/logs/h70_pipeline.pid
  nohup bash /root/mining_src/s4-h2-merge/watch_form_decision.sh h70 \
    /root/affine_data/h70_sim_result.json /root/affine_data/h70_decision.json \
    /root/logs/h70_form_decision.nohup \
    >/root/logs/h70_form_decision.launch.out 2>&1 &
  ...
  nohup bash /root/mining_src/s4-h70-m7-winner-za-lr501e6/watch_preempt_bare_tcache_pass264.sh \
    >/root/logs/h70_preempt_bare_pass264.launch.nohup 2>&1 &
  echo $! > /root/logs/h70_preempt_bare_pass264.pid
  echo PREEMPT_PID=$(cat /root/logs/h70_preempt_bare_pass264.pid)
  echo PIPELINE_PID=$(cat /root/logs/h70_pipeline.pid)
  sleep 3
  head -n 40 /root/logs/bootstrap_h70.log 2>/dev/null || head -n 40 /root/logs/h70_pipeline.nohup || true
  ps -p "$(cat /root/logs/h70_pipeline.pid)" -o pid,etime,cmd || true
'
