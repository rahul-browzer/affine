#!/usr/bin/env bash
# Host → mine-r6-fmt-2: Tok-init LoRA on *natural* long-z (≠ R6 short≤180).
# Reuses s4-h101-f6-short-format bootstrap/train/merge/n80 stack; overlays start + data.
set -euo pipefail

ROOT=/home/const/subnet120
POD_NAME=${POD_NAME:-mine-r6-fmt-2}
...
STAGE=$(mktemp -d /tmp/mine-r6b-stack.XXXXXX)
...
mkdir -p "$STAGE/affine_pkg/affine" "$STAGE/affine_pkg/evalsrv" \
         ...
         "$STAGE/s4-h1v2-sft" "$STAGE/$EXP" "$STAGE/r6b-long-thought"
...
cp -a "$ROOT/mining/experiments/r6b-long-thought/plan.md" "$STAGE/r6b-long-thought/"
cp -a "$ROOT/mining/experiments/r6b-long-thought/start_r6b.sh" "$STAGE/r6b-long-thought/"
# Overlay: H101 bootstrap calls start_h101.sh — replace with R6b long-z train.
cp -a "$ROOT/mining/experiments/r6b-long-thought/start_r6b.sh" "$STAGE/$EXP/start_h101.sh"

TAR=/tmp/mine-r6b-stack.tar.gz
tar -C "$STAGE" -czf "$TAR" .
ls -lh "$TAR"

ENV_TMP=$(mktemp /tmp/mine-r6b.env.XXXXXX)
...
DEAD=$(date -u -d '+23 hours 30 minutes' +%Y-%m-%dT%H:%M:%SZ)
{
  ...
  echo "export HF_MERGED_REPO=unconst/Affine-5czsc2fc98-r6b-fmt"
  echo "export R6B_AXIS=natural_long_nonlisty_zgt180"
} >"$ENV_TMP"
chmod 600 "$ENV_TMP"

DATA="$ROOT/mining/experiments/r6b-long-thought/results/za_long_natural.jsonl"
test -s "$DATA"
test "$(wc -l <"$DATA")" -ge 200

"${SSH[@]}" 'mkdir -p /root/mining_src /root/affine_data /root/logs /root/h101 /root/r6b /root/hf'
"${SCP[@]}" "$TAR" "root@${DST_HOST}:/tmp/mine-r6b-stack.tar.gz"
"${SCP[@]}" "$ENV_TMP" "root@${DST_HOST}:/root/mine.env"
# H101 bootstrap expects this path; content is R6b natural-long (not ultrashort rewrite).
"${SCP[@]}" "$DATA" "root@${DST_HOST}:/root/h101/za_ultrashort80.jsonl"
"${SCP[@]}" "$DATA" "root@${DST_HOST}:/root/r6b/za_long_natural.jsonl"
rm -f "$ENV_TMP"
...
for repo in (
    "unconst/Affine-5czsc2fc98-r6b-fmt",
    "unconst/Affine-5czsc2fc98-r6b-fmt-lora",
):
    try:
...
"${SSH[@]}" 'set -e
  tar -C /root/mining_src -xzf /tmp/mine-r6b-stack.tar.gz
  ...
  chmod +x /root/mining_src/s3-duel-sim/*.sh \
           ...
           /root/mining_src/r6b-long-thought/*.sh
  ...
  # Prove overlay is R6b, not stock H101 / R6 short.
  grep -q "R6b: Tok-init" /root/mining_src/s4-h101-f6-short-format/start_h101.sh
  set -a; source /root/mine.env; set +a
  echo "R6B_DEADLINES soft=$SOFT_DEADLINE_UTC dead=$DEADMAN_UTC axis=$R6B_AXIS"
  echo STACK_UPLOAD_OK
  nohup bash /root/mining_src/s4-h101-f6-short-format/bootstrap_h101.sh \
    >/root/logs/r6b_pipeline.nohup 2>&1 &
  echo $! > /root/logs/r6b_pipeline.pid
  cp -f /root/logs/r6b_pipeline.pid /root/logs/h101_pipeline.pid
  nohup bash /root/mining_src/s4-h2-merge/watch_form_decision.sh h101 \
    ...
    /root/logs/r6b_form_decision.nohup \
    >/root/logs/r6b_form_decision.launch.out 2>&1 &
  echo $! > /root/logs/r6b_form_decision.pid
  echo PIPELINE_PID=$(cat /root/logs/r6b_pipeline.pid)
  sleep 5
  head -n 40 /root/logs/bootstrap_h101.log 2>/dev/null || head -n 40 /root/logs/r6b_pipeline.nohup || true
  ps -p "$(cat /root/logs/r6b_pipeline.pid)" -o pid,etime,cmd || true
  nvidia-smi -L | wc -l
'
