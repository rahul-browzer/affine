#!/usr/bin/env bash
# Pass 322: king-only relaunch after mid-n80 Triton ENOENT on bare cache/king
# (EAUHKKZ…/__triton_launcher.so @14:41:55Z → EngineDead; :8001=000).
# Isolated TCACHE + util=0.72 (bare→ENOENT again H80 p312; util0.80→OOM H80 p314).
# Teacher+chall stay up. Do NOT touch chall (recover264 / GPUs 4,5).
set -euo pipefail
...
LOGN=/root/logs/h84_king_recover_pass322.nohup
log() { echo "[king322-h84] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOGN"; }

log "START king-only recover (p322 isolated util=0.72 after bare-cache ENOENT @14:41:55Z; leave chall)"
...
for line in apps.strip().splitlines():
    ...
    if len(parts) >= 2 and parts[0] in uuids:
        try:
            ...
            pass
print(f"[king322-h84] reap gpus2,3 pids={sorted(pids)}")
for pid in pids:
    try:
...
rm -f /root/affine_data/h84_sim_result.json /root/affine_data/h84_sim_progress.json \
  /root/affine_data/h84_decision.json /root/logs/h84_n80.done /root/logs/h84_n80_retry.aborted \
  /root/logs/h84_king_recover_pass311.done /root/logs/h84_sim_n80.done
log "cleared stale h84 sim progress/result"

# Wipe bare + prior king_* caches; use fresh isolated dir (chall pattern).
...
TCACHE="/root/.triton/isolated/h84_king_p322_$(date +%s)_$$"
mkdir -p "$TCACHE"
echo "$TCACHE" >/root/logs/h84_king_tcache_pass322.path
...
for line in out.strip().splitlines():
    ...
    if int(idx) in (2, 3):
        print(f"[king322-h84] gpu{idx} used_mib={used}")
        if float(used) < 500:
            ok += 1
if ok < 2:
    print("[king322-h84] ABORT: gpus 2,3 not free")
    sys.exit(1)
PY
...
for i in $(seq 1 180); do
  # Early abort: load-time Triton ghost ENOENT never reaches health=200.
  ...
  if [[ "$code" == "200" ]]; then
    ...
    if [[ -n "$mid" ]]; then
      pcode=$(curl -s -o /tmp/_probe_king322.json -w "%{http_code}" --max-time 90 \
      ...
      if [[ "$pcode" == "200" ]]; then
        ...
        date -u +%Y-%m-%dT%H:%M:%SZ >/root/logs/h84_king_recover_pass322.done
        echo "$REPO $REV $TCACHE util=0.72" >>/root/logs/h84_king_recover_pass322.done
        log "DONE — retry_h84_n80 should proceed (watcher already armed)"
        exit 0
      fi
    fi
  fi
