#!/usr/bin/env bash
# Restart H84 n80 after crash. Engines must already be up.
...
SIM=/root/affine_data/h84_sim_result.json
PROG=/root/affine_data/h84_sim_progress.json
DEC=/root/affine_data/h84_decision.json
LOG=/root/logs/h84_n80_retry.nohup
MAX_ATTEMPTS=${MAX_ATTEMPTS:-3}

log() { echo "[h84-n80-retry] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }

_promptable() {
  # health=200 ≠ alive (H30/H84 Triton __triton_launcher.so → ConnectError false REFUTE)
...
_wait_engines() {
  ...
  while (( i < max )); do
    if _engines_ok; then
      ...
      # __triton_launcher.so (H84/H38 p204/p205). Confirm again after settle.
      log "first promptable at poll=$i — settle 20s + re-probe"
      sleep 20
      if _engines_ok; then
        log "engines double-promptable after ${i} polls"
        rm -f /root/logs/h84_n80_retry.aborted
        return 0
      fi
...
if [[ -f "$DEC" ]]; then
  if python3 -c 'import json,sys; d=json.load(open(sys.argv[1])); sys.exit(0 if d.get("false_probe") else 1)' "$DEC" 2>/dev/null; then
    ...
    mv "$DEC" "/root/affine_data/false_probes/h84_decision_retryQ_${ts}.json"
    [[ -f "$SIM" ]] && mv "$SIM" "/root/affine_data/false_probes/h84_sim_retryQ_${ts}.json"
    rm -f /root/logs/h84_n80.done
...
if [[ -f "$SIM" ]]; then
  if python3 -c 'import json,sys; d=json.load(open(sys.argv[1])); sys.exit(0 if (d.get("false_probe") or "unpromptable" in str(d.get("rejection_reason","")) or "ConnectError" in str(d.get("rejection_reason",""))) else 1)' "$SIM" 2>/dev/null; then
    ts=$(date -u +%Y%m%dT%H%M%SZ)
    mkdir -p /root/affine_data/false_probes
    mv "$SIM" "/root/affine_data/false_probes/h84_sim_retryQ_${ts}.json"
    ...
    python3 /root/mining_src/s4-h2-merge/write_merge_decision.py \
      --hyp h84 --sim-result "$SIM" --out "$DEC"
    date -u +%Y-%m-%dT%H:%M:%SZ > /root/logs/h84_n80.done
    exit 0
  fi
fi
# Require python in argv — bare pgrep/awk patterns false-match (pass205).
if ps -eo pid,cmd | awk '/python/ && /[r]un_sim_duel.py/ && /local-h84/ { found=1 } END { exit !found }'; then
...
if ! _wait_engines 120; then
  ...
  echo "aborted_engines_unhealthy $(date -u +%Y-%m-%dT%H:%M:%SZ)" \
    >/root/logs/h84_n80_retry.aborted
  exit 1
fi
test -d "$MERGED"
test -f /root/logs/h84_merge.done
