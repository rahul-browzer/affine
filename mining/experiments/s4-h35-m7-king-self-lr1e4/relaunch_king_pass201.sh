#!/usr/bin/env bash
# Pass 201: H35 king never came up (Triton __triton_launcher.so race @ prewarm).
# Wipe caches, unique TRITON_CACHE_DIR, relaunch king on GPUs 2,3 while merge
# still writes on 6,7. Live post_train waits on health after merge.
set -euo pipefail
...
LOG=/root/logs/h35_king_recover_pass201.log
...
TCACHE=/root/.triton/cache/king_p201_$(date +%s)_$$
mkdir -p "$TCACHE" /root/logs
: >"$LOG"

log() { echo "[recover201-h35] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }
...
print(f"[recover201-h35] reap gpu={sorted(want)} workers={sorted(pids)} parents={sorted(parents)}")
for pid in kill_set:
    try:
...
mkdir -p "$TCACHE" /root/.cache/torchinductor_king_p201_$$
export TORCHINDUCTOR_CACHE_DIR=/root/.cache/torchinductor_king_p201_$$
...
log "DONE_LAUNCH (pipeline still merging — will pick up health after merge)"
