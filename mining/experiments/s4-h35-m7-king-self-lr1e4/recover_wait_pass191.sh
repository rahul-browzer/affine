...
log() { echo "[recover-wait191-h35] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }
...
if [[ -f /root/logs/h35_chall_serve.done ]] && \
   [[ "$(curl -s -o /dev/null -w "%{http_code}" --max-time 3 http://127.0.0.1:8002/health || true)" == "200" ]]; then
  log "chall already up — kick retry_h35_n80 if needed"
  ...
  nohup bash /root/mining_src/s4-h35-m7-king-self-lr1e4/retry_h35_n80.sh \
    >/root/logs/h35_n80_relaunch_pass191.nohup 2>&1 &
  echo $! >/root/logs/h35_n80_relaunch_pass191.pid
  log "relaunched retry pid=$(cat /root/logs/h35_n80_relaunch_pass191.pid)"
  exit 0
fi

# Pipeline aborted or still stuck: chall-only serve then n80.
if [[ -f /root/logs/h35_pipeline.aborted ]] || \
   ! ps -eo pid,cmd | awk '/[p]ost_train_pipeline.sh/ && /h35/' | grep -q .; then
  log "pipeline gone/aborted — chall-only serve + n80"
  # shellcheck disable=SC1091
  source /root/venv/bin/activate
  if [[ -f /root/mine.env ]]; then set -a; source /root/mine.env; set +a; fi
  export HF_HOME=${HF_HOME:-/root/hf}
  MERGED=/root/h35/merged
