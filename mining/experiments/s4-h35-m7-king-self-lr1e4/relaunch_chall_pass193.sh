#!/usr/bin/env bash
# Pass 193: H35 chall died on first completions — Triton __triton_launcher.so
# missing. n80 wrote ConnectError false REFUTE (margin=null). Quarantine,
# reap GPUs 4,5 by index, unique TRITON_CACHE_DIR, relaunch chall; recover_wait
# kicks n80 after /v1/completions probe.
set -euo pipefail
...
LOG=/root/logs/h35_chall_recover_pass193.log
CHALL_LOG=/root/logs/vllm_chall.log
PIDF=/root/logs/vllm_chall.pid
MERGE=/root/h35/merged
export GPUS=4,5
UTIL=0.72
...
log() { echo "[recover193-h35-chall] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }

log "START chall relaunch TCACHE=$TCACHE"

for f in h35_decision.json h35_sim_result.json h35_sim_progress.json \
  h35_sim_result_artifact.json; do
  if [[ -f "/root/affine_data/$f" ]]; then
    mv "/root/affine_data/$f" \
      "/root/affine_data/false_probes/${f%.json}_pass193_$(date -u +%H%M%S).json"
    log "quarantined $f"
  fi
done
...
done < <(ps -eo pid,cmd | awk '/[r]un_sim_duel.py/ && /local-h35/ {print $1}')
...
while read -r pid; do
  ...
  sleep 1
  kill -9 "$pid" 2>/dev/null || true
done < <(ps -eo pid,cmd | awk '/[p]ost_train_pipeline.sh/ && /h35/ {print $1}')
...
print(f"[recover193-h35-chall] reap gpu={sorted(want)} workers={sorted(pids)} parents={sorted(parents)}")
for pid in kill_set:
    try:
...
rm -f /root/logs/h35_pipeline.aborted /root/logs/h35_n80_retry.aborted \
  /root/logs/h35_n80.done /root/logs/h35_chall_serve.done

nohup bash /root/mining_src/s4-h35-m7-king-self-lr1e4/recover_wait_chall_pass193.sh \
  >/root/logs/h35_recover_wait193.nohup 2>&1 &
echo $! >/root/logs/h35_recover_wait193.pid
log "wait_pid=$(cat /root/logs/h35_recover_wait193.pid)"
log "DONE_LAUNCH"
