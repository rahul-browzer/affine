#!/usr/bin/env bash
# Pass 191: H35 king died on Triton __triton_launcher.so race (GPUs 2,3 empty).
# Wipe caches, unique TRITON_CACHE_DIR, relaunch king; live post_train_pipeline
# is still in its teacher/king health wait and will proceed to chall+n80.
set -euo pipefail
...
LOG=/root/logs/h35_king_recover_pass191.log
...
log() { echo "[recover191-h35] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }

log "START king relaunch TCACHE=$TCACHE"

mkdir -p /root/affine_data/false_probes
for f in h35_decision.json h35_sim_result.json h35_sim_progress.json; do
  if [[ -f "/root/affine_data/$f" ]]; then
    mv "/root/affine_data/$f" "/root/affine_data/false_probes/${f%.json}_pass191_$(date -u +%H%M%S).json"
    log "quarantined $f"
  fi
done
...
print(f"[recover191-h35] reap gpu={sorted(want)} workers={sorted(pids)} parents={sorted(parents)}")
for pid in kill_set:
    try:
...
# Background: wait health+completions; log only (pipeline resumes on health=200)
nohup bash /root/mining_src/s4-h35-m7-king-self-lr1e4/recover_wait_pass191.sh \
  >/root/logs/h35_recover_wait191.nohup 2>&1 &
echo $! >/root/logs/h35_recover_wait.pid
log "wait_pid=$(cat /root/logs/h35_recover_wait.pid)"
log "DONE_LAUNCH"
