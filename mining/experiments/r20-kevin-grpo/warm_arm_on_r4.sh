#!/usr/bin/env bash
# Host → lean warm-arm R20 onto TKC-hot mine-r4-fullft-1 after R14 REFUTE.
# Keeps teacher:8000 + ckp333:8001; trains kevin-GRPO on GPUs 6–7 (≠ R14 REINFORCE).
set -euo pipefail
...
cp -a "$ROOT/mining/experiments/r20-kevin-grpo/"*.sh "$STAGE/r20-kevin-grpo/"
cp -a "$ROOT/mining/experiments/r20-kevin-grpo/plan.md" "$STAGE/r20-kevin-grpo/"
# Overlay start/bootstrap for R20 identity grep.
cp -a "$ROOT/mining/experiments/r20-kevin-grpo/start_r20.sh" "$STAGE/$EXP/start_r3.sh"
cp -a "$ROOT/mining/experiments/r20-kevin-grpo/bootstrap_r20.sh" "$STAGE/$EXP/bootstrap_r3.sh"
...
TAR=/tmp/mine-r20-warm-stack.tar.gz
tar -C "$STAGE" -czf "$TAR" .
ls -lh "$TAR"
...
set +a
umask 077
ENV_TMP=$(mktemp /tmp/mine-r20.env.XXXXXX)
{
  ...
  echo "export HF_MERGED_REPO=unconst/Affine-5czsc2fc98-r20-kevin-grpo"
  echo "export R20_AXIS=kevin_grpo_reason"
  echo "export R20_LR=5e-6"
  echo "export R20_LORA_R=16"
  echo "export R20_LORA_ALPHA=32"
  echo "export R20_GROUP_SIZE=4"
  echo "export R20_MAX_STEPS=200"
  ...
  echo "export BASE=/root/hf/hub/models--kevin954--Affine-5dfqbbh8ev-sft/snapshots/6a5815fad8f4e34c983b1933c1fae5762fe25220"
} >"$ENV_TMP"
chmod 600 "$ENV_TMP"
...
"${SSH[@]}" 'mkdir -p /root/mining_src /root/affine_data /root/logs /root/r3 /root/r20 /root/hf'
"${SCP[@]}" "$TAR" "root@${DST_HOST}:/tmp/mine-r20-warm-stack.tar.gz"
"${SCP[@]}" "$ENV_TMP" "root@${DST_HOST}:/root/mine.env"
"${SCP[@]}" "$DATA" "root@${DST_HOST}:/root/r3/winner_za_high_l1.jsonl"
"${SCP[@]}" "$DATA" "root@${DST_HOST}:/root/r20/winner_za_high_l1.jsonl"
rm -f "$ENV_TMP"
...
for repo in (
    "unconst/Affine-5czsc2fc98-r20-kevin-grpo",
    "unconst/Affine-5czsc2fc98-r20-kevin-grpo-lora",
):
    try:
...
"${SSH[@]}" 'set -euo pipefail
  tar -C /root/mining_src -xzf /tmp/mine-r20-warm-stack.tar.gz
  chmod 600 /root/mine.env
  chmod +x /root/mining_src/r3-reason-grpo/*.sh \
           /root/mining_src/r20-kevin-grpo/*.sh \
  ...
  # Stop leftover R14 lean/post/form by pidfile only (never pkill -f).
  for f in /root/logs/r14_lean_warm.pid /root/logs/r14_post_train.pid \
           /root/logs/h135_post_train.pid /root/logs/r14_form_decision.pid \
           /root/logs/h135_dec_watch.pid /root/logs/h135_train.pid \
           /root/logs/r14_train.pid /root/logs/r3_train.pid \
           /root/logs/r3_post_train.pid; do
    if [[ -f "$f" ]]; then
  ...
  nohup bash /root/mining_src/r20-kevin-grpo/lean_warm_boot.sh \
    >/root/logs/r20_lean_warm.nohup 2>&1 &
  echo $! >/root/logs/r20_lean_warm.pid
  sleep 20
  echo LEAN_PID=$(cat /root/logs/r20_lean_warm.pid)
  head -n 100 /root/logs/r20_lean_warm.log 2>/dev/null \
    || head -n 100 /root/logs/r20_lean_warm.nohup || true
  ps -p "$(cat /root/logs/r20_lean_warm.pid)" -o pid,etime,cmd || true
  test -f /root/logs/r3_train.pid && echo TRAIN_PID=$(cat /root/logs/r3_train.pid)
  test -f /root/logs/r20_post_train.pid && echo POST_PID=$(cat /root/logs/r20_post_train.pid)
...
echo "R20_WARM_ARM_OK pod=$POD_NAME soft=$SOFT dead=$DEAD $(date -u +%Y-%m-%dT%H:%M:%SZ)"
