#!/usr/bin/env bash
# R20: kevin954-init → Reason-GRPO (Tok kept as n80 king only).
...
mkdir -p /root/logs /root/hf /root/affine_data /root/mining_src /root/r3 /root/r20
exec > >(tee -a "$LOG") 2>&1

echo "[bootstrap-r20] $(date -u +%Y-%m-%dT%H:%M:%SZ) start host=$(hostname)"
...
if [[ -z "${HF_TOKEN:-}" ]]; then
  echo "[bootstrap-r20] FATAL: HF_TOKEN missing in /root/mine.env"
  exit 1
fi

nvidia-smi -L || true
NGPU=$(nvidia-smi -L | wc -l)
echo "[bootstrap-r20] GPU_COUNT=$NGPU"
test "$NGPU" -ge 8
...
# Prove overlay is R20, not stock R3 / R14 / R19.
...
if p.is_file():
    ...
    if txt != orig:
        p.write_text(txt)
        print("[bootstrap-r20] patched vllm_client timeout=600 retries=5", flush=True)
    else:
        print("[bootstrap-r20] vllm_client already patched or pattern miss", flush=True)
PY
...
kevin_repo = "kevin954/Affine-5dfqbbh8ev-sft"
kevin_rev = "6a5815fad8f4e34c983b1933c1fae5762fe25220"
print("[bootstrap-r20] DOWNLOAD kevin-init start", kevin_repo, kevin_rev, flush=True)
kevin = snapshot_download(kevin_repo, revision=kevin_rev, token=token)
print(f"[bootstrap-r20] DOWNLOAD kevin-init done -> {kevin}", flush=True)
open("/root/logs/kevin.done", "w").write(kevin + "\n")
open("/root/r20/r20_base.path", "w").write(kevin + "\n")
assert kevin_rev in kevin, kevin

tok_repo = "Tok331102/affine-5EqYW8McUc-af10"
tok_rev = "eb8bf9a356a254f71faaa439e8abc3cfba572c53"
print("[bootstrap-r20] DOWNLOAD tok331102 king start", tok_repo, tok_rev, flush=True)
tok = snapshot_download(tok_repo, revision=tok_rev, token=token)
print(f"[bootstrap-r20] DOWNLOAD tok331102 done -> {tok}", flush=True)
open("/root/logs/tok_init.done", "w").write(tok + "\n")
open("/root/logs/tok331102.done", "w").write(tok + "\n")

print("[bootstrap-r20] DOWNLOAD teacher start", flush=True)
tpath = snapshot_download("zai-org/GLM-4.5-Air-FP8", token=token)
print(f"[bootstrap-r20] DOWNLOAD teacher done -> {tpath}", flush=True)
open("/root/logs/teacher.done", "w").write(tpath + "\n")
print("[bootstrap-r20] ALL_DOWNLOADS_OK", flush=True)
PY

export BASE=$(cat /root/r20/r20_base.path)
if [[ -f /root/mine.env ]]; then
...
echo "[bootstrap-r20] $(date -u +%Y-%m-%dT%H:%M:%SZ) serving teacher before train BASE=$BASE"
bash /root/mining_src/r3-reason-grpo/prewarm_engines.sh \
  >/root/logs/r3_prewarm.nohup 2>&1 &
echo $! >/root/logs/r3_prewarm.pid
cp -f /root/logs/r3_prewarm.pid /root/logs/r20_prewarm.pid

for i in $(seq 1 240); do
  if curl -sf --max-time 5 http://127.0.0.1:8000/v1/models >/dev/null 2>&1; then
    echo "[bootstrap-r20] teacher :8000 up at iter=$i"
    break
  fi
  if (( i == 240 )); then
    echo "[bootstrap-r20] FATAL teacher never up"
...
echo "[bootstrap-r20] $(date -u +%Y-%m-%dT%H:%M:%SZ) launching R20 kevin Reason-GRPO train"
...
cp -f /root/logs/r3_extra_dl.pid /root/logs/r20_extra_dl.pid
...
cp -f /root/logs/r3_post_train.pid /root/logs/r20_post_train.pid

echo "[bootstrap-r20] $(date -u +%Y-%m-%dT%H:%M:%SZ) BOOTSTRAP_DONE train=$(cat /root/logs/r3_train.pid) post=$(cat /root/logs/r3_post_train.pid)"
