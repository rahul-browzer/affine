#!/usr/bin/env bash
# Host → mine-r20-kevin-grpo-1: R3 stack + R20 Kevin-GRPO overlays, bootstrap.
# Axis R20: kevin954-init Reason-GRPO (≠ R3 Tok, ≠ R14 kevin REINFORCE, ≠ R19 Talent).
set -euo pipefail

ROOT=/home/const/subnet120
POD_NAME=${POD_NAME:-mine-r20-kevin-grpo-1}
...
STAGE=$(mktemp -d /tmp/mine-r20-stack.XXXXXX)
...
cp -a "$ROOT/mining/experiments/r20-kevin-grpo/start_r20.sh" "$STAGE/r20-kevin-grpo/"
cp -a "$ROOT/mining/experiments/r20-kevin-grpo/bootstrap_r20.sh" "$STAGE/r20-kevin-grpo/"
# Overlay: bootstrap_r3 + start_r3 → R20 kevin Reason-GRPO.
...
umask 077
{
  ...
  echo "export HF_LORA_REPO=unconst/Affine-5czsc2fc98-r20-lora"
  echo "export HF_MERGED_REPO=unconst/Affine-5czsc2fc98-r20-merged"
...
"${SCP[@]}" "$TAR" "root@${DST_HOST}:/tmp/mine-r20-stack.tar.gz"
...
for repo in (
    "unconst/Affine-5czsc2fc98-r20-lora",
    "unconst/Affine-5czsc2fc98-r20-merged",
):
    try:
...
"${SSH[@]}" 'set -e
  tar -C /root/mining_src -xzf /tmp/mine-r20-stack.tar.gz
  ...
  # Prove overlay is R20, not stock R3 / R19.
  grep -q "R20: Kevin-GRPO" /root/mining_src/r3-reason-grpo/start_r3.sh
  grep -q "DOWNLOAD kevin-init" /root/mining_src/r3-reason-grpo/bootstrap_r3.sh
  set -a; source /root/mine.env; set +a
  echo "R20_DEADLINES soft=$SOFT_DEADLINE_UTC dead=$DEADMAN_UTC axis=$R20_AXIS"
  echo "R20_KNOBS lr=${R20_LR} r=${R20_LORA_R} G=${R20_GROUP_SIZE}"
  echo STACK_UPLOAD_OK
  nohup bash /root/mining_src/r3-reason-grpo/bootstrap_r3.sh \
    >/root/logs/r20_pipeline.nohup 2>&1 &
  echo $! > /root/logs/r20_pipeline.pid
  cp -f /root/logs/r20_pipeline.pid /root/logs/r3_pipeline.pid
  echo PIPELINE_PID=$(cat /root/logs/r20_pipeline.pid)
  sleep 5
  head -n 40 /root/logs/bootstrap_r3.log 2>/dev/null || head -n 40 /root/logs/r20_pipeline.nohup || true
  ps -p "$(cat /root/logs/r20_pipeline.pid)" -o pid,etime,cmd || true
  nvidia-smi -L | wc -l
'
