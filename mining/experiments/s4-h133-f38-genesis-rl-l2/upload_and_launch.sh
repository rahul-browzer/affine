#!/usr/bin/env bash
# Host → mine-f38-1: upload stack + data, start bootstrap under nohup.
set -euo pipefail

ROOT=/home/const/subnet120
DST_HOST=${DST_HOST:?set DST_HOST}
DST_PORT=${DST_PORT:?set DST_PORT}
KNOWN=${KNOWN:-/tmp/mine-f38-1.known_hosts}
...
STAGE=$(mktemp -d /tmp/mine-h133-stack.XXXXXX)
trap 'rm -rf "$STAGE"' EXIT

EXP=s4-h133-f38-genesis-rl-l2
...
test -f "$STAGE/$EXP/bootstrap_h133.sh"
test -f "$STAGE/$EXP/start_h133.sh"

TAR=/tmp/mine-h133-stack.tar.gz
tar -C "$STAGE" -czf "$TAR" .
ls -lh "$TAR"

ENV_TMP=$(mktemp /tmp/mine-h133.env.XXXXXX)
...
# H133 data = H27 winner_za_high_l1 (406 ex); init = Tok331102 king.
DATA="$ROOT/mining/experiments/s4-h27-clip-l1-shape/results/winner_za_high_l1.jsonl"
test -s "$DATA"
test "$(wc -l <"$DATA")" -ge 300

"${SSH[@]}" 'mkdir -p /root/mining_src /root/affine_data /root/logs /root/h133 /root/hf'
"${SCP[@]}" "$TAR" "root@${DST_HOST}:/tmp/mine-h133-stack.tar.gz"
"${SCP[@]}" "$ENV_TMP" "root@${DST_HOST}:/root/mine.env"
"${SCP[@]}" "$DATA" "root@${DST_HOST}:/root/h133/winner_za_high_l1.jsonl"
rm -f "$ENV_TMP"
...
for repo in (
    "unconst/Affine-5czsc2fc98-h133-lora",
    "unconst/Affine-5czsc2fc98-h133-merged",
):
    try:
...
"${SSH[@]}" 'set -e
  tar -C /root/mining_src -xzf /tmp/mine-h133-stack.tar.gz
  chmod 600 /root/mine.env
  chmod +x /root/mining_src/s3-duel-sim/*.sh \
           /root/mining_src/s4-h2-merge/*.sh \
           /root/mining_src/s4-h133-f38-genesis-rl-l2/*.sh
  test -f /root/mining_src/affine_pkg/affine/score.py
  test -s /root/h133/winner_za_high_l1.jsonl
  test -x /root/mining_src/s4-h133-f38-genesis-rl-l2/bootstrap_h133.sh
  echo STACK_UPLOAD_OK
  nohup bash /root/mining_src/s4-h133-f38-genesis-rl-l2/bootstrap_h133.sh \
    >/root/logs/h133_pipeline.nohup 2>&1 &
  echo $! > /root/logs/h133_pipeline.pid
  nohup bash /root/mining_src/s4-h2-merge/watch_form_decision.sh h133 \
    /root/affine_data/h133_sim_result.json /root/affine_data/h133_decision.json \
    /root/logs/h133_form_decision.nohup \
    >/root/logs/h133_form_decision.launch.out 2>&1 &
  echo $! > /root/logs/h133_form_decision.pid
  nohup bash /root/mining_src/s4-h2-merge/watch_n80_retry.sh h133 \
    /root/mining_src/s4-h133-f38-genesis-rl-l2/retry_h133_n80_d203first.sh \
    >/root/logs/h133_watch_retry.launch.nohup 2>&1 &
  echo $! > /root/logs/h133_watch_retry.pid
  nohup bash /root/mining_src/s4-h133-f38-genesis-rl-l2/watch_preempt_bare_tcache_pass264.sh \
    >/root/logs/h133_preempt_bare_pass264.launch.nohup 2>&1 &
  echo $! > /root/logs/h133_preempt_bare_pass264.pid
  echo PREEMPT_PID=$(cat /root/logs/h133_preempt_bare_pass264.pid)
  echo PIPELINE_PID=$(cat /root/logs/h133_pipeline.pid)
  sleep 3
  head -n 40 /root/logs/bootstrap_h133.log 2>/dev/null || head -n 40 /root/logs/h133_pipeline.nohup || true
  ps -p "$(cat /root/logs/h133_pipeline.pid)" -o pid,etime,cmd || true
'
