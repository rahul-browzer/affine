...
LOG=/root/logs/h56_chall_recover_pass251.log
...
log() { echo "[recover251-h56-chall] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }

log "START chall relaunch (outer×3 + king-seed WRITABLE w1 + post-w1 freeze; p247 prefreeze-before-w1 dead)"
...
for f in h56_decision.json h56_sim_result.json h56_sim_result_artifact.json \
  ...
  if [[ -f "/root/affine_data/$f" ]]; then
    mv "/root/affine_data/$f" "$QDIR/${f%.json}_pass251_${ts}.json"
...
rm -f /root/logs/h56_n80.done /root/logs/h56_n80_retry.aborted \
  ...
  /root/logs/h56_sim_n80.done /root/logs/h56_chall_freeze_pass247.done \
  /root/logs/h56_chall_freeze_pass237.done \
...
print(f"[recover251-h56-chall] reap gpu={sorted(want)} workers={sorted(pids)} parents={sorted(parents)} grandparents={sorted(grandparents)}")
for pid in kill_set:
    try:
...
for attempt in 1 2 3; do
  ...
  wipe_caches
  TAG=h56_chall_p251_a${attempt}_$(date +%s)_$$
  ...
  log "warmup completion #1 (writable — allows JIT of non-king hashes)"
  if _comp "a${attempt}_w1"; then
    log "FREEZE TCACHE chmod -R a-w $TCACHE (post-w1 success)"
    chmod -R a-w "$TCACHE" || true
    chmod -R a-w "/root/.cache/torchinductor_$TAG" 2>/dev/null || true
    if finish_warmups "a${attempt}"; then
      log "triple-promptable after post-w1 freeze on attempt $attempt"
      ok=1
      break
    fi
    kill_chall
...
echo "TCACHE=$TCACHE mode=$(stat -c %a $TCACHE) probe=200 attempts_ok post_w1_freeze=1 king_seed=1" \
  > /root/logs/h56_chall_freeze_pass251.done
log "DONE_LAUNCH (TCACHE frozen post-w1; n80 should see double-promptable immediately)"
