...
LOG=/root/logs/h56_chall_recover_pass253.log
...
log() { echo "[recover253-h56-chall] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }
...
print(f"[recover253-h56-chall] reap gpu={sorted(want)} workers={sorted(pids)} parents={sorted(parents)} grandparents={sorted(grandparents)}")
for pid in kill_set:
    try:
...
for attempt in 1 2 3; do
  ...
  wipe_caches
  TAG=h56_chall_p253_a${attempt}_$(date +%s)_$$
...
echo "TCACHE=$TCACHE mode=$(stat -c %a $TCACHE) probe=200 attempts_ok post_diverse_freeze=1 king_seed=1" \
  > /root/logs/h56_chall_freeze_pass253.done
