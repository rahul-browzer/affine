#!/usr/bin/env bash
# Pass 234: teacher hung after Triton race (shm_broadcast + .ttir / __triton_launcher.so).
# King :8001 OK. Merged chall ready. Reap GPUs 0,1 → wipe teacher* → settle20 → unique TCACHE.
# Do NOT touch king/chall/merge. post_train wait loop will resume on t=200.
set -euo pipefail
HYPO=${HYPO:?set HYPO=h50|h51|h52}
...
LOG=/root/logs/${HYPO}_teacher_recover_pass234.log
TEACHER_LOG=/root/logs/vllm_teacher.log
PIDF=/root/logs/vllm_teacher.pid
export GPUS=0,1
UTIL=0.80
TEACHER_REPO=zai-org/GLM-4.5-Air-FP8
TAG=teacher_p234_$(date +%s)_$$
mkdir -p /root/logs
: >"$LOG"
log() { echo "[recover234-${HYPO}] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }
log "START teacher relaunch (wipe→settle20→unique cache); leave king alone"
...
print(f"[recover234] reap gpu={sorted(want)} workers={sorted(pids)} parents={sorted(parents)} grandparents={sorted(grandparents)}")
for pid in kill_set:
...
log "DONE_LAUNCH (king :8001 stays; post_train wait resumes on t=200)"
