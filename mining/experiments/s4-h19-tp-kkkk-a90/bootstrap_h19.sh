# mine-h19-1 bootstrap: TalentPigs×kkkk α0.90 + teacher → merge/serve/n80.
# Secrets from /root/mine.env only (never argv).
set -euo pipefail

LOG=/root/logs/bootstrap_h19.log
mkdir -p /root/logs /root/hf /root/affine_data /root/mining_src /root/merges
exec > >(tee -a "$LOG") 2>&1

echo "[bootstrap-h19] $(date -u +%Y-%m-%dT%H:%M:%SZ) start host=$(hostname)"
...
if [[ -z "${HF_TOKEN:-}" ]]; then
  echo "[bootstrap-h19] FATAL: HF_TOKEN missing in /root/mine.env"
  exit 1
fi
...
uv pip install \
  ...
  "numpy" \
  "scipy" \
  2>&1 | tee /root/logs/pip_h19.log | tail -40

python - <<'PY'
import torch, transformers, vllm
print("[bootstrap-h19] VERSIONS",
...
if txt == orig:
    print("[bootstrap-h19] vllm_client already patched or pattern miss", flush=True)
else:
    p.write_text(txt)
    print("[bootstrap-h19] patched vllm_client timeout=480 retries=5", flush=True)
PY
...
for repo, rev, label in jobs:
    ...
    if os.path.isfile(done):
        print(f"[bootstrap-h19] SKIP {label} (have {done})", flush=True)
        continue
    print(f"[bootstrap-h19] DOWNLOAD start {label} {repo} rev={rev}", flush=True)
    path = snapshot_download(repo, revision=rev, token=token)
    print(f"[bootstrap-h19] DOWNLOAD done {label} -> {path}", flush=True)
    open(done, "w").write(path + "\n")
print("[bootstrap-h19] ALL_DOWNLOADS_OK", flush=True)
...
python /root/mining_src/s4-h2-merge/merge_linear.py \
  ...
  --out "$OUT" \
  --hf-home "$HF_HOME" \
  2>&1 | tee /root/logs/h19_merge.log

date -u +%Y-%m-%dT%H:%M:%SZ > /root/logs/h19_merge.done
echo "[bootstrap-h19] MERGE_DONE alpha=$ALPHA out=$OUT b=$B_REPO@$B_REV"
...
export CHALL_REPO=/root/merges/h19-tp90
export CHALL_REV=
bash /root/mining_src/s3-duel-sim/serve_three.sh

touch /root/logs/bootstrap_h19.done
echo "[bootstrap-h19] $(date -u +%Y-%m-%dT%H:%M:%SZ) DONE — engines launching; next: wait_ready + n80"
