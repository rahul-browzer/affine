#!/usr/bin/env bash
# After bootstrap_h9.done + engines up: run n80 sim vs TalentPigs.
set -euo pipefail

LOG=/root/logs/h9_n80.nohup
...
test -f /root/logs/h9_merge.done
test -d /root/merges/h9-tp75

TIMEOUT_S=2400 bash /root/mining_src/s3-duel-sim/wait_ready.sh

# retry n80 up to 3× on ReadTimeout / crash (LESSON; died @60/80 pass136)
for attempt in 1 2 3; do
  echo "[h9-n80] attempt=$attempt $(date -u +%Y-%m-%dT%H:%M:%SZ)"
  set +e
  python /root/mining_src/s4-h2-merge/run_sim_duel.py \
    ...
    --chall-repo /root/merges/h9-tp75 \
    --chall-rev local \
    --n-turns 80 \
    --hotkey local-h9 \
    --out /root/affine_data/h9_sim_result.json \
    --progress-out /root/affine_data/h9_sim_progress.json \
    --save-artifact \
    2>&1 | tee /root/logs/h9_n80.log
  rc=${PIPESTATUS[0]}
  set -e
  if [[ $rc -eq 0 && -f /root/affine_data/h9_sim_result.json ]]; then
    break
  fi
  echo "[h9-n80] attempt=$attempt failed rc=$rc; sleep 30"
  sleep 30
done
