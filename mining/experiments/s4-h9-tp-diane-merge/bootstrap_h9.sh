#!/usr/bin/env bash
# mine-h9-1 bootstrap: pinned eval stack + TalentPigs/diane613/teacher → merge α0.75.
# Secrets from /root/mine.env only (never argv).
set -euo pipefail

LOG=/root/logs/bootstrap_h9.log
mkdir -p /root/logs /root/hf /root/affine_data /root/mining_src /root/merges
exec > >(tee -a "$LOG") 2>&1

echo "[bootstrap-h9] $(date -u +%Y-%m-%dT%H:%M:%SZ) start host=$(hostname)"
...
if [[ -z "${HF_TOKEN:-}" ]]; then
  echo "[bootstrap-h9] FATAL: HF_TOKEN missing in /root/mine.env"
  exit 1
fi
...
uv pip install \
  ...
  "numpy" \
  "scipy" \
  2>&1 | tee /root/logs/pip_h9.log | tail -40

python - <<'PY'
import torch, transformers, vllm
print("[bootstrap-h9] VERSIONS",
...
jobs = [
    ...
    ("diane613/Affine-5CQLBK7Mmw1vsk7eQcBok9Qn44JNU5YVrfNmZpJHPxLV271B",
     "ad0f3f116e44dc5154ca3f72b933faaefc4905fa", "diane"),
    ("zai-org/GLM-4.5-Air-FP8", None, "teacher"),
]
for repo, rev, label in jobs:
    print(f"[bootstrap-h9] DOWNLOAD start {label} {repo} rev={rev}", flush=True)
    path = snapshot_download(repo, revision=rev, token=token)
    print(f"[bootstrap-h9] DOWNLOAD done {label} -> {path}", flush=True)
    open(f"/root/logs/{label}.done", "w").write(path + "\n")
print("[bootstrap-h9] ALL_DOWNLOADS_OK", flush=True)
PY
...
python /root/mining_src/s4-h2-merge/merge_linear.py \
  ...
  --b-repo diane613/Affine-5CQLBK7Mmw1vsk7eQcBok9Qn44JNU5YVrfNmZpJHPxLV271B \
  --b-rev ad0f3f116e44dc5154ca3f72b933faaefc4905fa \
  --alpha "$ALPHA" \
  --out "$OUT" \
  --hf-home "$HF_HOME" \
  2>&1 | tee /root/logs/h9_merge.log

date -u +%Y-%m-%dT%H:%M:%SZ > /root/logs/h9_merge.done
echo "[bootstrap-h9] MERGE_DONE alpha=$ALPHA out=$OUT"
...
export CHALL_REPO=/root/merges/h9-tp75
export CHALL_REV=
bash /root/mining_src/s3-duel-sim/serve_three.sh

touch /root/logs/bootstrap_h9.done
echo "[bootstrap-h9] $(date -u +%Y-%m-%dT%H:%M:%SZ) DONE — engines launching; next: wait_ready + n80"
