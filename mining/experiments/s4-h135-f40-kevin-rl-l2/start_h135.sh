#!/usr/bin/env bash
# H135/F40: kevin954-init REINFORCE on teacher Λ2 (needs :8000 teacher ready).
set -euo pipefail
...
n=$(wc -l <"$DATA")
echo "[h135] $(date -u +%Y-%m-%dT%H:%M:%SZ) examples=$n data=$DATA base=$BASE gpus=$CUDA_VISIBLE_DEVICES"
test "$n" -ge 200

echo "[h135] wait teacher at $TEACHER_URL"
for i in $(seq 1 240); do
  if curl -sf --max-time 5 "$TEACHER_URL/models" >/dev/null 2>&1; then
    echo "[h135] teacher ready after ${i}×15s"
    break
  fi
  if (( i == 240 )); then
    echo "[h135] FATAL: teacher never came up"
    exit 1
  fi
  sleep 15
done
...
echo "[h135] $(date -u +%Y-%m-%dT%H:%M:%SZ) launch F40 REINFORCE-teacher-Λ2 lr=5e-6 r=16 G=2 on GPUs $CUDA_VISIBLE_DEVICES"
...
meta = {
    ...
    "out": "$TRAIN_DIR",
    "log": "$LOG",
    "note": "F40 kevin954×teacher-Λ2 REINFORCE: reward=lpC(y|z)-lpC(y|∅); non-king base",
}
Path("/root/affine_data/h135_train_launched.json").write_text(json.dumps(meta, indent=2) + "\n")
print(json.dumps(meta, indent=2))
print("[h135] TRAIN_LAUNCHED pid=%s" % meta["pid"])
PY
