#!/usr/bin/env bash
# Host → mine-f40-1: upload stack + data, start bootstrap under nohup.
set -euo pipefail

ROOT=/home/const/subnet120
DST_HOST=${DST_HOST:?set DST_HOST}
DST_PORT=${DST_PORT:?set DST_PORT}
KNOWN=${KNOWN:-/tmp/mine-f40-1.known_hosts}
...
STAGE=$(mktemp -d /tmp/mine-h135-stack.XXXXXX)
...
test -f "$STAGE/$EXP/bootstrap_h135.sh"
test -f "$STAGE/$EXP/start_h135.sh"

TAR=/tmp/mine-h135-stack.tar.gz
tar -C "$STAGE" -czf "$TAR" .
ls -lh "$TAR"

ENV_TMP=$(mktemp /tmp/mine-h135.env.XXXXXX)
...
# H135 data = H27 winner_za_high_l1 (406 ex); init = kevin954 @6a5815fa; n80 king = Tok.
DATA="$ROOT/mining/experiments/s4-h27-clip-l1-shape/results/winner_za_high_l1.jsonl"
test -s "$DATA"
test "$(wc -l <"$DATA")" -ge 300

"${SSH[@]}" 'mkdir -p /root/mining_src /root/affine_data /root/logs /root/h135 /root/hf'
"${SCP[@]}" "$TAR" "root@${DST_HOST}:/tmp/mine-h135-stack.tar.gz"
...
for repo in (
    "unconst/Affine-5czsc2fc98-h135-lora",
    "unconst/Affine-5czsc2fc98-h135-merged",
):
    try:
...
"${SSH[@]}" 'set -e
  tar -C /root/mining_src -xzf /tmp/mine-h135-stack.tar.gz
  ...
  nohup bash /root/mining_src/s4-h135-f40-kevin-rl-l2/bootstrap_h135.sh \
    >/root/logs/h135_pipeline.nohup 2>&1 &
  echo $! > /root/logs/h135_pipeline.pid
  ...
  nohup bash /root/mining_src/s4-h135-f40-kevin-rl-l2/watch_preempt_bare_tcache_pass264.sh \
    >/root/logs/h135_preempt_bare_pass264.launch.nohup 2>&1 &
  echo $! > /root/logs/h135_preempt_bare_pass264.pid
  echo PREEMPT_PID=$(cat /root/logs/h135_preempt_bare_pass264.pid)
  echo PIPELINE_PID=$(cat /root/logs/h135_pipeline.pid)
  sleep 3
  head -n 40 /root/logs/bootstrap_h135.log 2>/dev/null || head -n 40 /root/logs/h135_pipeline.nohup || true
  ps -p "$(cat /root/logs/h135_pipeline.pid)" -o pid,etime,cmd || true
'
