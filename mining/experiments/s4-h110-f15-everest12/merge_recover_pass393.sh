#!/usr/bin/env bash
# H110/F15 pass393: GPU merge save hung on gocryptfs request_wait_answer
...
LOG=/root/logs/h110_merge_recover_pass393.nohup
PIDF=/root/logs/h110_merge_recover_pass393.pid
MERGED=/root/h110/merged
...
log() { echo "[h110-merge393] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }

mkdir -p /root/logs /root/affine_data
: >"$LOG"
echo $$ >"$PIDF"

MERGE_PID=$(ps -eo pid,cmd | awk '/[p]ython .*merge_lora.py --base .*h110\/merged/{print $1; exit}')
POST_PID=$(ps -eo pid,cmd | awk '/[b]ash .*s4-h110-f15-everest12\/post_train_pipeline\.sh/{print $1; exit}')
