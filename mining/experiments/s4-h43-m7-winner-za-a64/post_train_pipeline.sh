#!/usr/bin/env bash
# After H43 train.done: merge LoRA → chall:8002 → n80 vs TalentPigs king.
...
TRAIN_DIR=${TRAIN_DIR:-/root/h43/train}
ADAPTER=${ADAPTER:-$TRAIN_DIR/adapter}
CKPT_ROOT=${CKPT_ROOT:-$TRAIN_DIR/checkpoints}
MERGED=${MERGED:-/root/h43/merged}
SIM_N80=/root/affine_data/h43_sim_result.json
PROG=/root/affine_data/h43_sim_progress.json
LOG=/root/logs/h43_pipeline.nohup
SOFT_DEADLINE_UTC=${SOFT_DEADLINE_UTC:-2026-08-08T04:30:00Z}
DEADMAN_UTC=${DEADMAN_UTC:-2026-08-08T05:30:00Z}

log() { echo "[h43-pipe] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }
...
_train_alive() {
  if [[ -f /root/logs/h43_train.pid ]]; then
    local tpid
    tpid=$(cat /root/logs/h43_train.pid 2>/dev/null || true)
...
_abort_on_exit() {
  ...
  if [[ -f /root/logs/h43_pipeline.done ]]; then
    return 0
  fi
  if [[ ! -f /root/logs/h43_pipeline.aborted ]]; then
    echo "aborted_err_rc=${rc} $(date -u +%Y-%m-%dT%H:%M:%SZ)" \
      >/root/logs/h43_pipeline.aborted
    echo "[h43-pipe] $(date -u +%Y-%m-%dT%H:%M:%SZ) EXIT trap wrote aborted_err_rc=${rc}" \
      | tee -a "$LOG" >/dev/null 2>&1 || true
  fi
}
trap _abort_on_exit EXIT

mkdir -p /root/logs /root/affine_data /root/h43
rm -f /root/logs/h43_pipeline.aborted /root/logs/h43_pipeline.done \
  /root/logs/h43_merge.done /root/logs/h43_chall_serve.done \
  /root/logs/h43_sim_n80.done
...
while true; do
  ...
  if (( now > soft - 3600 )); then
    ...
    echo "aborted_no_train $(date -u +%Y-%m-%dT%H:%M:%SZ)" > /root/logs/h43_pipeline.aborted
    exit 1
  fi
  _wait_i=$((_wait_i + 1))
...
if _train_alive; then
  ...
  echo "aborted_train_stuck $(date -u +%Y-%m-%dT%H:%M:%SZ)" > /root/logs/h43_pipeline.aborted
  exit 1
fi
sleep 15
...
python /root/mining_src/s4-h1-sft/merge_lora.py \
  ...
  --device-map auto \
  | tee -a "$LOG"
cp -f "$MERGED/merge_meta.json" /root/affine_data/h43_merge_meta.json 2>/dev/null || true
date -u +%Y-%m-%dT%H:%M:%SZ > /root/logs/h43_merge.done
...
Path("/root/affine_data/h43_identity.json").write_text(
...
print("[h43] OK_NON_IDENTICAL", flush=True)
PY

HF_LORA_REPO=${HF_LORA_REPO:-unconst/Affine-5czsc2fc98-h43-lora}
HF_MERGED_REPO=${HF_MERGED_REPO:-unconst/Affine-5czsc2fc98-h43-merged}
HF_BASE_HUB=${HF_BASE_HUB:-Radiant28/5eqdtdzqle-ckpt1000-m7}
SEEN_MID=${SEEN_MID:-/root/h43/mid_ckpt_salvaged.txt}
if [[ -n "${HF_TOKEN:-}" ]]; then
  if [[ -f "$SEEN_MID" ]] && grep -qx "adapter-final" "$SEEN_MID"; then
    ...
    echo "{\"skipped\":true,\"reason\":\"mid adapter-final present\"}" \
      >/root/affine_data/h43_adapter_salvage.json
    rm -f /root/logs/h43_push_adapter.pid
  elif pgrep -f "s4-h43-m7-winner-za-a64/mid_ckpt_salvage.sh" >/dev/null 2>&1; then
    ...
    nohup python3 /root/mining_src/s4-h1-sft/salvage_adapter.py \
      ...
      --commit-message "H43 m7-init winner-zA thought LoRA salvage (TTL insurance; not a submission)" \
      --out-meta /root/affine_data/h43_adapter_salvage.json \
      >>/root/logs/h43_push_adapter.nohup 2>&1 &
    echo $! >/root/logs/h43_push_adapter.pid
  fi
  ...
  nohup python3 /root/mining_src/s4-h1-sft/push_merged.py \
    ...
    --repo "$HF_MERGED_REPO" \
    --public \
    --commit-message "H43 m7-init winner-zA thought merged salvage (TTL insurance; not a submission)" \
    --out-meta /root/affine_data/h43_merged_salvage.json \
    >>/root/logs/h43_push_merged.nohup 2>&1 &
  echo $! >/root/logs/h43_push_merged.pid
  log "adapter push pid=$(cat /root/logs/h43_push_adapter.pid 2>/dev/null || echo skipped) merged push pid=$(cat /root/logs/h43_push_merged.pid)"
else
  log "WARN: HF_TOKEN unset; skipping H43 HF salvage pushes"
fi
...
if [[ "$code_t" != "200" || "$code_k" != "200" ]]; then
  ...
  echo "aborted_engines_unhealthy $(date -u +%Y-%m-%dT%H:%M:%SZ)" > /root/logs/h43_pipeline.aborted
  exit 1
fi
...
date -u +%Y-%m-%dT%H:%M:%SZ > /root/logs/h43_chall_serve.done
...
if (( dead - now < 2400 )); then
  ...
  echo "aborted_no_n80_budget $(date -u +%Y-%m-%dT%H:%M:%SZ)" > /root/logs/h43_pipeline.aborted
  exit 1
fi

N80_MAX_ATTEMPTS=${N80_MAX_ATTEMPTS:-3}
# Fresh block_hash per attempt (H32/H34/H43): default 0*64 dies teacher 400 @~40/80.
...
for attempt in $(seq 1 "$N80_MAX_ATTEMPTS"); do
  ...
  if ps -eo args | awk '/[r]un_sim_duel.py/ && /local-h43/' | grep -q .; then
    log "n80 already running under retry — skip post_train launch"
    n80_ok=1
    break
  fi
  if ps -eo args | awk '/[w]atch_n80_retry\.sh/ && / h43 /' | grep -q . \
    || ps -eo args | awk '/[r]etry_h43_n80\.sh/' | grep -q .; then
    log "watch_n80_retry/retry_h43 armed — defer n80 to retry; skip post_train launch"
    n80_ok=1
    break
  fi
  rm -f "$SIM_N80" "$PROG" /root/logs/h43_sim_n80.done
  ...
  python /root/mining_src/s4-h2-merge/run_sim_duel.py \
    ...
    --progress-out "$PROG" \
    --save-artifact \
    2>&1 | tee -a /root/logs/h43_sim.nohup
  sim_rc=${PIPESTATUS[0]}
  set -e
...
date -u +%Y-%m-%dT%H:%M:%SZ > /root/logs/h43_sim_n80.done
date -u +%Y-%m-%dT%H:%M:%SZ > /root/logs/h43_pipeline.done
