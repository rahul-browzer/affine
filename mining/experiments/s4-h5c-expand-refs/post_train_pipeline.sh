#!/usr/bin/env bash
# After H5c train.done: merge LoRA → chall:8002 → n80 vs TalentPigs king.
# Base = kevin; sim king = live TalentPigs. Prewarm may already hold teacher+king.
set -euo pipefail
...
TRAIN_DIR=${TRAIN_DIR:-/root/h5c/train}
ADAPTER=${ADAPTER:-$TRAIN_DIR/adapter}
CKPT_ROOT=${CKPT_ROOT:-$TRAIN_DIR/checkpoints}
MERGED=${MERGED:-/root/h5c/merged}
SIM_N80=/root/affine_data/h5c_sim_result.json
PROG=/root/affine_data/h5c_sim_progress.json
LOG=/root/logs/h5c_pipeline.nohup
...
_abort_on_exit() {
  ...
  if [[ -f /root/logs/h5c_pipeline.done ]]; then
    return 0
  fi
  if [[ ! -f /root/logs/h5c_pipeline.aborted ]]; then
    echo "aborted_err_rc=${rc} $(date -u +%Y-%m-%dT%H:%M:%SZ)" \
      >/root/logs/h5c_pipeline.aborted
    echo "[h5c-pipe] $(date -u +%Y-%m-%dT%H:%M:%SZ) EXIT trap wrote aborted_err_rc=${rc}" \
...
rm -f /root/logs/h5c_pipeline.aborted /root/logs/h5c_pipeline.done \
  /root/logs/h5c_merge.done /root/logs/h5c_chall_serve.done \
...
python /root/mining_src/s4-h1-sft/merge_lora.py \
  ...
  --device-map auto \
  | tee -a "$LOG"
cp -f "$MERGED/merge_meta.json" /root/affine_data/h5c_merge_meta.json 2>/dev/null || true
date -u +%Y-%m-%dT%H:%M:%SZ > /root/logs/h5c_merge.done

# Refuse weight-identical to kevin base OR TalentPigs king.
...
base_probe = probe(base, "kevin_base")
...
if [[ -n "${HF_TOKEN:-}" ]]; then
  if [[ -f "$SEEN_MID" ]] && grep -qx "adapter-final" "$SEEN_MID"; then
    ...
    echo "{\"skipped\":true,\"reason\":\"mid adapter-final present\"}" \
      >/root/affine_data/h5c_adapter_salvage.json
    rm -f /root/logs/h5c_push_adapter.pid
  elif pgrep -f "s4-h5c-expand-refs/mid_ckpt_salvage.sh" >/dev/null 2>&1; then
    ...
    nohup python3 /root/mining_src/s4-h1-sft/salvage_adapter.py \
      ...
      --commit-message "H5c kevin-init thought LoRA salvage (TTL insurance; not a submission)" \
      --out-meta /root/affine_data/h5c_adapter_salvage.json \
      >>/root/logs/h5c_push_adapter.nohup 2>&1 &
  ...
  echo $! >/root/logs/h5c_push_merged.pid
  log "adapter push pid=$(cat /root/logs/h5c_push_adapter.pid 2>/dev/null || echo skipped) merged push pid=$(cat /root/logs/h5c_push_merged.pid)"
else
  log "WARN: HF_TOKEN unset; skipping H5c HF salvage pushes"
fi
...
if [[ "$code_t" != "200" || "$code_k" != "200" ]]; then
  log "teacher/king not ready (t=$code_t k=$code_k); launching via serve_three (chall placeholder=kevin)"
...
if [[ "$code_t" != "200" || "$code_k" != "200" ]]; then
  ...
  echo "aborted_engines_unhealthy $(date -u +%Y-%m-%dT%H:%M:%SZ)" > /root/logs/h5c_pipeline.aborted
  exit 1
fi
...
date -u +%Y-%m-%dT%H:%M:%SZ > /root/logs/h5c_chall_serve.done
...
if (( dead - now < 2400 )); then
  ...
  echo "aborted_no_n80_budget $(date -u +%Y-%m-%dT%H:%M:%SZ)" > /root/logs/h5c_pipeline.aborted
  exit 1
fi
...
for attempt in $(seq 1 "$N80_MAX_ATTEMPTS"); do
  ...
  rm -f "$SIM_N80" "$PROG" /root/logs/h5c_sim_n80.done
  ...
  python /root/mining_src/s4-h2-merge/run_sim_duel.py \
    ...
    --progress-out "$PROG" \
    --save-artifact \
    2>&1 | tee -a /root/logs/h5c_sim.nohup
  sim_rc=${PIPESTATUS[0]}
  set -e
...
date -u +%Y-%m-%dT%H:%M:%SZ > /root/logs/h5c_sim_n80.done
date -u +%Y-%m-%dT%H:%M:%SZ > /root/logs/h5c_pipeline.done
