#!/usr/bin/env bash
# Pass 300: king-only relaunch after EngineDeadError / orphan workers on :8001.
...
LOGN=/root/logs/h76_king_recover_pass300.nohup
log() { echo "[king300-h76] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOGN"; }

log "START king-only recover (EngineDead @12:17Z; orphans ppid=1 on gpus2,3)"
...
for line in apps.strip().splitlines():
    ...
    if len(parts) >= 2 and parts[0] in uuids:
        try:
            ...
            pass
print(f"[king300-h76] reap gpus2,3 pids={sorted(pids)}")
for pid in pids:
    try:
...
for line in out.strip().splitlines():
    ...
    if int(idx) in (2, 3):
        print(f"[king300-h76] gpu{idx} used_mib={used}")
        if float(used) < 500:
            ok += 1
if ok < 2:
    print("[king300-h76] ABORT: gpus 2,3 not free")
    sys.exit(1)
PY
  ...
  if [[ "$code" == "200" ]]; then
    ...
    if [[ -n "$mid" ]]; then
      ...
      if [[ "$pcode" == "200" ]]; then
        ...
        date -u +%Y-%m-%dT%H:%M:%SZ >/root/logs/h76_king_recover_pass300.done
        echo "$REPO $REV" >>/root/logs/h76_king_recover_pass300.done
