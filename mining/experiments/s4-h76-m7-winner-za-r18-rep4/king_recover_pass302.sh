...
LOGN=/root/logs/h76_king_recover_pass302.nohup
log() { echo "[king302-h76] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOGN"; }

log "START king-only recover (re-fire: pass300 stuck orphan TP1 ppid=1 GPU3; APIServer gone)"
...
for line in apps.strip().splitlines():
    ...
    if len(parts) >= 2 and parts[0] in uuids:
        try:
            ...
            pass
print(f"[king302-h76] reap gpus2,3 pids={sorted(pids)}")
for pid in pids:
    try:
...
rm -f /root/affine_data/h76_sim_result.json /root/affine_data/h76_sim_progress.json \
  /root/affine_data/h76_decision.json /root/logs/h76_n80.done /root/logs/h76_n80_retry.aborted
log "cleared stale h76 sim progress/result"
...
for line in out.strip().splitlines():
    ...
    if int(idx) in (2, 3):
        print(f"[king302-h76] gpu{idx} used_mib={used}")
        if float(used) < 500:
            ok += 1
if ok < 2:
    print("[king302-h76] ABORT: gpus 2,3 not free")
    sys.exit(1)
PY
  ...
  if [[ "$code" == "200" ]]; then
    ...
    if [[ -n "$mid" ]]; then
      ...
      if [[ "$pcode" == "200" ]]; then
        ...
        date -u +%Y-%m-%dT%H:%M:%SZ >/root/logs/h76_king_recover_pass302.done
        echo "$REPO $REV" >>/root/logs/h76_king_recover_pass302.done
        log "DONE — retry_h76_n80 should proceed (watcher already armed)"
        exit 0
      fi
    fi
  fi
