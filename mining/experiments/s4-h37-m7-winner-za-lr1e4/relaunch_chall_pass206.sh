#!/usr/bin/env bash
# Pass 206: H37 chall p205 died on first completions (__triton_launcher.so) (__triton_launcher.so).
# Quarantine any false decision, wipe→settle20s→unique TCACHE, rearm watcher.
set -euo pipefail
...
for f in h37_decision.json h37_sim_result.json h37_sim_result_artifact.json \
  ...
  if [[ -f "/root/affine_data/$f" ]]; then
    mv "/root/affine_data/$f" "$QDIR/${f%.json}_pass206_${ts}.json"
...
print(f"[recover206-h37-chall] reap gpu={sorted(want)} workers={sorted(pids)} parents={sorted(parents)}")
for pid in kill_set:
    try:
