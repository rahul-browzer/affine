#!/usr/bin/env bash
# Pass 205: H37 chall p204 died again mid-load (__triton_launcher.so).
...
LOG=/root/logs/h37_chall_recover_pass205.log
...
log() { echo "[recover205-h37-chall] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }
...
print(f"[recover205-h37-chall] reap gpu={sorted(want)} workers={sorted(pids)} parents={sorted(parents)}")
for pid in kill_set:
    try:
