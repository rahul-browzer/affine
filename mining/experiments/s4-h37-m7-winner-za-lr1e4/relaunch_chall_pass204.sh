#!/usr/bin/env bash
# Pass 204: H37 chall died on first completions (__triton_launcher.so) →
# false REFUTE (ConnectError). Quarantine decision, wipe→settle→unique TCACHE,
# rearm watcher. Completions-gated retry prevents re-fire on health-only.
set -euo pipefail
...
LOG=/root/logs/h37_chall_recover_pass204.log
CHALL_LOG=/root/logs/vllm_chall.log
PIDF=/root/logs/vllm_chall.pid
MERGE=/root/h37/merged
export GPUS=4,5
UTIL=0.72
TAG=chall_p204_$(date +%s)_$$
TCACHE=/root/.triton/cache/$TAG
QDIR=/root/affine_data/false_probes
mkdir -p /root/logs /root/affine_data "$QDIR"
: >"$LOG"

log() { echo "[recover204-h37-chall] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }

log "START quarantine false ConnectError REFUTE + chall relaunch"

# Quarantine false probe artifacts so watcher/retry will re-fire n80.
ts=$(date -u +%Y%m%dT%H%M%SZ)
for f in h37_decision.json h37_sim_result.json h37_sim_result_artifact.json \
         h37_sim_progress.json; do
  if [[ -f "/root/affine_data/$f" ]]; then
    mv "/root/affine_data/$f" "$QDIR/${f%.json}_pass204_${ts}.json"
    log "quarantine $f → $QDIR"
  fi
done
rm -f /root/logs/h37_n80.done /root/logs/h37_n80_retry.aborted \
  /root/logs/h37_pipeline.aborted /root/logs/h37_chall_serve.done

# Kill any leftover sim / waiters.
while read -r pid; do
  [[ -n "${pid:-}" ]] || continue
...
print(f"[recover204-h37-chall] reap gpu={sorted(want)} workers={sorted(pids)} parents={sorted(parents)}")
for pid in kill_set:
    try:
...
# Rearm watcher (may have exited on false decision).
pkill -f 'watch_n80_retry\.sh h37 ' 2>/dev/null || true
sleep 1
