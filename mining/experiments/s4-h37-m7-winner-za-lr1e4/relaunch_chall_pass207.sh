#!/usr/bin/env bash
# Pass 207: H37 chall died again on __triton_launcher.so (brief health→500→
# false_probe ConnectError). Quarantine decision, wipe→settle20s→unique TCACHE,
# rearm watcher. Completions-gated retry + double-probe in retry_h37_n80.sh.
set -euo pipefail
...
LOG=/root/logs/h37_chall_recover_pass207.log
...
log() { echo "[recover207-h37-chall] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }
...
print(f"[recover207-h37-chall] reap gpu={sorted(want)} workers={sorted(pids)} parents={sorted(parents)}")
for pid in kill_set:
    try:
