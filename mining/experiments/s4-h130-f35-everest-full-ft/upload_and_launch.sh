#!/usr/bin/env bash
# Host → mine-f35-1: upload stack + data, start H130 full-FT bootstrap.
set -euo pipefail

ROOT=/home/const/subnet120
DST_HOST=${DST_HOST:?set DST_HOST}
DST_PORT=${DST_PORT:?set DST_PORT}
KNOWN=${KNOWN:-/tmp/mine-f35-1.known_hosts}
...
STAGE=$(mktemp -d /tmp/mine-h130-stack.XXXXXX)
trap 'rm -rf "$STAGE"' EXIT

EXP=s4-h130-f35-everest-full-ft
...
TAR=/tmp/mine-h130-stack.tar.gz
tar -C "$STAGE" -czf "$TAR" .
ls -lh "$TAR"

ENV_TMP=$(mktemp /tmp/mine-h130.env.XXXXXX)
...
"${SSH[@]}" 'mkdir -p /root/mining_src /root/affine_data /root/logs /root/h130 /root/hf'
"${SCP[@]}" "$TAR" "root@${DST_HOST}:/tmp/mine-h130-stack.tar.gz"
"${SCP[@]}" "$ENV_TMP" "root@${DST_HOST}:/root/mine.env"
"${SCP[@]}" "$DATA" "root@${DST_HOST}:/root/h130/winner_za_high_l2.jsonl"
rm -f "$ENV_TMP"
...
repo = "unconst/Affine-5czsc2fc98-h130-everestfullft"
try:
...
"${SSH[@]}" 'set -e
  tar -C /root/mining_src -xzf /tmp/mine-h130-stack.tar.gz
  chmod 600 /root/mine.env
  chmod +x /root/mining_src/s3-duel-sim/*.sh \
           /root/mining_src/s4-h2-merge/*.sh \
           /root/mining_src/s4-h130-f35-everest-full-ft/*.sh
  test -f /root/mining_src/affine_pkg/affine/score.py
  test -s /root/h130/winner_za_high_l2.jsonl
  test -x /root/mining_src/s4-h130-f35-everest-full-ft/bootstrap_h130.sh
  echo STACK_UPLOAD_OK
  nohup bash /root/mining_src/s4-h130-f35-everest-full-ft/bootstrap_h130.sh \
    >/root/logs/h130_pipeline.nohup 2>&1 &
  echo $! > /root/logs/h130_pipeline.pid
  nohup bash /root/mining_src/s4-h2-merge/watch_form_decision.sh h130 \
    /root/affine_data/h130_sim_result.json /root/affine_data/h130_decision.json \
    /root/logs/h130_form_decision.nohup \
    >/root/logs/h130_form_decision.launch.out 2>&1 &
  ...
  echo PIPELINE_PID=$(cat /root/logs/h130_pipeline.pid)
  sleep 5
  head -n 40 /root/logs/bootstrap_h130.log 2>/dev/null || head -n 40 /root/logs/h130_pipeline.nohup || true
  ps -p "$(cat /root/logs/h130_pipeline.pid)" -o pid,etime,cmd || true
  nvidia-smi -L | wc -l
'
