#!/usr/bin/env bash
# Host → mine-r14-kevin-rl-1: H135 kevin-RL stack + R14 Reason overlays, bootstrap.
# Axis R14: kevin954 REINFORCE on teacher Reason (≠ R3/R8 Tok RL, ≠ R5b Talent FT).
set -euo pipefail

ROOT=/home/const/subnet120
POD_NAME=${POD_NAME:-mine-r14-kevin-rl-1}
...
STAGE=$(mktemp -d /tmp/mine-r14-stack.XXXXXX)
...
cp -a "$ROOT/mining/experiments/r14-kevin-rl/start_r14.sh" "$STAGE/r14-kevin-rl/"
cp -a "$ROOT/mining/experiments/r14-kevin-rl/bootstrap_r14.sh" "$STAGE/r14-kevin-rl/"
# Overlay: H135 bootstrap + start → R14 Reason kevin-REINFORCE.
...
"${SCP[@]}" "$TAR" "root@${DST_HOST}:/tmp/mine-r14-stack.tar.gz"
...
"${SSH[@]}" 'set -e
  tar -C /root/mining_src -xzf /tmp/mine-r14-stack.tar.gz
  ...
  # Prove overlay is R14, not stock H135.
  grep -q "R14: kevin-REINFORCE" /root/mining_src/s4-h135-f40-kevin-rl-l2/start_h135.sh
  grep -q "DOWNLOAD kevin-init" /root/mining_src/s4-h135-f40-kevin-rl-l2/bootstrap_h135.sh
  set -a; source /root/mine.env; set +a
  echo "R14_DEADLINES soft=$SOFT_DEADLINE_UTC dead=$DEADMAN_UTC axis=$R14_AXIS"
  echo STACK_UPLOAD_OK
  nohup bash /root/mining_src/s4-h135-f40-kevin-rl-l2/bootstrap_h135.sh \
    >/root/logs/r14_pipeline.nohup 2>&1 &
  echo $! > /root/logs/r14_pipeline.pid
  cp -f /root/logs/r14_pipeline.pid /root/logs/h135_pipeline.pid
  ...
  echo PIPELINE_PID=$(cat /root/logs/r14_pipeline.pid)
  sleep 5
  head -n 40 /root/logs/bootstrap_h135.log 2>/dev/null || head -n 40 /root/logs/r14_pipeline.nohup || true
  ps -p "$(cat /root/logs/r14_pipeline.pid)" -o pid,etime,cmd || true
  nvidia-smi -L | wc -l
'
