...
mkdir -p /root/logs /root/hf /root/affine_data /root/mining_src /root/h135 /root/r14
exec > >(tee -a "$LOG") 2>&1

echo "[bootstrap-r14] $(date -u +%Y-%m-%dT%H:%M:%SZ) start host=$(hostname)"
...
if [[ -z "${HF_TOKEN:-}" ]]; then
  echo "[bootstrap-r14] FATAL: HF_TOKEN missing in /root/mine.env"
  exit 1
fi

nvidia-smi -L || true
NGPU=$(nvidia-smi -L | wc -l)
echo "[bootstrap-r14] GPU_COUNT=$NGPU"
test "$NGPU" -ge 8
...
# Prove overlay is R14, not stock H135 / F40.
...
if p.is_file():
    ...
    if txt != orig:
        p.write_text(txt)
        print("[bootstrap-r14] patched vllm_client timeout=600 retries=5", flush=True)
    else:
        print("[bootstrap-r14] vllm_client already patched or pattern miss", flush=True)
PY

# Blocking DL: kevin-init + teacher (train needs both).
python - <<'PY'
import os
...
print("[bootstrap-r14] DOWNLOAD kevin-init start", repo, rev, flush=True)
path = snapshot_download(repo, revision=rev, token=token)
print(f"[bootstrap-r14] DOWNLOAD kevin-init done -> {path}", flush=True)
open("/root/logs/kevin_init.done", "w").write(path + "\n")
print("[bootstrap-r14] DOWNLOAD teacher start", flush=True)
tpath = snapshot_download("zai-org/GLM-4.5-Air-FP8", token=token)
print(f"[bootstrap-r14] DOWNLOAD teacher done -> {tpath}", flush=True)
open("/root/logs/teacher.done", "w").write(tpath + "\n")
PY

echo "[bootstrap-r14] $(date -u +%Y-%m-%dT%H:%M:%SZ) serving teacher before train"
...
cp -f /root/logs/h135_prewarm.pid /root/logs/r14_prewarm.pid

for i in $(seq 1 240); do
  if curl -sf --max-time 5 http://127.0.0.1:8000/v1/models >/dev/null 2>&1; then
    echo "[bootstrap-r14] teacher :8000 up at iter=$i"
    break
  fi
  if (( i == 240 )); then
    echo "[bootstrap-r14] FATAL teacher never up"
...
echo "[bootstrap-r14] $(date -u +%Y-%m-%dT%H:%M:%SZ) launching R14 kevin-REINFORCE train"
...
from huggingface_hub import snapshot_download
token = os.environ["HF_TOKEN"]
print("[bootstrap-r14] DOWNLOAD tok331102 king start", flush=True)
kpath = snapshot_download(
    "Tok331102/affine-5EqYW8McUc-af10",
    revision="eb8bf9a356a254f71faaa439e8abc3cfba572c53",
    token=token,
)
print(f"[bootstrap-r14] DOWNLOAD tok331102 done -> {kpath}", flush=True)
...
cp -f /root/logs/h135_extra_dl.pid /root/logs/r14_extra_dl.pid
...
cp -f /root/logs/h135_post_train.pid /root/logs/r14_post_train.pid

echo "[bootstrap-r14] $(date -u +%Y-%m-%dT%H:%M:%SZ) BOOTSTRAP_DONE train=$(cat /root/logs/h135_train.pid) post=$(cat /root/logs/h135_post_train.pid)"
