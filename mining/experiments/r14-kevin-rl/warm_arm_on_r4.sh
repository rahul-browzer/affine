#!/usr/bin/env bash
# Host → lean warm-arm R14 onto TKC-hot mine-r4-fullft-1 after R13 REFUTE.
# Keeps teacher:8000 + ckp333:8001; trains kevin-REINFORCE on GPUs 6–7.
set -euo pipefail
...
STAGE=$(mktemp -d /tmp/mine-r14-warm.XXXXXX)
...
mkdir -p "$STAGE/affine_pkg/affine" "$STAGE/affine_pkg/evalsrv" \
         ...
         "$STAGE/s4-h1v2-sft" "$STAGE/$EXP" "$STAGE/r14-kevin-rl"
...
cp -a "$ROOT/mining/experiments/r14-kevin-rl/"*.sh "$STAGE/r14-kevin-rl/"
cp -a "$ROOT/mining/experiments/r14-kevin-rl/plan.md" "$STAGE/r14-kevin-rl/"
# Overlay start/bootstrap for R14 identity grep.
cp -a "$ROOT/mining/experiments/r14-kevin-rl/start_r14.sh" "$STAGE/$EXP/start_h135.sh"
cp -a "$ROOT/mining/experiments/r14-kevin-rl/bootstrap_r14.sh" "$STAGE/$EXP/bootstrap_h135.sh"
...
TAR=/tmp/mine-r14-warm-stack.tar.gz
tar -C "$STAGE" -czf "$TAR" .
ls -lh "$TAR"
...
set +a
umask 077
ENV_TMP=$(mktemp /tmp/mine-r14.env.XXXXXX)
{
  ...
  echo "export HF_MERGED_REPO=unconst/Affine-5czsc2fc98-r14-kevin-rl"
  echo "export R14_AXIS=kevin_reinforce_reason"
  echo "export R14_LR=5e-6"
  echo "export R14_LORA_R=16"
  echo "export R14_LORA_ALPHA=32"
  echo "export R14_GROUP_SIZE=2"
  echo "export R14_MAX_STEPS=200"
  echo "export R14_MAX_NEW=256"
...
"${SSH[@]}" 'mkdir -p /root/mining_src /root/affine_data /root/logs /root/h135 /root/r14 /root/hf'
"${SCP[@]}" "$TAR" "root@${DST_HOST}:/tmp/mine-r14-warm-stack.tar.gz"
"${SCP[@]}" "$ENV_TMP" "root@${DST_HOST}:/root/mine.env"
"${SCP[@]}" "$DATA" "root@${DST_HOST}:/root/h135/winner_za_high_l1.jsonl"
"${SCP[@]}" "$DATA" "root@${DST_HOST}:/root/r14/winner_za_high_l1.jsonl"
rm -f "$ENV_TMP"
...
for repo in (
    "unconst/Affine-5czsc2fc98-r14-kevin-rl",
    "unconst/Affine-5czsc2fc98-r14-kevin-rl-lora",
):
    try:
...
"${SSH[@]}" 'set -euo pipefail
  tar -C /root/mining_src -xzf /tmp/mine-r14-warm-stack.tar.gz
  chmod 600 /root/mine.env
  chmod +x /root/mining_src/s4-h135-f40-kevin-rl-l2/*.sh \
           /root/mining_src/r14-kevin-rl/*.sh \
  ...
  # Stop leftover R13 lean/post/form by pidfile only (never pkill -f).
  for f in /root/logs/r13_lean_warm.pid /root/logs/r13_post_train.pid \
           /root/logs/h138_post_train.pid /root/logs/r13_form_decision.pid \
           /root/logs/h138_dec_watch.pid /root/logs/r13_form_decision.launch.out \
           /root/logs/h138_train.pid /root/logs/r13_train.pid; do
    if [[ -f "$f" ]]; then
  ...
  nohup bash /root/mining_src/r14-kevin-rl/lean_warm_boot.sh \
    >/root/logs/r14_lean_warm.nohup 2>&1 &
  echo $! >/root/logs/r14_lean_warm.pid
  sleep 15
  echo LEAN_PID=$(cat /root/logs/r14_lean_warm.pid)
  head -n 80 /root/logs/r14_lean_warm.log 2>/dev/null \
    || head -n 80 /root/logs/r14_lean_warm.nohup || true
  ps -p "$(cat /root/logs/r14_lean_warm.pid)" -o pid,etime,cmd || true
  test -f /root/logs/h135_train.pid && echo TRAIN_PID=$(cat /root/logs/h135_train.pid)
  test -f /root/logs/r14_post_train.pid && echo POST_PID=$(cat /root/logs/r14_post_train.pid)
...
echo "R14_WARM_ARM_OK pod=$POD_NAME soft=$SOFT dead=$DEAD $(date -u +%Y-%m-%dT%H:%M:%SZ)"
