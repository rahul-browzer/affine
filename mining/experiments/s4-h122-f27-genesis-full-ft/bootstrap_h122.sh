#!/usr/bin/env bash
# mine-f27-1: Genesis init download → H122 full-FT; bg teacher+Tok DL; post-train after train.done.
set -euo pipefail
...
echo "[bootstrap-h122] $(date -u +%Y-%m-%dT%H:%M:%SZ) start host=$(hostname)"
...
if [[ -z "${HF_TOKEN:-}" ]]; then
  echo "[bootstrap-h122] FATAL: HF_TOKEN missing in /root/mine.env"
  exit 1
fi

nvidia-smi -L || true
NGPU=$(nvidia-smi -L | wc -l)
echo "[bootstrap-h122] GPU_COUNT=$NGPU"
test "$NGPU" -ge 8
...
if not p.is_file():
    print("[bootstrap-h122] no vllm_client to patch", flush=True)
else:
    txt = p.read_text()
    orig = txt
    ...
    if txt != orig:
        p.write_text(txt)
        print("[bootstrap-h122] patched vllm_client timeout=480 retries=5", flush=True)
    else:
        print("[bootstrap-h122] vllm_client already patched or pattern miss", flush=True)
PY

# Train init = genesis (full-FT). King Tok downloaded in post_train / extra_dl.
python - <<'PY'
import os
...
print("[bootstrap-h122] DOWNLOAD genesis-init start", repo, rev, flush=True)
path = snapshot_download(repo, revision=rev, token=token)
print(f"[bootstrap-h122] DOWNLOAD genesis-init done -> {path}", flush=True)
open("/root/logs/genesis_init.done", "w").write(path + "\n")
assert rev in path, path
PY

echo "[bootstrap-h122] $(date -u +%Y-%m-%dT%H:%M:%SZ) launching H122 full-FT train"
...
from huggingface_hub import snapshot_download
token = os.environ["HF_TOKEN"]
print("[bootstrap-h122] DOWNLOAD teacher start", flush=True)
path = snapshot_download("zai-org/GLM-4.5-Air-FP8", token=token)
print(f"[bootstrap-h122] DOWNLOAD teacher done -> {path}", flush=True)
open("/root/logs/teacher.done", "w").write(path + "\n")
print("[bootstrap-h122] DOWNLOAD tok-king start", flush=True)
kpath = snapshot_download("Tok331102/affine-5EqYW8McUc-af10", revision="eb8bf9a356a254f71faaa439e8abc3cfba572c53", token=token)
print(f"[bootstrap-h122] DOWNLOAD tok-king done -> {kpath}", flush=True)
...
echo "[bootstrap-h122] $(date -u +%Y-%m-%dT%H:%M:%SZ) BOOTSTRAP_DONE train=$(cat /root/logs/h122_train.pid) post=$(cat /root/logs/h122_post_train.pid)"
