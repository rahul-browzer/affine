#!/usr/bin/env bash
# R22: golden-crown-init → Reason-GRPO; n80 king = live guass (p2229).
# Overlay: upload_and_launch copies this to r3-reason-grpo/bootstrap_r3.sh.
# Order: pip → DL golden+teacher+guass → serve teacher → train (6,7) → post_train.
set -euo pipefail

LOG=/root/logs/bootstrap_r3.log
mkdir -p /root/logs /root/hf /root/affine_data /root/mining_src /root/r3 /root/r22
exec > >(tee -a "$LOG") 2>&1

echo "[bootstrap-r22] $(date -u +%Y-%m-%dT%H:%M:%SZ) start host=$(hostname)"
...
if [[ -z "${HF_TOKEN:-}" ]]; then
  echo "[bootstrap-r22] FATAL: HF_TOKEN missing in /root/mine.env"
  exit 1
fi

nvidia-smi -L || true
NGPU=$(nvidia-smi -L | wc -l)
echo "[bootstrap-r22] GPU_COUNT=$NGPU"
test "$NGPU" -ge 8
...
# Prove overlay is R22, not stock R3 / R16 / R18–R21.
...
if p.is_file():
    ...
    if txt != orig:
        p.write_text(txt)
        print("[bootstrap-r22] patched vllm_client timeout=600 retries=5", flush=True)
    else:
        print("[bootstrap-r22] vllm_client already patched or pattern miss", flush=True)
PY

# Blocking DL: golden (train init) + guass (live n80 king) + teacher.
python - <<'PY'
import os
from huggingface_hub import snapshot_download
token = os.environ["HF_TOKEN"]
golden_repo = "golden-crown/Affine-5EpvnXGu8jUAVc67oPGgJ3brR4JZqjBUSaTKhZuBoNAAzSJF"
golden_rev = "ee37f4f0457df943d957435d7c9c24222a7ca93d"
print("[bootstrap-r22] DOWNLOAD golden-init start", golden_repo, golden_rev, flush=True)
golden = snapshot_download(golden_repo, revision=golden_rev, token=token)
print(f"[bootstrap-r22] DOWNLOAD golden-init done -> {golden}", flush=True)
open("/root/logs/golden.done", "w").write(golden + "\n")
open("/root/r22/r22_base.path", "w").write(golden + "\n")
assert golden_rev in golden, golden
...
print("[bootstrap-r22] DOWNLOAD guass-king start", king_repo, king_rev, flush=True)
kpath = snapshot_download(king_repo, revision=king_rev, token=token)
print(f"[bootstrap-r22] DOWNLOAD guass-king done -> {kpath}", flush=True)
...
print("[bootstrap-r22] DOWNLOAD teacher start", flush=True)
tpath = snapshot_download("zai-org/GLM-4.5-Air-FP8", token=token)
print(f"[bootstrap-r22] DOWNLOAD teacher done -> {tpath}", flush=True)
open("/root/logs/teacher.done", "w").write(tpath + "\n")
print("[bootstrap-r22] ALL_DOWNLOADS_OK", flush=True)
PY

export BASE=$(cat /root/r22/r22_base.path)
if [[ -f /root/mine.env ]]; then
...
echo "[bootstrap-r22] $(date -u +%Y-%m-%dT%H:%M:%SZ) serving teacher before train BASE=$BASE"
bash /root/mining_src/r3-reason-grpo/prewarm_engines.sh \
  >/root/logs/r3_prewarm.nohup 2>&1 &
echo $! >/root/logs/r3_prewarm.pid
cp -f /root/logs/r3_prewarm.pid /root/logs/r22_prewarm.pid

for i in $(seq 1 240); do
  if curl -sf --max-time 5 http://127.0.0.1:8000/v1/models >/dev/null 2>&1; then
    echo "[bootstrap-r22] teacher :8000 up at iter=$i"
    break
  fi
  if (( i == 240 )); then
    echo "[bootstrap-r22] FATAL teacher never up"
...
echo "[bootstrap-r22] $(date -u +%Y-%m-%dT%H:%M:%SZ) launching R22 golden Reason-GRPO train"
...
cp -f /root/logs/r3_extra_dl.pid /root/logs/r22_extra_dl.pid
...
cp -f /root/logs/r3_post_train.pid /root/logs/r22_post_train.pid

echo "[bootstrap-r22] $(date -u +%Y-%m-%dT%H:%M:%SZ) BOOTSTRAP_DONE train=$(cat /root/logs/r3_train.pid) post=$(cat /root/logs/r3_post_train.pid)"
