#!/usr/bin/env bash
# R22: golden-crown-init → Reason-GRPO (Tok kept as n80 king only).
# Overlay: upload_and_launch copies this to r3-reason-grpo/bootstrap_r3.sh.
# Order: pip → DL golden+teacher+Tok → serve teacher → train (6,7) → post_train.
set -euo pipefail
...
# Blocking DL: golden (train init) + Tok (n80 king) + teacher.
python - <<'PY'
import os
...
print("[bootstrap-r22] DOWNLOAD tok331102 king start", tok_repo, tok_rev, flush=True)
tok = snapshot_download(tok_repo, revision=tok_rev, token=token)
print(f"[bootstrap-r22] DOWNLOAD tok331102 done -> {tok}", flush=True)
