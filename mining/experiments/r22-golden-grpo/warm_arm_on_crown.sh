#!/usr/bin/env bash
# Host → lean warm-arm R22 onto TKC-hot mine-crown-1 after R33 REFUTE.
# Keeps teacher:8000 + guass:8001; trains golden-crown-init Reason-GRPO on GPUs 6–7.
set -euo pipefail
...
STAGE=$(mktemp -d /tmp/mine-r22-warm.XXXXXX)
...
cp -a "$ROOT/mining/experiments/r22-golden-grpo/"*.sh "$STAGE/r22-golden-grpo/"
cp -a "$ROOT/mining/experiments/r22-golden-grpo/plan.md" "$STAGE/r22-golden-grpo/"
# Overlay start for R22 identity grep.
...
TAR=/tmp/mine-r22-warm-stack.tar.gz
tar -C "$STAGE" -czf "$TAR" .
ls -lh "$TAR"
...
ENV_TMP=$(mktemp /tmp/mine-r22.env.XXXXXX)
{
  ...
  echo "export BASE=/root/hf/hub/models--golden-crown--Affine-5EpvnXGu8jUAVc67oPGgJ3brR4JZqjBUSaTKhZuBoNAAzSJF/snapshots/ee37f4f0457df943d957435d7c9c24222a7ca93d"
} >"$ENV_TMP"
chmod 600 "$ENV_TMP"
...
"${SCP[@]}" "$TAR" "root@${DST_HOST}:/tmp/mine-r22-warm-stack.tar.gz"
...
"${SSH[@]}" 'set -euo pipefail
  tar -C /root/mining_src -xzf /tmp/mine-r22-warm-stack.tar.gz
  ...
  # Stop leftover R33 post/form/sim by pidfile only (never pkill -f).
  for f in /root/logs/r33_lean_warm.pid /root/logs/r33_post_train.pid \
           /root/logs/r33_form_decision.pid /root/logs/r33_train.pid \
           /root/logs/r3_train.pid /root/logs/r3_post_train.pid \
           /root/logs/r3_pipeline.pid /root/logs/watch_n80_retry.pid; do
    if [[ -f "$f" ]]; then
  ...
  nohup bash /root/mining_src/r22-golden-grpo/lean_warm_boot.sh \
    >/root/logs/r22_lean_warm.nohup 2>&1 &
  echo $! >/root/logs/r22_lean_warm.pid
  sleep 40
  echo LEAN_PID=$(cat /root/logs/r22_lean_warm.pid)
  head -n 80 /root/logs/r22_lean_warm.log 2>/dev/null \
    || head -n 80 /root/logs/r22_lean_warm.nohup || true
  ps -p "$(cat /root/logs/r22_lean_warm.pid)" -o pid,etime,cmd || true
  test -f /root/logs/r3_train.pid && echo TRAIN_PID=$(cat /root/logs/r3_train.pid)
  test -f /root/logs/r22_post_train.pid && echo POST_PID=$(cat /root/logs/r22_post_train.pid)
...
echo "R22_WARM_ARM_OK pod=$POD_NAME soft=$SOFT dead=$DEAD $(date -u +%Y-%m-%dT%H:%M:%SZ)"
