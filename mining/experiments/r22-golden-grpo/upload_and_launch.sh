#!/usr/bin/env bash
# Host → mine-r22-golden-grpo-1: R3 stack + R22 Golden-GRPO overlays, bootstrap.
# Axis R22: golden-crown-init Reason-GRPO (≠ R3 Tok, ≠ R16 golden REINFORCE, ≠ R18–R21).
set -euo pipefail

ROOT=/home/const/subnet120
POD_NAME=${POD_NAME:-mine-r22-golden-grpo-1}
...
STAGE=$(mktemp -d /tmp/mine-r22-stack.XXXXXX)
...
mkdir -p "$STAGE/affine_pkg/affine" "$STAGE/affine_pkg/evalsrv" \
         ...
         "$STAGE/$EXP" "$STAGE/r22-golden-grpo"
...
cp -a "$ROOT/mining/experiments/r22-golden-grpo/plan.md" "$STAGE/r22-golden-grpo/"
cp -a "$ROOT/mining/experiments/r22-golden-grpo/start_r22.sh" "$STAGE/r22-golden-grpo/"
cp -a "$ROOT/mining/experiments/r22-golden-grpo/bootstrap_r22.sh" "$STAGE/r22-golden-grpo/"
# Overlay: bootstrap_r3 + start_r3 → R22 golden Reason-GRPO.
cp -a "$ROOT/mining/experiments/r22-golden-grpo/start_r22.sh" "$STAGE/$EXP/start_r3.sh"
cp -a "$ROOT/mining/experiments/r22-golden-grpo/bootstrap_r22.sh" "$STAGE/$EXP/bootstrap_r3.sh"
...
_rem_raw=$(lium describe "$POD_NAME" --json 2>/dev/null \
  ...
  || true)
if [[ -z "${_rem_raw}" ]]; then
  echo "[r22-up] FATAL: no billing.removal_scheduled_at for POD_NAME=$POD_NAME" >&2
  exit 1
fi
...
echo "[r22-up] Removal=$REMOVAL Soft=$SOFT Dead=$DEAD (from lium describe $POD_NAME)"
...
KING_LOCAL_DEFAULT=/root/hf/hub/models--ttttxxxxsada--Affine-5guassq3tu/snapshots/e86758f5080d1e373e5fbbd7b4fbf6af327aeb44
{
  ...
  echo "export HF_LORA_REPO=unconst/Affine-5czsc2fc98-r22-lora"
  echo "export HF_MERGED_REPO=unconst/Affine-5czsc2fc98-r22-merged"
  echo "export R22_AXIS=golden_grpo_reason"
  echo "export R22_LR=5e-6"
  echo "export R22_LORA_R=16"
  echo "export R22_LORA_ALPHA=32"
  echo "export R22_GROUP_SIZE=4"
  echo "export R22_MAX_STEPS=200"
...
"${SSH[@]}" 'mkdir -p /root/mining_src /root/affine_data /root/logs /root/r3 /root/r22 /root/hf'
"${SCP[@]}" "$TAR" "root@${DST_HOST}:/tmp/mine-r22-stack.tar.gz"
"${SCP[@]}" "$ENV_TMP" "root@${DST_HOST}:/root/mine.env"
"${SCP[@]}" "$DATA" "root@${DST_HOST}:/root/r3/winner_za_high_l1.jsonl"
"${SCP[@]}" "$DATA" "root@${DST_HOST}:/root/r22/winner_za_high_l1.jsonl"
rm -f "$ENV_TMP"
...
for repo in (
    "unconst/Affine-5czsc2fc98-r22-lora",
    "unconst/Affine-5czsc2fc98-r22-merged",
):
    try:
...
"${SSH[@]}" 'set -e
  tar -C /root/mining_src -xzf /tmp/mine-r22-stack.tar.gz
  ...
  chmod +x /root/mining_src/s3-duel-sim/*.sh \
           ...
           /root/mining_src/r22-golden-grpo/*.sh
  ...
  # Prove overlay is R22, not stock R3 / R16 / R21; sim king = guass; Reason writer present.
  grep -q "R22: Golden-GRPO" /root/mining_src/r3-reason-grpo/start_r3.sh
  grep -q "DOWNLOAD golden-init" /root/mining_src/r3-reason-grpo/bootstrap_r3.sh
  ...
  echo "R22_DEADLINES soft=$SOFT_DEADLINE_UTC dead=$DEADMAN_UTC axis=$R22_AXIS king=$KING_REPO@$KING_REV"
  echo "R22_KNOBS lr=${R22_LR} r=${R22_LORA_R} G=${R22_GROUP_SIZE}"
  echo STACK_UPLOAD_OK
  nohup bash /root/mining_src/r3-reason-grpo/bootstrap_r3.sh \
    >/root/logs/r22_pipeline.nohup 2>&1 &
  echo $! > /root/logs/r22_pipeline.pid
  cp -f /root/logs/r22_pipeline.pid /root/logs/r3_pipeline.pid
  ...
  echo PIPELINE_PID=$(cat /root/logs/r22_pipeline.pid)
  sleep 5
  head -n 40 /root/logs/bootstrap_r3.log 2>/dev/null || head -n 40 /root/logs/r22_pipeline.nohup || true
  ps -p "$(cat /root/logs/r22_pipeline.pid)" -o pid,etime,cmd || true
  nvidia-smi -L | wc -l
'
