#!/usr/bin/env bash
# Pass 302: king-only relaunch after EngineDeadError / orphan workers on :8001.
...
LOGN=/root/logs/h77_king_recover_pass302.nohup
log() { echo "[king302-h77] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOGN"; }

log "START king-only recover (king Triton ENOENT ghost @12:25Z; gpus2,3 free; teacher+chall up)"
...
for line in apps.strip().splitlines():
    ...
    if len(parts) >= 2 and parts[0] in uuids:
        try:
            ...
            pass
print(f"[king302-h77] reap gpus2,3 pids={sorted(pids)}")
for pid in pids:
    try:
...
for line in out.strip().splitlines():
    ...
    if int(idx) in (2, 3):
        print(f"[king302-h77] gpu{idx} used_mib={used}")
        if float(used) < 500:
            ok += 1
if ok < 2:
    print("[king302-h77] ABORT: gpus 2,3 not free")
    sys.exit(1)
PY
  ...
  if [[ "$code" == "200" ]]; then
    ...
    if [[ -n "$mid" ]]; then
      ...
      if [[ "$pcode" == "200" ]]; then
        ...
        date -u +%Y-%m-%dT%H:%M:%SZ >/root/logs/h77_king_recover_pass302.done
        echo "$REPO $REV" >>/root/logs/h77_king_recover_pass302.done
