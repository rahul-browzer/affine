#!/usr/bin/env bash
# Pass 308: king-only relaunch after EngineDeadError / orphan workers on :8001.
...
LOGN=/root/logs/h77_king_recover_pass308.nohup
log() { echo "[king308-h77] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOGN"; }

log "START king-only recover (pass306 hung: Worker_TP1 Triton ImportError NODUTTS…/__triton_launcher.so @12:51Z then EngineCore shm_broadcast hang; health=000; teacher+chall :8000/:8002=200; leave chall)"
...
for line in apps.strip().splitlines():
    ...
    if len(parts) >= 2 and parts[0] in uuids:
        try:
            ...
            pass
print(f"[king308-h77] reap gpus2,3 pids={sorted(pids)}")
for pid in pids:
    try:
...
rm -f /root/affine_data/h77_sim_result.json /root/affine_data/h77_sim_progress.json \
  /root/affine_data/h77_decision.json /root/logs/h77_n80.done /root/logs/h77_n80_retry.aborted
log "cleared stale h77 sim progress/result"
...
for line in out.strip().splitlines():
    ...
    if int(idx) in (2, 3):
        print(f"[king308-h77] gpu{idx} used_mib={used}")
        if float(used) < 500:
            ok += 1
if ok < 2:
    print("[king308-h77] ABORT: gpus 2,3 not free")
    sys.exit(1)
PY
...
for i in $(seq 1 180); do
  ...
  if [[ "$code" == "200" ]]; then
    ...
    if [[ -n "$mid" ]]; then
      pcode=$(curl -s -o /tmp/_probe_king300.json -w "%{http_code}" --max-time 90 \
        http://127.0.0.1:8001/v1/completions \
        -H 'Content-Type: application/json' \
        -d "{\"model\":\"${mid}\",\"prompt\":\"hi\",\"max_tokens\":2}" || true)
      if [[ "$pcode" == "200" ]]; then
        log "KING PROMPTABLE mid=$mid poll=$i"
        date -u +%Y-%m-%dT%H:%M:%SZ >/root/logs/h77_king_recover_pass308.done
        echo "$REPO $REV" >>/root/logs/h77_king_recover_pass308.done
        log "DONE — retry_h77_n80 should proceed (watcher already armed)"
        exit 0
      fi
    fi
  fi
