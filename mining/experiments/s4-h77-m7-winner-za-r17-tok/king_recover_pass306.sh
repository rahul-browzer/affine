#!/usr/bin/env bash
# Pass 306: king-only relaunch after EngineDeadError / orphan workers on :8001.
...
LOGN=/root/logs/h77_king_recover_pass306.nohup
log() { echo "[king306-h77] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOGN"; }

log "START king-only recover (EngineDead TimeoutError sample_tokens @12:44Z mid-n80 chall15; gpus2,3 free 0MiB; no orphans; teacher+chall :8000/:8002=200)"
...
for line in apps.strip().splitlines():
    ...
    if len(parts) >= 2 and parts[0] in uuids:
        try:
            ...
            pass
print(f"[king306-h77] reap gpus2,3 pids={sorted(pids)}")
for pid in pids:
    try:
...
for line in out.strip().splitlines():
    ...
    if int(idx) in (2, 3):
        print(f"[king306-h77] gpu{idx} used_mib={used}")
        if float(used) < 500:
            ok += 1
if ok < 2:
    print("[king306-h77] ABORT: gpus 2,3 not free")
    sys.exit(1)
PY
  ...
  if [[ "$code" == "200" ]]; then
    ...
    if [[ -n "$mid" ]]; then
      ...
      if [[ "$pcode" == "200" ]]; then
        ...
        date -u +%Y-%m-%dT%H:%M:%SZ >/root/logs/h77_king_recover_pass306.done
        echo "$REPO $REV" >>/root/logs/h77_king_recover_pass306.done
