#!/usr/bin/env bash
# Restart H114 n80 after crash. Engines must already be up.
...
MERGED=${MERGED:-/root/h114/chall}
SIM=/root/affine_data/h114_sim_result.json
PROG=/root/affine_data/h114_sim_progress.json
DEC=/root/affine_data/h114_decision.json
LOG=/root/logs/h114_n80_retry.nohup
MAX_ATTEMPTS=${MAX_ATTEMPTS:-6}

log() { echo "[h114-n80-retry] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }
  ...
  while (( i < max )); do
    if _engines_ok; then
      ...
      if _engines_ok; then
        ...
        rm -f /root/logs/h114_n80_retry.aborted
        return 0
      fi
...
if [[ -f "$DEC" ]]; then
  if python3 -c 'import json,sys; d=json.load(open(sys.argv[1])); sys.exit(0 if d.get("false_probe") else 1)' "$DEC" 2>/dev/null; then
    ...
    mv "$DEC" "/root/affine_data/false_probes/h114_decision_retryQ_${ts}.json"
    [[ -f "$SIM" ]] && mv "$SIM" "/root/affine_data/false_probes/h114_sim_retryQ_${ts}.json"
    rm -f /root/logs/h114_n80.done
...
if [[ -f "$SIM" ]]; then
  if python3 -c 'import json,sys; d=json.load(open(sys.argv[1])); sys.exit(0 if (d.get("false_probe") or "unpromptable" in str(d.get("rejection_reason","")) or "ConnectError" in str(d.get("rejection_reason",""))) else 1)' "$SIM" 2>/dev/null; then
    ts=$(date -u +%Y%m%dT%H%M%SZ)
    mkdir -p /root/affine_data/false_probes
    mv "$SIM" "/root/affine_data/false_probes/h114_sim_retryQ_${ts}.json"
    ...
    python3 /root/mining_src/s4-h2-merge/write_merge_decision.py \
      --hyp h114 --sim-result "$SIM" --out "$DEC"
    date -u +%Y-%m-%dT%H:%M:%SZ > /root/logs/h114_n80.done
    exit 0
  fi
fi
# Require python in argv — bare pgrep/awk patterns false-match (pass205).
if ps -eo pid,cmd | awk '/python/ && /[r]un_sim_duel.py/ && /local-h114/ { found=1 } END { exit !found }'; then
...
if ! _wait_engines "${WAIT_ENGINE_POLLS:-360}"; then
  ...
  echo "aborted_engines_unhealthy $(date -u +%Y-%m-%dT%H:%M:%SZ)" \
    >/root/logs/h114_n80_retry.aborted
  exit 1
fi
test -d "$MERGED"
test -f /root/logs/h114_merge.done
...
for attempt in $(seq 1 "$MAX_ATTEMPTS"); do
  ...
  python /root/mining_src/s4-h2-merge/run_sim_duel.py \
    ...
    --progress-out "$PROG" \
    --save-artifact \
    2>&1 | tee /root/logs/h114_n80.log
  rc=${PIPESTATUS[0]}
  set -e
    ...
    if _is_false_probe_sim "$SIM"; then
      ...
      [[ -f "${SIM%.json}_artifact.json" ]] && \
        mv "${SIM%.json}_artifact.json" \
          "/root/affine_data/false_probes/h114_artifact_retryQ_${ts}.json" || true
