#!/usr/bin/env bash
# mine-f19-1: raw kevin954 chall (no train) vs Tok king — F19/H114 screen.
set -euo pipefail

LOG=/root/logs/bootstrap_h114.log
mkdir -p /root/logs /root/hf /root/affine_data /root/mining_src /root/h114
exec > >(tee -a "$LOG") 2>&1

echo "[bootstrap-h114] $(date -u +%Y-%m-%dT%H:%M:%SZ) start host=$(hostname)"
...
if [[ -z "${HF_TOKEN:-}" ]]; then
  echo "[bootstrap-h114] FATAL: HF_TOKEN missing in /root/mine.env"
  exit 1
fi

nvidia-smi -L || true
NGPU=$(nvidia-smi -L | wc -l)
echo "[bootstrap-h114] GPU_COUNT=$NGPU"
test "$NGPU" -ge 8
...
uv pip install \
  ...
  "numpy" \
  "scipy" \
  2>&1 | tee /root/logs/pip_h114.log | tail -40

python - <<'PY'
import torch, transformers, vllm
print("[bootstrap-h114] VERSIONS",
...
if p.is_file():
    ...
    if txt != orig:
        p.write_text(txt)
        print("[bootstrap-h114] patched vllm_client timeout=480 retries=5", flush=True)
    else:
        print("[bootstrap-h114] vllm_client already patched or pattern miss", flush=True)
PY
...
print("[bootstrap-h114] DOWNLOAD kevin954 chall start", flush=True)
gpath = snapshot_download(
    "kevin954/Affine-5dfqbbh8ev-sft",
    revision="3fb79cfbf3a21c7a2d2cf3ac5161a9a46277c152",
    token=token,
)
print(f"[bootstrap-h114] DOWNLOAD kevin954 done -> {gpath}", flush=True)
chall = Path("/root/h114/chall")
...
Path("/root/logs/kevin_chall.done").write_text(gpath + "\n")
# retry script expects merge.done for legacy gate — stamp it (no merge occurred)
Path("/root/logs/h114_merge.done").write_text(
    "RAW_KEVIN_NO_MERGE " + gpath + "\n"
)
print("[bootstrap-h114] DOWNLOAD teacher start", flush=True)
tpath = snapshot_download("zai-org/GLM-4.5-Air-FP8", token=token)
print(f"[bootstrap-h114] DOWNLOAD teacher done -> {tpath}", flush=True)
Path("/root/logs/teacher.done").write_text(tpath + "\n")
print("[bootstrap-h114] DOWNLOAD tok331102 king start", flush=True)
kpath = snapshot_download(
    "Tok331102/affine-5EqYW8McUc-af10",
    revision="eb8bf9a356a254f71faaa439e8abc3cfba572c53",
    token=token,
)
print(f"[bootstrap-h114] DOWNLOAD tok331102 done -> {kpath}", flush=True)
Path("/root/logs/tok331102.done").write_text(kpath + "\n")
print("[bootstrap-h114] ALL_DOWNLOADS_OK", flush=True)
PY

bash /root/mining_src/s3-duel-sim/sync_corpus.sh || true

echo "[bootstrap-h114] $(date -u +%Y-%m-%dT%H:%M:%SZ) serving teacher+Tok+kevin954"
...
export CHALL_REPO=/root/h114/chall
export CHALL_REV=
...
for i in $(seq 1 240); do
  ...
  if [[ "$code" == "200" ]]; then
    echo "chall_serve_ok poll=$i $(date -u +%Y-%m-%dT%H:%M:%SZ)" \
      | tee /root/logs/h114_chall_serve.done
    break
  fi
  sleep 15
done
test -f /root/logs/h114_chall_serve.done

echo "[bootstrap-h114] $(date -u +%Y-%m-%dT%H:%M:%SZ) DONE — engines up; n80 watcher owns screen"
touch /root/logs/bootstrap_h114.done
