#!/usr/bin/env bash
# Host → mine-f23-1: upload lean stack (no train), start raw-Bittob11040 bootstrap.
set -euo pipefail

ROOT=/home/const/subnet120
DST_HOST=${DST_HOST:?set DST_HOST}
DST_PORT=${DST_PORT:?set DST_PORT}
KNOWN=${KNOWN:-/tmp/mine-f23-1.known_hosts}
...
STAGE=$(mktemp -d /tmp/mine-h118-stack.XXXXXX)
trap 'rm -rf "$STAGE"' EXIT

EXP=s4-h118-f23-raw-bittob
...
TAR=/tmp/mine-h118-stack.tar.gz
tar -C "$STAGE" -czf "$TAR" .
ls -lh "$TAR"

ENV_TMP=$(mktemp /tmp/mine-h118.env.XXXXXX)
...
"${SSH[@]}" 'mkdir -p /root/mining_src /root/affine_data /root/logs /root/h118 /root/hf'
"${SCP[@]}" "$TAR" "root@${DST_HOST}:/tmp/mine-h118-stack.tar.gz"
"${SCP[@]}" "$ENV_TMP" "root@${DST_HOST}:/root/mine.env"
rm -f "$ENV_TMP"

"${SSH[@]}" 'set -e
  tar -C /root/mining_src -xzf /tmp/mine-h118-stack.tar.gz
  chmod 600 /root/mine.env
  chmod +x /root/mining_src/s3-duel-sim/*.sh \
           /root/mining_src/s4-h2-merge/*.sh \
           /root/mining_src/s4-h118-f23-raw-bittob/*.sh
  test -f /root/mining_src/affine_pkg/affine/score.py
  test -x /root/mining_src/s4-h118-f23-raw-bittob/bootstrap_h118.sh
  echo STACK_UPLOAD_OK
  nohup bash /root/mining_src/s4-h118-f23-raw-bittob/bootstrap_h118.sh \
    >/root/logs/h118_pipeline.nohup 2>&1 &
  echo $! > /root/logs/h118_pipeline.pid
  nohup bash /root/mining_src/s4-h2-merge/watch_form_decision.sh h118 \
    /root/affine_data/h118_sim_result.json /root/affine_data/h118_decision.json \
    /root/logs/h118_form_decision.nohup \
    >/root/logs/h118_form_decision.launch.out 2>&1 &
  echo $! > /root/logs/h118_form_decision.pid
  nohup bash /root/mining_src/s4-h2-merge/watch_n80_retry.sh h118 \
    /root/mining_src/s4-h118-f23-raw-bittob/retry_h118_n80_d203first.sh \
    >/root/logs/h118_watch_retry.launch.nohup 2>&1 &
  echo $! > /root/logs/h118_watch_retry.pid
  echo PIPELINE_PID=$(cat /root/logs/h118_pipeline.pid)
  sleep 3
  head -n 40 /root/logs/bootstrap_h118.log 2>/dev/null || head -n 40 /root/logs/h118_pipeline.nohup || true
  ps -p "$(cat /root/logs/h118_pipeline.pid)" -o pid,etime,cmd || true
  # COUNT=8 smoke
  nvidia-smi -L | wc -l
'
