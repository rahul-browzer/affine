#!/usr/bin/env bash
# mine-r28: Tok-init → R28 GRPO on teacher Reason; n80 king = live guass (p2230).
...
echo "[bootstrap-r28] $(date -u +%Y-%m-%dT%H:%M:%SZ) start host=$(hostname)"
...
if [[ -z "${HF_TOKEN:-}" ]]; then
  echo "[bootstrap-r28] FATAL: HF_TOKEN missing in /root/mine.env"
  exit 1
fi

nvidia-smi -L || true
NGPU=$(nvidia-smi -L | wc -l)
echo "[bootstrap-r28] GPU_COUNT=$NGPU"
test "$NGPU" -ge 8
...
if p.is_file():
    ...
    if txt != orig:
        p.write_text(txt)
        print("[bootstrap-r28] patched vllm_client timeout=600 retries=5", flush=True)
    else:
        print("[bootstrap-r28] vllm_client already patched or pattern miss", flush=True)
PY
...
if [[ -s /root/logs/tok_init.done && -s /root/logs/teacher.done ]]; then
  echo "[bootstrap-r28] DOWNLOAD skip — stamps already present"
  echo "[bootstrap-r28] tok=$(cat /root/logs/tok_init.done)"
  echo "[bootstrap-r28] teacher=$(cat /root/logs/teacher.done)"
elif [[ -f /root/logs/parallel_dl.pid ]] && kill -0 "$(cat /root/logs/parallel_dl.pid)" 2>/dev/null; then
  echo "[bootstrap-r28] waiting for parallel_dl pid=$(cat /root/logs/parallel_dl.pid)"
  for i in $(seq 1 720); do
    if [[ -s /root/logs/tok_init.done && -s /root/logs/teacher.done ]]; then
      echo "[bootstrap-r28] parallel_dl stamps ready at wait=$i"
      break
    fi
    if ! kill -0 "$(cat /root/logs/parallel_dl.pid)" 2>/dev/null; then
      echo "[bootstrap-r28] parallel_dl exited; falling through to inline download"
      break
    fi
    sleep 15
  done
...
print("[bootstrap-r28] DOWNLOAD tok-init start", repo, rev, flush=True)
path = snapshot_download(repo, revision=rev, token=token)
print(f"[bootstrap-r28] DOWNLOAD tok-init done -> {path}", flush=True)
open("/root/logs/tok_init.done", "w").write(path + "\n")
open("/root/logs/tok331102.done", "w").write(path + "\n")
print("[bootstrap-r28] DOWNLOAD teacher start", flush=True)
tpath = snapshot_download("zai-org/GLM-4.5-Air-FP8", token=token)
print(f"[bootstrap-r28] DOWNLOAD teacher done -> {tpath}", flush=True)
open("/root/logs/teacher.done", "w").write(tpath + "\n")
print("[bootstrap-r28] ALL_DOWNLOADS_OK", flush=True)
PY
else
  echo "[bootstrap-r28] ALL_DOWNLOADS_OK (from stamps/parallel_dl)"
fi
...
print("[bootstrap-r28] DOWNLOAD guass-king start", king_repo, king_rev, flush=True)
kpath = snapshot_download(king_repo, revision=king_rev, token=token)
print(f"[bootstrap-r28] DOWNLOAD guass-king done -> {kpath}", flush=True)
open("/root/logs/guass.done", "w").write(kpath + "\n")
assert king_rev in kpath, kpath
PYG
else
  echo "[bootstrap-r28] DOWNLOAD guass-king skip — $(cat /root/logs/guass.done)"
fi

# Serve teacher (+ king) on 0–3; chall placeholder stopped; train on 6,7.
echo "[bootstrap-r28] $(date -u +%Y-%m-%dT%H:%M:%SZ) serving teacher before train"
...
for i in $(seq 1 240); do
  if curl -sf --max-time 5 http://127.0.0.1:8000/v1/models >/dev/null 2>&1; then
    echo "[bootstrap-r28] teacher :8000 up at iter=$i"
    break
  fi
  if (( i == 240 )); then
    echo "[bootstrap-r28] FATAL teacher never up"
...
echo "[bootstrap-r28] $(date -u +%Y-%m-%dT%H:%M:%SZ) launching R3 Reason-GRPO train"
...
echo "[bootstrap-r28] $(date -u +%Y-%m-%dT%H:%M:%SZ) BOOTSTRAP_DONE train=$(cat /root/logs/r3_train.pid) post=$(cat /root/logs/r3_post_train.pid)"
