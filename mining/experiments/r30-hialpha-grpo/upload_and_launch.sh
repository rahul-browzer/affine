#!/usr/bin/env bash
# Host → mine-r30-hialpha-1: R3 stack + R30 HiAlpha-GRPO overlay, bootstrap.
# Axis R30: Tok-init Reason-GRPO lora_alpha=128 r=16 (≠ R3 α=32 / R29 r=64).
# p2231: n80 king = live guass + Reason writer + form-dec.
set -euo pipefail

ROOT=/home/const/subnet120
POD_NAME=${POD_NAME:-mine-r30-hialpha-1}
...
STAGE=$(mktemp -d /tmp/mine-r30-stack.XXXXXX)
...
mkdir -p "$STAGE/affine_pkg/affine" "$STAGE/affine_pkg/evalsrv" \
         ...
         "$STAGE/$EXP" "$STAGE/r30-hialpha-grpo"
...
cp -a "$ROOT/mining/experiments/r30-hialpha-grpo/plan.md" "$STAGE/r30-hialpha-grpo/"
cp -a "$ROOT/mining/experiments/r30-hialpha-grpo/start_r30.sh" "$STAGE/r30-hialpha-grpo/"
cp -a "$ROOT/mining/experiments/r30-hialpha-grpo/bootstrap_r30.sh" "$STAGE/r30-hialpha-grpo/"
# Overlay: bootstrap_r3 + start_r3 → R30 HiAlpha + guass n80 king.
cp -a "$ROOT/mining/experiments/r30-hialpha-grpo/start_r30.sh" "$STAGE/$EXP/start_r3.sh"
cp -a "$ROOT/mining/experiments/r30-hialpha-grpo/bootstrap_r30.sh" "$STAGE/$EXP/bootstrap_r3.sh"
...
_rem_raw=$(lium describe "$POD_NAME" --json 2>/dev/null \
  ...
  || true)
if [[ -z "${_rem_raw}" ]]; then
  echo "[r30-up] FATAL: no billing.removal_scheduled_at for POD_NAME=$POD_NAME" >&2
  exit 1
fi
...
echo "[r30-up] Removal=$REMOVAL Soft=$SOFT Dead=$DEAD (from lium describe $POD_NAME)"
...
ENV_TMP=$(mktemp /tmp/mine-r30.env.XXXXXX)
...
KING_LOCAL_DEFAULT=/root/hf/hub/models--ttttxxxxsada--Affine-5guassq3tu/snapshots/e86758f5080d1e373e5fbbd7b4fbf6af327aeb44
{
  ...
  echo "export HF_LORA_REPO=unconst/Affine-5czsc2fc98-r30-lora"
  echo "export HF_MERGED_REPO=unconst/Affine-5czsc2fc98-r30-merged"
  echo "export R30_AXIS=hialpha_grpo_reason"
  echo "export R30_LR=5e-6"
  echo "export R30_LORA_R=16"
  echo "export R30_LORA_ALPHA=128"
  echo "export R30_GROUP_SIZE=4"
  echo "export R30_MAX_STEPS=200"
  echo "export R30_MAX_LEN=6144"
  echo "export R30_MAX_NEW=512"
  echo "export R30_TEMPERATURE=0.8"
...
"${SSH[@]}" 'mkdir -p /root/mining_src /root/affine_data /root/logs /root/r3 /root/r30 /root/hf'
"${SCP[@]}" "$TAR" "root@${DST_HOST}:/tmp/mine-r30-stack.tar.gz"
"${SCP[@]}" "$ENV_TMP" "root@${DST_HOST}:/root/mine.env"
"${SCP[@]}" "$DATA" "root@${DST_HOST}:/root/r3/winner_za_high_l1.jsonl"
"${SCP[@]}" "$DATA" "root@${DST_HOST}:/root/r30/winner_za_high_l1.jsonl"
rm -f "$ENV_TMP"
...
for repo in (
    "unconst/Affine-5czsc2fc98-r30-lora",
    "unconst/Affine-5czsc2fc98-r30-merged",
):
    try:
...
"${SSH[@]}" 'set -e
  tar -C /root/mining_src -xzf /tmp/mine-r30-stack.tar.gz
  ...
  chmod +x /root/mining_src/s3-duel-sim/*.sh \
           ...
           /root/mining_src/r30-hialpha-grpo/*.sh
  ...
  # Prove overlay is R30 HiAlpha; sim king = guass; Reason writer present.
  grep -q "R30: HiAlpha-GRPO" /root/mining_src/r3-reason-grpo/start_r3.sh
  grep -q "R30_LORA_ALPHA:-128" /root/mining_src/r3-reason-grpo/start_r3.sh
  ...
  echo "R30_DEADLINES soft=$SOFT_DEADLINE_UTC dead=$DEADMAN_UTC axis=$R30_AXIS king=$KING_REPO@$KING_REV"
  echo "R30_KNOBS lr=${R30_LR} G=${R30_GROUP_SIZE} temp=${R30_TEMPERATURE} r=${R30_LORA_R} α=${R30_LORA_ALPHA} max_len=${R30_MAX_LEN}"
  echo STACK_UPLOAD_OK
  nohup bash /root/mining_src/r3-reason-grpo/bootstrap_r3.sh \
    >/root/logs/r30_pipeline.nohup 2>&1 &
  echo $! > /root/logs/r30_pipeline.pid
  cp -f /root/logs/r30_pipeline.pid /root/logs/r3_pipeline.pid
  nohup bash /root/mining_src/s4-h2-merge/watch_form_decision.sh r3 \
    ...
    /root/logs/r30_form_decision.nohup \
    >/root/logs/r30_form_decision.launch.out 2>&1 &
  echo $! > /root/logs/r30_form_decision.pid
  echo PIPELINE_PID=$(cat /root/logs/r30_pipeline.pid)
  sleep 5
  head -n 40 /root/logs/bootstrap_r3.log 2>/dev/null || head -n 40 /root/logs/r30_pipeline.nohup || true
  ps -p "$(cat /root/logs/r30_pipeline.pid)" -o pid,etime,cmd || true
  nvidia-smi -L | wc -l
'
