...
mkdir -p "$STAGE/affine_pkg/affine" "$STAGE/affine_pkg/evalsrv" \
         ...
         "$STAGE/s4-h1v2-sft" "$STAGE/$EXP" "$STAGE/r30-hialpha-grpo"
...
# Overlay: bootstrap_r3 calls start_r3.sh — replace with R30 HiAlpha knobs.
...
"${SSH[@]}" 'set -e
  ...
  # Prove overlay is R30 HiAlpha, not stock R3 / R24–R29 / parent-GRPO.
  ...
  echo "R30_DEADLINES soft=$SOFT_DEADLINE_UTC dead=$DEADMAN_UTC axis=$R30_AXIS"
