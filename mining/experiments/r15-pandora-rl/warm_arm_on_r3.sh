#!/usr/bin/env bash
# Host → lean warm-arm R15 onto TKC-hot mine-r3-grpo-1 after R3b REFUTE.
# Keeps teacher:8000 + ckp333:8001; trains pandora-REINFORCE on GPUs 6–7.
set -euo pipefail
...
STAGE=$(mktemp -d /tmp/mine-r15-warm.XXXXXX)
...
cp -a "$ROOT/mining/experiments/r15-pandora-rl/"*.sh "$STAGE/r15-pandora-rl/"
cp -a "$ROOT/mining/experiments/r15-pandora-rl/plan.md" "$STAGE/r15-pandora-rl/"
# Overlay start/bootstrap for R15 identity grep.
...
REMOVE_AT=${REMOVE_AT:-2026-08-12T16:29:36Z}
...
TAR=/tmp/mine-r15-warm-stack.tar.gz
tar -C "$STAGE" -czf "$TAR" .
ls -lh "$TAR"
...
"${SCP[@]}" "$TAR" "root@${DST_HOST}:/tmp/mine-r15-warm-stack.tar.gz"
...
"${SSH[@]}" 'set -euo pipefail
  tar -C /root/mining_src -xzf /tmp/mine-r15-warm-stack.tar.gz
  ...
  # Stop leftover R3b lean/post/form by pidfile only (never pkill -f).
  for f in /root/logs/r3b_lean_warm.pid /root/logs/r3b_post_train.pid \
           /root/logs/r3b_form_decision.pid /root/logs/r3b_train.pid \
           /root/logs/h135_train.pid /root/logs/h135_post_train.pid; do
    if [[ -f "$f" ]]; then
  ...
  nohup bash /root/mining_src/r15-pandora-rl/lean_warm_boot.sh \
    >/root/logs/r15_lean_warm.nohup 2>&1 &
  echo $! >/root/logs/r15_lean_warm.pid
  sleep 15
  echo LEAN_PID=$(cat /root/logs/r15_lean_warm.pid)
  head -n 80 /root/logs/r15_lean_warm.log 2>/dev/null \
    || head -n 80 /root/logs/r15_lean_warm.nohup || true
  ps -p "$(cat /root/logs/r15_lean_warm.pid)" -o pid,etime,cmd || true
  test -f /root/logs/h135_train.pid && echo TRAIN_PID=$(cat /root/logs/h135_train.pid)
  test -f /root/logs/r15_post_train.pid && echo POST_PID=$(cat /root/logs/r15_post_train.pid)
...
echo "R15_WARM_ARM_OK pod=$POD_NAME soft=$SOFT dead=$DEAD $(date -u +%Y-%m-%dT%H:%M:%SZ)"
