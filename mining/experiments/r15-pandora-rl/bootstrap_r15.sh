...
# Order: pip → DL pandora+teacher → serve teacher → train (6,7) → Tok king DL + post_train.
set -euo pipefail

LOG=/root/logs/bootstrap_h135.log
mkdir -p /root/logs /root/hf /root/affine_data /root/mining_src /root/h135 /root/r15
exec > >(tee -a "$LOG") 2>&1

echo "[bootstrap-r15] $(date -u +%Y-%m-%dT%H:%M:%SZ) start host=$(hostname)"
...
if [[ -z "${HF_TOKEN:-}" ]]; then
  echo "[bootstrap-r15] FATAL: HF_TOKEN missing in /root/mine.env"
  exit 1
fi

nvidia-smi -L || true
NGPU=$(nvidia-smi -L | wc -l)
echo "[bootstrap-r15] GPU_COUNT=$NGPU"
test "$NGPU" -ge 8
...
# Prove overlay is R15, not stock H135 / R14 kevin.
...
if p.is_file():
    ...
    if txt != orig:
        p.write_text(txt)
        print("[bootstrap-r15] patched vllm_client timeout=600 retries=5", flush=True)
    else:
        print("[bootstrap-r15] vllm_client already patched or pattern miss", flush=True)
PY

# Blocking DL: pandora-init + teacher (train needs both).
python - <<'PY'
import os
...
print("[bootstrap-r15] DOWNLOAD pandora-init start", repo, rev, flush=True)
path = snapshot_download(repo, revision=rev, token=token)
print(f"[bootstrap-r15] DOWNLOAD pandora-init done -> {path}", flush=True)
open("/root/logs/pandora_init.done", "w").write(path + "\n")
open("/root/h135/r15_base.path", "w").write(path + "\n")
assert rev in path, path
print("[bootstrap-r15] DOWNLOAD teacher start", flush=True)
tpath = snapshot_download("zai-org/GLM-4.5-Air-FP8", token=token)
print(f"[bootstrap-r15] DOWNLOAD teacher done -> {tpath}", flush=True)
...
echo "[bootstrap-r15] $(date -u +%Y-%m-%dT%H:%M:%SZ) serving teacher before train BASE=$BASE"
...
cp -f /root/logs/h135_prewarm.pid /root/logs/r15_prewarm.pid

for i in $(seq 1 240); do
  if curl -sf --max-time 5 http://127.0.0.1:8000/v1/models >/dev/null 2>&1; then
    echo "[bootstrap-r15] teacher :8000 up at iter=$i"
    break
  fi
  if (( i == 240 )); then
    echo "[bootstrap-r15] FATAL teacher never up"
...
echo "[bootstrap-r15] $(date -u +%Y-%m-%dT%H:%M:%SZ) launching R15 pandora-REINFORCE train"
...
from huggingface_hub import snapshot_download
token = os.environ["HF_TOKEN"]
print("[bootstrap-r15] DOWNLOAD tok331102 king start", flush=True)
kpath = snapshot_download(
    "Tok331102/affine-5EqYW8McUc-af10",
    revision="eb8bf9a356a254f71faaa439e8abc3cfba572c53",
    token=token,
)
print(f"[bootstrap-r15] DOWNLOAD tok331102 done -> {kpath}", flush=True)
...
cp -f /root/logs/h135_extra_dl.pid /root/logs/r15_extra_dl.pid
...
cp -f /root/logs/h135_post_train.pid /root/logs/r15_post_train.pid

echo "[bootstrap-r15] $(date -u +%Y-%m-%dT%H:%M:%SZ) BOOTSTRAP_DONE train=$(cat /root/logs/h135_train.pid) post=$(cat /root/logs/h135_post_train.pid)"
