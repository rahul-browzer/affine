#!/usr/bin/env bash
# H108/F13 pass393: GPU merge save hung on gocryptfs request_wait_answer
...
EXP=/root/mining_src/s4-h108-f13-diane613
LOG=/root/logs/h108_merge_recover_pass393.nohup
PIDF=/root/logs/h108_merge_recover_pass393.pid
MERGED=/root/h108/merged
ADAPTER=/root/h108/train/adapter
BASE=/root/hf/hub/models--diane613--Affine-5CQLBK7Mmw1vsk7eQcBok9Qn44JNU5YVrfNmZpJHPxLV271B/snapshots/ad0f3f116e44dc5154ca3f72b933faaefc4905fa

log() { echo "[h108-merge393] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }

mkdir -p /root/logs /root/affine_data
: >"$LOG"
echo $$ >"$PIDF"

MERGE_PID=$(ps -eo pid,cmd | awk '/[p]ython .*merge_lora.py --base .*h108\/merged/{print $1; exit}')
POST_PID=$(ps -eo pid,cmd | awk '/[b]ash .*s4-h108-f13-diane613\/post_train_pipeline\.sh/{print $1; exit}')
...
rm -f /root/logs/h108_pipeline.aborted
...
if [[ -x "$EXP/watch_preempt_bare_tcache_pass264.sh" ]]; then
  nohup bash "$EXP/watch_preempt_bare_tcache_pass264.sh" \
    >>/root/logs/h108_preempt_bare_pass264.log 2>&1 &
  echo $! >/root/logs/h108_preempt_bare_pass264.pid
  log "rearmed preempt264 pid=$(cat /root/logs/h108_preempt_bare_pass264.pid)"
fi
...
log "post_train resumed pid=$(cat /root/logs/h108_post_train.pid) SKIP_MERGE=1"
log "DONE recover393"
