#!/usr/bin/env bash
# Host → mine-h34-1: upload stack + data, start bootstrap under nohup.
set -euo pipefail

ROOT=/home/const/subnet120
DST_HOST=${DST_HOST:?set DST_HOST}
DST_PORT=${DST_PORT:?set DST_PORT}
KNOWN=${KNOWN:-/tmp/mine-h34-1.known_hosts}
...
STAGE=$(mktemp -d /tmp/mine-h34-stack.XXXXXX)
trap 'rm -rf "$STAGE"' EXIT

EXP=s4-h34-m7-king-self-ep2
...
TAR=/tmp/mine-h34-stack.tar.gz
tar -C "$STAGE" -czf "$TAR" .
ls -lh "$TAR"

ENV_TMP=$(mktemp /tmp/mine-h34.env.XXXXXX)
...
# H34 data = H29 king-self harvest (686 ex); init = m7 (same as H28).
DATA="$ROOT/mining/experiments/s4-h29-king-self-clip-l1/results/king_self_high_l1.jsonl"
test -s "$DATA"
test "$(wc -l <"$DATA")" -ge 300

"${SSH[@]}" 'mkdir -p /root/mining_src /root/affine_data /root/logs /root/h34 /root/hf'
"${SCP[@]}" "$TAR" "root@${DST_HOST}:/tmp/mine-h34-stack.tar.gz"
"${SCP[@]}" "$ENV_TMP" "root@${DST_HOST}:/root/mine.env"
"${SCP[@]}" "$DATA" "root@${DST_HOST}:/root/h34/king_self_high_l1.jsonl"
rm -f "$ENV_TMP"
...
for repo in (
    "unconst/Affine-5czsc2fc98-h34-lora",
    "unconst/Affine-5czsc2fc98-h34-merged",
):
    try:
...
"${SSH[@]}" 'set -e
  tar -C /root/mining_src -xzf /tmp/mine-h34-stack.tar.gz
  chmod 600 /root/mine.env
  chmod +x /root/mining_src/s3-duel-sim/*.sh \
           /root/mining_src/s4-h2-merge/*.sh \
           /root/mining_src/s4-h34-m7-king-self-ep2/*.sh
  test -f /root/mining_src/affine_pkg/affine/score.py
  test -s /root/h34/king_self_high_l1.jsonl
  test -x /root/mining_src/s4-h34-m7-king-self-ep2/bootstrap_h34.sh
  echo STACK_UPLOAD_OK
  nohup bash /root/mining_src/s4-h34-m7-king-self-ep2/bootstrap_h34.sh \
    >/root/logs/h34_pipeline.nohup 2>&1 &
  echo $! > /root/logs/h34_pipeline.pid
  nohup bash /root/mining_src/s4-h2-merge/watch_form_decision.sh h34 \
    /root/affine_data/h34_sim_result.json /root/affine_data/h34_decision.json \
    /root/logs/h34_form_decision.nohup \
    >/root/logs/h34_form_decision.launch.out 2>&1 &
  echo $! > /root/logs/h34_form_decision.pid
  nohup bash /root/mining_src/s4-h2-merge/watch_n80_retry.sh h34 \
    /root/mining_src/s4-h34-m7-king-self-ep2/retry_h34_n80.sh \
    >/root/logs/h34_watch_retry.launch.nohup 2>&1 &
  echo $! > /root/logs/h34_watch_retry.pid
  echo PIPELINE_PID=$(cat /root/logs/h34_pipeline.pid)
  sleep 3
  head -n 40 /root/logs/bootstrap_h34.log 2>/dev/null || head -n 40 /root/logs/h34_pipeline.nohup || true
  ps -p "$(cat /root/logs/h34_pipeline.pid)" -o pid,etime,cmd || true
'
