#!/usr/bin/env bash
# Restart H100 n80 after crash. Engines must already be up.
# p410: preemptively drop a203+c203 (H32 overflow: a203 known; c203 on F7
# p408 @~7/80). d203-first + longwait polls (360/120). MAX=6.
# FALSE_PROBE continues to next hash (p389).
set -euo pipefail
...
SIM=/root/affine_data/h100_sim_result.json
...
# B300/Range Tok: longwait 360×15s (p388/p391).
_wait_engines() {
  local max=${1:-360} i=0
  while (( i < max )); do
    if _engines_ok; then
      ...
      if _engines_ok; then
        ...
        rm -f /root/logs/h100_n80_retry.aborted
        return 0
      fi
...
if [[ -f "$DEC" ]]; then
  if python3 -c 'import json,sys; d=json.load(open(sys.argv[1])); sys.exit(0 if d.get("false_probe") else 1)' "$DEC" 2>/dev/null; then
    ...
    mv "$DEC" "/root/affine_data/false_probes/h100_decision_retryQ_${ts}.json"
    [[ -f "$SIM" ]] && mv "$SIM" "/root/affine_data/false_probes/h100_sim_retryQ_${ts}.json"
    rm -f /root/logs/h100_n80.done
...
if [[ -f "$SIM" ]]; then
  if python3 -c 'import json,sys; d=json.load(open(sys.argv[1])); sys.exit(0 if (d.get("false_probe") or "unpromptable" in str(d.get("rejection_reason","")) or "ConnectError" in str(d.get("rejection_reason",""))) else 1)' "$SIM" 2>/dev/null; then
    ts=$(date -u +%Y%m%dT%H%M%SZ)
    mkdir -p /root/affine_data/false_probes
    mv "$SIM" "/root/affine_data/false_probes/h100_sim_retryQ_${ts}.json"
    ...
    python3 /root/mining_src/s4-h2-merge/write_merge_decision.py \
      --hyp h100 --sim-result "$SIM" --out "$DEC"
    date -u +%Y-%m-%dT%H:%M:%SZ > /root/logs/h100_n80.done
    exit 0
  fi
fi
if ps -eo pid,cmd | awk '/python/ && /[r]un_sim_duel.py/ && /local-h100/ { found=1 } END { exit !found }'; then
  log "sim already running — noop"
  exit 0
fi
if ! _wait_engines 360; then
  log "ABORT: engines not promptable after wait"
  echo "aborted_engines_unhealthy $(date -u +%Y-%m-%dT%H:%M:%SZ)" \
    >/root/logs/h100_n80_retry.aborted
  exit 1
fi
test -d "$MERGED"
test -f /root/logs/h100_merge.done

# p408/p410: drop a203+c203 (H32 teacher 400). d203-first; e/f/g + b203 fallback.
BLOCK_HASHES=(
...
for attempt in $(seq 1 "$MAX_ATTEMPTS"); do
  ...
  python /root/mining_src/s4-h2-merge/run_sim_duel.py \
    ...
    --progress-out "$PROG" \
    --save-artifact \
    2>&1 | tee /root/logs/h100_n80.log
  rc=${PIPESTATUS[0]}
  set -e
    ...
    if _is_false_probe_sim "$SIM"; then
      ...
      [[ -f "${SIM%.json}_artifact.json" ]] && \
        mv "${SIM%.json}_artifact.json" \
          "/root/affine_data/false_probes/h100_artifact_retryQ_${ts}.json" || true
