# mine-h25-1 bootstrap: TalentPigs×adambell ckpt1000-m7 (Radiant28 mirror) α0.90 + teacher → merge/serve/n80.
# Secrets from /root/mine.env only (never argv).
set -euo pipefail

LOG=/root/logs/bootstrap_h25.log
mkdir -p /root/logs /root/hf /root/affine_data /root/mining_src /root/merges
exec > >(tee -a "$LOG") 2>&1

echo "[bootstrap-h25] $(date -u +%Y-%m-%dT%H:%M:%SZ) start host=$(hostname)"
...
if [[ -z "${HF_TOKEN:-}" ]]; then
  echo "[bootstrap-h25] FATAL: HF_TOKEN missing in /root/mine.env"
  exit 1
fi
...
uv pip install \
  ...
  "numpy" \
  "scipy" \
  2>&1 | tee /root/logs/pip_h25.log | tail -40

python - <<'PY'
import torch, transformers, vllm
print("[bootstrap-h25] VERSIONS",
...
if txt == orig:
    print("[bootstrap-h25] vllm_client already patched or pattern miss", flush=True)
else:
    p.write_text(txt)
    print("[bootstrap-h25] patched vllm_client timeout=480 retries=5", flush=True)
PY

B_REPO=Radiant28/5eqdtdzqle-ckpt1000-m7
B_REV=f766293ee878efef5f068a4053d6974017f11f26
...
jobs = [
    ...
    ("$B_REPO", "$B_REV", "adambellm7"),
    ("zai-org/GLM-4.5-Air-FP8", None, "teacher"),
]
for repo, rev, label in jobs:
    done = f"/root/logs/{label}.done"
    if os.path.isfile(done):
        print(f"[bootstrap-h25] SKIP {label} (have {done})", flush=True)
        continue
    print(f"[bootstrap-h25] DOWNLOAD start {label} {repo} rev={rev}", flush=True)
    path = snapshot_download(repo, revision=rev, token=token)
    print(f"[bootstrap-h25] DOWNLOAD done {label} -> {path}", flush=True)
    open(done, "w").write(path + "\n")
print("[bootstrap-h25] ALL_DOWNLOADS_OK", flush=True)
...
python /root/mining_src/s4-h2-merge/merge_linear.py \
  ...
  --out "$OUT" \
  --hf-home "$HF_HOME" \
  2>&1 | tee /root/logs/h25_merge.log

date -u +%Y-%m-%dT%H:%M:%SZ > /root/logs/h25_merge.done
echo "[bootstrap-h25] MERGE_DONE alpha=$ALPHA out=$OUT b=$B_REPO@$B_REV"
...
export CHALL_REPO=/root/merges/h25-tp90
export CHALL_REV=
bash /root/mining_src/s3-duel-sim/serve_three.sh

touch /root/logs/bootstrap_h25.done
echo "[bootstrap-h25] $(date -u +%Y-%m-%dT%H:%M:%SZ) DONE — engines launching; next: wait_ready + n80"
