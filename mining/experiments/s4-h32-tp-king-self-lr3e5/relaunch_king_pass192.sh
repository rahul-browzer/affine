...
log() { echo "[recover192-h32] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }
...
for f in h32_decision.json h32_sim_result.json h32_sim_progress.json; do
  if [[ -f "/root/affine_data/$f" ]]; then
    mv "/root/affine_data/$f" "/root/affine_data/false_probes/${f%.json}_pass192_$(date -u +%H%M%S).json"
    log "quarantined $f"
  fi
done
...
print(f"[recover192-h32] reap gpu={sorted(want)} workers={sorted(pids)} parents={sorted(parents)}")
for pid in kill_set:
    try:
...
nohup bash /root/mining_src/s4-h32-tp-king-self-lr3e5/recover_wait_pass192.sh \
