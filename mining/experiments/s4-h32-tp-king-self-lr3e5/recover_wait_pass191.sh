...
log() { echo "[recover-wait191-h32] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }
...
if [[ -f /root/logs/h32_chall_serve.done ]] && \
  ...
  nohup bash /root/mining_src/s4-h32-tp-king-self-lr3e5/retry_h32_n80.sh \
    >/root/logs/h32_n80_relaunch_pass191.nohup 2>&1 &
  echo $! >/root/logs/h32_n80_relaunch_pass191.pid
  log "relaunched retry pid=$(cat /root/logs/h32_n80_relaunch_pass191.pid)"
  exit 0
fi
