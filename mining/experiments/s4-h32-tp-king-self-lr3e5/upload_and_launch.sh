#!/usr/bin/env bash
# Host → mine-h32-1: upload stack + king-self data, start bootstrap under nohup.
set -euo pipefail

ROOT=/home/const/subnet120
DST_HOST=${DST_HOST:?set DST_HOST}
DST_PORT=${DST_PORT:?set DST_PORT}
KNOWN=${KNOWN:-/tmp/mine-h32-1.known_hosts}
...
STAGE=$(mktemp -d /tmp/mine-h32-stack.XXXXXX)
trap 'rm -rf "$STAGE"' EXIT
EXP=s4-h32-tp-king-self-lr3e5
...
TAR=/tmp/mine-h32-stack.tar.gz
tar -C "$STAGE" -czf "$TAR" .
ls -lh "$TAR"

ENV_TMP=$(mktemp /tmp/mine-h32.env.XXXXXX)
...
"${SSH[@]}" 'mkdir -p /root/mining_src /root/affine_data /root/logs /root/h32 /root/hf'
"${SCP[@]}" "$TAR" "root@${DST_HOST}:/tmp/mine-h32-stack.tar.gz"
"${SCP[@]}" "$ENV_TMP" "root@${DST_HOST}:/root/mine.env"
"${SCP[@]}" "$DATA" "root@${DST_HOST}:/root/h32/king_self_high_l1.jsonl"
rm -f "$ENV_TMP"
...
for repo in (
    "unconst/Affine-5czsc2fc98-h32-lora",
    "unconst/Affine-5czsc2fc98-h32-merged",
):
    try:
...
"${SSH[@]}" 'set -e
  tar -C /root/mining_src -xzf /tmp/mine-h32-stack.tar.gz
  chmod 600 /root/mine.env
  chmod +x /root/mining_src/s3-duel-sim/*.sh \
           /root/mining_src/s4-h2-merge/*.sh \
           /root/mining_src/s4-h32-tp-king-self-lr3e5/*.sh
  test -f /root/mining_src/affine_pkg/affine/score.py
  test -s /root/h32/king_self_high_l1.jsonl
  test -x /root/mining_src/s4-h32-tp-king-self-lr3e5/bootstrap_h32.sh
  echo STACK_UPLOAD_OK
  nohup bash /root/mining_src/s4-h32-tp-king-self-lr3e5/bootstrap_h32.sh \
    >/root/logs/h32_pipeline.nohup 2>&1 &
  echo $! > /root/logs/h32_pipeline.pid
  nohup bash /root/mining_src/s4-h2-merge/watch_form_decision.sh h32 \
    /root/affine_data/h32_sim_result.json /root/affine_data/h32_decision.json \
    /root/logs/h32_form_decision.nohup \
    >/root/logs/h32_form_decision.launch.out 2>&1 &
  echo $! > /root/logs/h32_form_decision.pid
  nohup bash /root/mining_src/s4-h2-merge/watch_n80_retry.sh h32 \
    /root/mining_src/s4-h32-tp-king-self-lr3e5/retry_h32_n80.sh \
    >/root/logs/h32_watch_retry.launch.nohup 2>&1 &
  echo $! > /root/logs/h32_watch_retry.pid
  echo PIPELINE_PID=$(cat /root/logs/h32_pipeline.pid)
  sleep 3
  head -n 40 /root/logs/bootstrap_h32.log 2>/dev/null || head -n 40 /root/logs/h32_pipeline.nohup || true
  ps -p "$(cat /root/logs/h32_pipeline.pid)" -o pid,etime,cmd || true
'
