#!/usr/bin/env bash
# Pass 191: H32 king died on Triton __triton_launcher.so (GPUs 2,3 still held).
...
LOG=/root/logs/h32_king_recover_pass191.log
...
log() { echo "[recover191-h32] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }

log "START king relaunch TCACHE=$TCACHE"

mkdir -p /root/affine_data/false_probes
for f in h32_decision.json h32_sim_result.json h32_sim_progress.json; do
...
print(f"[recover191-h32] reap gpu={sorted(want)} workers={sorted(pids)} parents={sorted(parents)}")
for pid in kill_set:
    try:
...
nohup bash /root/mining_src/s4-h32-tp-king-self-lr3e5/recover_wait_pass191.sh \
  >/root/logs/h32_recover_wait191.nohup 2>&1 &
echo $! >/root/logs/h32_recover_wait.pid
log "wait_pid=$(cat /root/logs/h32_recover_wait.pid)"
log "DONE_LAUNCH"
