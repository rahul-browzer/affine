...
for f in h46_decision.json h46_sim_result.json h46_sim_result_artifact.json \
         h46_sim_progress.json; do
  if [[ -f "/root/affine_data/$f" ]]; then
    mv "/root/affine_data/$f" "$QDIR/${f%.json}_pass224_${ts}.json"
    log "quarantine $f → $QDIR"
  fi
done
rm -f /root/logs/h46_n80.done /root/logs/h46_n80_retry.aborted \
  /root/logs/h46_pipeline.aborted /root/logs/h46_chall_serve.done
...
while read -r pid; do
  ...
  /[r]un_sim_duel\.py/ && /local-h46/ {print $1}
  /[r]etry_h46_n80\.sh/ {print $1}
')
...
print(f"[recover224-h46-chall] reap gpu={sorted(want)} workers={sorted(pids)} parents={sorted(parents)} grandparents={sorted(grandparents)}")
for pid in kill_set:
    try:
...
rm -rf /root/.triton/isolated/h46_chall_* || true
...
rm -rf /tmp/torchinductor_* /root/.cache/torch/inductor /root/.cache/torchinductor_chall_* /root/.cache/torchinductor_h46_* || true
...
_comp() {
  ...
  code=$(curl -s -o "/tmp/h46_warmup_${label}.json" -w "%{http_code}" --max-time 120 \
...
while read -r pid; do
  ...
  sleep 1
  kill -9 "$pid" 2>/dev/null || true
done < <(ps -eo pid,args | awk '/[w]atch_n80_retry\.sh/ && / h46 / {print $1}')
...
while read -r pid; do
  ...
  sleep 1
  kill -9 "$pid" 2>/dev/null || true
done < <(ps -eo pid,args | awk '/[r]etry_h46_n80\.sh/ {print $1}')
sleep 1
...
log "rearmed watcher pid=$(cat /root/logs/h46_watch_retry.pid)"
