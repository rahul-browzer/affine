#!/usr/bin/env bash
# H109/F14 pass429c: /root gocryptfs hang + /tmp safetensors EFAULT on save.
# Contiguous-clone state_dict then save 5GB shards to /tmp; symlink for serve.
set -euo pipefail

EXP=/root/mining_src/s4-h109-f14-bittob
LOG=/root/logs/h109_merge_recover_pass429c.nohup
PIDF=/root/logs/h109_merge_recover_pass429c.pid
...
MERGE_PY=/tmp/merge_lora_contig429.py

log() { echo "[h109-merge429c] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOG"; }

mkdir -p /root/logs /root/affine_data
: >"$LOG"
echo $$ >"$PIDF"

# Kill leftover merge/post_train by $0 / cmdline match (not SSH argv)
while read -r pid cmd; do
  case "$cmd" in
    *merge_lora*h109*|*merge_lora_sharded*|*merge_lora_contig*|*post_train_pipeline.sh*)
...
new = """    print("[merge] merge_and_unload", flush=True)
    model = model.merge_and_unload()
    print("[merge] contiguous-clone state_dict (EFAULT workaround)", flush=True)
...
log "CPU merge → $MERGED_TMP contiguous-clone + 5GB shards"
python "$MERGE_PY" \
  --base "$BASE" \
  --adapter "$ADAPTER" \
  --out "$MERGED_TMP" \
