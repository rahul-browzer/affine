#!/usr/bin/env bash
# After bootstrap_h21.done + engines up: run n80 sim vs TalentPigs.
set -euo pipefail
...
test -d /root/merges/h21-tp75

TIMEOUT_S=2400 bash /root/mining_src/s3-duel-sim/wait_ready.sh

for attempt in 1 2 3; do
  echo "[h21-n80] attempt=$attempt $(date -u +%Y-%m-%dT%H:%M:%SZ)"
  set +e
  ...
  echo "[h21-n80] attempt=$attempt failed rc=$rc; sleep 30"
  sleep 30
done

echo "[h21-n80] ERROR: all 3 attempts failed — watch_n80_retry will take over"
exit 1
