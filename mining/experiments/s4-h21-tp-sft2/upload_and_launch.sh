#!/usr/bin/env bash
# Host → mine-h21-1: upload harness + mine.env, start bootstrap+n80 under nohup.
set -euo pipefail

ROOT=/home/const/subnet120
DST_HOST=${DST_HOST:?set DST_HOST}
DST_PORT=${DST_PORT:?set DST_PORT}
KNOWN=${KNOWN:-/tmp/mine-h21-1.known_hosts}
...
STAGE=$(mktemp -d /tmp/mine-h21-stack.XXXXXX)
trap 'rm -rf "$STAGE"' EXIT

mkdir -p "$STAGE/affine_pkg/affine" "$STAGE/affine_pkg/evalsrv" \
         "$STAGE/s3-duel-sim" "$STAGE/s4-h2-merge" "$STAGE/s4-h21-tp-sft2"
...
cp -a "$ROOT/mining/experiments/s4-h21-tp-sft2/bootstrap_h21.sh" "$STAGE/s4-h21-tp-sft2/"
cp -a "$ROOT/mining/experiments/s4-h21-tp-sft2/start_h21_n80.sh" "$STAGE/s4-h21-tp-sft2/"
cp -a "$ROOT/mining/experiments/s4-h21-tp-sft2/retry_h21_n80.sh" "$STAGE/s4-h21-tp-sft2/"
cp -a "$ROOT/mining/experiments/s4-h21-tp-sft2/plan.md" "$STAGE/s4-h21-tp-sft2/"

TAR=/tmp/mine-h21-stack.tar.gz
tar -C "$STAGE" -czf "$TAR" .
ls -lh "$TAR"

ENV_TMP=$(mktemp /tmp/mine-h21.env.XXXXXX)
...
"${SCP[@]}" "$TAR" "root@${DST_HOST}:/tmp/mine-h21-stack.tar.gz"
"${SCP[@]}" "$ENV_TMP" "root@${DST_HOST}:/root/mine.env"
rm -f "$ENV_TMP"

"${SSH[@]}" 'set -e
  tar -C /root/mining_src -xzf /tmp/mine-h21-stack.tar.gz
  ...
  chmod +x /root/mining_src/s3-duel-sim/*.sh \
           ...
           /root/mining_src/s4-h21-tp-sft2/*.sh
  test -f /root/mining_src/affine_pkg/affine/score.py
  test -x /root/mining_src/s4-h21-tp-sft2/bootstrap_h21.sh
  test -x /root/mining_src/s4-h21-tp-sft2/start_h21_n80.sh
  test -x /root/mining_src/s4-h21-tp-sft2/retry_h21_n80.sh
  echo STACK_UPLOAD_OK
  nohup bash -lc "
    set -euo pipefail
    bash /root/mining_src/s4-h21-tp-sft2/bootstrap_h21.sh
    bash /root/mining_src/s4-h21-tp-sft2/start_h21_n80.sh
  " >/root/logs/h21_pipeline.nohup 2>&1 &
  echo $! > /root/logs/h21_pipeline.pid
  nohup bash /root/mining_src/s4-h2-merge/watch_fix_decision.sh h21 \
    >/root/logs/h21_fix_decision.nohup 2>&1 &
  echo $! > /root/logs/h21_fix_decision.pid
  nohup bash /root/mining_src/s4-h2-merge/watch_n80_retry.sh h21 \
    /root/mining_src/s4-h21-tp-sft2/retry_h21_n80.sh \
    >/root/logs/h21_watch_retry.nohup 2>&1 &
  echo $! > /root/logs/h21_watch_retry.pid
  echo PIPELINE_PID=$(cat /root/logs/h21_pipeline.pid)
  sleep 2
  head -n 20 /root/logs/h21_pipeline.nohup || true
  ps -p "$(cat /root/logs/h21_pipeline.pid)" -o pid,etime,cmd || true
'
