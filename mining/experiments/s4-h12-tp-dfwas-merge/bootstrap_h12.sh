#!/usr/bin/env bash
# mine-h12-1 bootstrap: pinned eval stack + TalentPigs/dfwas-alskdjf/teacher → merge α0.75.
...
echo "[bootstrap-h12] $(date -u +%Y-%m-%dT%H:%M:%SZ) start host=$(hostname)"
...
if [[ -z "${HF_TOKEN:-}" ]]; then
  echo "[bootstrap-h12] FATAL: HF_TOKEN missing in /root/mine.env"
  exit 1
fi
...
uv pip install \
  ...
  "numpy" \
  "scipy" \
  2>&1 | tee /root/logs/pip_h12.log | tail -40

python - <<'PY'
import torch, transformers, vllm
print("[bootstrap-h12] VERSIONS",
...
if txt == orig:
    print("[bootstrap-h12] vllm_client already patched or pattern miss", flush=True)
else:
    p.write_text(txt)
    print("[bootstrap-h12] patched vllm_client timeout=480 retries=5", flush=True)
PY
...
jobs = [
    ...
    ("dfwasfmdpwkjglnpwngwg/affine-5ccebdzvsj-alskdjf",
     "a733622145b81e89a900622a5554148054df0edd", "dfwas"),
...
python /root/mining_src/s4-h2-merge/merge_linear.py \
  ...
  --b-repo dfwasfmdpwkjglnpwngwg/affine-5ccebdzvsj-alskdjf \
  --b-rev a733622145b81e89a900622a5554148054df0edd \
  --alpha "$ALPHA" \
  --out "$OUT" \
  --hf-home "$HF_HOME" \
...
echo "[bootstrap-h12] MERGE_DONE alpha=$ALPHA out=$OUT"
