#!/usr/bin/env bash
# mine-f17-1: raw genesis chall (no train) vs Tok king — F17/H112 screen.
set -euo pipefail

LOG=/root/logs/bootstrap_h112.log
mkdir -p /root/logs /root/hf /root/affine_data /root/mining_src /root/h112
exec > >(tee -a "$LOG") 2>&1

echo "[bootstrap-h112] $(date -u +%Y-%m-%dT%H:%M:%SZ) start host=$(hostname)"
...
if [[ -z "${HF_TOKEN:-}" ]]; then
  echo "[bootstrap-h112] FATAL: HF_TOKEN missing in /root/mine.env"
  exit 1
fi

nvidia-smi -L || true
NGPU=$(nvidia-smi -L | wc -l)
echo "[bootstrap-h112] GPU_COUNT=$NGPU"
test "$NGPU" -ge 8
...
uv pip install \
  ...
  "numpy" \
  "scipy" \
  2>&1 | tee /root/logs/pip_h112.log | tail -40

python - <<'PY'
import torch, transformers, vllm
print("[bootstrap-h112] VERSIONS",
...
if p.is_file():
    ...
    if txt != orig:
        p.write_text(txt)
        print("[bootstrap-h112] patched vllm_client timeout=480 retries=5", flush=True)
    else:
        print("[bootstrap-h112] vllm_client already patched or pattern miss", flush=True)
PY

# Download chall=genesis, king=Tok, teacher — then serve + stamp chall ready.
python - <<'PY'
import os
from pathlib import Path
...
print("[bootstrap-h112] DOWNLOAD genesis chall start", flush=True)
gpath = snapshot_download(
    "dendriteholdings/albedo-qwen3.6-35b-king-genesis",
    revision="abe89194d6addf82e71f3f1ba9fef94b05404abf",
    token=token,
)
print(f"[bootstrap-h112] DOWNLOAD genesis done -> {gpath}", flush=True)
chall = Path("/root/h112/chall")
...
Path("/root/logs/genesis_chall.done").write_text(gpath + "\n")
# retry script expects merge.done for legacy gate — stamp it (no merge occurred)
Path("/root/logs/h112_merge.done").write_text(
    "RAW_GENESIS_NO_MERGE " + gpath + "\n"
)
print("[bootstrap-h112] DOWNLOAD teacher start", flush=True)
tpath = snapshot_download("zai-org/GLM-4.5-Air-FP8", token=token)
print(f"[bootstrap-h112] DOWNLOAD teacher done -> {tpath}", flush=True)
Path("/root/logs/teacher.done").write_text(tpath + "\n")
print("[bootstrap-h112] DOWNLOAD tok331102 king start", flush=True)
kpath = snapshot_download(
    "Tok331102/affine-5EqYW8McUc-af10",
    revision="eb8bf9a356a254f71faaa439e8abc3cfba572c53",
    token=token,
)
print(f"[bootstrap-h112] DOWNLOAD tok331102 done -> {kpath}", flush=True)
Path("/root/logs/tok331102.done").write_text(kpath + "\n")
print("[bootstrap-h112] ALL_DOWNLOADS_OK", flush=True)
PY

bash /root/mining_src/s3-duel-sim/sync_corpus.sh || true

echo "[bootstrap-h112] $(date -u +%Y-%m-%dT%H:%M:%SZ) serving teacher+Tok+genesis"
...
export CHALL_REPO=/root/h112/chall
export CHALL_REV=
...
for i in $(seq 1 240); do
  ...
  if [[ "$code" == "200" ]]; then
    echo "chall_serve_ok poll=$i $(date -u +%Y-%m-%dT%H:%M:%SZ)" \
      | tee /root/logs/h112_chall_serve.done
    break
  fi
  sleep 15
done
test -f /root/logs/h112_chall_serve.done

echo "[bootstrap-h112] $(date -u +%Y-%m-%dT%H:%M:%SZ) DONE — engines up; n80 watcher owns screen"
touch /root/logs/bootstrap_h112.done
