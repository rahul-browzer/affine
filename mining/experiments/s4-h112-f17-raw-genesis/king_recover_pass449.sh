#!/usr/bin/env bash
# Pass 449: king-only relaunch after mid-n80 EngineDead on :8001
# (challenger@14/80 → ConnectError @02:43:32Z; orphan VLLM::Worker ppid=1
# on GPUs 2,3; teacher+chall still :8000/:8002=200). Seed from live chall
# TCACHE (n_so≥16) + util=0.72 — bare/cold isolated → ENOENT (p434/p435).
set -euo pipefail
...
LOGN=/root/logs/h112_king_recover_pass449.nohup
log() { echo "[king449-h112] $(date -u +%Y-%m-%dT%H:%M:%SZ) $*" | tee -a "$LOGN"; }

log "START king-only recover (p449 mid-n80 EngineDead @02:43:32Z chall@14/80; leave teacher+chall)"
...
for line in apps.strip().splitlines():
    ...
    if len(parts) >= 2 and parts[0] in uuids:
        try:
            ...
            pass
print(f"[king449-h112] reap gpus2,3 pids={sorted(pids)}")
for pid in pids:
    try:
...
rm -f /root/affine_data/h112_sim_result.json /root/affine_data/h112_sim_progress.json \
  /root/affine_data/h112_decision.json /root/logs/h112_n80.done /root/logs/h112_n80_retry.aborted \
  /root/logs/h112_king_recover_pass332.done /root/logs/h112_sim_n80.done \
  /root/logs/h112_king_recover_pass449.done
log "cleared stale h112 sim progress/result"

log "wipe bare king caches before seeded isolated TCACHE"
rm -rf /root/.triton/cache/king
rm -rf /root/.triton/cache/king_* 2>/dev/null || true
rm -rf /root/.triton/isolated/h112_king_p449_* 2>/dev/null || true

SEED=""
# Prefer live chall TRITON_CACHE_DIR (promptable on :8002)
for pid in $(ps -eo pid=,args= | awk '/[v]llm serve .*\/root\/h112\/chall/{print $1}'); do
...
TCACHE="/root/.triton/isolated/h112_king_p449_$(date +%s)_$$"
mkdir -p "$TCACHE"
cp -a "$SEED"/. "$TCACHE/"
chmod -R u+w "$TCACHE" || true
n_so=$(find "$TCACHE" -name '__triton_launcher*.so' | wc -l)
echo "$TCACHE" >/root/logs/h112_king_tcache_pass332.path
echo "$TCACHE" >/root/logs/h112_king_tcache_pass449.path
...
print(f"[king449-h112] launcher ghosts={bad}")
...
for line in out.strip().splitlines():
    ...
    if int(idx) in (2, 3):
        print(f"[king449-h112] gpu{idx} used_mib={used}")
        if float(used) < 500:
            ok += 1
if ok < 2:
    print("[king449-h112] ABORT: gpus 2,3 not free")
    sys.exit(1)
PY
...
for i in $(seq 1 180); do
  ...
  if [[ "$code" == "200" ]]; then
    ...
    if [[ -n "$mid" ]]; then
      pcode=$(curl -s -o /tmp/_probe_king449.json -w "%{http_code}" --max-time 90 \
      ...
      if [[ "$pcode" == "200" ]]; then
        ...
        date -u +%Y-%m-%dT%H:%M:%SZ >/root/logs/h112_king_recover_pass449.done
        echo "$REPO $REV $TCACHE util=0.72 seeded_from_chall" >>/root/logs/h112_king_recover_pass449.done
        cp /root/logs/h112_king_recover_pass449.done /root/logs/h112_king_recover_pass332.done
        log "DONE — retry_h112_n80 should proceed (watcher already armed)"
        exit 0
      fi
    fi
  fi
