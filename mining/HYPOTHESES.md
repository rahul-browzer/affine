...
|---|---|---|---|---|
| 1 | H6 | highest | TalentPigs-init shortz-nolist thought LoRA lr5e-6 → clip-L1≥0.042, margin>0.04 | **open** (train~90/99; mid50 n40 retry) |
| 2 | H11 | high (cheap) | TalentPigs×adambell-ckpt450 α0.75 → margin>0.04 (chal-00274 +0.023) | **open** (mirror resume dl) |
| 3 | H9 | high (cheap) | TalentPigs×diane613 α0.75 → margin>0.04 vs TalentPigs | **open** (n80 ~26/80) |
| 4 | H10 | high (cheap) | TalentPigs×kevin α0.75 (TP-dom) → margin>0.04 | **open** (kevin dl ~65G) |
...
- **Status:** train **RUNNING** ~90/99; mid50 first n40 **ReadTimeout@29/40**, retry via `retry_mid50_n40.sh`. Detail: `experiments/s4-h6-talentpigs-shortz-mild/`.
...
- **Status:** resume **RUNNING** (mirror dl). Detail: `experiments/s4-h11-tp-adambell-merge/`.
...
- **Status:** n80 **RUNNING** ~26/80. Detail: `experiments/s4-h9-tp-diane-merge/`.
...
- **Status:** kevin dl ~65G (1 incomplete). Detail: `experiments/s4-h10-tp-kevin-merge/`.
