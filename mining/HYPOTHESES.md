...
|---|---|---|---|---|
| 1 | H67 | med | H42 cell @ **r=19** → m>0.04 | **open** (n80 ~40/80 old king) |
| 2 | H69 | med | H42 cell @ **r=17** → m>0.04 | **open** (n80 early old king) |
| 3 | H70 | med | H42 cell @ **lr=5.01e-6** → m>0.04 vs **Tok** | **open** (merge+retarget) |
| 4 | H66 | med | H42 cell @ **lr=5.08e-6** → m>0.04 | **open** (n80 ~71/80 old king) |
| 5 | H68 | med | H42 cell @ **lr=4.95e-6** → m>0.04 | **open** (n80 ~35/80 old king) |
...
- **Status:** n80 ~40/80 vs TalentPigs (ranking only until Tok re-sim).

### H69 — H42 @ LoRA r=17 (non-α) — open
- **Claim:** below H64@r18 best → m>0.04 (r≤8 dead).
- **Status:** n80 early vs TalentPigs. `…/results/pass278_n80_started.md`.

### H70 — H28 @ lr=5.01e-6 (non-α) — open
- **Claim:** between H42@5e-6 +0.016 and H65@5.02 +0.018 → m>0.04.
- **Status:** train DONE; merge+`retarget_king_pass279` → n80 vs
  **Tok331102** (live king S=0.04456). `…/results/pass279_king_retarget.md`.

### H66 — H28 @ lr=5.08e-6 (non-α)
- **Claim:** between H63@5.05 dead and H58@5.1 dead → m>0.04.
- **Status:** n80 ~71/80 vs **TalentPigs** (not submit-valid; need Tok re-sim).

### H68 — H28 @ lr=4.95e-6 (non-α)
- **Claim:** just under H42@5e-6 peak (H53@4e-6 dead) → m>0.04.
- **Status:** n80 ~35/80 vs TalentPigs (ranking only).
