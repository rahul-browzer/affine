...
- **Post-train arm (pass 104):** mid **5194** / prewarm **5206** /
  pipe **5222**; corpus 9000; HF salvage *claimed* (repos were not
  actually present — fixed pass 107).
...
- **Pipe pidfile fix (pass 106):** pipe → **10642** (`_train_alive` via
  `h5c_train.pid` + chall-placeholder kill).
- **HF shells (pass 107):** private empty
  `unconst/Affine-5czsc2fc98-h5c-lora` @ `bb26ad9…` +
  `…-h5c-merged` @ `128c6fa…`; pod HfApi write verified. Prewarm DONE
  (t/k 200). Train step **14**/99.
- **Verdict:** **open**.
