...
| # | id | claim | status |
|---|---|---|---|
| 1 | R1 | Teacher-ref SFT / distill on current king init raises Reason margin > 3·SE | **weak** — R1 +0.0005; R1b n80#1 ReadTimeout@~76; **relaunch** king~1/80 @600s×5 |
...
- **Status:** weak / R1b n80 relaunch. R1 LoRA@8192 → margin **+0.000516** (z=0.105). R1b train+merge DONE; n80#1 died `httpx.ReadTimeout` @~76/80 (180s×3). Patched client **600s×5**; relaunch pid95336 slice `b99bfc9c…` king~1/80 → `r1b_lora_decision.json`. R1c chain armed (83033). HF `r1lora@569a68be` not for submit.
