...
(n80 sim RUNNING attempt 1/3, ~42/80).**
...
pid **276121** advancing king**42**/chall**42** @ 09:03:52Z
(window ~1.61 tpm → ETA **~09:27Z**; wall-90s ~2.67 tpm). Advancing.
No submit.
...
| Lium balance | $33,760.86 (floor $28,000) |
| miner coldkey free | τ10.000 (unchanged) |
| mining spend to date | `mine-sim-1` spent **~$240** @ $23.60/h |
| our submissions | none |
| Stage 3 gate | **MET** |
| H2 / H1 / H1v2 / H5 merge | **all REFUTED** |
| H5b | **open** — n80 running (king42/chall42 @ 09:03Z; ETA ~09:27Z) |
...
- n80 sim pid **276121** (`run_sim_duel.py` attempt **1/3**); out
  ...
  `/root/affine_data/h5b_sim_progress.json` (king **42**/chall **42** @ 09:03:52Z)
- Rate: window ~1.61 k-tpm (recovered vs pass95 ~1.30); wall-90s ~2.67 → ETA
  **~09:27Z**; deadman slack OK (~153 min vs window ETA)
...
- Evidence: `results/h5b_sim_progress.json`, `h5b_time_budget_pass96.json`
...
Nothing hard. **Do not submit** H1/H1v2/H2/h5-kt*/h5b HF salvage. Cap remaining ~$3,760.
Ignore archived false abort. ETA n80 done ~09:27Z (window ~1.61 tpm; slack to deadman OK).
