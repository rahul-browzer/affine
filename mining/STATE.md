...
| B300 stock | **0** free 8×B300/B200 · fleet blind-fire →`mine-r7-datafilt-1` |
...
| name | huid | $/h | role |
|---|---|---|---|
| mine-crown-1 | gentle-orbit-bd | $52.25 | **R2bf** n80 ~15/80 dpo2@90ea78ff |
| mine-r3-grpo-1 | golden-hawk-ff | $64.00 | **R3** n80 ~chall 80 + king 79/80 |
| mine-r4-fullft-1 | noble-orbit-9d | $64.00 | **R6** train max_len=16384 (pid 51683) after R5 REFUTE |
| host fleet-rent | pid**2674606** | — | blind-fire →25 @POLL=0 next=**R7** |
...
R6: `tail -f /root/logs/h101_train.nohup` · meta `r6_train_launched.json` · dec `h101_decision.json`
Fleet: `tail -f experiments/fleet-rent/logs/wait_{fleet_b300,bootstrap_fleet}.log`
**p2126:** R5→R6 lean retarget on `mine-r4-fullft-1` (kept T/K); max_len 8192→16384 (33→121/202); fleet next R7. B300×8=0.

## Blocked
No free 8×B300/B200. Cannot hit $833/h burn until stock returns.

## Next action
**Rent:** snatch next B300 → `mine-r7-datafilt-1` (blind-fire@0).
**R3:** wait n80 finish → `r3_decision.json` (almost done).
