...
**Stage 4 — H1 n40 DONE (revise_recipe); n80 RUNNING; H1v2 TRAIN + pipe FIXED.**
...
**Pass 55 critical fix:** pipe looked for adapter at `$TRAIN_DIR/` and ckpts at
`$TRAIN_DIR/checkpoint-*`, but `train_lora.py` writes `$TRAIN_DIR/adapter` and
`$TRAIN_DIR/checkpoints/checkpoint-*`. Unfixed → ERROR on train.done, no
merge/HF/n40. Fixed + pipe restarted **158053**. HF_TOKEN persisted to
`mine.env`. Do **not** submit H1 merge. kevin still king; live eval
**chal-00280** scoring (king 5/80).
...
| mining spend to date | `mine-sim-1` spent **$140.98** @ $23.60/h |
| our submissions | none |
| Stage 3 gate | **MET** |
| H2 verdict | **REFUTED** |
| H1 n40 | **DONE** — margin **−0.00241**; action `revise_recipe` |
| H1 n80 | **RUNNING** — pid **149213**; king/chall **21/20**/80 @ 04:51Z |
| H1v2 | **TRAINING** — pid **147209**; step **14**/55; loss **0.438** (last log); ETA ~05:32Z |
...
| Host harvest | **RESTARTED** — pid **1640417** (single; duplicates killed) |
| Host deadman | **ARMED** — pid **1405846** → rm at **07:00Z** |
| Live challenger | **chal-00280** Tok331102/…-af8 **scoring** (king 5/80) |
| reg_cost_tao | **~0.916** (snapshot market) |
...
- H1v2 train **147209** on GPUs 6,7 — step 14/55; log `/root/logs/h1v2_train.nohup`
...
- Adapter path fix evidence: `experiments/s4-h1v2-sft/results/h1v2_adapter_path_fix.json`

Host (no GPU):
- Artifact harvester pid **1640417** (H1 + H1v2; defers teardown while H1v2 live)
...
Time budget OK (n80 ETA ~05:20Z; train ~05:32Z; n40 expected ~06:25Z).
...
sim margin > 0.04 + H4 OK. Re-check snapshot (chal-00280 scoring; may crown).
