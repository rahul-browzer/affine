...
`TalentPigs/affine-5ekxlcg3fx-abc` reign 3 @ S=0.0315. Pass 79 armed H5b HF
salvage (mid-ckpt + post-merge push) that the launch pipe lacked; train
**245350** untouched at step **4**/55. No submit.
...
| Lium balance | $33,893.52 (floor $28,000) |
| miner coldkey free | τ10.000 (unchanged) |
| mining spend to date | `mine-sim-1` spent **~$206** @ $23.60/h |
| our submissions | none |
| Stage 3 gate | **MET** |
| H2 / H1 / H1v2 / H5 merge | **all REFUTED** |
| H5b | **open** — train step 4/55 → merge → n80; HF salvage armed |
...
| Disk | host: text only; pod `h5b/` + engines |
...
- H5b train pid **245350** GPUs 6,7 (`--loss-on thought --lr 1e-5`) step **4**/55 @ ~62s/it → ETA train.done **~08:30Z**
- H5b pipe pid **246775** (restarted w/ HF push; was 245426) waiting `train.done`
- H5b mid-ckpt salvage pid **246776** → `unconst/Affine-5czsc2fc98-h5b-lora`
- Post-merge pipe will bg-push adapter+merged to HF (same pattern as H1v2)
...
- Evidence: `experiments/s4-h5b-talentpigs-distill/results/h5b_hf_salvage_armed.json`

Validator pods `affine-eval` / `affine-bench` — do not touch.

## Blocked

Nothing hard. **Do not submit** H1/H1v2/H2/h5-kt*/h5b HF salvage. Cap remaining ~$3,794.
...
If absent: poll train **245350** for steps/loss → `train.done`; confirm pipe
**246775** + mid **246776** + harvest **1871830** + deadman **1783662**; after
merge, confirm HF push pids `h5b_push_{adapter,merged}.pid`. ETA train ~08:30Z
then merge+serve+n80 → decision ~09:30–10:30Z. Re-check snapshot before any
