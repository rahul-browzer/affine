...
**Stage 4 — H5c train RUNNING; post-train pipe RESTARTED (pidfile wait); prewarm + host harvest/deadman ARMED.**
...
| min_submission_block | **8767079** (llms/contract; snapshot field null) |
| weight_version_key | 1 |
| min_margin | 0.02 (duel) |
| eval stack | vllm 0.22.1 / transformers 5.14.1 / torch 2.11.0 |
| Lium balance | $33,676.21 (floor $28,000) |
| miner coldkey free | τ10.000 (unchanged) |
| mining spend to date | prior ~$252 + `mine-h5c-1` **$8.72** @ $28/h |
| our submissions | none |
| Stage 3 gate | **MET** |
| H2 / H1 / H1v2 / H5 merge / H5b | **all REFUTED** |
| H5c | **open** — train pid **2820** step **11**/99; pipe **10642** (pidfile wait); prewarm **5206** t=1 k=0; mid **5194** |
...
| live eval | `chal-00301` kevin954 same-repo scoring (unrelated) |

## What's running

| name | huid | role | check |
|---|---|---|---|
| `mine-h5c-1` | `golden-hawk-dc` | H5c train + prewarm + post-train→n80 | SSH `152.236.142.234:40298`; train **2820** GPUs 6,7 step11/99; teacher **5268** :8000 READY; king **5270** :8001 loading; pipe **10642**; mid **5194**; corpus 9000; TTL remove **2026-08-07T19:37:46Z** |
...
ETA: train ~11:30Z (@~48s/it from step11); merge+chall ~30m; n80 ~45–60m →
decision ~13:10Z. Soft 18:00Z / host harvest stop 18:45Z / deadman 19:00Z / TTL 19:37Z.
Gate: margin **> 0.04**, r∈[0.70,0.85], clip-L1≥0.042. No submit below gate.

Pass 106 fix: pipe uses `_train_alive` via `h5c_train.pid` (not broad pgrep).
