...
| Lium bal | ~$116,589 · floor $10k OK |
| submissions | 0 |
| R33 | **REFUTE** vs guass n80 m=**−0.00685** z=**−1.00** (p2249) |
| R22 | **TRAINING** on crown (golden-init GRPO pid**371573**) |
| R25 | chall CUDA-graph on R3; n80 arm + recover watch |
| R19 | merge→chall serve on r4 (Talent-GRPO) |
...
| mine-r3-grpo-1 | golden-hawk-ff | $64.00 | **R25-on-R24** chall→n80 |
| mine-r4-fullft-1 | noble-orbit-9d | $64.00 | **R19** Talent-GRPO post_train |
...
| host p2249 recover | pid**4116276** | — | R25 n80 if serve times out |
...
R22 train: `ssh … crown 'tail -f /root/logs/r3_train.nohup'`
...
## Next action
1. Await R25-on-R24 n80 → `r25_decision.json` (watches 4107829 / 4116276).
2. If CLEAR → Stage-5; if SIGNAL_POS_BELOW/REFUTE → free/reuse R3.
3. R22 train→merge→n80 on crown; R19 chall serve→n80 on r4.
4. Burst snatch → bootstrap **R23** (R22 no longer queued).
