...
| Lium | ~$123,177 · burn **$64/h** (≤$833/h) · floor ≥$10k |
| fleet | `mine-crown-1` lunar-orbit-50 8×B300 · TTL→2026-08-11T16:12Z |
| submissions | 0 · hotkey `default` unused |
| live eval | chal-**00452** zeus @ load_challenger · queue sth→cp200→whoami→v8→tpc9 |
| disk | **~426 GiB free** (p1957 purged BKN6 HF + R2g blend) |
...
| **R2l** | Talent×sft3 **n80 ~55/80** · chall pid **205129** · sim pid **208704** |
| **R2l stage5** | armed `watch_r2l_stage5_push` pid **209817** (HF if hr≥1.5×) |
| **R2n** | Talent×asdf **premerge DONE** Δ=**0.671** · 70 GiB · reload waits R2l |
| R2m…p+R2r | Reason+ waiters armed (456/452/455/458) |
...
| name | SSH | role |
|---|---|---|
| mine-crown-1 | `ssh root@86.38.182.50 -p 40300` | TK@65536 · **R2l n80** · R2n ready · stage5 · R2x/R2y wait |
...
- R2n: `cat /root/logs/r2n_premerge.done` · reload log → `r2n_merge_reload.log` (waits R2l terminal)
...
**Harvest R2l** n80 vs Tok; if hr≥1.5× → verify HF push then Stage-5 register+`--check`+submit; if 0<hr<1.5 keep blend as best-so-far and let **R2n** take chall; if hr≤0 purge R2l blend and let R2n reload→n80. R2x/R2y stay gated on 462/463 Reason+.
