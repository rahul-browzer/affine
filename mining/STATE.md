...
(n80 retry + abort-aware harvest armed).**

Stage 0–3 complete. H2 / H1 / H1v2 / **H5 merge** **REFUTED**. Live king
`TalentPigs/affine-5ekxlcg3fx-abc` reign 3 @ S=0.0315. Pass 84 patched host
harvest: (1) only triage n80 after `h5b_sim_n80.done` / `pipeline.done`
(retry-safe), (2) surface `h5b_pipeline.aborted` immediately as
`h5b_decision.json`. Train **245350** untouched at step **28**/55. No submit.
...
| mining spend to date | `mine-sim-1` spent **~$215** @ $23.60/h |
| our submissions | none |
| Stage 3 gate | **MET** |
| H2 / H1 / H1v2 / H5 merge | **all REFUTED** |
| H5b | **open** — train step 28/55 → merge → n80 (≤3 attempts); harvest abort+done-marker gate armed |
...
- H5b train pid **245350** GPUs 6,7 (`--loss-on thought --lr 1e-5`) step **28**/55
  @ ~58s/it → ETA train.done **~08:31Z**; loss@step25 **0.429**
- H5b pipe pid **253801** waiting `train.done` then GPU settle → merge
  → identity → HF → chall → **n80 with ≤3 retries**
...
- Evidence: `results/h5b_harvest_abort_done_gate_fix.json`,
...
Nothing hard. **Do not submit** H1/H1v2/H2/h5-kt*/h5b HF salvage. Cap remaining ~$3,785.
...
pipe **253801** log lines `train proc gone` / `GPU settle done` → merge →
...
**1917667**; deadman **1783662**. ETA train ~08:31Z then merge+serve+n80 →
