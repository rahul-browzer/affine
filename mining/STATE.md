...
**Stage 4** — R2as pure-726 **n80 RUNNING** (~26/80); R2at…av + **R2ax** armed; **tt_chall prestaged**. `weight_version_key=3`.
...
| Lium | ~$121,630 · burn **$52.25/h** (≤$833/h) |
| fleet | `mine-crown-1` **gentle-orbit-bd** 8×B200 · TTL→2026-08-12T08:42Z |
| SSH | `ssh root@95.133.253.90 -p 40099` |
| submissions | 0 · hotkey `default` unused |
| board | Q **489**+; **486** scoring (~1171/2080); **485** stamped hr **0.601×** |
...
| R2as | pure 726 **n80** pid**141642** ~**26/80** · Stage-5 armed |
...
| R2ax | pure **tt** **ARMED** wait R2av · prefetch **DONE** · **tt_chall** ready · Stage-5 **143277** |
...
- R2ax wait **143276** · Stage-5 **143277** · `/root/r2_out/tt_chall` **24 files / 16 shards**
...
- Many Reason+ HF parents now `gated=manual` (sth/pig/sbs/whoami/nvidia/awesome) — prefer ungated or cached.

## Next action

**Poll** R2as n80→decision (`/root/affine_data/r2as_726_{reason_progress,decision}.json`). If hr≥1.5× → Stage-5 HF push → register+submit. Else R2at→…→R2ax pure-tt (chall already prestaged). Host-hist stamps 486+ when verdicts land.
