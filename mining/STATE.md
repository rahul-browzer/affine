...
| R2e Talent×awesome | **n80 RUNNING** · ~32/80 · sim **128291** · Δ=**0.626** |
...
| R2g Talent×saysth | premerge waiter **130003** + **reload→n80** waiter **130365** |

## What's running

| name | huid | SSH | TTL | role |
|---|---|---|---|---|
| mine-crown-1 | lunar-orbit-50 | `ssh root@86.38.182.50 -p 40300` | 2026-08-11T16:12Z | TK@65536 + R2e n80 + 440/R2g chain |
...
- R2g premerge: `/root/logs/r2g_premerge.{log,done,skip}` pid **130003** (gate hr>0).
- R2g reload→n80: `/root/logs/r2g_merge_reload.{log,done}` pid **130365** (waits premerge + R2e below-bar lane).
...
**Harvest R2e** when `/root/affine_data/r2e_alpha_decision.json` exists. If headroom ≥ 1.5× → Stage-5. Else confirm R2g chain: `r2g_premerge.done|skip` then `r2g_alpha_decision.json` (waiter 130365). If R2g SKIP → rescan new completed evals for Reason+ parents.
