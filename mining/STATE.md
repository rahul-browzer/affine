...
| name | huid | $/h | role |
|---|---|---|---|
| mine-crown-1 | gentle-orbit-bd | $52.25 | TK; **R2be** n80 ~25/80; **R2bf** dpo2 prefetch DONE + waiter |
| mine-r3-grpo-1 | golden-hawk-ff | $64.00 | **merge→/tmp** (p2122 unstick) → chall → n80 |
| mine-r4-fullft-1 | noble-orbit-9d | $64.00 | **R5** Genesis full-FT step≥22/26 |
...
R2bf: prefetch DONE; waiter `r2bf_dpo2_reload.log` + stage5 watch
R3: `tail -f /root/logs/r3_pipeline.nohup` · `MERGED=/tmp/r3_merged` · dec `r3_decision.json`
...
**p2122:** R3 merge hung on gocryptfs `/root` → killed; relaunch merge to `/tmp/r3_merged`.
...
**R3:** wait `/tmp` merge→chall reload→n80 decision.
**Crown:** wait R2be n80 → decision; else R2bf auto-serve (prefetch cached).
