...
| Lium / spend | **~$190,721** · cum mining ~$3,020 · **avail ~$181k** (floor $10k) |
| miner | τ10.000 free · 0 submissions |
| H28 | n80 **41/80** chall / **42/80** king (engines 200) |
| H29 | train ~40/46 · loss@40≈0.486 · **prewarm relaunched** (teacher up) |
| H30 | train ~22/46 · loss@20≈0.473 · **prewarm relaunched** |
| H31 | train ~7/46 · loss@5≈0.617 · prewarm teacher+king loading |
| H32 | train launched lr=3e-5 (BOOTSTRAP_DONE) |
...
| mine-h29-1 | golden-wolf-bc | 38.255.28.21:20100 | ~07:28Z | H29 LoRA→merge→n80 |
...
Corpus sync race: see LESSONS; fixed `s3-duel-sim/sync_corpus.sh` (flock+adopt).

## Next action

1. H29: poll `train.done` → merge → n80; confirm prewarm `:8000/:8001` ready
   before chall serve; m>0.04 → Stage 5 else REFUTE+rm.
2. H28: poll → `h28_decision.json`; m>0.04 → Stage 5; else REFUTE+rm.
3. H30: poll train → merge → n80 (prewarm recovering); same gate.
4. H31: poll train → merge → n80; same gate.
5. H32: poll train → merge → n80; same gate.
