...
**Stage 4 in progress — H2 α=0.5 sim duel RUNNING vs kevin (sampling ~20/80).**
...
| mining spend to date | `mine-sim-1` spent **$30.53** @ $23.60/h (TTL cap ≈ $141.60) |
...
| Stage 4 H2 sim | **RUNNING** — pid **68843**; log @ 00:11Z: `king 20/80`, `challenger 20/80` |
...
- Progress (pass 14): sampling advancing; teacher GPUs 0–1 ~100% (bottleneck:
  ~3–8 running / ~14–19 waiting); king/chall engines loaded, idle between batches
- Throughput: ~5 turns / ~3–4 min per side after bootstrap → sampling ETA
  ~00:40–00:50Z; force-echo after that; finish well inside TTL 04:53Z
...
Nothing hard. Soft: TTL ends 04:53Z (~4.7h left @ 00:11Z). Do **not**
