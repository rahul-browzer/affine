...
(n80≤3 + abort-aware harvest + pipe EXIT abort-trap armed).**

Stage 0–3 complete. H2 / H1 / H1v2 / **H5 merge** **REFUTED**. Live king
`TalentPigs/affine-5ekxlcg3fx-abc` reign 3 @ S=0.0315. Pass 85 armed
`post_train_pipeline` EXIT trap so merge/identity/serve/wait failures write
`h5b_pipeline.aborted` (harvest → `h5b_decision.json`). Train **245350**
untouched at step **31**/55. No submit.
...
| Lium balance | $33,854.29 (floor $28,000) |
| miner coldkey free | τ10.000 (unchanged) |
| mining spend to date | `mine-sim-1` spent **~$216** @ $23.60/h |
| our submissions | none |
| Stage 3 gate | **MET** |
| H2 / H1 / H1v2 / H5 merge | **all REFUTED** |
| H5b | **open** — train step 31/55 → merge → n80 (≤3); abort trap + harvest abort gate armed |
...
- H5b train pid **245350** GPUs 6,7 (`--loss-on thought --lr 1e-5`) step **31**/55
  @ ~58s/it → ETA train.done **~08:27Z**; loss@step30 **0.441**
- H5b pipe pid **256662** waiting `train.done` then GPU settle → merge
  → identity → HF → chall → **n80 with ≤3 retries**; EXIT trap writes
  `h5b_pipeline.aborted` on any unclean failure
...
- Evidence: `results/h5b_pipe_abort_trap_fix.json`,
  `h5b_time_budget_pass85.json`
...
Nothing hard. **Do not submit** H1/H1v2/H2/h5-kt*/h5b HF salvage. Cap remaining ~$3,784.
...
pipe **256662** log lines `train proc gone` / `GPU settle done` → merge →
`h5b_identity.json` (`identical_to_king=false`); if n80 fails once, confirm
retry in `h5b_sim_retries.log` / pipe log. Mid **251832**; harvest
**1917667**; deadman **1783662**. ETA train ~08:27Z then merge+serve+n80 →
decision ~09:30–10:40Z. Re-check snapshot before any submit. Do not rent
another pod.
