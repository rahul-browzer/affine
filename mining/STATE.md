...
| disk | `/root` **~510 GiB** free · R2z blend ~70 GiB · sbs prefetch ~70 GiB |
| board 455 sth | hr **0.79×** (margin 0.0091 / 3·SE 0.0116) — lost |
| **R2p** | Talent×sth n80 **~27/80** · Δ=**0.671** · chall `/tmp/r2p_alpha_merged` |
...
| **sbs prefetch** | chal-00468 @`c175fe8b…` **RUNNING** (pid 248095) |
| next parents | 469 sky / 470 google / 471 pig — after sbs DONE |

## What's running

| name | SSH | role |
|---|---|---|
| mine-crown-1 | `ssh root@86.38.182.50 -p 40300` | TK@65536 · **R2p n80** · sbs prefetch · R2z wait-Reason · R2x/y wait |
...
- Do not merge awesome-v8/v9/tpc9/sbs until post-verdict Reason+ (eager OK; DONE gated).
...
- Prefetch next queue parent **one-at-a-time** after sbs (sky first) — disk ~510 GiB.

## Next action

**Harvest R2p** when n80 finishes (`r2p_alpha_decision.json`); if hr≥1.5× → Stage-5 HF push+register+`--check`+submit; if 0<hr<1.5 keep blend and advance next Reason+ lane; if hr≤0 purge R2p and let R2x/R2y/R2z claim GPU after their Reason+ stamps. After sbs DONE: prefetch **chal-00469 sky** (2×st) one-at-a-time.
