...
| R1b train | **RUNNING** · max_len=16384 · **~32/126** @~37s/it · loss~0.31 · pid **79866** |
...
| R2 prefetch | **RUNNING** · TalentPigs+kevin954 pid **83501** (CPU only) |
...
|---|---|---|---|---|
| mine-crown-1 | lunar-orbit-50 | `ssh root@86.38.182.50 -p 40300` | 2026-08-11T16:12Z | TK@65536 + R1b + waiters + R2 prefetch |
...
- ETA R1b train ~1.0h; then merge+n80; chain auto-fires R1c on fail.
...
**Harvest** `/root/affine_data/r1b_lora_decision.json` (and/or chain stamp). If R1b clears 1.5× → Stage-5 prep. If chain launched R1c → monitor `/root/logs/r1c_train.log`. In parallel: harvest `/root/logs/r2_prefetch_parents.done` then stage R2 α-merge recipe (Tok×TalentPigs×kevin) for post-R1c GPU slot.
