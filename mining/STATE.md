...
| name | huid | $/h | role |
|---|---|---|---|
| mine-crown-1 | gentle-orbit-bd | $52.25 | **R2ax** n80 ~16/80; **R2ay** sbs-v2 armed; **R2az** vvv prefetch+waiter |
| mine-r3-grpo-1 | golden-hawk-ff | $64.00 | **GRPO** pid**23755** step≥10 mean_r≈0.025; T:8000 K:8001@65536 |
...
R2az check: `tail -f /root/logs/r2_prefetch_vvv.log` · `tail -f /root/logs/r2az_vvv_reload.log`
R3 check: `grep r3-log /root/logs/r3_train.nohup | tail` (logs every 5 steps)
Host-hist: pid**2113721** pending 490–495 + **497–504**
Crown disk: purged iynocr2p+h64+sft4 → **~339G** free
...
**Crown:** collect R2ax → R2ay auto → R2az vvv (chal-00497) auto.
