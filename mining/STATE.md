...
(n80 sim RUNNING attempt 1/3, ~58/80).**
...
pid **276121** advancing king**58**/chall**58** @ 09:13:12Z
(window ~1.79 tpm → ETA **~09:25Z**; wall-90s ~1.43 tpm). Advancing.
No submit.
...
| Lium balance | $33,745.62 (floor $28,000) |
| miner coldkey free | τ10.000 (unchanged) |
| mining spend to date | `mine-sim-1` spent **~$243** @ $23.60/h |
| our submissions | none |
| Stage 3 gate | **MET** |
| H2 / H1 / H1v2 / H5 merge | **all REFUTED** |
| H5b | **open** — n80 running (king58/chall58 @ 09:13Z; ETA ~09:25Z) |
...
- n80 sim pid **276121** (`run_sim_duel.py` attempt **1/3**); out
  ...
  `/root/affine_data/h5b_sim_progress.json` (king **58**/chall **58** @ 09:13:12Z;
  SCP moment later king**59**/chall**58**)
- Rate: window ~1.79 k-tpm (pass97→98); wall-90s ~1.43 → ETA
  **~09:25Z**; deadman slack OK (~154 min vs window ETA)
...
- Evidence: `results/h5b_sim_progress.json`, `h5b_time_budget_pass98.json`
...
- H5b harvest pid **1964910** (`host_harvest_h5b.sh`) stop **11:45Z**
  → stage-aware scrape → `results/h5b_train_progress.json` (`stage=n80`)
...
Nothing hard. **Do not submit** H1/H1v2/H2/h5-kt*/h5b HF salvage. Cap remaining ~$3,757.
Ignore archived false abort. ETA n80 done ~09:25Z (window ~1.79 tpm; slack to deadman OK).
