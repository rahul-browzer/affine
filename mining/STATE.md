...
(n80 retry insurance armed).**

Stage 0–3 complete. H2 / H1 / H1v2 / **H5 merge** **REFUTED**. Live king
`TalentPigs/affine-5ekxlcg3fx-abc` reign 3 @ S=0.0315. Pass 83 patched
post-train n80 to retry up to **3** attempts with deadman budget gate
(H1 died once on ReadTimeout; client already 360s×5). Train **245350**
untouched at step **24**/55. No submit.
...
| Lium balance | $33,862.41 (floor $28,000) |
| miner coldkey free | τ10.000 (unchanged) |
| mining spend to date | `mine-sim-1` spent **~$214** @ $23.60/h |
| our submissions | none |
| Stage 3 gate | **MET** |
| H2 / H1 / H1v2 / H5 merge | **all REFUTED** |
| H5b | **open** — train step 24/55 → merge → n80 (≤3 attempts); prior GPU-release + identity + HF fixes still in pipe |
...
- H5b train pid **245350** GPUs 6,7 (`--loss-on thought --lr 1e-5`) step **24**/55
  @ ~55–59s/it → ETA train.done **~08:26Z**; loss@step20 **0.521**
...
- Evidence: `results/h5b_n80_retry_fix.json`, `h5b_time_budget_pass83.json`
...
Nothing hard. **Do not submit** H1/H1v2/H2/h5-kt*/h5b HF salvage. Cap remaining ~$3,786.
...
confirm pipe **253801** log lines `train proc gone` / `GPU settle done` → merge
→ `h5b_identity.json` (`identical_to_king=false`); if n80 fails once, confirm
retry in `h5b_sim_retries.log` / pipe log. Mid **251832**; harvest **1884718**;
deadman **1783662**. ETA train ~08:26Z then merge+serve+n80 → decision
