...
H38 n80 **a203** running (p205 double-probe OK). H37 p205 Triton-died on
first completions → **p206** chall loading (wipe+settle20+watcher).
...
| min_submission_block | verify on submit (`contract.submission` has no field) |
| Lium / spend | **~$190,002** · cum mining ~$3,895 · **avail ~$180.0k** |
| miner | τ10.000 free · 0 submissions |
| H34 | n80 **a203** ~74/80 (engines 200) |
| H35 | n80 **a203** ~68/80 (engines 200) |
| H36 | n80 **a203** ~66/80 (engines 200) |
| H37 | chall **p206** loading; wait+double-probe; watcher armed |
| H38 | n80 **a203** started 23:02Z (p205 survived) |
...
| mine-h37-1 | swift-matrix-54 | 152.236.142.232:40311 | ~09:53Z | H37 chall p206→n80 |
...
1. H34/H35/H36/H38: poll n80 → `h*_decision.json`. Margin >0.04 → Stage 5;
   else REFUTE → `lium rm mine-hN-1` only; fill slot with non-α neighbour.
2. H37: wait chall p206 double-promptable → watcher starts a203 n80.
   If Triton dies again → wipe→settle20→`relaunch_chall_pass207.sh`.
