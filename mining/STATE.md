...
| Lium balance | ~$124,328 · floor ≥$10k · burn **$64/h** (≤$833/h ok) |
...
| R1b train | **RUNNING** · max_len=16384 · **~93/126** @~37s/it · loss~0.33 · pid **79866** |
...
| R2 CPU premerge | **DONE** @18:47Z · `max_abs_delta=0.27734375` · 66 GiB · path `/root/r2_out/alpha_tok_talent_kevin` |
| R2 α→n80 | **ARMED** · waiter pid **85408** · reuses premerge; waits R1 lane |
...
|---|---|---|---|---|
| mine-crown-1 | lunar-orbit-50 | `ssh root@86.38.182.50 -p 40300` | 2026-08-11T16:12Z | TK@65536 + R1b + R1/R2 waiters |
...
- R2 premerge stamp: `/root/logs/r2_premerge.done`; meta `/root/r2_out/alpha_tok_talent_kevin/merge_alpha_meta.json`.
- R2 n80: `/root/logs/r2_merge_reload.log` → `/root/affine_data/r2_alpha_decision.json`.
- ETA R1b train ~0.35h then merge+n80; R1c auto if below bar; R2 α after R1 lane frees.
...
**Harvest** R1b decision (`/root/affine_data/r1b_lora_decision.json`). If headroom ≥ 1.5× → Stage-5 prep. Else confirm R1c chain launch, then R1c/R2 decisions.
