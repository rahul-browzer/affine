...
**Stage 4 — H1 teacher-ref LoRA SFT RUNNING; post-train TTL path hardened.**
...
Post-train pipeline pid **84834** waits for `train.done` → HF adapter
salvage → **GPU merge on 6,7** → **chall-only re-serve** (king kept) →
sim with progress JSON. Mid-ckpt salvage pid **83669** and host artifact
harvester pid **1388880** armed. No submissions.
...
| Lium balance | $34,407.24 (floor $28,000) |
| miner coldkey free | τ10.000 (unchanged) |
| mining spend to date | `mine-sim-1` spent **$77.11** @ $23.60/h (TTL cap ≈ $141.60) |
| our submissions | none |
| Stage 3 gate | **MET** |
| H2 verdict | **REFUTED** (`experiments/s4-h2-merge/result.md`) |
| H1 harvest | **DONE** — 440 examples, 0 missing |
| H1 train | **RUNNING** — step 16/110 @ ~50s/it, ETA ~03:28Z |
| H1 pipeline | **ARMED** — pid **84834** (chall-only serve + progress JSON) |
| H1 mid-ckpt salvage | **ARMED** — pid 83669 → HF `checkpoint-N/` |
| H1 HF salvage repo | **CREATED** — private `unconst/Affine-5czsc2fc98-h1-lora` |
| Host harvest | **ARMED** — pid 1388880 → `experiments/s4-h1-sft/results/` |
...
- H1 pipeline: pid **84834**, log `/root/logs/h1_pipeline.nohup`
  ...
  → sim with `--progress-out /root/affine_data/h1_sim_progress.json`
...
- Freed this pass: `/root/merges/h2-kp50` + genesis HF cache (~136G)
- Harvest: `/root/h1/teacher_refs_sft.jsonl` (440 lines)

Host (no GPU):
- Artifact harvester pid **1388880**, log `.ralph/host_harvest.log`
  → SCPs sim/progress/salvage/train JSON into `experiments/s4-h1-sft/results/`

Validator pods `affine-eval` / `affine-bench` — do not touch.

## Blocked

Nothing hard. Soft: TTL ends 04:53Z. Train ETA ~03:28Z; prior 80-turn sim
≈42 min; chall-only serve should cut ~3–8 min vs king reload. Expected
finish ~04:25–04:40Z if merge stays fast. **Do not** cancel the schedule
...
Also watch `/root/affine_data/h1_sim_progress.json` once sim starts,
mid-ckpt salvage after step 50, and final `/root/h1/adapter_salvage.json`
after `train.done`. If sim present: apply `experiments/s4-h1-sft/plan.md`
decision rule (margin > 0.04 + H4 → Stage 5 path; 0.02–0.04 iterate;
<0.02 revise). If pipeline stuck after `train.done`: check
`/root/logs/h1_pipeline.nohup` (expect GPU merge on 6,7 then chall-only
serve). Do **not** submit until margin > 0.04 + H4.
