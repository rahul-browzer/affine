...
**Stage 4 — H1 n40 DONE (revise_recipe); n80 RUNNING; H1v2 TRAIN + pipe ARMED; harvest H1v2-gated.**
...
on GPUs 6,7 + **post_train_pipeline** waiting on `train.done` → merge → chall
serve → n40. **Pass 53:** host harvest patched so H1 n80 completion cannot
early-teardown while H1v2 still runs; H1v2 progress now scraped to
`experiments/s4-h1v2-sft/results/`. Do **not** submit H1 merge. kevin still
...
| mining spend to date | `mine-sim-1` spent **$138.57** @ $23.60/h |
| our submissions | none |
| Stage 3 gate | **MET** |
| H2 verdict | **REFUTED** |
| H1 n40 | **DONE** — margin **−0.00241**; action `revise_recipe` |
| H1 n80 | **RUNNING** — pid **149213**; king/chall **~6/5**/80 @ 04:44Z |
| H1v2 | **TRAINING** — pid **147209**; step **6**/55; loss **0.493**; ETA ~05:35Z |
| H1v2 pipe | **ARMED** — pid **149216** waits `train.done` → merge→serve→n40 |
| H1 merged HF | `unconst/Affine-5czsc2fc98-h1-merged` @ **3364892cefcc…** |
| Host harvest | **RESTARTED** — pid **1627557** (H1v2 gate + scrape) |
...
- H1v2 train **147209** on GPUs 6,7 — step 6/55; log `/root/logs/h1v2_train.nohup`
...
Host (no GPU):
- Artifact harvester pid **1627557** (H1 + H1v2; defers teardown while H1v2 live)
- TTL deadman pid **1405846** @ 07:00Z
- H1v2 code: `experiments/s4-h1v2-sft/` (plan + train + emit + post_train_pipeline)
...
Host harvest no longer kills the pod on H1 n80 alone.

## Next action (single, highest value)

**Poll H1v2 train** (`train.done` / `h1v2_train_progress.json`) and **n80**
