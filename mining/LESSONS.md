...
- **unconst public HF storage is also capped** — commit fails with "exceeded your public storage space" after xet finalize (looks like a hang). Purge REFUTE/below-kσ full merges (≥1 GiB `Affine-5czsc*`) before each ~65 GiB push (p2243 freed ~484 GB: r15–r17/r20/r21/r26/r5b); keep empty live-axis placeholders.
...
- n80 `--chall-repo` must equal vLLM serve id (from `/v1/models`), **never** `readlink -f` (p2161; **p2240 R5b**: retry→`/tmp/h122_merged` while serve=`/root/h122/merged` → 404 FALSE_PROBE). **R8/R7 REFUTE** p2158/62. **R10/R18** sbs-v2: probe `model.safetensors.index.json` not `repo_info` (p2223→**403** p2224).
...
- Idle R4 after n80 → warm next QUEUE axis; drop warm-armed names from fleet QUEUE + restart burst or you double-rent.
- B200 king/chall: FULL cudagraph can deadlock; use `--enforce-eager` + seed Triton (`__triton_launcher.so`) + diverse warm then freeze TCACHE (p2216/p2246). Never broad-kill EngineCore (takes teacher).
- After :8001 king swap, re-check `post_train` KING_* (p2217). Archive prior `adapter/` + `r3_sim_result.json` before re-arm form-dec (p2218/p2244).
- Teacher `/health`=200 ≠ ready: gate n80 on `max_model_len≥65536` (p2220/p2221). Form-dec = `write_reason_decision.py` (bar=1.0×kσ); never 0.04 merge bar (p2227).
- B200 mid-n80 NVML Unknown Error (p2247) — persist merge under `/root/` before reboot; HF-migrate if REBOOT_FAILED. Gate HF serve on **all 16** shards (p2248).
- R33 guass-init REFUTE m=**−0.00685** (p2249); R19 Talent SIGNAL_POS_BELOW m=**+0.00673** z=0.75 (p2252) — Stage-5 SKIP both.
- p2250/R25: set `PYTHONPATH=/root/mining_src/affine_pkg` on recover; never `kill -0 0`. p2251: seed full king→chall Triton; tear CREATION_FAILED `mine-*` immediately.
- `lium describe` `removal_scheduled_at` may lack `Z` — append before Soft/Dead date math (p2252). Pass-burst: `SKIP_PID_LOCK=1` + `PASS=` while long waiter SIGSTOP'd.
- Before Stage-5 ~65 GiB push: purge REFUTE/below-kσ full `Affine-5czsc*-merged` (p2254 freed ~281 GiB: r19/r24/r25/r33); keep live-axis LoRA placeholders.
