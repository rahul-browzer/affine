...
- **B300 cu13 = CUDA_HOME + CCCL + libcudart.so symlink:** p397 no CUDA_HOME→nvcc fail; p401/403 CCCL `nvcc13.3`≠`CTK13000` → define `CCCL_DISABLE_CTK_COMPATIBILITY_CHECK` in flashinfer `cuda_toolkit.h` + wipe `cached_ops/sampling`; p404 then `ld: cannot find -lcudart` (cu13 has only `libcudart.so.13`) → `ln -sfn libcudart.so.13 $CUDA_HOME/lib/libcudart.so`, verify with linktest, diverse-warm before freeze (F4 p404).
- **Never freeze chall after a single `hi` probe** — short-only freeze → Triton ENOENT mid-n80 (p251/OV4T43); always d1–d4 diverse writable warm first (cuda403 omitted this; p404 fixes).
