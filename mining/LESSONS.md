...
- **RL `mean_r=0`+empty `z0`**: (1) dead teacher (F40 p507 `:8000` refused) **or** (2) kevin/albedo emit `</think>\nTHOUGHT:` and old `_normalize_z` collapses to "" (p508; offline gen proved; fix leading-close+prefer ```bash cut → mean_r 0.016/0.022/0.005). Always offline-decode once before blaming teacher.
