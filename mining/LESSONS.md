...
- **`/root/hN/merged` symlink ≠ vLLM model id:** post_train serves `/tmp/hN_merged`; `--chall-repo /root/hN/merged` → completions 404 → FALSE_PROBE in ~48s (F27 p476×3). Fix: `MERGED=$(readlink -f …)` before `run_sim_duel` (`retry_*_p476.sh`).
