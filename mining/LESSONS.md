...
- **R9 merge graft:** `save_file` after `del blob` → EFAULT — `.clone()` visual tensors (p2187). Kill chall PID before rewriting its serve dir (p2189 R3b Bad address while `:8002` mapped `/tmp/r3_merged`). Prefer `graft_visual_weights.py` resume if language shards already written.
- **R9 REFUTE** p2189 m=−0.0172 z=−3.36; **R12 REFUTE** p2188 m=+0.00085; **R13 REFUTE** p2189 m=−0.0191 z=−2.86 — teacher-z_C LoRA / BoN-CE / offline-DPO all closed vs ckp333.
