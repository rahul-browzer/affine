...
- H7/H8/H10 TP×{pandora,golden-crown,kevin} α0.75 all **INVALID** band (base×2.21/1.97/**1.98**);
  even reign-2 kevin at 25% sabotages empty-baseline — no α0.85 on gate-fail.
- Dual-side n40/n80 teacher `httpx.ReadTimeout`: 180s×3 dies (H6@29/40); 480s×5 still dies (H9@60/80) — outer 3× retry required; H9/H12 inline; H6/H10/H11 live sims use `watch_n80_retry.sh`→`retry_*_n80.sh` (do not edit live start_*.sh).
