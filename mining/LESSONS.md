...
- Idle R4 after n80 close → warm next QUEUE axis (R21 p2212; **R5b SIGNAL_POS_BELOW m=+0.00393 z=0.43→R19 Talent-GRPO p2241**); drop warm-armed axes from fleet QUEUE + restart burst or you double-rent the name.
...
- p2251/R19: post_train re-serve chall can die mid-load on missing `__triton_launcher.so` even when chall TCACHE has a partial entry — seed **full king→chall** Triton cache (2→23 launchers), relaunch without `pkill -f`, freeze after `:8002=200`. Tear CREATION_FAILED `mine-*` immediately (one-shot template 400) so burst keeps polling.
- `lium describe` `removal_scheduled_at` often lacks trailing `Z` — append before GNU `date -d '… -1 hour'` or Soft/Dead parse fails (p2252 R23 warm). Pass-burst needs `SKIP_PID_LOCK=1` + env `PASS=` (not PASS_TAG) while long waiter is SIGSTOP'd.
- R19 Talent-GRPO vs guass: n80 m=**+0.00673** z=0.75 hr**0.38×** → SIGNAL_POS_BELOW (p2252); Stage-5 SKIP. Idle R3→R23 diane warm; drop R23 from fleet QUEUE.
