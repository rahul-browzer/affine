...
- New B300 pods: prefer **crown→pod rsync** of Tok+teacher HF caches (parallel) over Hub when crown already has them; hold bootstrap with a sleep pid in `parallel_dl.pid` until stamps land so it does not fall through to a second Hub download.
