...
- Script-path self-match in SSH argv → match `$0` via `/proc/*/cmdline`. Mid-n80 king EngineDead/Triton ENOENT → orphans ppid=1 GPUs 2,3 → king-only recover (H76 p300/p302; H77 p302); leave chall alone. If recover waits health=000 with orphan TP1+APIServer gone → kill recover PID, reap, re-fire (do not wait out 180 polls).
...
- recover264 DONE rearms form+watch_n80 only — **not** preempt. One-shot
  `watch_preempt_bare_tcache_pass264` **exits** on isolated chall (p303 rearm
  → immediate "leave alone"). Mid-n80 bare needs **continuous**
  `watch_mid_n80_bare_tcache_pass304.sh` while sim alive (H76–H80 p304).
