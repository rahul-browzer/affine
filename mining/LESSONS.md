...
- n80 `--chall-repo` must be the **vLLM serve-time model id** (often symlink `/root/h121/merged`), never `readlink -f` → `/tmp/…` (404 “model does not exist” → FALSE_PROBE). Resolve from `:8002/v1/models` **after** engines up (p2161).
