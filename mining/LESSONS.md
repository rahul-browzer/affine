...
- Chall recover must seed from **live king isolated TCACHE** (`:8001` env), not bare `cache/king` — H91/p382: script checked only bare → "no king TCACHE" while isolated n_so=19; mid-load rsync 0→19. Patched `relaunch_chall_pass264.sh`. Early-abort load-time launcher ImportError (don't wait :8002=200 for preempt).
