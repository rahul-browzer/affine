...
- **`--chall-repo` = `/v1/models` id, not blind realpath:** F27 serve `/tmp` + symlink chall-repo → 404 (p476). F28 serve symlink `/root/h123/merged` + realpath → 404 (p477). Query models id; king cold-TCACHE ENOENT → seed chall n_so≥16 (`king_recover_pass435`).
