...
- **R8 REFUTE** p2158 m=−0.027; king-swap retry_* KING defaults (p2159); R9 train-wait ≠ merge-block (p2160); n80 `--chall-repo`=vLLM serve id not `readlink -f` (p2161). **R7 REFUTE** p2162 m=+0.0123 z=1.978 <2σ (hr0.99×) — knife-edge. **R10** was 403 on sbs-v2 (p2162); **Hub public again p2223** — re-check `model_info` before leaving axis blocked; promote distinct merge/sbs axes over knob-family when stock is scarce.
