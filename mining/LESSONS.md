...
- Script-path self-match in SSH/`awk` argv false-fires ALREADY_RUNNING (p300); match `$0` via `/proc/*/cmdline`. Mid-n80 king `EngineDead`/`sample_tokens` timeout → orphans ppid=1 GPUs 2,3 → king-only recover (H76 p300); leave chall alone.
