...
- Default `block_hash=0*64` n80 dies teacher **400** @~40/80 — rotate a203/b203/c203. **FALSE_PROBE quarantine + watcher restart resets attempt→1/a203** (rehits H32); kill a203 (rc=143)→b203 (F7 p387/p389). Also: retry `rc=0`+rejection writes N80_DONE → never reaches b203 — treat FALSE_PROBE as attempt fail; prefer b203-first. Mid-n80: scp fix under **new name** + re-point watcher (F7 p390 `retry_*_b203first.sh`); never edit the running retry script. Soft-deadline: patch `:-` ≥TTL−1h; never tear down. `watch_n80_retry` must not `exec` retry.
...
- recover264 DONE rearms form+n80 only (not preempt/mid304). Match `$0` via `/proc/*/cmdline` (H82/H93). Seed chall from **live king :8001 TRITON_CACHE_DIR** (not bare `cache/king`) — H91/p382/p384 F7 had 19 .so in isolated king but recover said "no king TCACHE"; mid-load rsync works. clone: `test -x` rearm (H94); king cold-JIT→peer king TCACHE (H95).
- Peer-seed mid-load when local king TCACHE misses a hash (F6 a1 `6YKNXZRS…` absent from 16-so seed; rsync from F7 iso king 19so → a2). **Salvage pre-freeze 555 hangs load** if torchinductor twin is also 555 — APIServer stalls before EngineCore at ~4 MiB; `chmod 755` both mid-load unblocks (F7 p386).
- **huggingface_hub≥1.27 never resumes** `.incomplete`: unique `{etag}.{uuid}.incomplete` opened `"wb"`, deleted on fail (PR#4228). Orphan large incompletes need HTTP `Range` resume (H100/F4 p383). **post_train 120×15s king-wait races Range** — kill post_train before ABORT + arm tok.done→king→chall watcher (F4 p388: 7m left vs ETA 10m).
