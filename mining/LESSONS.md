...
- **B300 cu13 = CUDA_HOME + CCCL patch:** p397 missing CUDA_HOME→nvcc fail; p401 CUDA_HOME alone→flashinfer JIT `nvcc 13.3` vs `CUDA_VERSION 13000` CCCL incompatible. Define `CCCL_DISABLE_CTK_COMPATIBILITY_CHECK` in flashinfer `cuda_toolkit.h`, wipe `cached_ops/sampling`, leave TCACHE writable till promptable (F4 p403).
