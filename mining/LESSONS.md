...
- R3/s3 `serve_three.sh` default `MAXLEN` must be **65536** (live eval); 32768 aborts n80 on long prefixes. If teacher dies mid-GRPO (NCCL orphan on GPU1), `SIGSTOP` train → clear GPU0/1 orphans → relaunch teacher → `SIGCONT` so steps do not burn on Connection refused.
