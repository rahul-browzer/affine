...
- **Raw past-king/genesis CLOSED:** F19−0.006…F23−0.084 + **F22 everest m=−0.06273** z=−7.07 (p499). **Full-FT×Λ2 CLOSED/dying:** Tok/Genesis/Bittob/refs/kevin/pandora/everest/golden/diane F34−0.063 + **TalentPigs F32 m=−0.02626** z=−3.94 λ2_c=−0.0069 (p502) all ≤0; only af-k1 F36 screen left — do not rent more of this class.
...
- **`--chall-repo` = `/v1/models` id; preempt EXP = real dir** (p480/p491). **Teacher-Λ2 RL needs teacher :8000 before train** (F37 p492). **post_train SOFT_DEADLINE = this rent's TTL−1h** (p496). **Family-clone `sed` breaks `"\\n"` inside `python <<'PY'` heredocs** → SyntaxError (F38 p502 genesis_init.done); verify compile or use python rewrite, never sed on those lines.
